unit VorbisFile;
// Unit bracket (inferred): .text 0x007F4934..0x007F4FFD; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create(SharedLock: PCriticalSection; UseExternalLibrary: Boolean); // @addr $7F49B4 @ida "TOggWorker *__userpurge $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, TCriticalSection **SharedLock@<ecx>, bool UseExternalLibrary@<^0>);"
    destructor Destroy; override; // @addr $7F4A58 @ida "void __usercall $name(TOggWorker *Self@<eax>, __int8 DestroyFlags@<dl>);" @note "Decrements the shared use count without unloading or clearing the decoder."
...
var
  VorbisLoaded: Boolean = False; // @addr $881C20
  VorbisClear: TVorbisFileStatus; // @addr $88BE44
  VorbisFOpen: TVorbisFOpen; // @addr $88BE48
  VorbisOpenCallbacks: TVorbisOpenCallbacks; // @addr $88BE4C
  VorbisTestCallbacks: TVorbisOpenCallbacks; // @addr $88BE50
  VorbisTestOpen: TVorbisFileStatus; // @addr $88BE54
  VorbisBitrate: TVorbisLinkStatus; // @addr $88BE58
  VorbisBitrateInstant: TVorbisFileStatus; // @addr $88BE5C
  VorbisStreams: TVorbisFileStatus; // @addr $88BE60
  VorbisSeekable: TVorbisFileStatus; // @addr $88BE64
  VorbisSerialNumber: TVorbisLinkStatus; // @addr $88BE68
  VorbisRawTotal: TVorbisLinkCount; // @addr $88BE6C
  VorbisPcmTotal: TVorbisLinkCount; // @addr $88BE70
  VorbisTimeTotal: TVorbisLinkTime; // @addr $88BE74
  VorbisRawSeek: TVorbisSeekOffset; // @addr $88BE78
  VorbisPcmSeek: TVorbisSeekOffset; // @addr $88BE7C
  VorbisPcmSeekPage: TVorbisSeekOffset; // @addr $88BE80
  VorbisTimeSeek: TVorbisSeekTime; // @addr $88BE84
  VorbisTimeSeekPage: TVorbisSeekTime; // @addr $88BE88
  VorbisRawTell: TVorbisTellOffset; // @addr $88BE8C
  VorbisPcmTell: TVorbisTellOffset; // @addr $88BE90
  VorbisTimeTell: TVorbisTellTime; // @addr $88BE94
  VorbisInfo: TVorbisLinkInfo; // @addr $88BE98
  VorbisComment: TVorbisLinkInfo; // @addr $88BE9C
  VorbisReadFloat: TVorbisReadFloat; // @addr $88BEA0
  VorbisRead: TVorbisRead; // @addr $88BEA4

  VorbisUseCount: Integer; // @addr $88BEA8
  VorbisCallbacks: TVorbisCallbacks; // @addr $88BEAC
  VorbisLibrary: Cardinal; // @addr $88BEBC

procedure LoadVorbisLibrary; // @addr $7F4CF8
function ReadVorbisSource(Buffer: Pointer; Size, Count: Cardinal; Source: Pointer): Cardinal; cdecl; // @addr $7F498C @note "Returns bytes read, rather than fread's element count."
function OpenVorbisStream(Decoder: TOggWorker; var Format: TSoundWaveFormat; var Stream: TFileStreamEC): Integer; stdcall; // @addr $7F4AC4 @note "Always advertises stereo 44100-Hz signed 16-bit PCM; raises if ov_open_callbacks fails."

function ReadVorbisSamples(Decoder: TOggWorker; Buffer: Pointer; var ByteCount: Integer): Integer; stdcall; // @addr $7F4BDC
...
{ @routine $7F498C ReadVorbisSource }
...
{ @routine $7F49B4 TOggWorker_Create }
...
{ @routine $7F4A58 TOggWorker_Destroy }
...
{ @routine $7F4AC4 OpenVorbisStream }
...
{ @routine $7F4BDC ReadVorbisSamples }
...
{ @routine $7F4CF8 LoadVorbisLibrary }
