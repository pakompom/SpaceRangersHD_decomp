unit aSaveLoad;
// Unit bracket (inferred): .text 0x0060012C..0x00602695; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    procedure Execute; override; // @addr 0x600198 @note "Owns and frees all five buffers. Writes Save.tmp before replacing the destination; shares SaveLoadLock with the loader."
    procedure QueueSave(AFileName: WideString; Header, Preview, SecondaryPreview, GameState, Films: TBufEC); // @addr 0x600854 @note "Waits for the preceding job, then takes ownership of all five buffers."
  end;
...
function SaveGameToFile(FileName, Description: WideString): Boolean; // @addr 0x6008F8 @note "Queues the write; true does not mean the background writer has finished."
function LoadGameFromFile(FileName: WideString): Boolean; // @addr 0x601150 @note "Replaces the current galaxy."
procedure LoadGameFromSaveBuffer(Buffer: TBufEC); // @addr 0x60106C @note "Requires an existing galaxy object and a decoded buffer positioned at the player-hold section."
procedure SaveGameToMemorySnapshot; // @addr 0x60196C @note "Requires a live galaxy/player and no outstanding snapshot. Obfuscates and detaches the live galaxy until restoration; does not increment the persistent save count."
procedure RestoreGameFromMemorySnapshot; // @addr 0x601CC8 @note "Consumes the snapshot, destroys the detached galaxy and rebuilds it. Preserves the persistent load count and restores UI references by object ID."
procedure InitializeSaveWriter; // @addr 0x60264C
procedure FinalizeSaveWriter; // @addr 0x602660 @note "Waits for a pending write before freeing the worker."

var
  MemorySnapshotBuffer: TBufEC = nil; // @addr 0x87B6B4
  SaveLoadLock: TCriticalSection = nil; // @addr 0x87B6B8
  SaveWriter: TSaver = nil; // @addr 0x87B6BC
  MemorySnapshotXorSeed: Integer; // @addr 0x889C0C
  MemorySnapshotGalaxyToken: Cardinal; // @addr 0x889C10 @note "Detached galaxy address plus 0x17557455, modulo 2^32."
...
{ @routine $600854 TSaver_QueueSave }
...
{ @routine $60106C LoadGameFromSaveBuffer }
...
{ @routine $601150 LoadGameFromFile }
...
{ @routine $60196C SaveGameToMemorySnapshot }
...
{ @routine $601CC8 RestoreGameFromMemorySnapshot }
...
{ @routine $60264C InitializeSaveWriter }
procedure InitializeSaveWriter;
begin SaveWriter := TSaver.Create; end;
{ @end $60264C }

{ @routine $602660 FinalizeSaveWriter }
