unit aAsteroid;
// Unit bracket (inferred): .text 0x00769418..0x0076A1C2; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create; // @addr 0x769470 @ida "TAsteroid *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x7694D8 @ida "void __usercall $name(TAsteroid *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Init(Star: TStar; const GraphKey: WideString); // @addr 0x769520 @note "Requires an unassigned GraphObject."
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x769588
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); // @addr 0x76960C @note "Caller sets CurrentStar. Requires an unassigned GraphObject."
    procedure RespawnIfOutsideSystem; // @addr 0x76977C
    procedure PrepareTurnMovement(StartStepIndex: Integer; RecordFilm: Boolean); // @addr 0x7697C4
    procedure AdvanceOrbitStep(StepIndex: Integer; RecordFilm: Boolean); // @addr 0x769830
    procedure Respawn; // @addr 0x76988C @note "Keeps the ID and visual. May spawn another asteroid under the galaxy's special mode."
    procedure SpawnSiblingAsteroidInCurrentStar; // @addr 0x769B68 @note "The new asteroid belongs to CurrentStar.Asteroids; it does not copy this asteroid's visual or motion."
    procedure IntegrateMotion(TimeScale: Single); // @addr 0x769D94 @ida "void __userpurge $name(TAsteroid *Self@<eax>, float TimeScale@<^0>);"
    procedure WritePredictedPositions(Positions: PPointF; Count: Integer); // @addr 0x769F04 @note "Writes Count future positions at TimeScale=1, excluding the current position, then restores the live motion state. Caller supplies Count * 8 bytes."
    function GetDisplayName: WideString; // @addr 0x769FE0 @ida "void __usercall $name(TAsteroid *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetInfoText: WideString; // @addr 0x76A0B4 @ida "void __usercall $name(TAsteroid *Self@<eax>, unsigned __int16 **Result@<edx>);"
  end;

const
  AsteroidGravitationalConstant: Single = 6.672041391597716e-11; // @addr $87C0F4
...
{ @routine $769470 TAsteroid_Create }
...
{ @routine $7694D8 TAsteroid_Destroy }
...
{ @routine $769588 TAsteroid_SaveToBuffer }
...
{ @routine $76960C TAsteroid_LoadFromBuffer }
...
{ @routine $76977C TAsteroid_RespawnIfOutsideSystem }
...
{ @routine $7697C4 TAsteroid_PrepareTurnMovement }
...
{ @routine $769830 TAsteroid_AdvanceOrbitStep }
...
{ @routine $76988C TAsteroid_Respawn }
...
{ @routine $769B68 TAsteroid_SpawnSiblingAsteroidInCurrentStar }
...
{ @routine $769D94 TAsteroid_IntegrateMotion }
...
{ @routine $769F04 TAsteroid_WritePredictedPositions }
...
{ @routine $769FE0 TAsteroid_GetDisplayName }
...
{ @routine $76A0B4 TAsteroid_GetInfoText }
