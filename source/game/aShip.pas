unit aShip;
// Unit bracket (inferred): .text 0x006F7B6C..0x0072F5C8; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    ChameleonDisplayCount: Integer; // @offset $464 Serialized counter displayed in active chameleon info ($6F35F0); no gameplay update recovered.
    ...
    constructor Create; // @addr 0x6F7CB0 @ida "TShip *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x6F8110 @ida "void __usercall $name(TShip *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure SaveToBuffer(Buffer: TBufEC); virtual; // @addr 0x6F87A8 @slot 0x00
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); virtual; // @addr 0x6F9670 @slot 0x04
    procedure ResolveLoadedReferences(Galaxy: TGalaxy); virtual; // @addr 0x6FD438 @slot 0x08 @note "Converts saved IDs to object references; requires all referenced objects to have been loaded."
    procedure ClearObjectReferences; virtual; // @addr 0x6FDA5C @slot 0x0C @note "Clears navigation, docking and ship references, including item references; preserves CurrentStar, ScriptShip and LiberationGroup."
    procedure SaveToBlock(Block: TBlockParEC); virtual; // @addr 0x6FAB04 @slot 0x10 @note "Editable subset; not a full save."
    procedure LoadFromBlock(Block: TBlockParEC); virtual; // @addr 0x6FBCE0 @slot 0x14 @note "Applies edits to existing state and can append items or issue new orders. Cargo quantities change without updating cost basis."
    procedure NextDay; virtual; // @addr 0x6FE39C @slot 0x18 @calls "0x863BD6 0x84D0BB"
    procedure NextDayLogic; virtual; // @addr 0x6FF00C @slot 0x1C
    procedure ProcessBrokenFuelTankLeak; // @addr 0x6FDC88 @note "Can cancel the player's jump when remaining fuel is insufficient."
    procedure RefreshTechKnowledgeAtLocation; // @addr 0x6FDFA8 @note "Can unlock carried equipment use/repair and notify the player."
    procedure DerivedStateCompatibilityHook; // @addr 0x6FF058 @note "Native no-op, called after loading/editing and refreshing derived stats."
    procedure AssignWeaponTargetsInStar; virtual; // @addr 0x6FF018 @slot 0x20 @note "The base implementation clears all weapon targets."

    // Abstract entries in native TShip VMT 0x6F7BB8 share the RTL stub at 0x40358C.
    ...
    function GetFullName(const Separator: WideString): WideString; virtual; abstract; // @slot 0x28 @calls "0x727E77 0x727EA7 0x727ED5" @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 *Separator@<edx>, unsigned __int16 **Result@<ecx>);"
    ...
    procedure TransferToStar(Star: TStar); // @addr 0x7022B0 @note "Moves Self and docked ships between star lists; does not clear InHyperspace."
    procedure TryRelocateUnseenShip; // @addr 0x702568 @note "Can subsidize and queue an unseen NPC for relocation to a peaceful Coalition system; excludes protected quest/script/partner ships."
    procedure LeaveLiberationGroup; // @addr 0x70E3A8 @note "Requires a current liberation group."
    procedure ProcessLiberationGroupRoute; // @addr 0x70DE38 @note "Requires an in-range route index when a group is assigned; may leave the group or issue travel/combat orders."
    procedure ReassignActiveItemSlots(ItemType: TItemType); // @addr 0x72457C @note "Repairs duplicate/out-of-range equipped slots; unequips overflow and preserves slot flag bit 7."
    procedure RefreshAssignedItemSlots; // @addr 0x7247B8
    procedure RefreshInactiveItemSlotAssignments; // @addr 0x724914
    procedure RepairDuplicateSatelliteTrajectoryIndices; // @addr 0x724A3C
    function GetSatelliteTrajectoryIndexLimit: Integer; // @addr 0x724B14 @note "One past the largest carried satellite index, or zero."
    function FindFreeSatelliteTrajectoryIndex: Integer; // @addr 0x724BA4
    function FindSatelliteByTrajectoryIndex(Index: Integer): TSatellite; // @addr 0x724C20
    procedure InsertSatelliteTrajectoryIndex(Index: Integer); // @addr 0x724C98 @note "Shifts indices at or above Index upward."
    procedure RemoveEmptySatelliteTrajectoryIndex(Index: Integer); // @addr 0x724D10 @note "Does nothing if Index is occupied."
    procedure CompactSatelliteTrajectoryIndices; // @addr 0x724D98
    procedure AssignSatelliteIndicesFromHoldOrder; // @addr 0x724DF8 @note "Uses and refreshes global hold-view state."
    procedure ArrangeHoldSatellitesByTrajectoryIndex; // @addr 0x725040 @note "Reorders the satellite entries in the global hold view."
    function UseDominatorTransmitter(Artefact: TArtefactTransmitter): Boolean; // @addr 0x72538C @note "Requires minimum charge; records a galaxy event and consumes charge even if no Dominators respond."
    function HasMatchingArtefactOrCustomItem(Item: TItem): Boolean; // @addr 0x725988 @note "Custom items match by configuration name; generic artefacts also compare names."
    procedure ApplyNanoArtefactRepair; // @addr 0x72683C @note "Prefers installed inventory equipment; selects at most one repairable item and may clear BrokenFlag."
    function CanContactShip(OtherShip: TShip): Boolean; // @addr 0x726B04 @note "Uses Self's radar with a 500-unit minimum and both NoTalk flags; does not test system membership."
    function OpenPlayerConversation(RespectChameleon: Boolean): Boolean; // @addr 0x7270D4 @note "Turn-worker/UI handshake; waits for conversation completion or shutdown. Requires an active visible-space turn."
    function ShowPlayerDialogue(Kind: Byte; const Text: WideString; Amount: Integer): Byte; // @addr 0x727230 @note "Returns the global dialogue response, or zero when conversation cannot open. Amount only replaces the global amount when positive."
    procedure NotifyMoneyDemand(OtherShip: TShip; Response: WideString; Amount: Integer); // @addr 0x727294
    procedure NotifyCargoDemand(OtherShip: TShip; Response: WideString); // @addr 0x727540
    procedure NotifyFearCargoDrop(OtherShip: TShip); // @addr 0x7277A0
    procedure NotifyTruceOffer(OtherShip: TShip; Response: WideString; Amount: Integer); // @addr 0x727AA4
    procedure NotifyAttackRequest(OtherShip: TShip; Response: WideString; Target: TShip); // @addr 0x727D9C
    procedure NotifyPartnershipOffer(OtherShip: TShip; Response: WideString; Amount: Integer); // @addr 0x728090
    procedure NotifyPartnerBreak(Leader: TShip); // @addr 0x728364
    procedure NotifyPartnershipExpired(Leader: TShip); // @addr 0x728674
    procedure NotifyPartnerRebellion(Leader: TShip); // @addr 0x728A20
    procedure ShowMessageToPlayer(Text: WideString); // @addr 0x728D2C
    procedure NotifyPiratePartnerRelationBreak(Leader: TShip); // @addr 0x728E68
    procedure NotifyPiratePartnerRatingBreak(Leader: TShip); // @addr 0x729184
    procedure NotifyPiratePartnershipExpired(Leader: TShip); // @addr 0x72949C
    procedure NotifyPiratePartnerRebellion(Leader: TShip); // @addr 0x729840
    function UnknownVirtualC0(Argument: Pointer): Boolean; virtual; // @addr 0x729B48 @slot 0xC0 @note "Native base implementation returns false; argument role and intended operation remain unresolved."
    function CalculatePartnershipMonths(Amount: Integer; OtherShip: TShip): Integer; // @addr 0x729B60 @note "Payment/wealth and relation determine contract months; a player stimulant can double the result."
    function GetGreetingText: WideString; // @addr 0x729C5C @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    procedure InitializeScriptStateOrders; // @addr 0x72A764 @note "Requires ScriptShip; clears EndState, applies state orders and refreshes completion/pickup state."
    procedure ApplyScriptStateOrders; // @addr 0x72A7A0 @note "Requires ScriptShip; may issue travel orders and assign script-selected weapon targets."
    procedure UpdateScriptStateCompletionAndPickups; // @addr 0x72ACDC @note "Requires ScriptShip; updates EndState and queues state-requested pickups."
    function FindScriptFollowTarget: TShip; // @addr 0x72B324 @note "Requires ScriptShip; matches the state's group in the current system."
    procedure RemoveExperience(Amount: Integer); // @addr 0x72B5A8 @note "Subtracts independently from total and free experience, capped at each current balance."
    function CanTrainSkill(Skill: TPilotSkill): Boolean; // @addr 0x72B884
    function GetRelativeStrengthCategory: Byte; // @addr 0x72BCF8 @note "Returns a category from 1 to 5 using StrengthInBestRanger."
    function GetHullConditionCategory: Byte; // @addr 0x72BE5C @note "Categories 1..5 split rounded hull percentage at 20, 50, 70 and 90."
    function IsFemaleHumanPilot: Boolean; // @addr 0x72CF08 @note "Human portrait IDs 25..32 on normal NPC ships; excludes special simulation mode."
    function UsesVeteranHumanRangerAppearance: Boolean; // @addr 0x72CF74 @note "Native resource key RangerOldFag; deterministic ID/creation-turn selection, excluding female pilots and the player."
    function SelectInterceptorTarget: TShip; // @addr 0x72D1F0
    procedure LaunchInterceptors; // @addr 0x72D38C @note "Clears an explicit target before checking energy; successful launch installs source/pass state and a graphic on the target."
    procedure ClearIncomingInterceptors; // @addr 0x72D4EC @note "Clears source/pass state and releases the interceptor graphic."
    function CountActiveInterceptorTargets: Integer; // @addr 0x72D534 @note "Counts ships referring to Self as interceptor source across all galaxy systems."
    function GetHullEnergyRegeneration: Integer; // @addr 0x72D5C8
    function GetInterceptorEnergyCost: Integer; // @addr 0x72D6C0
    function GetInterceptorPassCount: Byte; // @addr 0x72D6E8 @note "Hull override or five when zero."
    procedure SetStatBonus(BonusKind: TEquipmentBonusKind; Value: Integer); // @addr 0x72D884 @note "Zero removes and frees the matching bonus entry; nonzero inserts or replaces it."
    procedure UpdateAfterburnerState; virtual; // @addr 0x72E540 @slot 0x9C @note "Native base implementation is a no-op."
    function HasScriptStateText: Boolean; // @addr 0x72E588
    function CheckDockingPermission(Ship: TShip; var Response: WideString): Boolean; virtual; // @addr 0x72F33C @slot 0xCC @note "Base implementation clears Response and returns false."

    procedure ScriptNextDay; // @addr 0x72AEC8 @note "Requires ScriptShip; script execution can remove the binding."
    function ScriptItemsAct(ActionType: Byte; Object1, Object2: TObject; Param: Integer): Integer; // @addr 0x72E77C @note "Returns Param after script handlers modify it; object slots may carry event-specific integer values."
    procedure RefreshCurrentStanding; virtual; // @addr 0x72E54C @slot 0xC4 @calls "0x863B87 0x86CC02 0x86CCA9"
    function ScanForCollectableItems: Boolean; // @addr 0x71B2B0
    function TryCollectBestFloatingItem(MaximumTravelTurns: Integer): Boolean; // @addr 0x71B44C @note "May queue nearby pickups and issue/cancel a move order; true means a move order remains."
    function AcceptPickupItem(Item: TItem): Boolean; virtual; // @addr 0x71B900 @slot 0x94 @note "Base implementation returns false."
    function AcceptPickupDistance(Item: TItem; Distance: Double): Boolean; virtual; // @addr 0x71BCE0 @slot 0x98 @ida "bool __userpurge $name@<al>(TShip *Self@<eax>, TItem *Item@<edx>, double Distance@<^0>);" @note "Base implementation returns true."
    procedure RemoveInvalidPickupTargets; // @addr 0x71BE14 @note "Frees the target list when it becomes empty."
    function GetPickupApproachPosition(ItemPosition: TPointF): TPointF; // @addr 0x71C1AC @ida "void __usercall $name(TShip *Self@<eax>, TPointF *ItemPosition@<edx>, TPointF *Result@<ecx>);"
    function GetCurrentPickupItem: TItem; // @addr 0x71C248
    function GetArrivalPosition(DestinationStar: TStar): TPointF; // @addr 0x71C590 @ida "void __usercall $name(TShip *Self@<eax>, TStar *DestinationStar@<edx>, TPointF *Result@<ecx>);" @note "Point on the destination map boundary facing the current system."
    function GetMovementPathTurnCount: Integer; // @addr 0x71CEAC @note "Ceiling of active path-node count times the star's MovementStepScale."
    function IsTravelCompletionPathReady: Boolean; // @addr 0x720FB8 @note "Tests landing, jump, hole and teleport completion conditions against the prepared path."
    procedure RepelFollowingShips; // @addr 0x7211C4 @note "Adjusts RepulsionPosition on Self and nearby following ships using their collision radii."
    procedure RebuildMovePath; // @addr 0x7217D8 @note "Uses the current turn's step limit; rewrites OrderDestination to the resulting endpoint."
    procedure BuildFullPathTo(Destination: TPointF); // @addr 0x721854 @ida "void __usercall $name(TShip *Self@<eax>, TPointF *Destination@<edx>);" @note "Clears the old path and uses aGroup, a 999999-node limit."
    procedure BuildPlanetLandingPath; // @addr 0x7218A8
    procedure BuildOrderMovementPath(MaximumNodes: Integer); // @addr 0x7219B0 @note "Clears the path, normalizes heading and handles the current order; may commit jump departure or adjust the destination."
    procedure AppendPathToWithTurnPadding(Destination: TPointF; MaximumNodes: Integer); // @addr 0x722848 @ida "void __usercall $name(TShip *Self@<eax>, TPointF *Destination@<edx>, int MaximumNodes@<ecx>);" @note "Turning then straight movement; may pad the player's visible turn to 200 nodes."
    procedure AppendPathTo(Destination: TPointF; MaximumNodes: Integer); // @addr 0x722964 @ida "void __usercall $name(TShip *Self@<eax>, TPointF *Destination@<edx>, int MaximumNodes@<ecx>);"
    procedure AppendStarAvoidingPathWithTurnPadding(Destination: TPointF; MaximumNodes: Integer); // @addr 0x722A10 @ida "void __usercall $name(TShip *Self@<eax>, TPointF *Destination@<edx>, int MaximumNodes@<ecx>);"
    procedure AppendOrbitalPath(MaximumNodes: Integer); // @addr 0x722B1C @note "Appends up to 200 nodes around the system origin, with visible-turn padding."
    procedure AppendTurningPath(Destination: TPointF; AvoidStar: Boolean; MaximumNodes: Integer); // @addr 0x722CC0 @ida "void __userpurge $name(TShip *Self@<eax>, TPointF *Destination@<edx>, bool AvoidStar@<cl>, int MaximumNodes@<^0>);"
    procedure AppendStraightPath(Destination: TPointF; MaximumNodes: Integer); // @addr 0x723524 @ida "void __usercall $name(TShip *Self@<eax>, TPointF *Destination@<edx>, int MaximumNodes@<ecx>);"
    procedure AppendHyperspaceTransitionPath(Direction: Single); // @addr 0x723868 @ida "void __userpurge $name(TShip *Self@<eax>, float Direction@<^0>);" @note "Positive/negative Direction selects the outgoing/incoming transition path."
    procedure AppendStarAvoidingPath(Destination: TPointF; MaximumNodes: Integer); // @addr 0x723A6C @ida "void __usercall $name(TShip *Self@<eax>, TPointF *Destination@<edx>, int MaximumNodes@<ecx>);"
    procedure AppendCircularDetour(Destination: TPointF; MaximumNodes: Integer; Radius: Double); // @addr 0x723DF4 @ida "void __userpurge $name(TShip *Self@<eax>, TPointF *Destination@<edx>, int MaximumNodes@<ecx>, double Radius@<^0>);"

    procedure PrepareTurnMovement(StartStepIndex: Integer; RecordFilm: Boolean); // @addr 0x71D054
    function ProcessMovementStep(StepIndex: Integer; RecordFilm: Boolean): Boolean; // @addr 0x720090 @note "The native result remains false."
    procedure ClearCompletedTakeoffOrHoleOrder(UnusedStepIndex: Integer; UnusedRecordFilm: Boolean); // @addr 0x720F5C
    procedure RefreshDerivedStats(UpdateRelativeRatings: Boolean); // @addr 0x70FF00
    procedure UpdateBestRangerRelativeRatings; // @addr 0x70020C
    procedure UpdateAverageRangerRelativeStrength; // @addr 0x700294 @note "Does not guard against zero AverageRangerStrength."
    function GetRangerRatingBand: Byte; // @addr 0x72BF6C @note "Zero for non-rangers; otherwise 1..5 from the rounded experience-rank percentile. Uses all galaxy rangers, including excluded entries."
    function GetCaptainPortraitResourceBase: WideString; // @addr 0x7009F4 @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);" @note "May assign PortraitFaceId lazily. Returns a resource base without the GI prefix or animation suffix."
    function IsInPrison: Boolean; // @addr 0x70CA24
    function GetPrisonTermRemaining: Integer; // @addr 0x70CA9C @note "Rangers and pirates only; other classes return zero."
    procedure ClearPrisonTerm; // @addr 0x70CAF4
    function GetPrisonReleaseCost: Integer; // @addr 0x70CB40 @note "Zero without a positive term; otherwise at least 100, using cached Wealth."
    function CalculateStrength: Double; // @addr 0x700768 @note "Updates Strength; calculating the player's strength also refreshes galaxy ranger strength statistics."
    function CalculateAttackStrength: Double; // @addr 0x700420
    function CalculateDefenseStrength: Double; // @addr 0x700628
    function GetRepairStrengthFactor: Double; // @addr 0x70070C
    function GetFullHullRelativeStrengthPercent: Byte; // @addr 0x7007D4 @note "Temporarily restores hull points. Leaves cached Strength and player galaxy-strength statistics at the full-hull values; byte result is not clamped."
    function GetEstimatedMemoryUsage: Integer; virtual; // @addr 0x6FFE04 @slot 0x44 @note "Counts the instance and selected list storage; excludes item objects and managed strings."
    function GetDefaultHullType: Byte; // @addr 0x6FF248
    function SelectRandomHullSeries: Integer; // @addr 0x71AF58 @note "Normal ships use PilotRace rather than OwnerId; selects rarity 1..100 through the galaxy RNG."
    function NextRandomInteger(Minimum, Maximum: Integer): Integer; // @addr 0x6FF338
    function GetTurnSeedFraction(TurnOffset: Integer): Single; // @addr 0x6FFDC0 @note "Fractional part of signed Seed divided by CurrentTurn + TurnOffset; denominator must be nonzero. Does not advance RandomState."
    function GetWealthScaledAmount(ScaleIndex: Byte): Integer; // @addr 0x70083C @note "Uses cached Wealth and the configured scale table; no index validation."
    function GetTypeNameKey: WideString; virtual; // @addr 0x6FF064 @slot 0x2C @calls "0x727EE9 0x75BDD7" @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetLocalizedTypeName: WideString; // @addr 0x6FF08C @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetFactionNameKey: WideString; // @addr 0x6FF1C0 @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetSpaceInfoText: WideString; // @addr 0x6FF368 @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);" @note "Requires a player; includes current order, hull, speed and relation information."
    function GetShipPortraitImagePath: WideString; // @addr 0x700908 @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);" @note "Leaves the result storage unchanged when Graphic is not a supported graphic class."
    procedure RefreshGraphicSize; // @addr 0x710078 @note "Requires Graphic; chooses dimensions from ship class, hull and special equipment."
    function GetCargoGoodsWeight: Integer; // @addr 0x710820
    function CalculateFollowRadius: Integer; // @addr 0x710854 @note "Requires a follow order; uses weapon ranges or the ships' collision radii."
    function GetFollowMode: Byte; // @addr 0x710A00 @note "Raises when the current order is not follow."
    function GetEffectiveFollowMode: Byte; // @addr 0x710A64 @note "Requires a follow order; applies tactical and map-edge adjustments without modifying OrderStateData."
    function NeedsEquipmentType(ItemType: TItemType): Boolean; // @addr 0x710B98
    function CountCarriedEquipmentByType(ItemType: TItemType): Integer; // @addr 0x710C84 @note "Skips inventory index zero; any weapon request counts all weapon types."
    function SelectBestUnequippedWeapon: TWeapon; // @addr 0x710D00
    procedure DropUnequippedItemsAndGoods; // @addr 0x710F78 @note "Preserves named script items; drops through the normal item/artefact helpers."
    procedure AddItemToPlayerStorage(Item: TItem; Location: TObject; Slot: Integer); // @addr 0x71105C @note "Always adds to the player's storage, even when Self is an NPC. Caller must detach the item from its previous owner. Negative Slot allocates a free slot."
    procedure MergeItemIntoPlayerStorage(Item: TItem; Location: TObject; Slot: Integer); // @addr 0x711154 @note "Transfers ownership; a successful goods/countable merge frees Item."
    procedure AddGoodsToPlayerStorage(Good: Byte; Quantity, Cost: Integer; Location: TObject; Slot: Integer); // @addr 0x711304 @note "Positive quantities only; Cost is the total cost basis, not a unit price."
    function StoreLooseInventoryAt(Location: TObject): Boolean; // @addr 0x7114C0 @note "Uses the player's storage filters. Returns true for an eligible location even if nothing was moved."
    function RetrieveStoredItems(Location: TObject): Boolean; // @addr 0x7116A8 @note "Transfers matching player storage entries into Self; returns location eligibility, not whether items were retrieved."
    procedure AutoEquipArtefacts; // @addr 0x711A6C
    function CalculateItemEffectiveness(Item: TItem): Single; // @addr 0x711E08 @note "Temporarily changes weapon installation and disables ChaoticRandom while evaluating equipment; restores them on the normal path."
    function EstimateWeaponDamageAgainstTypicalDefense(Item: TItem): Single; // @addr 0x7127F4 @note "Returns one for a non-weapon; adjusts weapon damage for galaxy technology and accuracy."
    function GetItemFuelTankCapacity(Item: TItem): Integer; // @addr 0x712CB8 @note "Zero unless Item is fuel tanks."
    function GetEquipmentEvaluationSynergyBonus(Item: TEquipment): Single; // @addr 0x713518
    function GetWeaponArtefactDamageFactor(Weapon: TWeapon): Single; // @addr 0x713F50
    procedure RefreshEquipmentEvaluationMetrics; // @addr 0x714274
    function EvaluateItem(Item: TItem; PriceMode: Byte): Single; // @addr 0x714598
    function AdjustItemEvaluation(Item: TItem; PriceMode: Byte; Effectiveness: Single): Single; virtual; // @addr 0x7145D4 @slot 0x50 @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, TItem *Item@<edx>, unsigned __int8 PriceMode@<cl>, float Effectiveness@<^0>);" @note "Price modes: 1 negated item cost, 3 resale value, 4 item cost; other modes omit the price term. Mode 0 also omits weight/fragility penalties; the supplied effectiveness is recomputed."
    function EvaluateStatBonus(BonusKind: TEquipmentBonusKind; Value: Integer): Single; virtual; // @addr 0x714A60 @slot 0x54
    function EvaluateWeaponDamage(Weapon: TWeapon; IncludeAdditiveBonuses: Boolean; BaseDamage: Single): Single; virtual; // @addr 0x7156B0 @slot 0x58 @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, TWeapon *Weapon@<edx>, bool IncludeAdditiveBonuses@<cl>, float BaseDamage@<^0>);"
    function EvaluateMicroModuleGain(Item: TEquipment; ModuleIndex: Integer): Single; virtual; // @addr 0x715CE0 @slot 0x5C @note "Evaluates a temporary clone; restores NextItemId but leaves LoadedSaveVersion set to CurrentSaveVersion. ModuleIndex is zero-based."
    procedure AutoApplyMicroModules; // @addr 0x715E84 @note "Consumes beneficial carried modules, preferring installed equipment; refreshes derived stats after each application."
    procedure DropCargoUntilNotOverloaded; // @addr 0x716508 @note "Sells at a location or jettisons in space. May flag a long-stranded NPC for destruction when overload cannot be resolved."
    procedure DropRandomCheapItemsOnDestruction(Count: Integer); // @addr 0x716DF0
    procedure DropRandomValuableItemsOnDestruction(Count: Integer); // @addr 0x717074
    procedure DropItemsForDominatorProgram(Count: Integer); // @addr 0x7170D4 @note "Requires a Dominator; used by dialogue and Script.DomikProgramm. Attempts at most 100 selections."

    procedure RefreshGraphic; // @addr 0x7017B0 @note "ScriptChameleon preserves the assigned graphic while updating GraphDominator."
    procedure CreateNormalGraphic; // @addr 0x701840
    procedure CreateDominatorGraphic; // @addr 0x7020DC
    function SelectChameleonVisualType: Byte; // @addr 0x7015A4
    function HasPlayerChameleonCharges: Boolean; // @addr 0x701568 @note "Reads the player rather than Self; requires a player."
    function IsPlayerChameleonEffectiveAgainstSelf: Boolean; // @addr 0x701698 @note "Requires a player; action-17 script handlers can override the default result."

    // Except OrderJumpHole, new orders respect AbsoluteScriptOrder.
    procedure OrderNone(OverrideScriptOrder: Boolean); // @addr 0x71C3F0
    procedure OrderMove(Destination: TPointF; Absolute: Boolean); // @addr 0x71C43C @ida "void __usercall $name(TShip *Self@<eax>, TPointF *Destination@<edx>, bool Absolute@<cl>);"
    procedure OrderJump(Star: TStar; Absolute: Boolean); // @addr 0x71C6B4
    procedure OrderJumpHole(Hole: THole; Absolute: Boolean); // @addr 0x71C75C @note "Does not check AbsoluteScriptOrder."
    procedure OrderLanding(Location: TObject; Absolute: Boolean); // @addr 0x71C870 @note "Location is a planet or dockable ship."
    procedure OrderTakeoff; // @addr 0x71C8FC
    procedure OrderFollowShip(Ship: TShip; FollowMode: Byte; Absolute: Boolean); // @addr 0x71CE50
    procedure OrderTeleport(Star: TStar; Destination: TPointF; TransitionData: Integer; Absolute: Boolean); // @addr 0x71C804 @ida "void __userpurge $name(TShip *Self@<eax>, TStar *Star@<edx>, TPointF *Destination@<ecx>, int TransitionData@<^4>, bool Absolute@<^0>);"
    procedure ClearMovementPath; // @addr 0x7240C0
    function HasLockedOrFollowOrder: Boolean; // @addr 0x700024 @note "True for OrderAbsolute, AbsoluteScriptOrder, or a follow-ship order."
    procedure CancelInvalidTravelOrder; // @addr 0x702B40 @note "Checks only ships in star space; clears the player auto-follow target when cancelling."
    procedure SynchronizeDockedLocation; // @addr 0x7028F0 @note "Copies the carrier's star and position, moving star-list membership when necessary. Does nothing without DockedTo."
    procedure OrderRandomFreeFlightMove; // @addr 0x703630
    function TryMirrorPartnerTravelOrders: Boolean; // @addr 0x702FB0 @note "Requires a ranger PartnerShip. True reports handled travel, not necessarily a changed or accepted order."
    function CanEscapePursuer(Pursuer: TShip): Boolean; // @addr 0x7036F8 @note "AI estimate using range, hull and travel time. Pursuer weapon range is evaluated using Self's bonuses."
    function FindNextStarTowardDestination(Destination: TStar; RequireFuelMargin: Boolean): TStar; // @addr 0x703A44 @note "Returns a borrowed intermediate star or nil. Uses raw full-fuel range and current distance caches; excludes sector 20. A direct reachable destination returns the current star."
    function IsTargetStillPursuable(Target: TShip): Boolean; // @addr 0x7036A4 @note "False only when Target is jumping and its distance is at least twice Self.Speed; does not validate shared star or speed."
    function HasLandablePlanetInStar(Star: TStar): Boolean; // @addr 0x703990
    function FindFirstInhabitedPlanetInStar: TPlanet; // @addr 0x702DD0 @note "Requires a nonempty planet list; returns the last planet if all are uninhabited."
    function FindNearestDockableStation(StandingMask: TStationStandingMask): TShip; // @addr 0x702CF0 @note "Types 6..13 whose CanDock(Self) succeeds. Zero mask permits every standing; does not independently filter hyperspace/docking."
    function GetFullFuelBaseJumpRange: Integer; // @addr 0x7039F8 @note "Minimum of raw tank Capacity and engine JumpRange; requires both items and ignores bonuses/condition."
    function DistanceToNearestShipByTypeMask(ShipTypeMask: TShipTypeMask): Double; // @addr 0x703D0C @note "Excludes Self but includes docked/hyperspace entries; no match returns sqrt(1000000000)."
    function EstimateOrderTravelTurns: Integer; // @addr 0x703DB4 @note "Uses rounded distance / (Speed + 1) + 1; unsupported orders return zero."
    function EstimateTravelTurnsToObject(Target: TObject): Integer; // @addr 0x703F78 @note "Accepts planet, ship or star; a star estimates travel to the current system boundary, excluding hyperspace transit. Other classes return zero."
    function EstimateTravelTurnsToPlanet(Planet: TPlanet): Integer; // @addr 0x704110 @note "Returns -1 for zero speed, nil planet/star, or a different star."
    function CanDock(Ship: TShip): Boolean; virtual; // @addr 0x72F324 @slot 0xC8 @note "Base implementation always returns false."
    function CalculateJumpTravelDays(Origin, Destination: TStar): Integer; // @addr 0x71C634 @note "Minimum two days; independent of equipment and fuel."
    function GetJumpDeparturePoint(Destination: TStar): TPointF; // @addr 0x71C4B4 @ida "void __usercall $name(TShip *Self@<eax>, TStar *Destination@<edx>, TPointF *Result@<ecx>);"

    function LookupTalkText(const Path: WideString): WideString; // @addr 0x726C44 @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 *Path@<edx>, unsigned __int16 **Result@<ecx>);" @note "Selects among at most ten contiguous variants using ship seed and turn; substitutes ship names and HomePlanet. Missing text returns an unavailable marker."
    function LookupVisibleTalkText(const Path: WideString; OtherShip: TShip): WideString; // @addr 0x70012C @ida "void __userpurge $name(TShip *Self@<eax>, unsigned __int16 *Path@<edx>, TShip *OtherShip@<ecx>, unsigned __int16 **Result@<^0>);" @note "Returns empty unless the player shares CurrentStar. Substitutes OtherShip for <TalkShip>."
    function RelationToShip(Ship: TShip): TPercent; // @addr 0x72F360
    function GetRelationLevelToShip(Ship: TShip): TRelationLevel; // @addr 0x70BE0C
    function GetRelationLevelTextToShip(Ship: TShip): WideString; // @addr 0x70BE74 @ida "void __usercall $name(TShip *Self@<eax>, TShip *Ship@<edx>, unsigned __int16 **Result@<ecx>);" @note "Stations can display their stored ranger relation instead of the effective relation."

    function InNormalSpace: Boolean; // @addr 0x7029A4
    function IsOutsideStarSpace: Boolean; // @addr 0x702A2C @note "True without a current star, while docked/in hyperspace, or with a saved player ruins docking target."
    function IsOnPlanet: Boolean; // @addr 0x702A94
    function IsDockedToShip: Boolean; // @addr 0x702AB0
    function IsDocked: Boolean; // @addr 0x702ACC
    function IsHullDestroyed: Boolean; // @addr 0x6FFDA0
    function HasPositiveSpeed: Boolean; // @addr 0x702AF8
    procedure UpdateSpeedTrackingMetrics; // @addr 0x7141E8
    function HasHullDamageOrBrokenEquippedItems: Boolean; // @addr 0x700064
    procedure FireWeaponAtAsteroid(Weapon: TWeapon; Target: TObject; RecordFilm: Boolean); // @addr 0x709798 @note "Ignores non-asteroid targets. Respawning preserves the target object but invalidates its previous motion and mineral reserve."
    procedure FireWeaponAtMissile(Weapon: TWeapon; Target: TObject; RecordFilm: Boolean); // @addr 0x707160 @note "Ignores non-missile targets; can free the target and nearby missiles."
    procedure FireWeaponAtShip(Weapon: TWeapon; Target: TShip; RecordFilm: Boolean); // @addr 0x707CD4 @note "Can affect additional ships through chained, area or penetrating fire."
    procedure FireWeaponAtItem(Weapon: TWeapon; Target: TItem; RecordFilm: Boolean); // @addr 0x7090B0 @note "Weapon may be nil. Script handlers can change the target; a destroyed item may explode and be freed."
    function ApplyDamage(Source: TObject; Damage: Integer; HitRange: Single; out DamageColor: Cardinal; DamageFlags: TDamageFlagSet): Integer; // @addr 0x704490 @ida "int __userpurge $name@<eax>(TShip *Self@<eax>, TObject *Source@<edx>, int Damage@<ecx>, float HitRange@<^8>, unsigned int *DamageColor@<^4>, unsigned int DamageFlags@<^0>);" @note "Source may be nil, ship or missile. HitRange=-1 selects direct-hit rules; other values select area-hit rules. Returns adjusted damage, zero for rejection, or negative damage for an impulse-shield block; not actual hull loss. May run death handling without freeing Self."
    function ApplyWeaponHit(Source: TShip; Weapon: TWeapon; HitRange: Single; out DamageColor: Cardinal; out DamageFlags: Dword; DamageScale: Single; FixedDamage: Integer): Integer; // @addr 0x70671C @ida "int __userpurge $name@<eax>(TShip *Self@<eax>, TShip *Source@<edx>, TWeapon *Weapon@<ecx>, float HitRange@<^16>, unsigned int *DamageColor@<^12>, unsigned int *DamageFlags@<^8>, float DamageScale@<^4>, int FixedDamage@<^0>);" @note "Returns ApplyDamage's signed result. Positive FixedDamage bypasses the initial roll/armor stage unless weapon flag 0x800 is already set; later effects still apply."
    function ApplyMissileHit(Missile: TObject; out DamageColor: Cardinal; out DamageFlags: Dword): Integer; // @addr 0x706E00 @note "Requires a TMissile; returns ApplyDamage's signed result."
    function ApplyInterceptorDamage(out DamageColor: Cardinal): Integer; // @addr 0x7099F4 @note "Uses InterceptorSourceShip; absent source gives base damage 25. Returns ApplyDamage's signed result."
    function GetInterceptorDamage: Integer; // @addr 0x72D664
    function ApplyShockStatusDamage(out DamageColor: Cardinal): Integer; // @addr 0x709B30 @note "Uses rounded shock strength and nonlethal flag 0x1000; returns ApplyDamage's signed result."
    function ApplyExplosionDamage(SourceShip: TShip; ExplodingObject: TObject; ExtraDamage: Integer; Missile: TObject): Integer; // @addr 0x709F30 @note "ExplodingObject may be an item or ship; SourceShip and Missile may be nil. Returns script-adjusted damage, not actual hull loss; can trigger death handling without freeing Self."
    function ApplyStarHeatDamage: Integer; // @addr 0x70A428 @note "Also refuels up to five units inside the damage radius. Returns script-adjusted damage; Dominator bosses survive with at least one hull point."
    procedure NotifyCompanionDeath; // @addr 0x70418C @note "Ranger, pirate or Tranclucator death notification; does not itself check player partnership."
    function ApplyAsteroidImpactDamage(Asteroid: TAsteroid; out DamageColor: Cardinal): Integer; // @addr 0x709B74 @note "Returns script-adjusted damage, which may exceed actual hull loss. DamageColor uses the current packed pixel format and is zero for nonpositive damage."
    function GetDesiredCargoFreeSpace: Integer; virtual; // @addr 0x6FFD88 @slot 0x40
    procedure ClearPlanetQueue; // @addr 0x702BD4
    procedure TruceWithShip(Ship: TShip); // @addr 0x70C1E0 @note "Cancels reciprocal attacks and pursuers, including wingmen/Tranclucators and missiles. Only the player's TruceShip is set reciprocally."
    procedure SetJointAttackTarget(Ally, Target: TShip); // @addr 0x70B970 @note "Assigns both ships' enemies and eligible weapons, then issues pursuit orders where possible."
    function IsEnemyPursuingSelf: Boolean; // @addr 0x70BB50
    function TryExtortShip(Target: TShip): Boolean; // @addr 0x70BF44 @note "Attempts a cargo or money demand through the target's dialogue handler; requires a player."
    procedure RequestAlliesAttackShip(Target: TShip); // @addr 0x70C5E8 @note "May ask the player or nearby allied NPCs; requires a player."
    procedure SetStoredRangerRelationLevel(Ranger: TShip; Level: TRelationLevel); // @addr 0x70BC70 @note "Requires a non-nil relation list; nonempty lists require a registered ranger. Stores representative values 5, 20, 45, 70 or 90."
    function SelectNearestQueuedPlanet: TPlanet; // @addr 0x702C04 @note "Requires a non-nil queue ordered with current-star planets first; otherwise falls back to the first entry. Empty queue returns nil."
    function NavigateToQueuedPlanet(Absolute: Boolean): TPlanet; // @addr 0x702E3C @note "Can build PlanetQueue and issue a landing or jump order. Result is borrowed and may be nil."
    function NavigateToEscapePlanet(Absolute: Boolean): Boolean; // @addr 0x702ECC @note "Uses the existing PlanetQueue; false means it is nil or empty. True does not guarantee that the order was accepted."
    procedure SetMoney(Value: Integer); // @addr 0x6FDBB8 @note "Clamps to 0..100000000."
    function CalculateWealth: Integer; // @addr 0x7002BC @note "Updates Wealth. Includes player storage and accrued deposit but does not subtract debt; capped at MaxInt."
    function CalculateEquippedItemCostWithoutHull: Integer; // @addr 0x710BDC @note "Includes equipped artefacts."

    function GetHull: THull; // @addr 0x70E5E0
    function GetFuelTanks: TFuelTanks; // @addr 0x70E5FC
    function GetEngine: TEngine; // @addr 0x70E618
    function GetRadar: TRadar; // @addr 0x70E634
    function GetScanner: TScaner; // @addr 0x70E650
    function GetRepairRobot: TRepairRobot; // @addr 0x70E66C
    function GetCargoHook: TCargoHook; // @addr 0x70E688
    function GetDefGenerator: TDefGenerator; // @addr 0x70E6A4
    function CanUseEquipmentTech(Item: TEquipment): Boolean; // @addr 0x70E764
    function CanRepairEquipmentTech(Item: TEquipment): Boolean; // @addr 0x70E834
    function IsEquipmentUsable(Item: TEquipment): Boolean; // @addr 0x70E934 @note "Does not require EquippedFlag."
    procedure RebuildEquipmentCache; // @addr 0x7105A8 @note "Can unequip items whose slots are unavailable."
    procedure EquipItem(Item: TEquipment); // @addr 0x70E980 @note "Does not add Item to the inventory."
    procedure UnequipSlot(ItemType: Byte; WeaponIndex: Integer); // @addr 0x70EA50 @note "Direct types 42..49 require a populated slot; weapon types 50..68 require a valid one-based WeaponIndex. Compacts the weapon cache; does not remove/free inventory."
    procedure UnequipItem(Item: TEquipment); // @addr 0x70EB28 @note "Only affects cached installed equipment. Requires non-nil Item."
    function CanRepairArtefactsAtLocation: Boolean; // @addr 0x70E6C0 @note "Pirate/science bases, licensed station names and the main pirate planet; follows DockedTo recursively."
    function CountEquippedWeapons: TWeaponCount; // @addr 0x70A974
    function CountMissileWeapons: TWeaponCount; // @addr 0x70A9E0 @note "Counts torpedo/missile/rocket shot types; does not check usability or ammunition."
    function CountDirectFireWeapons: Byte; // @addr 0x70AA34 @note "Complement of CountMissileWeapons over cached equipped weapons."
    function CountWeaponsByDamageFlags(Flags: TDamageFlagSet): TWeaponCount; // @addr 0x70AB10 @note "Counts any intersection; does not check usability or ammunition."
    procedure ClearUnequippedWeaponTargets; // @addr 0x70AA88
    procedure ClearWeaponTargets(Target: TObject); // @addr 0x70B254 @note "Nil clears every cached weapon target; otherwise clears only matches."
    function IsAttackingShip(Target: TShip): Boolean; // @addr 0x70B2E8 @note "Includes weapon targets, interceptor attribution and shock/acid source IDs; requires non-nil Target."
    function CanSafelyDetonateItem(Item: TItem): Boolean; // @addr 0x70BBB0 @note "Rejects bomb/explosive cargo near non-hostile normal-space ships, including Self. Does not test shot range."
    function GetWeaponDamageSummary: WideString; // @addr 0x70AB70 @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetManeuverabilitySummary: WideString; // @addr 0x70AE84 @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetRepairPointsSummary: WideString; // @addr 0x70AF20 @ida "void __usercall $name(TShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function HasScannerArtefact(UnusedTarget: TShip): Boolean; // @addr 0x70AFA8 @note "Callers pass the target ship in EDX. This routine ignores it and only checks Self's active scanner artefact count."
    function GetWeaponActionRange(Weapon: TWeapon): Integer; // @addr 0x70AFCC @note "Same result as GetWeaponRange in this binary."
    function HasNoUsableWeapons: Boolean; // @addr 0x72D770
    function GetWeaponRange(Weapon: TWeapon): Integer; // @addr 0x712A24
    function GetWeaponSlotRange(SlotIndex: Integer): Integer; // @addr 0x70AFF0 @note "Weapon slots are numbered 1..5."
    function GetMaxWeaponRange: Integer; // @addr 0x70B1E8
    function ChanceToWin(Target: TShip): Double; // @addr 0x70B380 @note "Returns a combat strength ratio, not a probability."
    function GetWinChancePercent(Target: TShip): TPercent; // @addr 0x70B8E0
    function GetWeaponMinDamage(Weapon: TWeapon): Integer; // @addr 0x70B01C
    function GetWeaponMaxDamage(Weapon: TWeapon): Integer; // @addr 0x70B058
    function GetAttackMultiplier: Integer; // @addr 0x70FDF4
    procedure ReloadWeaponAmmo; // @addr 0x718C5C

    function CreateAndEquipHull(Capacity: Word; Level, Owner: Byte; Series: Integer; PirateBuilt: Boolean): THull; // @addr 0x71AED0
    function CreateAndEquipFuelTanks(Weight: Integer; Level, Owner: Byte): TFuelTanks; // @addr 0x71AFCC
    function CreateAndEquipEngine(Weight: Integer; Level, Owner: Byte): TEngine; // @addr 0x71B028
    function CreateAndEquipRadar(Weight: Integer; Level, Owner: Byte): TRadar; // @addr 0x71B084
    function CreateAndEquipScanner(Weight: Integer; Level, Owner: Byte): TScaner; // @addr 0x71B0E0
    function CreateAndEquipRepairRobot(Weight: Integer; Level, Owner: Byte): TRepairRobot; // @addr 0x71B13C
    function CreateAndEquipCargoHook(Weight: Integer; Level, Owner: Byte): TCargoHook; // @addr 0x71B198
    function CreateAndEquipDefGenerator(Weight: Integer; Level, Owner: Byte): TDefGenerator; // @addr 0x71B1F4
    function CreateAndEquipWeapon(ItemType: Byte; Weight: Integer; Level, Owner: Byte): TWeapon; // @addr 0x71B250

    function GetEquipmentStatBonus(BonusKind: Byte; Item: TEquipment): Integer; // @addr 0x70F578
    function GetTotalStatBonus(BonusKind: Byte): Integer; // @addr 0x70F5C0
    function GetOwnStatBonus(BonusKind: Byte): Integer; // @addr 0x72D804

    function CalculateHullArmor(Hull: THull): Integer; // @addr 0x712BF4
    function CalculateEngineSpeed(Engine: TEngine; ApplyBrokenPenalty: Boolean): Integer; // @addr 0x712CE8
    function CalculateEngineJumpRange(Engine: TEngine): Integer; // @addr 0x712E58
    function CalculateRadarRange(Radar: TRadar): Integer; // @addr 0x712F90
    function CalculateScannerPower(Scanner: TScaner): Integer; // @addr 0x713044
    function CalculateRepairPoints(RepairRobot: TRepairRobot): Integer; // @addr 0x7130F8
    function CalculateCargoHookPower(CargoHook: TCargoHook): Integer; // @addr 0x7131AC
    procedure AutoEquipInventory; // @addr 0x716124
    function DropCarriedItemAsMovingLoot(Item: TItem): Boolean; // @addr 0x717340 @note "Requires membership in Inventory; honors ship/item NoDrop and rejects the hull. True means accepted, even if a script consumes the drop."
    function DropCarriedArtefactAsMovingLoot(Item: TArtefact): Boolean; // @addr 0x7173CC @note "Requires membership in Artefacts; honors ship/item NoDrop."
    procedure DropGuaranteedDeathDropItems; // @addr 0x717478
    function CanDropTreasureMap: Boolean; // @addr 0x7174FC @note "Deterministic eligibility roll for an ordinary pirate without a script binding or player partnership."
    function SelectTreasureMapPlanet: TPlanet; // @addr 0x717578 @note "Searches reachable uninhabited planets with accessible artefact/module loot; excludes maps already carried or stored by the player."
    function TryDropTreasureMap: Boolean; // @addr 0x7177FC
    function DropItemIntoStar(Item: TItem): Boolean; // @addr 0x717884 @note "Player drops are placed immediately and may be consumed by scripts or stellar heat. Native artefact branch mistakenly deletes Inventory at the Artefacts index (0x717964); preserved here as observed behavior."
    procedure DropAllArtefactsOnDestruction; // @addr 0x717C28
    procedure DropGoodsIntoSpace(Good: Byte; Count: Integer); // @addr 0x717C74 @note "Caller supplies a valid good and quantity; does not itself honor ship NoDrop."
    function JettisonCargoGoodsTowardTargetValue(TargetValue: Integer): Boolean; // @addr 0x717CF8
    procedure DropAllCargoGoods; // @addr 0x717E74
    procedure QueueMovingItemDrop(Item: TItem; UseFlag: Byte); // @addr 0x717EC0 @note "Caller detaches Item first. Script action 33 can suppress transfer or free Item; otherwise the moving-drop descriptor takes ownership."
    function SelectLeastValuableInventoryItem: TItem; // @addr 0x71814C @note "Considers protection, essential equipment and value per mass; can return the hull when no alternative qualifies."
    function SelectLeastValuableArtefact: TArtefact; // @addr 0x7182D8 @note "Native scan starts at index one; an artefact list with one element yields nil."
    function SelectCheapestCargoGood: Byte; // @addr 0x7183B4 @note "Returns 255 if no cargo qualifies."
    procedure LiquidateInventoryItem(Item: TItem); // @addr 0x718930 @note "Normally credits resale value and frees Item; eligible NPC node stacks instead feed DepositCarriedNodes and automatic training."
    procedure LiquidateArtefact(Item: TArtefact); // @addr 0x718A58 @note "Removes and frees Item after crediting resale value."
    procedure ApplyCombatItemDegradation(BaseDurabilityDamage: Double); // @addr 0x718CB8 @ida "void __userpurge $name(TShip *Self@<eax>, double BaseDurabilityDamage@<^0>);"
    procedure ApplyArtefactUseDegradation(BaseDurabilityDamage: Double); // @addr 0x718F54 @ida "void __userpurge $name(TShip *Self@<eax>, double BaseDurabilityDamage@<^0>);"
    procedure ApplyAfterburnerItemDegradation; // @addr 0x719020
    function ApplyItemDegradation(Item: TEquipment; Kind: TItemDegradationKind; DurabilityDamage: Double): Boolean; // @addr 0x7190B8 @ida "bool __userpurge $name@<al>(TShip *Self@<eax>, TEquipment *Item@<edx>, TItemDegradationKind Kind@<cl>, double DurabilityDamage@<^0>);" @note "True only when the item becomes newly broken. Nil is accepted; script actions 36..39 can modify the damage."
    function CanGenerateMicroModuleForLoadout: Boolean; // @addr 0x7195B8
    function CanGenerateSpecialHullModule: Boolean; // @addr 0x7196F4 @note "True for the player or when the current hull already has a special module."
    procedure ImproveRandomEquipment(ResolveOverload: Boolean); // @addr 0x719728
    procedure GenerateAndApplyMicroModule(Item: TEquipment; ResolveOverload: Boolean); // @addr 0x719B24 @note "Selects up to three qualifying candidates within 51 attempts, applies the best positive gain, then auto-equips. Nil Item is accepted."
    procedure GenerateExtraWeapon; // @addr 0x719C8C @note "Adds a generated weapon, may improve/module it, then auto-equips and optimizes inventory."
    function NeedsMicroModule(ModuleIndexPlusOne: Integer): Boolean; // @addr 0x719ED4 @note "Requires an installable target and no carried module of the same index."
    function GetSatelliteLimit: Integer; // @addr 0x71A000
    function IsEssentialInventoryItem(Item: TItem): Boolean; // @addr 0x71A03C @note "Includes protected/script-named items and indispensable installed equipment; nil is false."
    function IsOptionalUtilityEquipment(Item: TItem): Boolean; // @addr 0x71A0EC @note "Radar/scanner, or cargo hook on a ship other than a ranger or pirate. Nil is false."

    procedure OptimizeInventory; // @addr 0x718530 @note "Can sell equipment, artefacts and excess cargo; outside a market, excess goods are refunded at their cost basis."
    function RepairHullAtLocation: Boolean; // @addr 0x718AC4 @note "Returns whether AI should remain docked; false does not guarantee full repair. Does not charge Money."
    function NeedsEssentialEquipment: Boolean; // @addr 0x71A138
    procedure RestoreEssentialEquipment; // @addr 0x71A190 @note "Can subsidize equipment purchases; does not guarantee success."
    procedure BuyEquipmentAtLocation(ForceGeneratedOffers: Boolean); // @addr 0x71A3EC
    function CalculateDefGeneratorFactor(DefGenerator: TDefGenerator): Single; // @addr 0x713260
    function GetRadarRange: Integer; // @addr 0x70F6F8
    function GetScannerPower: Integer; // @addr 0x70F78C
    function CanResolveObjectWithScanner(Target: TObject): Boolean; // @addr 0x70F7E8 @note "Ignores radar range and Dominator scanner series; non-ship targets require only a usable scanner."
    function GetCargoHookRange: Integer; // @addr 0x70FCC0
    function GetCargoHookMinPullSpeed: Single; // @addr 0x70FB08
    function GetCargoHookMaxPullSpeed: Single; // @addr 0x70FBE4
    function GetDefenseDamageFactor: Double; // @addr 0x70FD80 @note "1 means no damage reduction."
    procedure ApplyRepairDroidHealing; // @addr 0x70F8B0
    function GetHullIntegrityPercent: TPercent; // @addr 0x70EE74 @note "Not clamped to 0..100."
    function GetArmor: Integer; // @addr 0x70EEB8
    function GetJumpRange: Integer; // @addr 0x70F230 @note "Ignores fuel; broken engines retain 60% range."
    function GetFuelLimitedJumpRange: Integer; // @addr 0x70F190
    function GetCarriedItemWeight: Integer; // @addr 0x710784 @note "Excludes the hull."
    function CalculateMass: Integer; // @addr 0x70EBC0 @note "Uses cached CargoFreeSpace; includes cargo, artefact and status modifiers."
    function CalculateEquippedMass(ItemForModule: TEquipment): Integer; // @addr 0x70ECF4 @note "Excludes cargo and unequipped items. Uses the current hull, adding ItemForModule's micromodule mass bonus when supplied."
    function GetCargoFreeSpace: Integer; // @addr 0x710750 @note "Can be negative."
    function GetCarriedNodeCount: Integer; // @addr 0x6FFFC4
    function HasCargoGoods: Boolean; // @addr 0x6FFF54
    function CountCargoGoodsTypes: Byte; // @addr 0x6FFF8C @note "Counts positive cargo quantities."
    function FindCarriedItemById(Id: Cardinal): TItem; // @addr 0x702374 @note "Includes artefacts, guaranteed drops, stored Tranclucator inventories and station shop stock; result is borrowed."
    function CanRefuel: Boolean; // @addr 0x7000E8
    function GetFullRefuelCost: Integer; // @addr 0x70EF5C @note "Zero without tanks. Uses current planet owner, or owner six off-planet; overfilled tanks can produce a negative cost."
    function GetJumpDestinationDistance: Integer; // @addr 0x70EF0C @note "Truncated map distance for a jump order; zero otherwise."
    function GetAfterburnerWear: Integer; // @addr 0x70F0B0 @note "Uses engine owner and current turn; no engine gives one before artefact modifiers."
    function HasLooseNonScriptItemsOrGoods: Boolean; // @addr 0x710E8C
    function GetAverageCargoCost(Good: Byte): Double; // @addr 0x70D344 @note "Returns 0 for an empty cargo entry."
    function GetLocationGoodsEntry(Good: Byte): PGoodsTradePriceEntry; // @addr 0x70CBA8 @note "Result is borrowed; raises when no market is available."
    function ShopGoodsPurchasePrice(Good: Byte; Location: TObject): Integer; // @addr 0x70CE54 @note "Location is a planet or station; nil selects the current trade context."
    function ShopGoodsSellPrice(Good: Byte; Location: TObject): Integer; // @addr 0x70D09C @note "Includes the Trading skill bonus. Location=nil selects the current trade context."
    procedure ConsumeCargoGoods(Good: Byte; Count: Integer); // @addr 0x70D394 @note "Count is not clamped; remaining cost basis uses the previous average price."
    procedure BuyGoodsFromLocation(Good: Byte; Count: Integer); // @addr 0x70DB5C @note "Checks stock and cash, but not free cargo space or negative Count."
    procedure SellGoodsToLocation(Good: Byte; Count: Integer); // @addr 0x70D4A8 @note "Rejects Count above carried stock; does not reject a negative Count. Player trade losses offset later profit before trade experience is awarded."
    function IsCargoGoodIllegalOnCurrentPlanet(Good: Byte): Boolean; // @addr 0x70D410

    function GetSlotCount(SlotKind: TShipSlotKind): Integer; // @addr 0x7240D8
    function GetSlotCountForItemType(ItemType: Byte): Integer; // @addr 0x724554
    // Slot indices are zero-based.
    function FindEquippedItemInSlot(ItemType: Byte; SlotIndex: Integer): TEquipment; // @addr 0x724824
    function CountUnequippedItemsInSlot(SlotIndex: Integer): Integer; // @addr 0x724994
    function FindFreeUnequippedSlot: Integer; // @addr 0x724A0C
    function HasScriptControl: Boolean; // @addr 0x72D720
    function HasScriptBindings: Boolean; // @addr 0x7008C4 @note "For the player, checks the script-binding list; for NPC ships, checks ScriptShip."
    function HasIndependentScriptFaction: Boolean; // @addr 0x72E5C4 @note "Requires a nonempty faction not beginning with SubFaction. The native substring result is used as Boolean, so absence also returns true."
    function HasNamedScriptFaction: Boolean; // @addr 0x72E638 @note "Requires a nonempty faction other than the exact SubFactionFixedStanding marker."
    function GetScriptStandingOverrideMode: Integer; // @addr 0x72E6C0 @note "0 normal, 1 independent faction, 2 fixed standing. The SubFaction substring test accepts absence as mode one."

    function CountActiveArtefacts(ArtefactType: Byte): Integer; // @addr 0x725A74 @note "Uses custom SharedEffect types and excludes broken items. Activation exceptions can count some unequipped artefacts."
    function HasEquippedArtefactOfSameUseGroup(Item: TItem): Boolean; // @addr 0x725B38 @note "Uses custom SharedUse and ConfigBlockName. Includes Item itself if equipped, and does not exclude broken items."
    function CanBoostArtefact(ArtefactType: Byte; Item: TEquipment; IgnoreArtefactAvailability: Boolean): Boolean; // @addr 0x725CB8 @note "Item=nil checks cached installed equipment. A supplied item need not be equipped; eligible equipment types depend on ArtefactType."

    procedure AddPickupTarget(Item: TItem; Prioritize: Boolean); // @addr 0x71BCFC @note "Existing targets keep their position."
    procedure RemovePickupTarget(Item: TItem); // @addr 0x71BD74
    procedure ClearPickupTargets; // @addr 0x71BDE4
    function HasPickupTarget(Item: TItem): Boolean; // @addr 0x71BFC0
    function GetReservedPickupWeight: Integer; // @addr 0x71B844
    function CountOtherShipsTargetingItem(Item: TItem): Integer; // @addr 0x71BFFC
    procedure TogglePickupTargets(IgnoreRange: Boolean); // @addr 0x71BEEC @note "Adds missing eligible targets; removes eligible targets only if none were added."
    procedure QueueItemsWithinPickupRange; // @addr 0x71B390
    function IsItemInPickupRange(Item: TItem): Boolean; // @addr 0x71B8A8 @note "Also checks hook eligibility."
    function GetCargoHookRangeSquared: Integer; // @addr 0x70FD2C
    function GetBaseCargoHookPower: Integer; // @addr 0x70FD4C @note "Raw PickupPower; zero without a hook. Does not check usability."
    function GetDefensePercent: TPercent; // @addr 0x70FDCC
    function IsMicroModuleRaciallyRestricted(ModuleIndex: Integer): Boolean; // @addr 0x70F2AC @note "Zero-based template index; true means disallowed. Includes custom faction, Dominator series and pilot-race restrictions."
    procedure AddAward(AwardId: Byte); // @addr 0x70FE2C @note "Rejects ID 255 and appends without deduplication; stops when list count equals 255. Extends the visible prefix only when all previous awards were visible."
    function ShouldPickUpItem(Item: TItem): Boolean; // @addr 0x71B918
    function CanReachItemBeforeOtherShips(Item: TItem): Boolean; // @addr 0x71C080 @note "Ties are allowed; requires positive speed."
    procedure AddRecentlyDroppedItem(Item: TItem); // @addr 0x71C350
    procedure ClearRecentlyDroppedItems; // @addr 0x71C320
    function IsRecentlyDroppedItem(Item: TItem): Boolean; // @addr 0x71C3B0

    function HasActiveDisease: Boolean; // @addr 0x72C0A8
    function CountActiveDiseases: Integer; // @addr 0x72C0F4
    function HasPresentDisease: Boolean; // @addr 0x72C13C
    function CountPresentDiseases: Integer; // @addr 0x72C188
    function HasActiveStimulant: Boolean; // @addr 0x72C1D0
    function CountActiveStimulants: Integer; // @addr 0x72C21C
    function CountPresentDiseasesAndActiveStimulants: Integer; // @addr 0x72C264
    function IsHealthEffectActive(Index: Integer): Boolean; // @addr 0x72C40C @note "Captain effect is active only when Progress equals 100."
    function HasRadiationSickness: Boolean; // @addr 0x72C8B0
    function HasDiseaseFromCurrentPlanet: Boolean; // @addr 0x72C290
    function HasDiseaseFromCurrentDockedShip: Boolean; // @addr 0x72C354
    procedure SimulateNpcHealthEffects; // @addr 0x72C444
    procedure ClearCombatStatusEffects; // @addr 0x72DC2C @note "Frees entries but keeps the list."
    function FindCombatStatusEffect(EffectType: TCombatStatusEffectType): Integer; // @addr 0x72D994 @note "Returns the list index, or -1."
    procedure AddCombatStatusStrength(EffectType: TCombatStatusEffectType; Strength: Single; Source: TShip); // @addr 0x72DA10 @ida "void __userpurge $name(TShip *Self@<eax>, TCombatStatusEffectType EffectType@<dl>, float Strength@<^0>, TShip *Source@<ecx>);" @note "Scales by hull and existing strength. Always replaces the source ID, including clearing it for nil Source; no sign validation."
    procedure ReduceCombatStatusStrength(EffectType: TCombatStatusEffectType; Amount: Single); // @addr 0x72DB90 @ida "void __userpurge $name(TShip *Self@<eax>, TCombatStatusEffectType EffectType@<dl>, float Amount@<^0>);" @note "Removes only entries reduced below zero; exactly zero remains. Negative Amount increases strength."
    procedure DecayCombatStatusEffects; // @addr 0x72DC88 @note "Daily decay; exactly zero remains until a subsequent reduction."
    function GetShockStatusDecay(Strength: Single): Single; // @addr 0x72DDE8 @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, float Strength@<^0>);"
    function GetAcidStatusDecay(UnusedStrength: Single): Single; // @addr 0x72DE9C @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, float UnusedStrength@<^0>);"
    function GetMagneticStatusDecay(Strength: Single): Single; // @addr 0x72DF64 @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, float Strength@<^0>);"
    function GetWeaponBlockStatusDecay(Strength: Single): Single; // @addr 0x72DFCC @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, float Strength@<^0>);"
    function GetDroidBlockStatusDecay(Strength: Single): Single; // @addr 0x72DFF0 @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, float Strength@<^0>);"
    function GetBWBuffStatusDecay(Strength: Single): Single; // @addr 0x72E014 @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, float Strength@<^0>);"
    function GetBWRepairDebuffStatusDecay(Strength: Single): Single; // @addr 0x72E04C @ida "float __userpurge $name@<st0>(TShip *Self@<eax>, float Strength@<^0>);"
    procedure ClearCombatStatusSourceReferences(Source: TShip); // @addr 0x72E084 @note "Requires non-nil Source; clears matching IDs without changing effect strengths."
    function GetCombatStatusStrength(EffectType: TCombatStatusEffectType): Single; // @addr 0x72E108 @note "Zero when absent."
    function GetCombatStatusSourceId(EffectType: TCombatStatusEffectType): Integer; // @addr 0x72E14C @note "Zero when absent or unattributed."
    function GetCombatStatusDescription(out Count: Integer; ShowStrength: Boolean): WideString; // @addr 0x72E2A8 @ida "void __userpurge $name(TShip *Self@<eax>, int *Count@<edx>, bool ShowStrength@<cl>, unsigned __int16 **Result@<^0>);" @note "Includes rounded-positive shock, acid, magnetic, BW buff and custom status entries; omits transient blocking effects."

    function GetBaseSkillLevel(Skill: TPilotSkill): Byte; // @addr 0x72B8E4
    function GetEffectiveSkillLevel(Skill: TPilotSkill; IgnoreStatusEffects: Boolean = False): TPilotSkillLevel; // @addr 0x72B90C @note "Clamps to 0..6; equipment bonuses still apply when status effects are ignored."
    function CalculateSpeed: Integer; virtual; // @addr 0x72C8DC @slot 0x4C @calls "0x70FF2A"
    function CountUnequippedDominatorEquipment: Integer; // @addr 0x719F9C @note "Skips inventory index 0."
    function TrainSkill(Skill: TPilotSkill): Boolean; // @addr 0x72B730
    procedure GainExperience(Amount: Integer; SourceKind: Byte); // @addr 0x72B3C4 @note "Source kind 0 bypasses diminishing returns."
    procedure DepositCarriedNodes; // @addr 0x72B620 @note "Deposits every carried stack and awards experience."
  end;

var
  SimulationContext: ShortInt = 0; // @addr $87BC10  Suppresses duplicate Pirate Clan abduction effects during turn simulation.
  DamageScriptActionTypes: array[0..2] of Byte = (7, 8, 9); // @addr $87BC14 Energy, splinter and missile hit callbacks.
  TradeGoodsSold: TGoods = nil; // @addr $87BC18 Reused script-event payload for the goods leaving the ship.
  TradeGoodsCostBasis: TGoods = nil; // @addr $87BC1C Reused payload for the purchased portion of the sale.
  DominatorShipSmallSizes: array[0..2, 0..7] of Integer = ((127, 110, 70, 60, 45, 40, 130, 40), (127, 110, 70, 60, 45, 40, 130, 40), (127, 110, 70, 60, 45, 40, 130, 40)); // @addr $87BC20
  DominatorShipLargeSizes: array[0..2, 0..7] of Integer = ((127, 127, 100, 90, 65, 60, 160, 60), (127, 127, 100, 90, 65, 60, 160, 60), (127, 127, 100, 90, 65, 60, 160, 60)); // @addr $87BC80
  RangerSmallSizes: array[0..7] of Integer = (50, 50, 50, 50, 50, 50, 50, 50); // @addr $87BCE0
  RangerLargeSizes: array[0..7] of Integer = (80, 80, 80, 80, 80, 80, 80, 80); // @addr $87BD00
  TransportSmallSizes: array[3..5, 0..7] of Integer = ((50, 50, 50, 50, 50, 50, 50, 50), (50, 50, 50, 50, 50, 50, 50, 50), (50, 50, 50, 50, 50, 50, 50, 50)); // @addr $87BD20
  TransportLargeSizes: array[3..5, 0..7] of Integer = ((90, 90, 90, 90, 90, 90, 90, 90), (90, 90, 90, 90, 90, 90, 90, 90), (90, 90, 90, 90, 90, 90, 90, 90)); // @addr $87BD80
  PirateSmallSizes: array[0..7] of Integer = (45, 45, 45, 55, 45, 45, 45, 45); // @addr $87BDE0
  PirateLargeSizes: array[0..7] of Integer = (80, 80, 80, 90, 80, 80, 80, 80); // @addr $87BE00
  PirateClanSmallSizes: array[0..7] of Integer = (45, 45, 45, 55, 45, 45, 45, 45); // @addr $87BE20
  PirateClanLargeSizes: array[0..7] of Integer = (80, 80, 80, 90, 80, 80, 80, 80); // @addr $87BE40
  WarriorSmallSizes: array[0..7] of Integer = (45, 45, 45, 55, 45, 45, 45, 45); // @addr $87BE60
  WarriorLargeSizes: array[0..7] of Integer = (80, 80, 80, 80, 80, 80, 80, 80); // @addr $87BE80
  BigWarriorSmallSizes: array[0..7] of Integer = (80, 80, 80, 80, 80, 80, 80, 80); // @addr $87BEA0
  BigWarriorLargeSizes: array[0..7] of Integer = (130, 130, 130, 130, 130, 130, 130, 130); // @addr $87BEC0
  TranclucatorSmallSize: Integer = 40; // @addr $87BEE0
  TranclucatorLargeSize: Integer = 50; // @addr $87BEE4
  SpecialHullSmallSize: Integer = 50; // @addr $87BEE8

var
  SpecialHullLargeSize: Integer = 80; // @addr $87BEEC
  StationSize: Integer = 128; // @addr $87BEF0
  DefaultShipSmallSize: Integer = 50; // @addr $87BEF4
  DefaultShipLargeSize: Integer = 80; // @addr $87BEF8
  SkillBonusEvaluationWeights: array[22..27] of Integer = (100, 100, 80, 80, 60, 60); // @addr $87BEFC bonSkill1..bonSkill6.
  SlotBonusEvaluationWeights: array[13..20] of Integer = (100, 100, 200, 100, 200, 75, 10, 30); // @addr $87BF14 bonSlotRadar..bonSlotForsage.

function CreateShipByType(ShipType: Byte): TShip; // @addr 0x70E4B4 @note "Allocates an unregistered instance; caller must initialize or deserialize it."

function CompareShipGroupsStrength(Ships, Opponents: TList): Single; // @addr 0x70E408 @note "Lists contain TShip. Sum of pairwise ChanceToWin divided by Opponents.Count squared; requires nonempty Opponents when Ships is nonempty."
function CalculateFuelCost(Amount: Integer; OwnerId: Byte): Single; // @addr 0x70EFE8 @note "Owner six skips racial scaling. Uses active galaxy turn and difficulty."
function CalculateRoundedFuelCost(Amount: Integer; OwnerId: Byte): Integer; // @addr 0x70F088

var
  KlingCheapDropValueFactors: array[0..7] of Double = (0.1, 0.85, 0.9, 1, 1.2, 1.5, 0.7, 4); // @addr $87BF34 Indexed by KlingType.
  KlingValuableDropValueFactors: array[0..7] of Double = (0.1, 0.8, 0.9, 1, 2, 4, 0.7, 8); // @addr $87BF74 Indexed by KlingType.

// Nested LookupTalkText helper; the native caller removes its unused static link.

const
  DominatorProgramDropCostFactors: array[0..7] of Double = (0.2, 1.6, 1.8, 2.0, 4.0, 8.0, 1.4, 16.0); // @addr $87BFB4 TKlingType order.
...
{ @routine $6F87A8 TShip_SaveToBuffer }
...
begin
  ...
  if InterceptorSourceShip <> nil then Buffer.AddDWord((TObject(InterceptorSourceShip) as TShip).Id)
  else Buffer.AddDWord(0);
...
{ @routine $6F9670 TShip_LoadFromBuffer }
...
{ @routine $6FAB04 TShip_SaveToBlock }
...
{ @routine $6FBCE0 TShip_LoadFromBlock }
...
{ @routine $6FD438 TShip_ResolveLoadedReferences }
...
{ @routine $6FDA5C TShip_ClearObjectReferences }
...
{ @routine $6FDC88 TShip_ProcessBrokenFuelTankLeak }
...
{ @routine $6FDFA8 TShip_RefreshTechKnowledgeAtLocation }
...
{ @routine $6FF00C TShip_NextDayLogic }
...
{ @routine $6FF018 TShip_AssignWeaponTargetsInStar }
...
{ @routine $6FF058 TShip_DerivedStateCompatibilityHook }
...
{ @routine $6FF064 TShip_GetTypeNameKey }
...
{ @routine $6FF08C TShip_GetLocalizedTypeName }
...
{ @routine $6FF1C0 TShip_GetFactionNameKey }
...
{ @routine $6FF248 TShip_GetDefaultHullType }
...
{ @routine $6FF338 TShip_NextRandomInteger }
...
{ @routine $6FF368 TShip_GetSpaceInfoText }
...
{ @routine $6FFD88 TShip_GetDesiredCargoFreeSpace }
...
{ @routine $6FFDA0 TShip_IsHullDestroyed }
...
{ @routine $6FFDC0 TShip_GetTurnSeedFraction }
...
{ @routine $6FFE04 TShip_GetEstimatedMemoryUsage }
...
{ @routine $6FFF54 TShip_HasCargoGoods }
...
{ @routine $6FFF8C TShip_CountCargoGoodsTypes }
...
{ @routine $6FFFC4 TShip_GetCarriedNodeCount }
...
{ @routine $700024 TShip_HasLockedOrFollowOrder }
...
{ @routine $700064 TShip_HasHullDamageOrBrokenEquippedItems }
...
{ @routine $7000E8 TShip_CanRefuel }
...
{ @routine $70012C TShip_LookupVisibleTalkText }
...
{ @routine $70020C TShip_UpdateBestRangerRelativeRatings }
...
{ @routine $700294 TShip_UpdateAverageRangerRelativeStrength }
...
{ @routine $7002BC TShip_CalculateWealth }
...
{ @routine $700420 TShip_CalculateAttackStrength }
...
{ @routine $700628 TShip_CalculateDefenseStrength }
...
{ @routine $70070C TShip_GetRepairStrengthFactor }
...
{ @routine $700768 TShip_CalculateStrength }
...
{ @routine $7007D4 TShip_GetFullHullRelativeStrengthPercent }
...
{ @routine $70083C TShip_GetWealthScaledAmount }
...
{ @routine $7008C4 TShip_HasScriptBindings }
...
{ @routine $700908 TShip_GetShipPortraitImagePath }
...
{ @routine $7009F4 TShip_GetCaptainPortraitResourceBase }
...
{ @routine $701568 TShip_HasPlayerChameleonCharges }
...
{ @routine $7015A4 TShip_SelectChameleonVisualType }
...
{ @routine $701698 TShip_IsPlayerChameleonEffectiveAgainstSelf }
...
{ @routine $7017B0 TShip_RefreshGraphic }
...
{ @routine $701840 TShip_CreateNormalGraphic }
...
{ @routine $7020DC TShip_CreateDominatorGraphic }
...
{ @routine $7022B0 TShip_TransferToStar }
...
{ @routine $702374 TShip_FindCarriedItemById }
...
{ @routine $702568 TShip_TryRelocateUnseenShip }
...
{ @routine $7028F0 TShip_SynchronizeDockedLocation }
...
{ @routine $7029A4 TShip_InNormalSpace }
...
{ @routine $702A2C TShip_IsOutsideStarSpace }
...
{ @routine $702A94 TShip_IsOnPlanet }
...
{ @routine $702AB0 TShip_IsDockedToShip }
...
{ @routine $702AF8 TShip_HasPositiveSpeed }
...
{ @routine $702B40 TShip_CancelInvalidTravelOrder }
procedure TShip.CancelInvalidTravelOrder;
  // @nested $702B18 Cancel
  procedure Cancel; // @addr 0x702B18 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; ship at ParentFrame-4."
  begin
...
{ @routine $702BD4 TShip_ClearPlanetQueue }
...
{ @routine $702C04 TShip_SelectNearestQueuedPlanet }
...
{ @routine $702CF0 TShip_FindNearestDockableStation }
...
{ @routine $702DD0 TShip_FindFirstInhabitedPlanetInStar }
...
{ @routine $702E3C TShip_NavigateToQueuedPlanet }
...
{ @routine $702ECC TShip_NavigateToEscapePlanet }
...
{ @routine $702FB0 TShip_TryMirrorPartnerTravelOrders }
...
{ @routine $703630 TShip_OrderRandomFreeFlightMove }
...
{ @routine $7036A4 TShip_IsTargetStillPursuable }
...
{ @routine $7036F8 TShip_CanEscapePursuer }
...
{ @routine $703990 TShip_HasLandablePlanetInStar }
...
{ @routine $7039F8 TShip_GetFullFuelBaseJumpRange }
...
{ @routine $703A44 TShip_FindNextStarTowardDestination }
...
{ @routine $703D0C TShip_DistanceToNearestShipByTypeMask }
...
{ @routine $703DB4 TShip_EstimateOrderTravelTurns }
...
{ @routine $703F78 TShip_EstimateTravelTurnsToObject }
...
{ @routine $704110 TShip_EstimateTravelTurnsToPlanet }
...
{ @routine $70418C TShip_NotifyCompanionDeath }
...
{ @routine $704490 TShip_ApplyDamage }
...
{ @routine $70671C TShip_ApplyWeaponHit }
...
{ @routine $706E00 TShip_ApplyMissileHit }
...
{ @routine $707160 TShip_FireWeaponAtMissile }
...
{ @routine $707CD4 TShip_FireWeaponAtShip }
...
var
  ...
  // @nested $707744 ApplyChainExplosion
  function ApplyChainExplosion(ExplodingShip: TShip): Integer; // @addr 0x707744 @ida "int __usercall $name@<eax>(TShip *ExplodingShip@<eax>, void *ParentFrame@<^0>);" @note "Caller-popped static link; recursively damages nearby hostiles. Returns the accumulated signed damage results."
  var
    I: Integer;
    Ship: TShip;
  begin
  ...
  // @nested $707AE8 FinishChainExplosionFilm
  procedure FinishChainExplosionFilm; // @addr 0x707AE8 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; merges repeated targets' damage in the parent's film lists."
  var
    Damage: Integer;
    Color: Cardinal;
    Ship: TShip;
...
{ @routine $7090B0 TShip_FireWeaponAtItem }
...
{ @routine $709798 TShip_FireWeaponAtAsteroid }
...
{ @routine $7099F4 TShip_ApplyInterceptorDamage }
...
{ @routine $709B30 TShip_ApplyShockStatusDamage }
...
{ @routine $709B74 TShip_ApplyAsteroidImpactDamage }
...
{ @routine $709F30 TShip_ApplyExplosionDamage }
...
{ @routine $70A428 TShip_ApplyStarHeatDamage }
...
{ @routine $70A974 TShip_CountEquippedWeapons }
...
{ @routine $70A9E0 TShip_CountMissileWeapons }
...
{ @routine $70AA34 TShip_CountDirectFireWeapons }
...
{ @routine $70AA88 TShip_ClearUnequippedWeaponTargets }
...
{ @routine $70AB10 TShip_CountWeaponsByDamageFlags }
...
{ @routine $70AB70 TShip_GetWeaponDamageSummary }
...
{ @routine $70AE84 TShip_GetManeuverabilitySummary }
...
{ @routine $70AF20 TShip_GetRepairPointsSummary }
...
{ @routine $70AFA8 TShip_HasScannerArtefact }
...
{ @routine $70AFCC TShip_GetWeaponActionRange }
...
{ @routine $70AFF0 TShip_GetWeaponSlotRange }
...
{ @routine $70B01C TShip_GetWeaponMinDamage }
...
{ @routine $70B058 TShip_GetWeaponMaxDamage }
...
{ @routine $70B1E8 TShip_GetMaxWeaponRange }
...
{ @routine $70B254 TShip_ClearWeaponTargets }
...
{ @routine $70B2E8 TShip_IsAttackingShip }
...
{ @routine $70B380 TShip_ChanceToWin }
...
{ @routine $70B8E0 TShip_GetWinChancePercent }
...
{ @routine $70B970 TShip_SetJointAttackTarget }
...
{ @routine $70BB50 TShip_IsEnemyPursuingSelf }
...
{ @routine $70BBB0 TShip_CanSafelyDetonateItem }
...
{ @routine $70BC70 TShip_SetStoredRangerRelationLevel }
...
{ @routine $70BE0C TShip_GetRelationLevelToShip }
...
{ @routine $70BE74 TShip_GetRelationLevelTextToShip }
...
{ @routine $70BF44 TShip_TryExtortShip }
...
{ @routine $70C1E0 TShip_TruceWithShip }
...
{ @routine $70C5E8 TShip_RequestAlliesAttackShip }
...
{ @routine $70CA24 TShip_IsInPrison }
...
{ @routine $70CA9C TShip_GetPrisonTermRemaining }
...
{ @routine $70CAF4 TShip_ClearPrisonTerm }
...
{ @routine $70CB40 TShip_GetPrisonReleaseCost }
...
{ @routine $70CBA8 TShip_GetLocationGoodsEntry }
...
{ @routine $70CE54 TShip_ShopGoodsPurchasePrice }
...
{ @routine $70D09C TShip_ShopGoodsSellPrice }
...
{ @routine $70D344 TShip_GetAverageCargoCost }
...
{ @routine $70D394 TShip_ConsumeCargoGoods }
...
{ @routine $70D410 TShip_IsCargoGoodIllegalOnCurrentPlanet }
...
{ @routine $70D4A8 TShip_SellGoodsToLocation }
...
{ @routine $70DB5C TShip_BuyGoodsFromLocation }
...
{ @routine $70DE38 TShip_ProcessLiberationGroupRoute }
...
{ @routine $70E3A8 TShip_LeaveLiberationGroup }
...
{ @routine $70E408 CompareShipGroupsStrength }
...
{ @routine $70E4B4 CreateShipByType }
...
{ @routine $70E5FC TShip_GetFuelTanks }
...
{ @routine $70E618 TShip_GetEngine }
...
{ @routine $70E650 TShip_GetScanner }
...
{ @routine $70E66C TShip_GetRepairRobot }
...
{ @routine $70E688 TShip_GetCargoHook }
...
{ @routine $70E6A4 TShip_GetDefGenerator }
...
{ @routine $70E6C0 TShip_CanRepairArtefactsAtLocation }
...
{ @routine $70E764 TShip_CanUseEquipmentTech }
...
{ @routine $70E834 TShip_CanRepairEquipmentTech }
...
{ @routine $70E934 TShip_IsEquipmentUsable }
...
{ @routine $70E980 TShip_EquipItem }
...
{ @routine $70EA50 TShip_UnequipSlot }
...
{ @routine $70EB28 TShip_UnequipItem }
...
{ @routine $70EBC0 TShip_CalculateMass }
...
{ @routine $70ECF4 TShip_CalculateEquippedMass }
...
{ @routine $70EE74 TShip_GetHullIntegrityPercent }
...
{ @routine $70EF0C TShip_GetJumpDestinationDistance }
...
{ @routine $70EF5C TShip_GetFullRefuelCost }
...
{ @routine $70EFE8 CalculateFuelCost }
...
{ @routine $70F088 CalculateRoundedFuelCost }
...
{ @routine $70F0B0 TShip_GetAfterburnerWear }
...
{ @routine $70F190 TShip_GetFuelLimitedJumpRange }
...
{ @routine $70F230 TShip_GetJumpRange }
...
{ @routine $70F2AC TShip_IsMicroModuleRaciallyRestricted }
...
{ @routine $70F578 TShip_GetEquipmentStatBonus }
...
{ @routine $70F5C0 TShip_GetTotalStatBonus }
...
{ @routine $70F6F8 TShip_GetRadarRange }
...
{ @routine $70F78C TShip_GetScannerPower }
...
{ @routine $70F7E8 TShip_CanResolveObjectWithScanner }
...
{ @routine $70F8B0 TShip_ApplyRepairDroidHealing }
...
{ @routine $70FB08 TShip_GetCargoHookMinPullSpeed }
...
{ @routine $70FBE4 TShip_GetCargoHookMaxPullSpeed }
...
{ @routine $70FCC0 TShip_GetCargoHookRange }
...
{ @routine $70FD2C TShip_GetCargoHookRangeSquared }
...
{ @routine $70FD4C TShip_GetBaseCargoHookPower }
...
{ @routine $70FD80 TShip_GetDefenseDamageFactor }
...
{ @routine $70FDCC TShip_GetDefensePercent }
...
{ @routine $70FDF4 TShip_GetAttackMultiplier }
...
{ @routine $70FF00 TShip_RefreshDerivedStats }
...
{ @routine $710078 TShip_RefreshGraphicSize }
...
{ @routine $7105A8 TShip_RebuildEquipmentCache }
...
{ @routine $710750 TShip_GetCargoFreeSpace }
...
{ @routine $710784 TShip_GetCarriedItemWeight }
...
{ @routine $710820 TShip_GetCargoGoodsWeight }
...
{ @routine $710854 TShip_CalculateFollowRadius }
...
{ @routine $710A00 TShip_GetFollowMode }
...
{ @routine $710A64 TShip_GetEffectiveFollowMode }
...
{ @routine $710B98 TShip_NeedsEquipmentType }
...
{ @routine $710BDC TShip_CalculateEquippedItemCostWithoutHull }
...
{ @routine $710C84 TShip_CountCarriedEquipmentByType }
...
{ @routine $710D00 TShip_SelectBestUnequippedWeapon }
...
{ @routine $710E8C TShip_HasLooseNonScriptItemsOrGoods }
...
{ @routine $710F78 TShip_DropUnequippedItemsAndGoods }
...
{ @routine $71105C TShip_AddItemToPlayerStorage }
...
{ @routine $711154 TShip_MergeItemIntoPlayerStorage }
...
{ @routine $711304 TShip_AddGoodsToPlayerStorage }
...
{ @routine $7114C0 TShip_StoreLooseInventoryAt }
...
{ @routine $7116A8 TShip_RetrieveStoredItems }
...
{ @routine $711A6C TShip_AutoEquipArtefacts }
...
var
  ...
  // @nested $711954 EquipType
  procedure EquipType(ArtefactType: TItemType); // @addr 0x711954 @ida "void __usercall $name(unsigned __int8 ArtefactType@<al>, void *ParentFrame@<^0>);" @note "Caller-popped static link; remaining slots -4, ship -8."
  var
    I: Integer;
...
{ @routine $711E08 TShip_CalculateItemEffectiveness }
...
{ @routine $7127F4 TShip_EstimateWeaponDamageAgainstTypicalDefense }
...
{ @routine $712A24 TShip_GetWeaponRange }
...
{ @routine $712BF4 TShip_CalculateHullArmor }
...
{ @routine $712CB8 TShip_GetItemFuelTankCapacity }
...
{ @routine $712CE8 TShip_CalculateEngineSpeed }
...
{ @routine $712E58 TShip_CalculateEngineJumpRange }
...
{ @routine $712F90 TShip_CalculateRadarRange }
...
{ @routine $713044 TShip_CalculateScannerPower }
...
{ @routine $7130F8 TShip_CalculateRepairPoints }
...
{ @routine $7131AC TShip_CalculateCargoHookPower }
...
{ @routine $713260 TShip_CalculateDefGeneratorFactor }
...
{ @routine $713518 TShip_GetEquipmentEvaluationSynergyBonus }
...
var
  ...
  ItemType: TItemType;
  // @nested $7134DC AccumulateEquipmentBonus
  procedure AccumulateEquipmentBonus(Value: Single); // @addr 0x7134DC @ida "void __usercall $name(float Value@<^0>, void *ParentFrame@<^4>);" @stackpop 0x4 @calls "0x7135B0 0x7135D4 0x7135F2 0x713622 0x713652 0x713682 0x713700 0x71371E 0x7137A9 0x7137DF 0x7138E1 0x71390B 0x713929 0x713947 0x713965 0x713983 0x7139A4 0x7139D3 0x7139FD 0x713A21 0x713A57 0x713A8D 0x713AC3 0x713B47 0x713B6B 0x713C06 0x713C42 0x713D56 0x713D86 0x713DAA 0x713DCE 0x713DF2 0x713E16 0x713E3A 0x713E58 0x713E9A" @note "Adds positive and nonpositive values to separate Single accumulators at ParentFrame-4/-8."
  begin
...
{ @routine $713F50 TShip_GetWeaponArtefactDamageFactor }
...
{ @routine $7141E8 TShip_UpdateSpeedTrackingMetrics }
...
{ @routine $714274 TShip_RefreshEquipmentEvaluationMetrics }
...
{ @routine $714598 TShip_EvaluateItem }
...
{ @routine $7145D4 TShip_AdjustItemEvaluation }
...
{ @routine $714A60 TShip_EvaluateStatBonus }
...
{ @routine $7156B0 TShip_EvaluateWeaponDamage }
...
{ @routine $715CE0 TShip_EvaluateMicroModuleGain }
...
{ @routine $715E84 TShip_AutoApplyMicroModules }
...
var
  ...
  // @nested $715DA8 FitsMicroModuleCapacity
  function FitsMicroModuleCapacity: Boolean; // @addr 0x715DA8 @ida "bool __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; equipment -4, module item -8, ship -12."
  begin
...
{ @routine $716124 TShip_AutoEquipInventory }
...
{ @routine $716508 TShip_DropCargoUntilNotOverloaded }
...
{ @routine $716DF0 TShip_DropRandomCheapItemsOnDestruction }
procedure TShip.DropRandomCheapItemsOnDestruction(Count: Integer);
var
  Index, I: Integer;
  Item: TItem;
  // @nested $716BE0 SelectCheapItem
  function SelectCheapItem: TItem; // @addr 0x716BE0 @ida "TItem *__cdecl $name(void *ParentFrame);" @note "Caller-popped static link; cycling inventory index -4, ship -8."
  var
    Attempt: Integer;
    Candidate: TItem;
    Value: Double;
...
{ @routine $717074 TShip_DropRandomValuableItemsOnDestruction }
procedure TShip.DropRandomValuableItemsOnDestruction(Count: Integer);
var
  Index, I: Integer;
  Item: TItem;
  // @nested $716E50 SelectValuableItem
  function SelectValuableItem: TItem; // @addr 0x716E50 @ida "TItem *__cdecl $name(void *ParentFrame);" @note "Caller-popped static link; cycling inventory index -4, ship -8."
  var
    Attempt: Integer;
    Candidate: TItem;
    Value: Double;
...
{ @routine $7170D4 TShip_DropItemsForDominatorProgram }
...
{ @routine $717340 TShip_DropCarriedItemAsMovingLoot }
...
{ @routine $7173CC TShip_DropCarriedArtefactAsMovingLoot }
...
{ @routine $717478 TShip_DropGuaranteedDeathDropItems }
...
{ @routine $7174FC TShip_CanDropTreasureMap }
...
{ @routine $717578 TShip_SelectTreasureMapPlanet }
...
{ @routine $7177FC TShip_TryDropTreasureMap }
...
{ @routine $717884 TShip_DropItemIntoStar }
...
{ @routine $717C28 TShip_DropAllArtefactsOnDestruction }
...
{ @routine $717C74 TShip_DropGoodsIntoSpace }
...
{ @routine $717CF8 TShip_JettisonCargoGoodsTowardTargetValue }
...
{ @routine $717E74 TShip_DropAllCargoGoods }
...
{ @routine $717EC0 TShip_QueueMovingItemDrop }
...
{ @routine $71814C TShip_SelectLeastValuableInventoryItem }
...
{ @routine $7182D8 TShip_SelectLeastValuableArtefact }
...
{ @routine $7183B4 TShip_SelectCheapestCargoGood }
...
{ @routine $718530 TShip_OptimizeInventory }
...
var
  ...
  // @nested $718428 GetRetainedCapacity
  function GetRetainedCapacity: Integer; // @addr 0x718428 @ida "int __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; ship -4. Hull capacity minus equipped and protected items."
...
{ @routine $718930 TShip_LiquidateInventoryItem }
...
{ @routine $718A58 TShip_LiquidateArtefact }
...
{ @routine $718AC4 TShip_RepairHullAtLocation }
...
{ @routine $718C5C TShip_ReloadWeaponAmmo }
...
{ @routine $718CB8 TShip_ApplyCombatItemDegradation }
...
{ @routine $718F54 TShip_ApplyArtefactUseDegradation }
...
{ @routine $719020 TShip_ApplyAfterburnerItemDegradation }
...
{ @routine $7190B8 TShip_ApplyItemDegradation }
...
{ @routine $7195B8 TShip_CanGenerateMicroModuleForLoadout }
...
{ @routine $7196F4 TShip_CanGenerateSpecialHullModule }
...
{ @routine $719728 TShip_ImproveRandomEquipment }
...
{ @routine $719B24 TShip_GenerateAndApplyMicroModule }
...
var ModuleIndex, Attempts, Priority, Accepted, BestModule: Integer; Gain, BestGain: Single;
  ...
  // @nested $7199E4 FitsGeneratedModuleCapacity
  function FitsGeneratedModuleCapacity: Boolean; // @addr 0x7199E4 @ida "bool __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; equipment -4, zero-based module index -8, ship -12. Uses strict remaining-capacity comparison."
  begin
    if Item.ItemType <> t_Hull then
  ...
  // @nested $719A9C AcceptDuplicate
  function AcceptDuplicate: Boolean; // @addr 0x719A9C @ida "bool __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; module index -8, ship -12. Random rejection for matching installed modules."
...
{ @routine $719C8C TShip_GenerateExtraWeapon }
...
{ @routine $719ED4 TShip_NeedsMicroModule }
...
{ @routine $719F9C TShip_CountUnequippedDominatorEquipment }
...
{ @routine $71A000 TShip_GetSatelliteLimit }
...
{ @routine $71A03C TShip_IsEssentialInventoryItem }
...
{ @routine $71A0EC TShip_IsOptionalUtilityEquipment }
...
{ @routine $71A138 TShip_NeedsEssentialEquipment }
...
{ @routine $71A190 TShip_RestoreEssentialEquipment }
...
{ @routine $71A3EC TShip_BuyEquipmentAtLocation }
...
var
  ...
  // @nested $71A24C SelectReplacedWeapon
  function SelectReplacedWeapon: TWeapon; // @addr 0x71A24C @ida "TWeapon *__cdecl $name(void *ParentFrame);" @note "Caller-popped static link; ship -4. Selects the lowest evaluated weapon when all slots are occupied."
  var J, Best: Integer;
  begin
    Result := nil;
  ...
  // @nested $71A30C TemporarilyUnequipWeapon
  procedure TemporarilyUnequipWeapon(Weapon: TWeapon); // @addr 0x71A30C @ida "void __usercall $name(TWeapon *Weapon@<eax>, void *ParentFrame@<^0>);" @note "Caller-popped static link; ship -4, fallback weapon -8; saves five weapon pointers and target at -32."
  ...
  // @nested $71A398 RestoreWeapon
  procedure RestoreWeapon; // @addr 0x71A398 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; ship -4, weapon -8; restores saved target and weapon pointers."
  var J: Integer;
  begin
...
{ @routine $71AED0 TShip_CreateAndEquipHull }
...
{ @routine $71AF58 TShip_SelectRandomHullSeries }
...
{ @routine $71AFCC TShip_CreateAndEquipFuelTanks }
...
{ @routine $71B028 TShip_CreateAndEquipEngine }
...
{ @routine $71B084 TShip_CreateAndEquipRadar }
...
{ @routine $71B0E0 TShip_CreateAndEquipScanner }
...
{ @routine $71B13C TShip_CreateAndEquipRepairRobot }
...
{ @routine $71B198 TShip_CreateAndEquipCargoHook }
...
{ @routine $71B1F4 TShip_CreateAndEquipDefGenerator }
...
{ @routine $71B250 TShip_CreateAndEquipWeapon }
...
{ @routine $71B2B0 TShip_ScanForCollectableItems }
...
{ @routine $71B390 TShip_QueueItemsWithinPickupRange }
...
{ @routine $71B44C TShip_TryCollectBestFloatingItem }
...
{ @routine $71B844 TShip_GetReservedPickupWeight }
...
{ @routine $71B8A8 TShip_IsItemInPickupRange }
...
{ @routine $71B900 TShip_AcceptPickupItem }
...
{ @routine $71B918 TShip_ShouldPickUpItem }
...
{ @routine $71BCE0 TShip_AcceptPickupDistance }
...
{ @routine $71BCFC TShip_AddPickupTarget }
...
{ @routine $71BD74 TShip_RemovePickupTarget }
...
{ @routine $71BDE4 TShip_ClearPickupTargets }
...
{ @routine $71BE14 TShip_RemoveInvalidPickupTargets }
...
{ @routine $71BEEC TShip_TogglePickupTargets }
procedure TShip.TogglePickupTargets(IgnoreRange: Boolean);
var
  I: Integer;
  Item: TItem;
  Added: Boolean;
// @nested $71BEA0 AcceptPickupTarget
function AcceptPickupTarget(Item: TItem): Boolean; // @addr 0x71BEA0 @ida "bool __usercall $name@<al>(TItem *Item@<eax>, void *ParentFrame@<^0>);" @note "Caller-popped static link; mode -1, ship -8."
begin
  Result := False;
  if IgnoreRange then
  begin
...
{ @routine $71BFC0 TShip_HasPickupTarget }
...
{ @routine $71BFFC TShip_CountOtherShipsTargetingItem }
...
{ @routine $71C080 TShip_CanReachItemBeforeOtherShips }
...
{ @routine $71C1AC TShip_GetPickupApproachPosition }
...
{ @routine $71C248 TShip_GetCurrentPickupItem }
...
{ @routine $71C320 TShip_ClearRecentlyDroppedItems }
...
{ @routine $71C350 TShip_AddRecentlyDroppedItem }
...
{ @routine $71C3B0 TShip_IsRecentlyDroppedItem }
...
{ @routine $71C3F0 TShip_OrderNone }
...
{ @routine $71C43C TShip_OrderMove }
...
{ @routine $71C4B4 TShip_GetJumpDeparturePoint }
...
{ @routine $71C590 TShip_GetArrivalPosition }
...
{ @routine $71C634 TShip_CalculateJumpTravelDays }
...
{ @routine $71C6B4 TShip_OrderJump }
...
{ @routine $71C75C TShip_OrderJumpHole }
...
{ @routine $71C804 TShip_OrderTeleport }
...
{ @routine $71C870 TShip_OrderLanding }
...
{ @routine $71C8FC TShip_OrderTakeoff }
...
{ @routine $71CE50 TShip_OrderFollowShip }
...
{ @routine $71CEAC TShip_GetMovementPathTurnCount }
...
{ @routine $71D054 TShip_PrepareTurnMovement }
...
var
  ...
  CanLand: Boolean;
  Effect: TObjectSE;
// @nested $71CEE8 InitializeFilm
procedure InitializeFilm(Alpha: Byte); // @addr 0x71CEE8 @ida "void __usercall $name(unsigned __int8 Alpha@<al>, void *ParentFrame@<^0>);" @note "Caller-popped static link; step index -4, ship -8."
begin
...
{ @routine $720090 TShip_ProcessMovementStep }
...
var
  ...
  EffectFilm: TEFilmObj;
  Fuel: Integer;
// @nested $71FE10 AdvancePath
procedure AdvancePath; // @addr 0x71FE10 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; ship -4, node -8, recording flag -9, step index -16. Removes one path node and updates position, heading and film."
begin
...
{ @routine $720F5C TShip_ClearCompletedTakeoffOrHoleOrder }
...
{ @routine $720FB8 TShip_IsTravelCompletionPathReady }
...
{ @routine $7211C4 TShip_RepelFollowingShips }
...
{ @routine $7217D8 TShip_RebuildMovePath }
...
{ @routine $721854 TShip_BuildFullPathTo }
...
{ @routine $7218A8 TShip_BuildPlanetLandingPath }
...
{ @routine $7219B0 TShip_BuildOrderMovementPath }
...
{ @routine $722848 TShip_AppendPathToWithTurnPadding }
...
{ @routine $722964 TShip_AppendPathTo }
...
{ @routine $722A10 TShip_AppendStarAvoidingPathWithTurnPadding }
...
{ @routine $722B1C TShip_AppendOrbitalPath }
...
{ @routine $722CC0 TShip_AppendTurningPath }
...
{ @routine $723524 TShip_AppendStraightPath }
...
{ @routine $723868 TShip_AppendHyperspaceTransitionPath }
...
{ @routine $723A6C TShip_AppendStarAvoidingPath }
...
{ @routine $723DF4 TShip_AppendCircularDetour }
...
{ @routine $7240C0 TShip_ClearMovementPath }
...
{ @routine $7240D8 TShip_GetSlotCount }
...
{ @routine $724554 TShip_GetSlotCountForItemType }
...
{ @routine $72457C TShip_ReassignActiveItemSlots }
...
{ @routine $7247B8 TShip_RefreshAssignedItemSlots }
...
{ @routine $724824 TShip_FindEquippedItemInSlot }
...
{ @routine $724914 TShip_RefreshInactiveItemSlotAssignments }
...
{ @routine $724994 TShip_CountUnequippedItemsInSlot }
...
{ @routine $724A0C TShip_FindFreeUnequippedSlot }
...
{ @routine $724A3C TShip_RepairDuplicateSatelliteTrajectoryIndices }
...
{ @routine $724B14 TShip_GetSatelliteTrajectoryIndexLimit }
...
{ @routine $724BA4 TShip_FindFreeSatelliteTrajectoryIndex }
...
{ @routine $724C20 TShip_FindSatelliteByTrajectoryIndex }
...
{ @routine $724C98 TShip_InsertSatelliteTrajectoryIndex }
...
{ @routine $724D10 TShip_RemoveEmptySatelliteTrajectoryIndex }
...
{ @routine $724D98 TShip_CompactSatelliteTrajectoryIndices }
...
{ @routine $724DF8 TShip_AssignSatelliteIndicesFromHoldOrder }
...
{ @routine $725040 TShip_ArrangeHoldSatellitesByTrajectoryIndex }
...
{ @routine $72538C TShip_UseDominatorTransmitter }
...
{ @routine $725988 TShip_HasMatchingArtefactOrCustomItem }
...
{ @routine $725A74 TShip_CountActiveArtefacts }
...
{ @routine $725B38 TShip_HasEquippedArtefactOfSameUseGroup }
...
{ @routine $725CB8 TShip_CanBoostArtefact }
...
var Equipment: TEquipment;
  ...
  // @nested $725C80 IsArtefactBoostEquipment
  function IsArtefactBoostEquipment(Item: TEquipment): Boolean; // @addr $725C80 @ida "bool __usercall $name@<al>(TEquipment *Item@<eax>, void *ParentFrame@<^0>);" @note "Nested in CanBoostArtefact with unused caller-popped static link. OwnerId=6 and empty CustomFaction; nil returns false."
  begin
...
{ @routine $72683C TShip_ApplyNanoArtefactRepair }
procedure TShip.ApplyNanoArtefactRepair;
var Item: TEquipment; I, EquippedCount, UnequippedCount: Integer;

  // @nested $7266DC SelectOrdinal
  procedure SelectOrdinal(Ordinal: Integer; Equipped: Boolean); // @addr 0x7266DC @ida "void __usercall $name(int Ordinal@<eax>, bool Equipped@<dl>, void *ParentFrame@<^0>);" @note "Caller-popped static link; ship -4, selected-item output -8."
  ...
  // @nested $726754 SelectRepairable
  procedure SelectRepairable(Count: Integer; Equipped: Boolean); // @addr 0x726754 @ida "void __usercall $name(int Count@<eax>, bool Equipped@<dl>, void *ParentFrame@<^0>);" @note "Caller-popped static link; ship -4, selected-item output -8. At most 30 deterministic selections."
...
{ @routine $726B04 TShip_CanContactShip }
...
{ @routine $726C44 TShip_LookupTalkText }
...
var Count, I: Integer; Key: WideString; Variants: array[0..9] of WideString;
  ...
  // @nested $726B9C GetTalkContextPrefix
  function GetTalkContextPrefix(Ship: TShip): WideString; // @addr $726B9C @ida "void __usercall $name(TShip *Ship@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);"
  begin
    Result := 'Talk.';
...
{ @routine $7270D4 TShip_OpenPlayerConversation }
...
{ @routine $727230 TShip_ShowPlayerDialogue }
...
{ @routine $727294 TShip_NotifyMoneyDemand }
...
{ @routine $727540 TShip_NotifyCargoDemand }
...
{ @routine $7277A0 TShip_NotifyFearCargoDrop }
...
{ @routine $727AA4 TShip_NotifyTruceOffer }
...
{ @routine $727D9C TShip_NotifyAttackRequest }
...
{ @routine $728090 TShip_NotifyPartnershipOffer }
  ...
  if (GetPlayer.CurrentStar = CurrentStar) and GetPlayer.InNormalSpace then begin
    if (GetPlayer = Leader) and not PlayerAutomaticControl then begin
      ...
      if (RadarSquared >= PointDistanceSquared(Position, GetPlayer.Position)) or
        ...
        if not NoTalk and not Leader.NoTalk then begin
...
{ @routine $728674 TShip_NotifyPartnershipExpired }
...
{ @routine $728A20 TShip_NotifyPartnerRebellion }
  ...
  if (GetPlayer.CurrentStar = CurrentStar) and GetPlayer.InNormalSpace then begin
    if (GetPlayer = Leader) and not PlayerAutomaticControl then begin
      ...
      if (RadarSquared >= PointDistanceSquared(Position, GetPlayer.Position)) or
        ...
        if not NoTalk and not Leader.NoTalk then begin
          Header := '<color=255,240,100>' + GetFullName(' ') + '</color>' + LookupTalkText('Talk.To') + '<color=255,240,100>' + Leader.GetFullName(' ') + '</color>';
          Request := '- ' + LookupTalkText('Talk.Partner.MateRiot');
          Response := '- ' + LookupTalkText('Talk.Partner.AnswerLiderRiot');
          with AddOrUpdatePlayerBubble(1, Galaxy.CurrentTurn, Header + #13#10 + Request + #13#10 + Response, '') do begin
            Targets[0].ShipId := Self.Id;
            Targets[1].ShipId := Leader.Id;
          end;
...
{ @routine $728D2C TShip_ShowMessageToPlayer }
...
{ @routine $728E68 TShip_NotifyPiratePartnerRelationBreak }
  ...
  if (GetPlayer.CurrentStar = CurrentStar) and GetPlayer.InNormalSpace then begin
    if (GetPlayer = Leader) and not PlayerAutomaticControl then begin
      ...
      if (RadarSquared >= PointDistanceSquared(Position, GetPlayer.Position)) or
        ...
        if not NoTalk and not Leader.NoTalk then begin
          Header := '<color=255,240,100>' + GetFullName(' ') + '</color>' + LookupTalkText('Talk.To') + '<color=255,240,100>' + Leader.GetFullName(' ') + '</color>';
          Request := '- ' + LookupTalkText('Talk.Pirate.MateBreakRelation');
          Response := '- ' + LookupTalkText('Talk.Pirate.AnswerLiderBreak');
          with AddOrUpdatePlayerBubble(1, Galaxy.CurrentTurn, Header + #13#10 + Request + #13#10 + Response, '') do begin
            Targets[0].ShipId := Self.Id;
            Targets[1].ShipId := Leader.Id;
          end;
...
{ @routine $729184 TShip_NotifyPiratePartnerRatingBreak }
  ...
  if (GetPlayer.CurrentStar = CurrentStar) and GetPlayer.InNormalSpace then begin
    if (GetPlayer = Leader) and not PlayerAutomaticControl then begin
      ...
      if (RadarSquared >= PointDistanceSquared(Position, GetPlayer.Position)) or
        ...
        if not NoTalk and not Leader.NoTalk then begin
          Header := '<color=255,240,100>' + GetFullName(' ') + '</color>' + LookupTalkText('Talk.To') + '<color=255,240,100>' + Leader.GetFullName(' ') + '</color>';
          Request := '- ' + LookupTalkText('Talk.Pirate.MateBreakRating');
          Response := '- ' + LookupTalkText('Talk.Pirate.AnswerLiderBreak');
          with AddOrUpdatePlayerBubble(1, Galaxy.CurrentTurn, Header + #13#10 + Request + #13#10 + Response, '') do begin
            Targets[0].ShipId := Self.Id;
            Targets[1].ShipId := Leader.Id;
...
{ @routine $72949C TShip_NotifyPiratePartnershipExpired }
  ...
  if (GetPlayer.CurrentStar = CurrentStar) and GetPlayer.InNormalSpace then begin
    if (GetPlayer = Leader) and not PlayerAutomaticControl then begin
      ...
      if (RadarSquared >= PointDistanceSquared(Position, GetPlayer.Position)) or
        ...
        if not NoTalk and not Leader.NoTalk then begin
          Header := '<color=255,240,100>' + GetFullName(' ') + '</color>' + LookupTalkText('Talk.To') + '<color=255,240,100>' + Leader.GetFullName(' ') + '</color>';
          Request := '- ' + LookupTalkText('Talk.Pirate.MateTheEnd');
          Response := '- ' + LookupTalkText('Talk.Pirate.AnswerLiderTheEnd');
          with AddOrUpdatePlayerBubble(1, Galaxy.CurrentTurn, Header + #13#10 + Request + #13#10 + Response, '') do begin
            Targets[0].ShipId := Self.Id;
            Targets[1].ShipId := Leader.Id;
          end;
...
{ @routine $729840 TShip_NotifyPiratePartnerRebellion }
  ...
  if (GetPlayer.CurrentStar = CurrentStar) and GetPlayer.InNormalSpace then begin
    if (GetPlayer = Leader) and not PlayerAutomaticControl then begin
      ...
      if (RadarSquared >= PointDistanceSquared(Position, GetPlayer.Position)) or
        ...
        if not NoTalk and not Leader.NoTalk then begin
          Header := '<color=255,240,100>' + GetFullName(' ') + '</color>' + LookupTalkText('Talk.To') + '<color=255,240,100>' + Leader.GetFullName(' ') + '</color>';
          Request := '- ' + LookupTalkText('Talk.Pirate.MateRiot');
          Response := '- ' + LookupTalkText('Talk.Pirate.AnswerLiderRiot');
          with AddOrUpdatePlayerBubble(1, Galaxy.CurrentTurn, Header + #13#10 + Request + #13#10 + Response, '') do begin
            Targets[0].ShipId := Self.Id;
            Targets[1].ShipId := Leader.Id;
          end;
...
{ @routine $729B48 TShip_UnknownVirtualC0 }
...
{ @routine $729B60 TShip_CalculatePartnershipMonths }
...
{ @routine $729C5C TShip_GetGreetingText }
...
{ @routine $72A764 TShip_InitializeScriptStateOrders }
...
{ @routine $72A7A0 TShip_ApplyScriptStateOrders }
...
{ @routine $72ACDC TShip_UpdateScriptStateCompletionAndPickups }
...
{ @routine $72AEC8 TShip_ScriptNextDay }
...
{ @routine $72B324 TShip_FindScriptFollowTarget }
...
{ @routine $72B3C4 TShip_GainExperience }
...
{ @routine $72B5A8 TShip_RemoveExperience }
...
{ @routine $72B620 TShip_DepositCarriedNodes }
...
{ @routine $72B730 TShip_TrainSkill }
...
{ @routine $72B884 TShip_CanTrainSkill }
...
{ @routine $72B8E4 TShip_GetBaseSkillLevel }
...
{ @routine $72B90C TShip_GetEffectiveSkillLevel }
...
{ @routine $72BCF8 TShip_GetRelativeStrengthCategory }
...
{ @routine $72BE5C TShip_GetHullConditionCategory }
...
{ @routine $72BF6C TShip_GetRangerRatingBand }
...
{ @routine $72C0A8 TShip_HasActiveDisease }
...
{ @routine $72C0F4 TShip_CountActiveDiseases }
...
{ @routine $72C13C TShip_HasPresentDisease }
...
{ @routine $72C188 TShip_CountPresentDiseases }
...
{ @routine $72C1D0 TShip_HasActiveStimulant }
...
{ @routine $72C21C TShip_CountActiveStimulants }
...
{ @routine $72C264 TShip_CountPresentDiseasesAndActiveStimulants }
...
{ @routine $72C290 TShip_HasDiseaseFromCurrentPlanet }
...
{ @routine $72C354 TShip_HasDiseaseFromCurrentDockedShip }
...
{ @routine $72C40C TShip_IsHealthEffectActive }
...
{ @routine $72C444 TShip_SimulateNpcHealthEffects }
...
{ @routine $72C8B0 TShip_HasRadiationSickness }
...
{ @routine $72C8DC TShip_CalculateSpeed }
...
{ @routine $72CF08 TShip_IsFemaleHumanPilot }
...
{ @routine $72CF74 TShip_UsesVeteranHumanRangerAppearance }
...
{ @routine $72D38C TShip_LaunchInterceptors }
...
{ @routine $72D4EC TShip_ClearIncomingInterceptors }
...
{ @routine $72D534 TShip_CountActiveInterceptorTargets }
...
{ @routine $72D5C8 TShip_GetHullEnergyRegeneration }
...
{ @routine $72D664 TShip_GetInterceptorDamage }
...
{ @routine $72D6C0 TShip_GetInterceptorEnergyCost }
...
{ @routine $72D6E8 TShip_GetInterceptorPassCount }
...
{ @routine $72D720 TShip_HasScriptControl }
...
{ @routine $72D770 TShip_HasNoUsableWeapons }
...
{ @routine $72D804 TShip_GetOwnStatBonus }
...
{ @routine $72D884 TShip_SetStatBonus }
...
{ @routine $72D994 TShip_FindCombatStatusEffect }
...
{ @routine $72DA10 TShip_AddCombatStatusStrength }
...
{ @routine $72DB90 TShip_ReduceCombatStatusStrength }
...
{ @routine $72DC2C TShip_ClearCombatStatusEffects }
...
{ @routine $72DC88 TShip_DecayCombatStatusEffects }
...
{ @routine $72DDE8 TShip_GetShockStatusDecay }
...
{ @routine $72DE9C TShip_GetAcidStatusDecay }
...
{ @routine $72DF64 TShip_GetMagneticStatusDecay }
...
{ @routine $72DFCC TShip_GetWeaponBlockStatusDecay }
...
{ @routine $72DFF0 TShip_GetDroidBlockStatusDecay }
...
{ @routine $72E014 TShip_GetBWBuffStatusDecay }
...
{ @routine $72E04C TShip_GetBWRepairDebuffStatusDecay }
...
{ @routine $72E084 TShip_ClearCombatStatusSourceReferences }
...
{ @routine $72E108 TShip_GetCombatStatusStrength }
...
{ @routine $72E14C TShip_GetCombatStatusSourceId }
...
{ @routine $72E2A8 TShip_GetCombatStatusDescription }
function TShip.GetCombatStatusDescription(out Count: Integer; ShowStrength: Boolean): WideString;
var
  Strength, I: Integer;
  Info: PCustomShipInfo;
// @nested $72E190 AppendStatusLine
procedure AppendStatusLine(TextKey: WideString); // @addr 0x72E190 @ida "void __usercall $name(unsigned __int16 *TextKey@<eax>, void *ParentFrame@<^0>);" @note "Caller-popped static link; show-strength flag -1, strength -8, count output -12, string-result output +8."
begin
...
{ @routine $72E540 TShip_UpdateAfterburnerState }
procedure TShip.UpdateAfterburnerState;
begin
end;
{ @end $72E540 }

{ @routine $72E54C TShip_RefreshCurrentStanding }
...
{ @routine $72E588 TShip_HasScriptStateText }
...
{ @routine $72E5C4 TShip_HasIndependentScriptFaction }
...
{ @routine $72E638 TShip_HasNamedScriptFaction }
...
{ @routine $72E6C0 TShip_GetScriptStandingOverrideMode }
...
{ @routine $72E77C TShip_ScriptItemsAct }
...
{ @routine $72F33C TShip_CheckDockingPermission }
...
{ @routine $72F360 TShip_RelationToShip }
