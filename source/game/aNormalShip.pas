unit aNormalShip;
// Unit bracket (inferred): .text 0x007819A0..0x0078BD65; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    procedure ClearObjectReferences; override; // @addr $782074 @slot $0C Clears pending liberation and last-docked planet after inherited cleanup.
    procedure SaveToBlock(Block: TBlockParEC); override; // @addr $78209C @slot $10
    procedure LoadFromBlock(Block: TBlockParEC); override; // @addr $7822DC @slot $14 Native editable import truncates PirateRankPoints to Word.
    procedure CheckKillCountAwards(Victim: TShip); // @addr $787158
    function SelectAward(Owner: Byte; Kinds: TAwardTypeMask; VictimTypes: TShipTypeMask): Byte; // @addr $787390 @ida "unsigned __int8 __userpurge $name@<al>(TNormalShip *Self@<eax>, unsigned __int8 Owner@<dl>, unsigned __int8 Kinds@<cl>, unsigned __int16 VictimTypes@<^0>);" @note "Returns 255 when no award qualifies; retries duplicates twice."
    procedure ProcessShipKill(Victim: TShip); // @addr $7841FC Distributes kill rewards, career/rank changes and event counters.
    function CollectLiberationRewards: WideString; // @addr $7828F4 @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);" @note "Consumes the pending ceremony, grants rewards and returns the player's presentation text."
    function SelectSituationalMessage(Automatic: Boolean): WideString; // @addr $788330 @ida "void __usercall $name(TNormalShip *Self@<eax>, bool Automatic@<dl>, unsigned __int16 **Result@<ecx>);" @note "Automatic messages suppress object links and select the automatic-message category."
    procedure UpdateRelationsForNearbyCombat; // @addr $7871E0 Nearby rangers attacking a friend incur a penalty; attacks on an enemy can improve relations.
    procedure NextDay; override; // @addr 0x78250C @slot 0x18
    constructor Create; // @addr 0x781AC8 @ida "TNormalShip *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x781BBC @ida "void __usercall $name(TNormalShip *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure SaveToBuffer(Buffer: TBufEC); override; // @addr 0x781C24 @slot 0x00
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); override; // @addr 0x781DC0 @slot 0x04
    procedure ResolveLoadedReferences(Galaxy: TGalaxy); override; // @addr 0x782008 @slot 0x08
    function AwardRandomMedal: WideString; // @addr $784024 @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);" Selects a merit award using the current planet, station or default human owner and returns its name.
    function GetPirateRankName: WideString; // @addr $787DA8 @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetNextPirateRankName: WideString; // @addr $787F9C @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);" Does not check maximum rank.
    function CanPromotePirateRank: Boolean; // @addr $78816C
    procedure UpdateAfterburnerState; override; // @addr $78BC04 @slot $9C Enables afterburner for multi-turn orders with a serviceable engine.
    procedure TrainSkillsAutomatically; // @addr 0x78BC68

    function GetAwardInfo(AwardId: Byte): TRewardInfo; // @addr 0x787784 @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int8 AwardId@<dl>, TRewardInfo *Result@<ecx>);"
    function GetRankLongName: WideString; // @addr $787964 @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetRankDescription: WideString; // @addr $787A04 @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetPirateRankLongName: WideString; // @addr $787E4C @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetPirateRankDescription: WideString; // @addr $787EF8 @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetRankName: WideString; // @addr 0x7878CC @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetNextRankName: WideString; // @addr 0x787A9C @ida "void __usercall $name(TNormalShip *Self@<eax>, unsigned __int16 **Result@<edx>);" @note "Does not check for maximum rank."

    function GetRankPointsToNextRank: Word; // @addr 0x787B34
    procedure AddRankPoints(Amount: Word); // @addr 0x787B9C @note "Caps the addition at the points needed for the next rank."
    function CanPromoteRank: Boolean; // @addr 0x787D74
    function TryPromoteRank: Boolean; // @addr 0x787D2C @note "Maximum rank is 7; promotion resets RankPoints."
    function GetPirateRankPointsToNextRank: Word; // @addr 0x788040
    procedure AddPirateRankPoints(Amount: Cardinal); // @addr 0x7880A8 @note "Caps the addition at the points needed for the next rank."
    function TryPromotePirateRank: Boolean; // @addr 0x788108 @note "Maximum rank is 7; promotion resets PirateRankPoints."
  end;

procedure ProcessSystemLiberationRewards(SourceShip: TNormalShip; Star: TStar); // @addr 0x7863B0 @note "Processes qualifying ships after a control change; SourceShip supplies the news-variant seed."
...
{ @routine $781AC8 TNormalShip_Create }
...
{ @routine $781BBC TNormalShip_Destroy }
...
{ @routine $781C24 TNormalShip_SaveToBuffer }
...
{ @routine $781DC0 TNormalShip_LoadFromBuffer }
...
{ @routine $782008 TNormalShip_ResolveLoadedReferences }
...
{ @routine $782074 TNormalShip_ClearObjectReferences }
...
{ @routine $78209C TNormalShip_SaveToBlock }
...
{ @routine $7822DC TNormalShip_LoadFromBlock }
...
{ @routine $78250C TNormalShip_NextDay }
...
{ @routine $7828F4 TNormalShip_CollectLiberationRewards }
...
{ @routine $784024 TNormalShip_AwardRandomMedal }
...
{ @routine $7841FC TNormalShip_ProcessShipKill }
...
var
  ...
  // @nested $784120 RecordShipKillCategory
  procedure RecordShipKillCategory(Ship: TNormalShip; Victim: TShip); // @addr $784120 @ida "void __usercall $name(TNormalShip *Ship@<eax>, TShip *Victim@<edx>, void *ParentFrame@<^0>);" @note "Nested helper; unused caller-popped static link."
  begin
...
{ @routine $7863B0 ProcessSystemLiberationRewards }
...
var
  ...
  // @nested $7862F8 LogPlayerEvent
  procedure LogPlayerEvent(Ship: TShip); // @addr $7862F8 @ida "void __usercall $name(TShip *Ship@<eax>, void *ParentFrame@<^0>);" @note "Nested helper; caller-popped link, Star at ParentFrame-4."
  var
    Event: TGalaxyEvent;
  begin
...
{ @routine $787158 TNormalShip_CheckKillCountAwards }
procedure TNormalShip.CheckKillCountAwards(Victim: TShip);
  // @nested $786EFC Check
  procedure Check(InitialThreshold, Multiplier: Integer; Count: Word; VictimType: Byte); // @addr $786EFC @ida "void __userpurge $name(int InitialThreshold@<eax>, int Multiplier@<edx>, unsigned __int16 Count@<cx>, unsigned __int8 VictimType@<^0>, void *ParentFrame@<^4>);" @stackpop $4 @calls "0x78718F 0x7871AE 0x7871D6" @note "Caller-popped static link; ship at ParentFrame-4."
  const
    BadAwards = [atPerfidy];
  var
...
{ @routine $7871E0 TNormalShip_UpdateRelationsForNearbyCombat }
...
{ @routine $787390 TNormalShip_SelectAward }
...
{ @routine $787784 TNormalShip_GetAwardInfo }
...
{ @routine $7878CC TNormalShip_GetRankName }
...
{ @routine $787964 TNormalShip_GetRankLongName }
...
{ @routine $787A04 TNormalShip_GetRankDescription }
...
{ @routine $787A9C TNormalShip_GetNextRankName }
...
{ @routine $787B34 TNormalShip_GetRankPointsToNextRank }
...
{ @routine $787B9C TNormalShip_AddRankPoints }
...
{ @routine $787D2C TNormalShip_TryPromoteRank }
...
{ @routine $787D74 TNormalShip_CanPromoteRank }
...
{ @routine $787DA8 TNormalShip_GetPirateRankName }
...
{ @routine $787E4C TNormalShip_GetPirateRankLongName }
...
{ @routine $787EF8 TNormalShip_GetPirateRankDescription }
...
{ @routine $787F9C TNormalShip_GetNextPirateRankName }
...
{ @routine $788040 TNormalShip_GetPirateRankPointsToNextRank }
...
{ @routine $7880A8 TNormalShip_AddPirateRankPoints }
...
{ @routine $788108 TNormalShip_TryPromotePirateRank }
...
var
  ...
  Other: TShip;
  // @nested $7881C4 ShuffleDefinitions
  procedure ShuffleDefinitions; // @addr $7881C4 @ida "void __usercall $name(void *ParentFrame@<^0>);" @note "Nested helper; caller-popped static link. Copies definitions and swaps the first half against deterministic random positions."
...
{ @routine $78BC04 TNormalShip_UpdateAfterburnerState }
...
{ @routine $78BC68 TNormalShip_TrainSkillsAutomatically }
