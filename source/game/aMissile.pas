unit aMissile;
// Unit bracket (inferred): .text 0x005E5554..0x005E8A48; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    destructor Destroy; override; // @addr $5E56A8 @ida "void __usercall $name(TMissile *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure SaveToBuffer(Buffer: TBufEC); virtual; // @addr $5E5C08 @slot $00
    procedure LoadFromBuffer(Buffer: TBufEC; World: TGalaxy); virtual; // @addr $5E6084 @slot $04
    procedure ResolveLoadedReferences(World: TGalaxy); // @addr $5E6534
    procedure InitializeUnownedShot(Star: TStar; Target: TObject; X, Y: Integer; Direction: Single; MinDamage, MaxDamage: Integer; MaximumSpeed: Single; ItemType: Byte; ModuleIndex, SpecialIndex: Integer); // @addr $5E5AB4 @ida "void __userpurge $name(TMissile *Self@<eax>, TStar *Star@<edx>, TObject *Target@<ecx>, int X@<^32>, int Y@<^28>, float Direction@<^24>, int MinDamage@<^20>, int MaxDamage@<^16>, float MaximumSpeed@<^12>, unsigned __int8 ItemType@<^8>, int ModuleIndex@<^4>, int SpecialIndex@<^0>);"
    function CanBeHit(Attacker: TShip; UnusedWeapon: TWeapon): Boolean; // @addr $5E87D0
    procedure RetargetTorpedo; // @addr $5E7E28
    function TryReturnToOwner(StepIndex: Integer; RecordFilm: Boolean; PreviousPosition: TPointF; Ship: TShip): Boolean; // @addr $5E7BEC @ida "bool __userpurge $name@<al>(TMissile *Self@<eax>, int StepIndex@<edx>, bool RecordFilm@<cl>, TPointF *PreviousPosition@<^4>, TShip *Ship@<^0>);"
    procedure ClearReferencesTo(Obj: TObject); // @addr 0x5E8868
    constructor Create; // @addr 0x5E5628 @ida "TMissile *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    procedure InitializeShot(Star: TStar; OwnerShip: TShip; Weapon: TWeapon; Target: TObject; ShotIndex: Integer); // @addr 0x5E5730 @note "Registers the missile in Star, copies weapon data and initializes position, heading and speed."
    procedure PrepareTurnMovement(StepIndex: Integer; RecordFilm, PlayShotSound: Boolean); // @addr 0x5E67E8
    function GetDisplayName: WideString; // @addr $5E7FD8 @ida "void __usercall $name(TMissile *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetInfoText: WideString; // @addr $5E80D0 @ida "void __usercall $name(TMissile *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetGraphObject: TObjectSE; // @addr $5E66D8 Lazily creates and initializes the retained missile scene object.
    function GetGraphSuffix: WideString; virtual; // @addr $5E890C @slot $08 @ida "void __usercall $name(TMissile *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetWeaponInfo: PWeaponInfo; virtual; // @addr 0x5E8A00 @slot 0x0C
    function GetShotVisual: Integer; // @addr 0x5E88B4 @note "Special micromodule override unless -1, otherwise the weapon-info default."
    function StepDay(StepIndex: Integer; RecordFilm: Boolean): TObject; // @addr 0x5E6D4C @note "Returns a hit ship, item or asteroid, or nil when no object was hit."
  end;
  ...
  public
    ...
    procedure SaveToBuffer(Buffer: TBufEC); override; // @addr $5E5FFC
    procedure LoadFromBuffer(Buffer: TBufEC; World: TGalaxy); override; // @addr $5E64C4
    function GetGraphSuffix: WideString; override; // @addr $5E89A4 @ida "void __usercall $name(TCustomMissile *Self@<eax>, unsigned __int16 **Result@<edx>);"
    procedure InitializeUnownedShot(Star: TStar; Target: TObject; X, Y: Integer; Direction: Single; MinDamage, MaxDamage: Integer; MaximumSpeed: Single; WeaponName: WideString; ModuleIndex, SpecialIndex: Integer); // @addr $5E5B74 @ida "void __userpurge $name(TCustomMissile *Self@<eax>, TStar *Star@<edx>, TObject *Target@<ecx>, int X@<^32>, int Y@<^28>, float Direction@<^24>, int MinDamage@<^20>, int MaxDamage@<^16>, float MaximumSpeed@<^12>, unsigned __int16 *WeaponName@<^8>, int ModuleIndex@<^4>, int SpecialIndex@<^0>);"
    function GetWeaponInfo: PWeaponInfo; override; // @addr 0x5E8A30 @slot 0x0C
    procedure InitializeShot(Star: TStar; OwnerShip: TShip; Weapon: TWeapon; Target: TObject; ShotIndex: Integer); // @addr 0x5E5A74 @note "Caches Weapon.GetWeaponInfo before the base initializer."
  end;
...
{ @routine $5E5628 TMissile_Create }
...
{ @routine $5E56A8 TMissile_Destroy }
...
{ @routine $5E5730 TMissile_InitializeShot }
...
{ @routine $5E5A74 TCustomMissile_InitializeShot }
...
{ @routine $5E5AB4 TMissile_InitializeUnownedShot }
...
{ @routine $5E5B74 TCustomMissile_InitializeUnownedShot }
...
{ @routine $5E5C08 TMissile_SaveToBuffer }
...
{ @routine $5E5FFC TCustomMissile_SaveToBuffer }
...
{ @routine $5E6084 TMissile_LoadFromBuffer }
procedure TMissile.LoadFromBuffer(Buffer: TBufEC; World: TGalaxy);
var
  ModuleNumber: Integer;
// @nested $5E6028 FindLegacyMissileMicroModuleIndex
function FindLegacyMissileMicroModuleIndex(ConfigNumber: Integer): Integer; // @addr $5E6028 @ida "int __usercall $name@<eax>(int ConfigNumber@<eax>, void *ParentFrame@<^0>);" @note "Nested legacy lookup; caller pops the unused static link."
var
  I: Integer;
begin
  Result := 0;
...
{ @routine $5E64C4 TCustomMissile_LoadFromBuffer }
...
{ @routine $5E6534 TMissile_ResolveLoadedReferences }
...
{ @routine $5E66D8 TMissile_GetGraphObject }
...
{ @routine $5E67E8 TMissile_PrepareTurnMovement }
...
{ @routine $5E6D4C TMissile_StepDay }
...
{ @routine $5E7BEC TMissile_TryReturnToOwner }
...
{ @routine $5E7E28 TMissile_RetargetTorpedo }
...
{ @routine $5E7FD8 TMissile_GetDisplayName }
...
{ @routine $5E80D0 TMissile_GetInfoText }
...
{ @routine $5E87D0 TMissile_CanBeHit }
...
{ @routine $5E8868 TMissile_ClearReferencesTo }
...
{ @routine $5E88B4 TMissile_GetShotVisual }
...
{ @routine $5E890C TMissile_GetGraphSuffix }
...
{ @routine $5E89A4 TCustomMissile_GetGraphSuffix }
...
{ @routine $5E8A00 TMissile_GetWeaponInfo }
...
{ @routine $5E8A30 TCustomMissile_GetWeaponInfo }
