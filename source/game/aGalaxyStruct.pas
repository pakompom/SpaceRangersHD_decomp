...
const
  // TShip.TypeId names from ShipTypeNames at $87C518 (initialized by the table
  // at $7E737C) and the subclass initializers.
  ...
  // CurrentStanding categories: native GetControlPresence ($85EE04),
  // ResetControlFaction ($860458), and subclass RefreshCurrentStanding methods.
  ...
  // GetScriptStandingOverrideMode ($72E6C0); its SubFaction test is preserved.
  ssmNormal = 0;
  ssmCustomFaction = 1;
  ssmFixed = 2;

  // Greeting category bits from InitializeShipGreetingDefinitions ($809360) and
  ...
  // Hull categories from ShipToHullType ($7DE530), GetDefaultHullType ($6FF248),
  // and ApplySpecialMicroModule ($74B808). They are not TShip.TypeId values.
  htRanger = 0;
  htWarrior = 1;
  htPirate = 2;
  htTransport = 3;
  ...
  // ProgramNames at $87EAC4 (initializer descriptors $7E7380..$7E73DC).
  ...
  // CoalitionProjectNames ($87EA64), investment dispatch ($5867B4/$589E4C),
  // and the military-base war operation ($57CD20) share these cooldown indices.
  ...
  // Conversation IDs shared by ShowPlayerDialogue ($727230),
  // TfTalk.BuildBuiltinChoices ($6096F8), and script Talk* constants.
  tkMoneyDemand = 0;
  tkGoodsDemand = 1;
  tkTruceOffer = 2;
  tkAttack = 3;
  ...
  // Award categories from SysToReward ($7DDD7C); SelectAward returns $FF on failure.
  atLiberation = 0;
  atAccomplishment = 1;
  atSecretMission = 2;
  atCowardice = 3;
...
const
  // Semantic aliases follow WeaponDamageFlagNames at $87E830 and the native
  ...
  // The native aShip pools at $7127C8/$7127EC and $719594/$7195A8 are separate.
...
type
  ...
  // OwnerToSys ($7DD828) and RaceToSys ($7DD210) establish these IDs.
  ...
  TPlanetRaceMarketInfo = packed record // @size 0xA8
    ...
    FriendlyRelationScale: Single; // @offset $94 Scales transport-to-transport relations ($5C19C4) and partner-gift gains ($612088/$61613C).
  ...
  // Native record RTTI at $7DBF14.

  // Native record RTTI at $7D960C.
