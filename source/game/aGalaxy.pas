unit aGalaxy;
// Unit bracket (inferred): .text 0x00838A68..0x00871045; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create; // @addr 0x86D528 @ida "TInterfaceStateOverride *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x86D56C @ida "void __usercall $name(TInterfaceStateOverride *Self@<eax>, __int8 DestroyFlags@<dl>);" @note "Restores the original control value when the control still exists."
    procedure Initialize(FormName, ControlPath: WideString; State: Byte); // @addr 0x86D608 @note "Captures the original value before applying the override."
    procedure SetState(State: Byte); // @addr 0x86D85C
    function GetState: Byte; // @addr 0x86DA38
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x86DA54
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr 0x86DA9C @note "Loads both values and immediately reapplies the override."
    procedure Reapply; // @addr 0x86DB68
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x86DD30 @ida "TInterfaceTextOverride *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x86DD74 @ida "void __usercall $name(TInterfaceTextOverride *Self@<eax>, __int8 DestroyFlags@<dl>);" @note "Restores the original control value when the control still exists."
    procedure Initialize(FormName, ControlPath: WideString; Text: WideString); // @addr 0x86DE98 @note "Captures the original value before applying the override."
    procedure SetText(Text: WideString); // @addr 0x86E158
    function GetText: WideString; // @addr 0x86E3A4 @ida "void __usercall $name(TInterfaceTextOverride *Self@<eax>, unsigned __int16 **Result@<edx>);"
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x86E3C4
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr 0x86E40C @note "Loads both values and immediately reapplies the override."
    procedure Reapply; // @addr 0x86E4C0
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x86E6EC @ida "TInterfaceImageOverride *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x86E730 @ida "void __usercall $name(TInterfaceImageOverride *Self@<eax>, __int8 DestroyFlags@<dl>);" @note "Restores the original value when the control still exists; an empty original style is not restored."
    procedure Initialize(FormName, ControlPath: WideString; ImagePath: WideString); // @addr 0x86E938 @note "Captures the original value before applying the override."
    procedure SetImagePath(ImagePath: WideString); // @addr 0x86ED54 @note "Also accepts Style:name for any control. Do not change between image and style modes: they share one original-value slot."
    function GetImagePath: WideString; // @addr 0x86F0A8 @ida "void __usercall $name(TInterfaceImageOverride *Self@<eax>, unsigned __int16 **Result@<edx>);"
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x86F0C8
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr 0x86F110 @note "Loads both values and immediately reapplies the override."
    procedure Reapply; // @addr 0x86F1C4
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x86F4F8 @ida "TInterfacePosOverride *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x86F53C @ida "void __usercall $name(TInterfacePosOverride *Self@<eax>, __int8 DestroyFlags@<dl>);" @note "Restores the original control value when the control still exists."
    procedure Initialize(FormName, ControlPath: WideString; DeltaX, DeltaY, DeltaDepth: Integer); // @addr 0x86F5C8 @note "Captures the original value before applying the override."
    procedure SetPosition(DeltaX, DeltaY, DeltaDepth: Integer); // @addr 0x86F824 @note "Offsets are relative to the captured original position and depth, not the current control values."
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x86FA18
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr 0x86FAA0 @note "Loads both values and immediately reapplies the override."
    procedure Reapply; // @addr 0x86FB78
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x86FD34 @ida "TInterfaceSizeOverride *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x86FD78 @ida "void __usercall $name(TInterfaceSizeOverride *Self@<eax>, __int8 DestroyFlags@<dl>);" @note "Restores the original control value when the control still exists."
    procedure Initialize(FormName, ControlPath: WideString; Width, Height: Integer); // @addr 0x86FDF4 @note "Captures the original value before applying the override."
    procedure SetSize(Width, Height: Integer); // @addr 0x870034 @note "Nonpositive dimensions restore the corresponding original dimension."
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x870220
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr 0x870284 @note "Loads both values and immediately reapplies the override."
  ...
  public
    ...
    constructor CreateEmpty; // @addr 0x8704EC @ida "TStoredItem *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    constructor Create(Name: WideString; Item: TObject); // @addr 0x870540 @ida "TStoredItem *__userpurge $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, unsigned __int16 *Name@<ecx>, TObject *Item@<^0>);"
    destructor Destroy; override; // @addr 0x8705D0 @ida "void __usercall $name(TStoredItem *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x870620 @note "Requires a non-nil Item."
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); // @addr 0x87065C @note "Creates the owned item; overwrites a previous Item without freeing it."
  end;
  ...
  public
    ...
    RangerSpawnQuotas: array[0..4] of Integer; // @offset 0x100  Pilot-race indices; may be negative.
    ...
    constructor Create; // @addr 0x839198 @ida "TGalaxy *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x839A18 @ida "void __usercall $name(TGalaxy *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure AssignTextQuestsToPlanets; // @addr 0x8576B4 @note "Uses quest target-owner filters; leaves planets with an existing quest unchanged."
    procedure RunConfigOnStartHandlers; // @addr 0x84022C
    procedure RunConfigOnLoadHandlers; // @addr 0x8402F8
    procedure RunConfigOnSaveHandlers; // @addr 0x8403C4
    procedure InitializeCampaignState; // @addr 0x83A034
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x83A1A4 @note "Increments SaveCount and restores temporary shop stock before serialization."
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr 0x83B6CC
    procedure SaveEditableState; // @addr 0x83E920 @note "Replaces the shared editable-save block, including player, holes and stars. Requires a player."
    procedure ApplyEditableState; // @addr 0x83F3B8 @note "Only runs when FinalizationNameEncoded is empty; consumes and clears the shared editable-save block. Requires a player."
    procedure GenerateGalaxyLayout(PlayerRace: Byte); // @addr 0x84FF74
    function FindConstellationIndexForStar(Star: TStar): Integer; // @addr 0x84F220 @note "Uses polygon containment, not Star.Constellation; returns -1 when no polygon contains the star."
    procedure InitializeConstellationDistanceTiers; // @addr 0x84F28C @note "Requires a player/home planet and generated outlines. Boss sectors are forced to tier three."
    procedure HideSpecialConstellation; // @addr 0x851710 @note "Requires constellation ID 20 and generated compatible outlines. Merges its visible outline into a neighbor and retains backups for RestoreHiddenForm."
    procedure BuildConstellationOutlineJunctions; // @addr 0x84F418
    function ShouldKeepConstellationOutlineVertex(Point: TPointF): Boolean; // @addr 0x84F800 @ida "bool __usercall $name@<al>(TGalaxy *Self@<eax>, TPointF *Point@<edx>);"
    procedure SimplifyConstellationOutline(ConstellationIndex: Integer); // @addr 0x84F938
    function BuildConstellationStarGraphs: Boolean; // @addr 0x84FB7C @note "Attempts every constellation; false means at least one graph is disconnected."
    procedure BuildConstellationPolygonsAndAdjacency(WorkingPolygon: TPolygon2D); // @addr 0x84FBD8 @note "Requires at least eight constellations."
    function CountVisibleConstellationsWithBoundaryPoints(FirstPoint, SecondPoint: TPointF): Integer; // @addr 0x854ACC @ida "int __usercall $name@<eax>(TGalaxy *Self@<eax>, TPointF *FirstPoint@<edx>, TPointF *SecondPoint@<ecx>);"
    procedure ShowLocalizedWarning(TextKey: WideString); // @addr 0x86D4C0 @note "Adds a kind-five player notification using localized text."
    procedure ReapplyInterfaceOverrides; // @addr 0x840490
    procedure BindScriptImports; // @addr 0x8405C0
    procedure RefreshAllShipDerivedState; // @addr 0x841DD0
    procedure RebuildStarDistances; // @addr 0x841D80 @note "Refreshes every star's cache; called during generation and loading, not by Script.CoordX/CoordY."
    procedure CompleteDay(UnusedRecordFilm: Boolean); // @addr 0x840F08
    procedure NextDay; // @addr 0x840608 @note "Processes off-screen stars; the player's star is simulated separately."
    procedure ProcessStationSpawning; // @addr 0x85A744
    procedure ReplenishStationType(StationType: TStationType); // @addr 0x85A830
    function TryCreateLiberationGroup: Boolean; // @addr 0x85C2E8 @note "Returns BuildLiberationOrders on accepted groups; false for early rejection or disbanding."
    function TryDispatchMilitaryBaseToEnemyStar: Boolean; // @addr 0x85C9A8
    function SelectStarForLiberationAttack(Origin: TStar; FriendlyFaction: TStarFaction): TStar; // @addr 0x858484 @note "Nil Origin omits the origin-distance penalty. Searches active Galaxy; advances Self.RandomState."
    function FindMilitaryBaseInTransit: Pointer; // @addr 0x85D048 @note "Searches active Galaxy; requires an assigned destination different from the current normal-space star."
    function HasMilitaryBaseAssignedToStar(Star: TStar): Boolean; // @addr 0x85D120 @note "Searches active Galaxy; includes a base already at its assigned destination."
    function HasLiberationGroupTargetingStar(Star: TStar): Boolean; // @addr 0x85D1E0 @note "Tests the fourth order target of each liberation group, unlike TStar.HasLiberationGroupOrder."
    procedure CancelEnemyJumpsToStar(Star: TStar); // @addr 0x858338 @note "Cancels normal-space Dominator and hostile-pirate jumps from other stars, plus assigned Dominion relocations."
    procedure ProcessCoalitionDefeat; // @addr 0x85D89C @note "Can complete pirate ending five, clear eminent ranger titles and publish defeat news."
    procedure ApplyWingmanLeadershipPenalty; // @addr 0x85D738
    procedure TransferShipsInTransit; // @addr 0x841BE8

    function IdToConstellation(Id: Cardinal): TConstellation; // @addr 0x841E60 @note "Zero returns nil; an unknown nonzero ID raises."
    function IdToStar(Id: Cardinal): TStar; // @addr 0x841F24 @note "Zero returns nil; an unknown nonzero ID raises."
    function IdToHole(Id: Cardinal): THole; // @addr 0x84201C @note "Zero returns nil; an unknown nonzero ID raises."
    function IdToPlanet(Id: Cardinal; RaiseIfMissing: Boolean = True): Pointer; // @addr 0x8420D8 @note "Zero always returns nil."
    function IdToShip(Id: Cardinal; RaiseIfMissing: Boolean): Pointer; // @addr 0x842234 @note "Zero always returns nil. Includes docked ships and stored Tranclucators."
    function IdToItem(Id: Cardinal; RaiseIfMissing: Boolean): Pointer; // @addr 0x842630 @note "Zero always returns nil. Includes inventories, shops, storage and moving drops."
    function IdToAsteroid(Id: Cardinal): Pointer; // @addr 0x842C74 @note "Returns nil when absent."
    function IdToMissile(Id: Cardinal): Pointer; // @addr 0x842D1C @note "Returns nil when absent."
    function ContainsShipReference(Ship: Pointer): Boolean; // @addr 0x842DC4
    function ContainsPlanetReference(Planet: Pointer): Boolean; // @addr 0x842F60
    function FindHoleInStarByKind(Star: TStar; HoleKind: Integer): THole; // @addr 0x843168 @note "Accepts either endpoint star; returns nil when absent."
    function CreateJumpGate(WithEffect: Boolean): PJumpGateEntry; // @addr 0x843088 @note "Registers an owned gate descriptor; returned storage is borrowed until ClearJumpGates."
    procedure ClearJumpGates; // @addr 0x842FF4
    procedure ReleaseItemGraphics; // @addr 0x8431E0 @note "Visits loose items and ship equipment/artifacts; excludes shop stock and stored items."
    procedure GenerateSpaceBackground(BackgroundIndex: Integer); // @addr 0x8433B0 @note "Replaces SpaceBackgroundEntries. Requires PlayerStar, a nonempty star list and differing minimum/maximum map diameters."
    procedure EnableDominatorSurfaces; // @addr 0x84427C
    procedure DisableDominatorSurfaces; // @addr 0x844380

    procedure XorProtectedState(Seed: Integer); // @addr 0x844750 @note "Includes the active text quest; applying the same seed twice restores the state."
    procedure ObfuscateProtectedState; // @addr 0x84564C @note "Already-obfuscated state is left unchanged."
    procedure RestoreProtectedState; // @addr 0x8456A8
    function ComputeIntegrityChecksum(Mode: Integer): Cardinal; // @addr 0x8458D4 @note "An unconditional jump disables the checksum body; always returns zero in this binary."
    procedure PrimeIntegrityChecksum(StatusCode: Integer); // @addr 0x84680C
    procedure CheckIntegrityChecksum(ErrorCode: Integer); // @addr 0x8468F8 @note "A matching checksum clears the integrity status."
    procedure PrimeIntegrityChecksum1(StatusCode: Integer); // @addr 0x846858
    procedure PrimeIntegrityChecksum2(StatusCode: Integer); // @addr 0x8468A8
    procedure CheckIntegrityChecksumAndSetStatus(StatusCode: Integer); // @addr 0x84699C @note "Channel zero; unlike CheckIntegrityChecksum, a matching checksum stores StatusCode rather than zero."
    procedure CheckIntegrityChecksum1(ErrorCode: Integer); // @addr 0x846A40
    procedure CheckIntegrityChecksum2(ErrorCode: Integer); // @addr 0x846AE4
    procedure ClearIntegrityStatus; // @addr 0x846B88
    procedure AppendIntegritySnapshot; // @addr 0x846BB0 @note "An unconditional jump disables the snapshot body; no-op in this binary."

    function GetStoredItem(Name: WideString; Remove: Boolean): TObject; // @addr 0x8709E8 @note "Returns nil when absent. Remove detaches the item and frees its named entry, transferring ownership to the caller."
    procedure StoreItem(Name: WideString; Item: TObject); // @addr 0x8706E4 @note "Takes ownership. Replacing a name frees the previously stored item; detach Item from its old container first."

    function CanRecordAchievements: Boolean; // @addr $87108C Checks special simulation, cheat points and all protected integrity flags.
    function GetCheatPoints: Integer; // @addr 0x84752C
    procedure SetCheatPoints(Value: Integer); // @addr 0x847554
    function HasVisibleScoreModFlags: Boolean; // @addr 0x8474BC
    function CountEligibleRangers: Integer; // @addr 0x856E20
    procedure RefreshRangerWealthStats; // @addr 0x856E78
    procedure RefreshRangerStrengthStats; // @addr 0x85705C
    procedure RefreshRangerRatingPlaces; // @addr 0x857238 @note "Assigns one-based positions by descending TotalExperience, including excluded rangers; leaves the Rangers list order unchanged."
    function FindStrongestRanger: Pointer; // @addr 0x8573E0 @note "Ignores ExcludedFromRating rangers; returns nil if none has positive strength."
    function FindWealthiestRanger: Pointer; // @addr 0x85745C @note "Ignores ExcludedFromRating rangers; returns nil if none has positive wealth."
    function CountFactionStars(Faction: Byte): Integer; // @addr 0x8574DC @note "Excludes stars with a custom faction."
    function GetFactionControlPercent(Faction: Byte): TPercent; // @addr 0x857544
    function GetDominatorSeriesControlShare(Series: TDominatorSeries): Single; // @addr 0x857588 @note "Active Galaxy only. Fraction of Dominator systems in Series, multiplied by the number of unresolved series; not a percentage."
    function CountStarsInBattle: Integer; // @addr 0x857660
    function TurnToDateTime(Turn: Integer): Double; // @addr 0x857BFC @note "Delphi TDateTime; -1 selects CurrentTurn."
    function FormatTurnDate(Turn: Integer): WideString; // @addr 0x857C3C @ida "void __usercall $name(TGalaxy *Self@<eax>, int Turn@<edx>, unsigned __int16 **Result@<ecx>);" @note "-1 selects CurrentTurn."
    procedure AddPlanetNews(NewsType: Byte; Text: WideString); // @addr 0x857E64 @note "Rejects empty text; identical existing text suppresses insertion regardless of NewsType."
    procedure AddPlanetNewsWithPlayerBubble(NewsType: Byte; Text: WideString); // @addr 0x857DF0 @note "Adds a player bubble only after turn 300; news insertion still uses duplicate-text suppression."
    function CountPlanetNewsByType(NewsType: Byte): Integer; // @addr 0x857FB4
    procedure PrunePlanetNews; // @addr 0x858014 @note "Removes entries more than 30 days old."
    procedure UpdateConstellationMilitaryStats; // @addr 0x8580C8
    procedure CreateDominatorSpawnProxy(Star: TStar); // @addr 0x858088 @note "Replaces the global spawn-planet pointer without freeing its previous value; does not register the proxy in star or galaxy planet lists."
    function RefreshTechLevel: Byte; // @addr 0x8581D0 @note "Updates TechLevel but normally returns zero. The special mode at +0x1D8 returns 8 without updating it."
    function HasPlayerQuestHistory(QuestType: TQuestType; QuestNumber: Word): Boolean; // @addr 0x857B94
    procedure ComputeGlobalGoodsPriceBands; // @addr 0x85861C
    function ScaleGoodsPriceByGalaxyAge(BaseValue: Integer): Integer; // @addr 0x8588B4
    function ScaleGoodsStockByGalaxyAge(BaseValue: Integer): Integer; // @addr 0x858920
    function GetGoodsPricePercent(GoodsType: Byte; Price: Integer): TPercent; // @addr 0x858844 @note "Maps the global minimum/maximum price band to 0..100 with clamping."
    function ScaleIntByTechLevel(AtLevelTwo, AtLevelSeven: Integer): Integer; // @addr 0x85898C @note "Clamps TechLevel to 2..7, linearly interpolates the endpoints, then rounds."
    function InterpolateSingleByTechLevel(AtLevelTwo, AtLevelSeven: Single): Single; // @addr 0x8589E8 @ida "float __userpurge $name@<st0>(TGalaxy *Self@<eax>, float AtLevelTwo@<^4>, float AtLevelSeven@<^0>);"
    function GetOrCreateCustomWeaponInfo(Name: WideString): PWeaponInfo; // @addr $870BC8 Inserts a new custom template in the case-insensitive sorted pool.
    function RequireCustomWeaponInfo(Name: WideString): PWeaponInfo; // @addr $870DB8 Binary search of CustomWeaponTypes; raises if absent.
    function SelectWeaponInfo(Seed: Cardinal; AvailabilityMask: TWeaponAvailabilityMask; MaximumTechLevel, MinimumTechLevel: Byte): PWeaponInfo; // @addr 0x858AA4 @ida "PWeaponInfo __userpurge $name@<eax>(TGalaxy *Self@<eax>, unsigned int Seed@<edx>, unsigned __int16 AvailabilityMask@<cx>, unsigned __int8 MaximumTechLevel@<^4>, unsigned __int8 MinimumTechLevel@<^0>);" @note "Borrowed template. Uses the closest eligible technology when the interval has no match; falls back to the first built-in template when no availability matches."
    function SelectMicroModule(MinimumPriority, MaximumPriority: Byte; Seed: Cardinal; Context: TObject): Integer; // @addr 0x858C7C @note "Zero-based index; Context may be a planet or ship, or nil. Relaxes the priority interval after repeated misses. No termination guarantee when every template fails the context filter."
    function SelectMicroModuleForEquipment(MinimumPriority, MaximumPriority: Byte; Seed: Cardinal; Context: TObject; Item: Pointer): Integer; // @addr 0x859084 @note "Zero-based index. The final attempt-limit fallback can return an incompatible module; callers must check CanInstallMicroModule. Context rejection can bypass the attempt-limit check."
    function SelectHullSeries(OwnerId, HullType, MinimumRarity, MaximumRarity: Byte): Integer; // @addr 0x859500 @note "Zero-based series index or -1; advances Self.RandomState."
    function ResolveMoneySizeTag(Tag: WideString; ScaleIndex: Byte): Integer; // @addr 0x859F18 @note "Accepts Zero, Mini, Small, Average, Big and Huge; unknown tags raise. Uses active Galaxy for scaling."

    function GetMiniGoodsQuantity(GoodsType: Byte): Integer; // @addr 0x85A11C
    function GetSmallGoodsQuantity(GoodsType: Byte): Integer; // @addr 0x85A160
    function GetAverageGoodsQuantity(GoodsType: Byte): Integer; // @addr 0x85A19C
    function GetBigGoodsQuantity(GoodsType: Byte): Integer; // @addr 0x85A1D4
    function GetHugeGoodsQuantity(GoodsType: Byte): Integer; // @addr 0x85A210
    function GetGoodsQuantityBySize(Size: Byte; GoodsType: Byte): Integer; // @addr 0x85A24C @note "Sizes 0..5 select zero through huge; invalid sizes raise."
    function ClassifyGoodsQuantity(Quantity: Integer; GoodsType: Byte): Byte; // @addr 0x85A36C @note "Zero maps to zero; otherwise chooses levels 1..5, with ties favoring the larger level."
    function GetMinimumGoodsPrice(GoodsType: Byte): Integer; // @addr 0x85A448
    function GetLowGoodsPrice(GoodsType: Byte): Integer; // @addr 0x85A474
    function GetAverageGoodsPrice(GoodsType: Byte): Integer; // @addr 0x85A4BC
    function GetHighGoodsPrice(GoodsType: Byte): Integer; // @addr 0x85A4E8
    function GetMaximumGoodsPrice(GoodsType: Byte): Integer; // @addr 0x85A530
    function GetGoodsPriceByLevel(Level: Byte; GoodsType: Byte): Integer; // @addr 0x85A55C @note "Levels 1..5 select minimum through maximum; invalid levels raise."
    function ClassifyGoodsPrice(Price: Integer; GoodsType: Byte): Byte; // @addr 0x85A668 @note "Zero maps to zero; otherwise chooses levels 1..5, with ties favoring the larger level."
    function HasUnresolvedDominatorSeries(Series: TDominatorSeriesSet): Boolean; // @addr 0x8597D0 @ida "bool __usercall $name@<al>(TGalaxy *Self@<eax>, TDominatorSeriesSet Series@<dl>);" @note "True if any selected series is unresolved; false for an empty set."
    function IsDominatorSeriesUnresolved(Series: TDominatorSeries): Boolean; // @addr 0x859778 @note "Invalid series values return false."
    function IsDominatorResearchComplete(Series: TDominatorSeriesSet): Boolean; // @addr 0x85AF80 @ida "bool __usercall $name@<al>(TGalaxy *Self@<eax>, TDominatorSeriesSet Series@<dl>);" @note "Every selected series must have at least 100 progress; the empty set returns true."
    procedure ComputeRangerSpawnQuotas; // @addr 0x85DCAC
    procedure PruneExpiredGalaxyEvents; // @addr 0x85DFAC @note "Retains the most recent 1825 days."
    function GetCoalitionToPirateSystemRatio: Single; // @addr 0x85E010 @note "Uses the active Galaxy, not Self; denominator is max(pirate systems - 1, 1)."

    function GetEffectiveDifficultyLevel: Integer; // @addr 0x85E060 @note "With custom rules disabled, reads the active Galaxy difficulty array rather than Self."
    function GetDifficultyTierIndex: TDifficultyTier; // @addr 0x85E0B0 @note "Returns 0..9; tier boundaries are 6, 14, 22, and subsequent increments of eight."
    function InterpolateDifficulty(Level: Integer; AtZero, AtEight, AtSixteen, AtTwentyFour: Single): Single; // @addr 0x85E114 @ida "float __userpurge $name@<st0>(TGalaxy *Self@<eax>, int Level@<edx>, float AtZero@<^12>, float AtEight@<^8>, float AtSixteen@<^4>, float AtTwentyFour@<^0>);" @note "A negative Level selects the effective difficulty; values above 24 extrapolate."
    function ScaleDifficultyExponentially(Level: Integer; BaseValue, FactorPerEightLevels: Single): Single; // @addr 0x85E230 @ida "float __userpurge $name@<st0>(TGalaxy *Self@<eax>, int Level@<edx>, float BaseValue@<^4>, float FactorPerEightLevels@<^0>);" @note "A negative Level selects the effective difficulty."
    function GetTurnsBetweenLiberationGroups: Integer; // @addr 0x85E318
    function GetDominatorBossHullScale: Single; // @addr 0x85E290
    function GetDominatorKillExperienceScale: Single; // @addr 0x85E2C4
    function GetInitialDominatorControlPercent: Integer; // @addr 0x85E3AC

    procedure ProcessDominatorResearchProgress; // @addr 0x85AC64
    function GetDominatorResearchRate(Series: TDominatorSeries): Single; // @addr 0x85AFDC @note "Percentage points per day."
    function GetDominatorResearchEfficiency(Series: TDominatorSeries): TPercent; // @addr 0x85B058 @note "Returns 20..100 percent."
    procedure ProcessBankDebtAndDeposits; // @addr 0x85B4E8 @note "Debt pauses deposit accrual. Both states are cleared when no business centers remain."
    procedure TryAwardDepositPrize; // @addr 0x85B168 @note "Eligible only at positive multiples of 365 accrued deposit days."
    function FindStationByTypeAndIndex(Index: Integer; StationType: TStationType): Pointer; // @addr 0x85B0A8 @note "One-based index over active Galaxy star/ship order. Uses Self's cached type count as an early gate; missing entries return nil."
    procedure ProcessRangerCenterNewYearEvent; // @addr 0x85BCF4 @note "Also clears the player's deposited nodes when no ranger centers remain."
    procedure ProcessPlayerSatelliteExploration; // @addr 0x859848 @note "Advances deployed probes' terrain exploration and wear; idle completed planets still incur reduced wear."
    function CountExistingSatellites: Integer; // @addr 0x859B0C @note "Requires a player. Counts deployed/player-storage probes, loose and carried probes in active Galaxy, and Self.StoredItems; excludes shop stock."
    function IsChaoticRandomEnabled: Boolean; // @addr 0x85E48C @note "When enabled, seeded helpers ignore their supplied seed."
    function ComputeScaledMiniMoney(ScaleIndex: Byte): Integer; // @addr 0x859C98
    function ComputeScaledSmallMoney(ScaleIndex: Byte): Integer; // @addr 0x859D18
    function ComputeScaledAverageMoney(ScaleIndex: Byte): Integer; // @addr 0x859D98
    function ComputeScaledBigMoney(ScaleIndex: Byte): Integer; // @addr 0x859E18
    function ComputeScaledHugeMoney(ScaleIndex: Byte): Integer; // @addr 0x859E98
    function GetDominatorAggressionLevel: Integer; // @addr 0x85E3E0
    function GetDominatorSpawnLevel: Integer; // @addr 0x85E418
    function GetPirateAggressionLevel: Integer; // @addr 0x85E450
    function AreStationsNearStarsEnabled: Boolean; // @addr 0x85E4C0
    function IsFullStationTargetingEnabled: Boolean; // @addr 0x85E4F4
    function IsEquipmentKnowledgeUnrestricted: Boolean; // @addr 0x85E528
    function GetAsteroidModifier: Single; // @addr 0x85E55C
    function GetStarDamageDifficultyScale: Single; // @addr 0x85E5A8
    function AreSpecialShipsEnabled: Boolean; // @addr 0x85E5F8
    function GetMicroModuleOfferRollThresholdPercent: Single; // @addr 0x85E62C
    function GetNodeDropModifier: Single; // @addr 0x85E664
    function GetArcadeDropValueModifier: Single; // @addr 0x85E6B0
    function GetDropValueModifier: Single; // @addr 0x85E6FC
    function GetAgriculturalPlanetWeight: Integer; // @addr 0x85E748
    function GetMixedPlanetWeight: Integer; // @addr 0x85E7A8
    function GetIndustrialPlanetWeight: Integer; // @addr 0x85E808
    function IsZeroStartingExperienceEnabled: Boolean; // @addr 0x85E868
    function GetExtraRangerCount: Integer; // @addr 0x85E89C
    function IsArcadeBattleRoyaleEnabled: Boolean; // @addr 0x85E8CC
    function GetArcadeHitpointsModifier: Single; // @addr 0x85E900
    function GetArcadeDamageModifier: Single; // @addr 0x85E94C
    function AreDominatorRacialWeaponsEnabled: Boolean; // @addr 0x85E998
    function GetAIJunkToleranceLevel: Integer; // @addr 0x85E9CC
    function AreMaxRangeMissilesEnabled: Boolean; // @addr 0x85E9FC
    function IsOldHyperspaceEnabled: Boolean; // @addr 0x85EA30
    function ArePirateNodesEnabled: Boolean; // @addr 0x85EA64
    function IsAIShoppingEnabled: Boolean; // @addr 0x85EA98
    function IsStationShopUpdateEnabled: Boolean; // @addr 0x85EACC
    function AreDuplicateArtefactsEnabled: Boolean; // @addr 0x85EB00
    function GetHullGrowthMod: Byte; // @addr 0x85EB34
    function IsArcadeEquipmentChangeEnabled: Boolean; // @addr 0x85EB64
    function IsOldSpeedCalculationEnabled: Boolean; // @addr 0x85EB98
    function AreOldMissileBonusesEnabled: Boolean; // @addr 0x85EBCC
    procedure AssignSpecialStationService; // @addr 0x85D248
end;
  ...
  public
    ...
    constructor Create; // @addr 0x854B50 @ida "TConstellation *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x854C8C @ida "void __usercall $name(TConstellation *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x854D78
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); // @addr 0x855224 @note "Requires a fresh instance; saved object IDs remain unresolved."
    procedure ResolveLoadedReferences(Galaxy: TGalaxy); // @addr 0x855744
    function GetName: WideString; // @addr 0x856B48 @ida "void __usercall $name(TConstellation *Self@<eax>, unsigned __int16 **Result@<edx>);" @note "Localization key uses the current position in Galaxy.Constellations, not Id."

    procedure AddStar(Star: TStar); // @addr 0x855AC0 @note "Also sets Star.Constellation; does not remove earlier membership."
    procedure ClearStars; // @addr 0x855BE8 @note "Clears only the borrowed list; leaves Star.Constellation unchanged."
    procedure AddAdjacentConstellation(Constellation: TConstellation); // @addr 0x855AEC @note "Suppresses duplicates; does not add the reciprocal relationship."
    procedure ClearAdjacentConstellations; // @addr 0x855C08
    function HasAdjacentConstellation(Constellation: TConstellation): Boolean; // @addr 0x8565E0
    function SharesOutlineSegment(Constellation: TConstellation): Boolean; // @addr 0x855B1C @note "Self compares true; otherwise matches complete segment endpoints in either direction."

    procedure ClearStarLinks; // @addr 0x8557E8
    function FindNextClosestStarPair(var FirstStar, SecondStar: TStar; MinimumDistance: Integer): Integer; // @addr 0x855C4C @note "Resumes equal-distance pairs using the input stars; zero clears both outputs and means no pair remains."
    function HasStarGraphCycle: Boolean; // @addr 0x855E70 @note "Changes link traversal marks; returns false without checking when Stars.Count exceeds 100."
    function IsStarGraphConnected: Boolean; // @addr 0x85609C @note "Clears link traversal marks; returns false when Stars.Count exceeds 100."
    function BuildStarGraph: Boolean; // @addr 0x85623C @note "Replaces StarLinks; result reports connectivity."

    procedure ClearBoundaryRaySamples; // @addr 0x85583C
    procedure GenerateBoundaryRaySamples(Count: Integer); // @addr 0x855890 @note "Requires a positive Count."
    procedure SetOutlinePolygon(Polygon: TPolygon2D); // @addr 0x8559B4 @note "Takes ownership and frees the previous polygon chain; do not pass the current chain."
    procedure RebuildOutlineSegments; // @addr 0x8559F4 @note "Replaces the list without freeing its old segment records."
    procedure ClearOutlineSegmentsAndBounds; // @addr 0x855A30
    procedure ResetGeneratedMapShape; // @addr 0x855B90
    procedure RestoreHiddenForm; // @addr 0x854A54
    function GetOutlineArea: Single; // @addr 0x855C28
    procedure ExpandOutlineBounds(Point: TPointF); // @addr 0x8563E4 @ida "void __usercall $name(TConstellation *Self@<eax>, TPointF *Point@<edx>);"
    procedure RefreshOutlineBounds; // @addr 0x856470
    function ContainsPoint(Point: TPointF): Boolean; // @addr 0x856594 @ida "bool __usercall $name@<al>(TConstellation *Self@<eax>, TPointF *Point@<edx>);"
    function HasOutlineSegment(FirstPoint, SecondPoint: TPointF): Boolean; // @addr 0x856630 @ida "bool __usercall $name@<al>(TConstellation *Self@<eax>, TPointF *FirstPoint@<edx>, TPointF *SecondPoint@<ecx>);" @note "Endpoint matching uses a tolerance and accepts either direction."
    function AreBothPointsOnOutline(FirstPoint, SecondPoint: TPointF): Boolean; // @addr 0x8566AC @ida "bool __usercall $name@<al>(TConstellation *Self@<eax>, TPointF *FirstPoint@<edx>, TPointF *SecondPoint@<ecx>);" @note "The points may lie on different outline segments."
    function CalculateLabelPosition: TPointF; // @addr 0x856750 @ida "void __usercall $name(TConstellation *Self@<eax>, TPointF *Result@<edx>);" @note "Requires an outline yielding interior samples; the sample mean need not lie inside a concave outline."
    function HasOutlineVertex(Point: TPointF): Boolean; // @addr 0x856918 @ida "bool __usercall $name@<al>(TConstellation *Self@<eax>, TPointF *Point@<edx>);"
    function IsPointNearOutline(Point: TPointF): Boolean; // @addr 0x856990 @ida "bool __usercall $name@<al>(TConstellation *Self@<eax>, TPointF *Point@<edx>);" @note "Tests a distance of at most two map units."
    procedure NormalizeOutlineSegmentOrder; // @addr 0x856A2C

    function HasDominatorPresence: Boolean; // @addr 0x856C04 @note "Uses member stars' cached population counts."
    function HasPirateClanPresence: Boolean; // @addr 0x856C5C
    function HasBertorOfSeries(Series: TDominatorSeries): Boolean; // @addr 0x856D08
    function CountShipsByTypeMask(ShipTypeMask: TShipTypeMask): Integer; // @addr 0x856DD4 @note "Uses cached population counts."
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x84757C @ida "THole *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x8475DC @ida "void __usercall $name(THole *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure InitializeGraphic(GraphKey: WideString); // @addr 0x847624 @note "Empty GraphKey chooses a seeded Hole template. Clears ArcadeMapName; replacing an existing Graphic does not release the old reference."
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x847768 @note "Requires both endpoint stars and Graphic."
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); // @addr 0x84781C @note "Leaves endpoint IDs unresolved until ResolveLoadedReferences."
    procedure ResolveLoadedReferences(Galaxy: TGalaxy); // @addr 0x847980
    procedure SaveToBlock(Block: TBlockParEC); // @addr 0x8479D0 @note "Editable subset; writes remaining lifetime as CreatedTurn + 200 - Galaxy.CurrentTurn."
    procedure LoadFromBlock(Block: TBlockParEC); // @addr 0x847DEC @note "Updates endpoints, positions, remaining lifetime and ArcadeMapName; preserves Id, HoleType and Graphic."
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x848170 @ida "TCustomSystemInfo *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x8481B4 @ida "void __usercall $name(TCustomSystemInfo *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr 0x8481E8
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x8482A0
  end;
  ...
  public
    ...
    Status: TStarStatus; // @offset $40 Native managed-record RTTI at $838A20.
    ...
    constructor Create; // @addr 0x8482F8 @ida "TStar *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x848490 @ida "void __usercall $name(TStar *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x849198 @note "Removes empty moving-drop descriptors; includes module-integrity checks."
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); // @addr 0x849A68 @note "Appends owned objects; requires a fresh instance. References are resolved separately."
    procedure ResolveLoadedReferences(Galaxy: TGalaxy); // @addr 0x84A408
    procedure SaveToBlock(Block: TBlockParEC); // @addr 0x84A5C4 @note "Editable subset of the system; excludes the player ship."
    procedure LoadFromBlock(Block: TBlockParEC); // @addr 0x84AE18 @note "Updates existing objects and can create stations, planets, items and asteroids. Moves star-link endpoints but does not rebuild distance caches."
    procedure GenerateSystemContents(TerronSystem: Boolean); // @addr 0x848600
    procedure PrepareNextDay; // @addr 0x84CFC4 @note "Daily preparation without movement/combat simulation; the caller tracks whether it has already run."
    procedure NextDay(RecordFilm: Boolean); // @addr 0x863998
    procedure RefreshMovementStepParameters; // @addr 0x860A74 @note "200 steps in the player's star, 50 elsewhere; MovementStepScale is the reciprocal."
    procedure RefreshDerivedStats; // @addr 0x85ED1C
    procedure RefreshDominatorSeries; // @addr 0x85EC00 @note "Changes the series only when exactly one series has eligible local forces; includes the bosses."
    procedure RefreshMapDiameterAndStats; // @addr 0x860A54
    function ComputeMapDiameter: Integer; // @addr 0x860AF4 @note "Uses the final planet-list entry when nonempty; otherwise SystemRadius. Does not update MapDiameter."
    procedure UpdateControlFaction; // @addr 0x85F4E4 @note "Can transfer planet ownership and emit capture news. Pirate ending 3 prevents new Pirate Clan captures."
    procedure GetControlPresence(out PlayerPartyPresent, CoalitionPresent, DominatorsPresent, PiratesPresent, CustomPresent: Boolean); // @addr 0x85EE90 @note "Applies campaign-ending and liberation-contribution rules; not a raw ship-presence query."
    procedure ResetControlFaction; // @addr 0x8606E8 @note "Reevaluates ownership after clearing a custom faction, resets liberation contributions and can clear NPC prison terms. No-op without a player."
    procedure RefreshShipTypeCounts; // @addr 0x860D9C
    procedure RebuildStarDistances(Galaxy: TGalaxy); // @addr 0x861DD4
    function IsConstellationVisible: Boolean; // @addr 0x861934
    procedure ProcessItemScripts(TurnPhase: Integer); // @addr 0x84E884
    function DropMinerals(Quantity: Integer; Position: TPointF; Seed: Cardinal): Integer; // @addr 0x84BEB8 @ida "int __userpurge $name@<eax>(TStar *Self@<eax>, int Quantity@<edx>, TPointF *Position@<ecx>, unsigned int Seed@<^0>);" @note "Returns the sum of the new goods' Cost, not their quantity. Nonpositive Quantity creates no drops and returns zero."
    procedure ProcessPlayerAsteroidKill(MineralValue: Integer; Position: TPointF; AsteroidId: Cardinal); // @addr 0x84C310 @ida "void __userpurge $name(TStar *Self@<eax>, int MineralValue@<edx>, TPointF *Position@<ecx>, unsigned int AsteroidId@<^0>);" @note "Requires a player. Achievement progress is independent of eligibility for a planet's reward or complaint."
    procedure ClearTargetReferences(Target: TObject); // @addr 0x84C74C @note "Clears weapon, missile and queued attack references; accepts any target class. Nil is a no-op."
    procedure ClearShipReferences(Ship: Pointer); // @addr 0x84CA04 @note "Clears attack, landing and combat-event references without removing or freeing Ship. Nil is a no-op."
    procedure ClearItemReferences(Item: Pointer); // @addr 0x84CCE4 @note "Clears targets, pickups, ReferencedItems and moving-drop payload references; does not remove Item from Items or free it."
    procedure ClearCombatEventWeaponReferences(Weapon: Pointer); // @addr 0x84CF68
    procedure HandleObjectLeavingStar(Obj: TObject); // @addr 0x84D1CC @note "Updates followers and targets recursively for docked ships. Does not remove the object from Ships or free it; selected pursuers may receive a jump order."
    procedure AvoidShipPathCollisions; // @addr 0x84D744 @note "Trims or clears planned movement paths; does not apply collision damage."
    procedure RebuildShipMovementPaths; // @addr 0x84DBAC @note "Normal-space ships only; uses MovementStepCount."
    procedure PruneWeaponTargetsAfterTurn; // @addr 0x84BC28
    procedure MarkConnectedCombatEvents(Events: TList; Target: TObject; Group: Integer); // @addr 0x84BE14 @note "Recursively marks unmarked events sharing an attacker or target. Events contains PStarCombatEvent; Group must be nonzero."
    procedure OpenSpaceScene(MapPanel: TPanelGI; Minimap: TObjectGI; Screen: TMessageLoopGI); // @addr 0x84DC08
    procedure RefreshSpaceObjectPositions; // @addr 0x84E23C
    procedure QueueSpaceImageLoads(PendingLoads: TList; Owner: TObjectGI); // @addr 0x84E458
    procedure QueueHyperspaceShipImageLoads(PendingLoads: TList; Owner: TObjectGI); // @addr 0x84E73C
    function GetBackgroundImagePath(out Size: Integer): WideString; // @addr 0x84E7A4 @ida "void __usercall $name(TStar *Self@<eax>, int *Size@<edx>, unsigned __int16 **Result@<ecx>);" @note "Sets Size to 2000."
    procedure TryGenerateSystemNews; // @addr 0x8621E8 @note "Requires a visible peaceful system, no custom faction and an undefeated Coalition."

    function CountPlanetsByOwner(OwnerId: Byte): Integer; // @addr 0x860B5C
    function CountDistinctInhabitedPlanetOwners: Integer; // @addr 0x860BB4 @note "Counts owners 0..5 and 7; includes Dominators."
    function FindFirstInhabitedPlanet: Pointer; // @addr 0x860BFC @note "If every planet is uninhabited, returns the last planet; nil only for an empty list."
    function SelectRandomInhabitedPlanet: Pointer; // @addr 0x860C58 @note "Returns nil when no inhabited planet exists; advances the star RNG."
    function FindFastestResearchPlanet: Pointer; // @addr 0x860D14 @note "Excludes uninhabited planets; first entry wins equal progress rates."
    function CountEligibleRangersInSpace: Integer; // @addr 0x860E3C
    function CountShipsByTypeMask(ShipTypeMask: TShipTypeMask): Integer; // @addr 0x860EB0 @note "Uses cached population counts."
    function CountDominatorForces(Series: TDominatorSeries; ExcludeAbsoluteOrders, OtherSeries: Boolean; out Strength: Extended): Integer; // @addr 0x860F00 @note "Excludes bosses, scripted-standing ships and Dominators with a positive ActiveProgramAppliedTurn. Strength is an Extended output."
    function CountStandardDominatorsOfLocalSeries: Integer; // @addr 0x861054 @note "Counts normal-space types 1..5; excludes bosses and scripted standing."
    function CountPirateForces(ExcludeAbsoluteOrders: Boolean; out Strength: Extended; IncludeClanVariants, IncludeIndependent: Boolean): Integer; // @addr 0x86112C @note "Requires pirate ownership. IncludeIndependent also includes the pirate player."
    function CountCustomFactionForces(ExcludeAbsoluteOrders: Boolean; out Faction: WideString; out Strength: Extended): Integer; // @addr 0x86126C
    function CountOtherCustomFactionForces(ExcludeAbsoluteOrders: Boolean; out Faction: WideString; out Strength: Extended): Integer; // @addr 0x861360 @note "Faction is empty when counted ships do not share one nonempty faction tag."
    function CountPirateShips(IncludeOutsideStarSpace: Boolean): Integer; // @addr 0x8614EC @note "Includes the pirate player; excludes scripted standing."
    function CountForcesByOwnerGroups(out Strength: Extended; IncludeCoalition, IncludeDominators, IncludePirates, IncludeCustom: Boolean): Integer; // @addr 0x8616B8 @note "Includes local garrison ships absent from Ships, avoiding duplicate list entries."
    function CountRatedRangersByCareerMask(CareerMask: TRangerCareerSet): Byte; // @addr 0x8617C4 @note "Includes docked and hyperspace entries in Ships; excludes ExcludedFromRating. Byte count can wrap."
    function GetRangerNamesByCareerMask(CareerMask: TRangerCareerSet): WideString; // @addr 0x861854 @ida "void __usercall $name(TStar *Self@<eax>, unsigned __int8 CareerMask@<dl>, unsigned __int16 **Result@<ecx>);" @note "Unlike CountRatedRangersByCareerMask, includes ExcludedFromRating entries."
    function GetCachedFactionStrength(FactionGroup: Byte): Single; // @addr 0x861954 @note "Group 0 Coalition, 1 Dominators/custom, 2 pirates. Lazily refreshes all three once per active Galaxy.CurrentTurn."
    function SumBestRangerRelativeStrength(ShipTypeMask: TShipTypeMask): Single; // @addr 0x861D40 @note "Sums StrengthInBestRanger over Ships, excluding the three bosses; no docking/hyperspace filter."
    function FindNearestStarByFaction(Faction: TStarFaction; InBattle: Boolean): TStar; // @addr 0x862000 @note "Starts at distance-cache index one and excludes custom factions. Requires a current distance cache."
    function GetBoundaryPointTowardStar(Star: TStar): TPointF; // @addr 0x862088 @ida "void __usercall $name(TStar *Self@<eax>, TStar *Star@<edx>, TPointF *Result@<ecx>);"
    function HasLiberationGroupOrder: Boolean; // @addr 0x862134 @note "Searches every order target in active Galaxy.LiberationGroups."
    function HasHostilePresenceForScriptBinding: Boolean; // @addr 0x861F88 @note "Includes any TKling, standing eight, or scripted ship with nonempty faction not beginning with SubFaction. No docking/hyperspace filter; the substring test also accepts absence."
  end;

function ShouldContinuePlayerTravel: Boolean; // @addr 0x84ED48 @note "May prepare movement or start black-hole entry; false without a player."
function EstimatePlayerTravelTurns: Single; // @addr 0x84EEF0 @note "Uses distance divided by Speed + 1; zero for interrupted travel or unsupported orders."
function GetLocalObjectLink(Obj: TObject; Suppress: Boolean): WideString; // @addr 0x84F0AC @ida "void __usercall $name(TObject *Obj@<eax>, bool Suppress@<dl>, unsigned __int16 **Result@<ecx>);" @note "Object markup embeds the native pointer, not an ID. Ships/planets/loose items must be in the player's star. Suppress returns empty; otherwise supported objects require a player."
...
var
  ReservedMessageCounter: Integer = 0; // @addr $882090 Reset by galaxy construction; meaning unresolved.
  TurnsSinceLastShipMessage: Cardinal = 0; // @addr $882094 Reset by ship messages; incremented by Galaxy.NextDay and saved with galaxy state.
  ModuleSizeIntegrityStatus: Integer = 0; // @addr $882098 Startup size-check marker: positive means mismatch; nonpositive is accepted.
  ModuleCrcIntegrityStatus: Byte = 0; // @addr $88209C 0 unchecked, 1 accepted, 2 mismatch.
  ModuleCrcFailureValue: Integer = 0; // @addr $8820A0 Cleared on mismatch while saving star 1; no native readers.
  Galaxy: TGalaxy; // @addr 0x88C288
  PlayerStar: TStar; // @addr 0x88C28C
  PlayerDialogueRequestCount: Byte; // @addr $88C290 Incremented when a ship dialogue passes the shared message-delay threshold; only count <= 1 is accepted.
  WingmenPendingLeadershipPenalty: TList; // @addr 0x88C294  Borrowed TShip entries.

function GameTurnToDateTime(Turn: Integer): Double; // @addr 0x84EB6C
function FormatGameTurnDate(Turn: Integer): WideString; // @addr 0x84EB90 @ida "void __usercall $name(int Turn@<eax>, unsigned __int16 **Result@<edx>);"

var
  CameraSpeed: Integer = 10; // @addr 0x8820A4 Native initial camera-step limit.
  FastCameraSpeed: Integer = 20; // @addr 0x8820A8 Native initial camera-step limit.
...
var
  ...
  // @nested $838FF0 CheckModuleSize
  procedure CheckModuleSize(EncodedSize: Cardinal); // @addr 0x838FF0 @ida "void __usercall $name(unsigned int EncodedSize@<eax>, void *ParentFrame@<^0>);" @note "Caller-popped static link; filename at ParentFrame-4. Updates the startup integrity marker."
  ...
  // @nested $8390A0 ParseCheatsDisabledFlag
  function ParseCheatsDisabledFlag(Value: WideString): Boolean; // @addr 0x8390A0 @ida "bool __usercall $name@<al>(unsigned __int16 *Value@<eax>, void *ParentFrame@<^0>);" @note "Nested in TGalaxy.Create; unused caller-popped static link. Accepts exactly Yes, yes, True, true, TRUE or 1; no trimming."
  begin
...
{ @routine $839A18 TGalaxy_Destroy }
...
{ @routine $83A034 TGalaxy_InitializeCampaignState }
...
{ @routine $83A1A4 TGalaxy_SaveToBuffer }
...
begin
  ...
  for Race := 0 to 4 do Buffer.AddIntegerValue(RangerSpawnQuotas[Race]);
...
{ @routine $83B6CC TGalaxy_LoadFromBuffer }
...
var I, J, K, Count, TemplateIndex, ShipCount: Integer; X, Y: Single;
  ...
  WeaponInfo: PWeaponInfo; Crc: Cardinal; Race: Byte;
  // @nested $83B6C4 CompatibilityHook
  procedure CompatibilityHook; // @addr 0x83B6C4 @ida "void __cdecl $name(void *ParentFrame);" @note "Native no-op with an unused caller-popped static link."
  begin
  end;
begin
  Stage := 0;
  try
    ...
    if LoadedSaveVersion >= 133 then
      for Race := 0 to 4 do RangerSpawnQuotas[Race] := Buffer.GetInt32;
    ...
    if (LoadedSaveVersion < 121) and (MainPiratePlanet <> nil) then
      for I := 0 to Stars.Count - 1 do begin
        ...
        for J := 0 to Star.Ships.Count - 1 do begin
          ...
          if (Ship is TPirate) and (Ship.OwnerId = Byte(oiPirate)) and ((Ship as TPirate).PirateType <> 0) then
...
{ @routine $83E920 TGalaxy_SaveEditableState }
...
{ @routine $83F3B8 TGalaxy_ApplyEditableState }
...
{ @routine $84022C TGalaxy_RunConfigOnStartHandlers }
...
{ @routine $8402F8 TGalaxy_RunConfigOnLoadHandlers }
...
{ @routine $8403C4 TGalaxy_RunConfigOnSaveHandlers }
...
{ @routine $840490 TGalaxy_ReapplyInterfaceOverrides }
...
{ @routine $8405C0 TGalaxy_BindScriptImports }
...
{ @routine $840608 TGalaxy_NextDay }
...
{ @routine $840F08 TGalaxy_CompleteDay }
...
{ @routine $841BE8 TGalaxy_TransferShipsInTransit }
...
{ @routine $841D80 TGalaxy_RebuildStarDistances }
...
{ @routine $841DD0 TGalaxy_RefreshAllShipDerivedState }
...
{ @routine $841E60 TGalaxy_IdToConstellation }
...
{ @routine $841F24 TGalaxy_IdToStar }
...
{ @routine $84201C TGalaxy_IdToHole }
...
{ @routine $8420D8 TGalaxy_IdToPlanet }
...
{ @routine $842234 TGalaxy_IdToShip }
...
{ @routine $842630 TGalaxy_IdToItem }
...
{ @routine $842C74 TGalaxy_IdToAsteroid }
...
{ @routine $842D1C TGalaxy_IdToMissile }
...
{ @routine $842DC4 TGalaxy_ContainsShipReference }
...
{ @routine $842F60 TGalaxy_ContainsPlanetReference }
...
{ @routine $842FF4 TGalaxy_ClearJumpGates }
...
{ @routine $843088 TGalaxy_CreateJumpGate }
...
{ @routine $843168 TGalaxy_FindHoleInStarByKind }
...
{ @routine $8431E0 TGalaxy_ReleaseItemGraphics }
...
{ @routine $8433B0 TGalaxy_GenerateSpaceBackground }
...
var
  ...
  // @nested $843360 AdvanceEntry
  procedure AdvanceEntry; // @addr 0x843360 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; entry index -4, capacity -8, galaxy -12."
  begin
    Inc(EntryIndex);
...
{ @routine $84427C TGalaxy_EnableDominatorSurfaces }
...
{ @routine $844380 TGalaxy_DisableDominatorSurfaces }
...
{ @routine $844750 TGalaxy_XorProtectedState }
...
var ExclusionCount: Integer; Exclusions: array[0..10] of Cardinal;
  ...
  // @nested $844484 NextStateXorMask
  function NextStateXorMask: Cardinal; // @addr 0x844484 @ida "unsigned int __usercall $name@<eax>(void *ParentFrame@<^0>);" @note "Park-Miller state at ParentFrame-4; returns the updated state minus one."
  begin
  ...
  // @nested $8444E0 XorStateUInt64
  procedure XorStateUInt64(var Value: UInt64); // @addr 0x8444E0 @ida "void __usercall $name(unsigned __int64 *Value@<eax>, void *ParentFrame@<^0>);"
  begin
  ...
  // @nested $84450C XorStateUInt32
  procedure XorStateUInt32(var Value: Cardinal); // @addr 0x84450C @ida "void __usercall $name(unsigned int *Value@<eax>, void *ParentFrame@<^0>);"
  begin Value := Value xor NextStateXorMask; end;

  // @nested $844528 XorStateByte
  procedure XorStateByte(var Value: Byte); // @addr 0x844528 @ida "void __usercall $name(unsigned __int8 *Value@<eax>, void *ParentFrame@<^0>);"
  begin Value := Value xor Byte(NextStateXorMask); end;

  // @nested $844544 XorStateWords
  procedure XorStateWords(Data: PWord; Count: Cardinal); // @addr 0x844544 @ida "void __usercall $name(unsigned __int16 *Data@<eax>, unsigned int Count@<edx>, void *ParentFrame@<^0>);"
  var I: Integer;
  begin
    I := 0;
  ...
  // @nested $844580 XorStateBytes
  procedure XorStateBytes(Data: PByte; Count: Cardinal); // @addr 0x844580 @ida "void __usercall $name(unsigned __int8 *Data@<eax>, unsigned int Count@<edx>, void *ParentFrame@<^0>);"
  var I: Integer;
  begin
    I := 0;
    while Cardinal(I) < Count do begin
  ...
  // @nested $8445B8 XorStateObject
  procedure XorStateObject(Instance: TObject); // @addr 0x8445B8 @ida "void __usercall $name(TObject *Instance@<eax>, void *ParentFrame@<^0>);" @note "Preserves the VMT pointer."
  begin
    XorStateBytes(PByte(PAnsiChar(Instance) + 4), Instance.InstanceSize - 4);
  end;

  // @nested $8445E4 XorStateObjectExceptField
  procedure XorStateObjectExceptField(Instance: TObject; ExcludedField: Pointer); // @addr 0x8445E4 @ida "void __usercall $name(TObject *Instance@<eax>, void *ExcludedField@<edx>, void *ParentFrame@<^0>);" @note "Preserves the VMT and one four-byte field."
  ...
  // @nested $84466C XorStateObjectExceptFields
  procedure XorStateObjectExceptFields(Instance: TObject); // @addr 0x84466C @ida "void __usercall $name(TObject *Instance@<eax>, void *ParentFrame@<^0>);" @note "Preserves the VMT and sorted four-byte exclusions supplied by the parent frame."
...
{ @routine $84564C TGalaxy_ObfuscateProtectedState }
...
{ @routine $8456A8 TGalaxy_RestoreProtectedState }
...
{ @routine $8458D4 TGalaxy_ComputeIntegrityChecksum }
...
var
  ...
  // @nested $8456E8 AccumulateIntegrityUInt32
  procedure AccumulateIntegrityUInt32(Value: Cardinal); // @addr 0x8456E8 @ida "void __usercall $name(unsigned int Value@<eax>, void *ParentFrame@<^0>);"
  begin
    State := UpdateCrc32(State, @Value, 4);
  end;

  // @nested $84570C AccumulateIntegritySingle
  procedure AccumulateIntegritySingle(Value: Single); // @addr 0x84570C @ida "void __usercall $name(float Value@<^0>, void *ParentFrame@<^4>);" @stackpop 0x4 @calls "0x84593F 0x84594C 0x84595C 0x84596C 0x84672C"
  begin
    State := UpdateCrc32(State, @Value, 4);
  end;

  // @nested $84572C AccumulateIntegrityDouble
  procedure AccumulateIntegrityDouble(Value: Double); // @addr 0x84572C @ida "void __usercall $name(double Value@<^0>, void *ParentFrame@<^8>);" @stackpop 0x8 @calls "0x846478"
  begin
    State := UpdateCrc32(State, @Value, 8);
  end;

  // @nested $84574C AccumulateIntegrityByte
  procedure AccumulateIntegrityByte(Value: Byte); // @addr 0x84574C @ida "void __usercall $name(unsigned __int8 Value@<al>, void *ParentFrame@<^0>);"
  begin
    State := UpdateCrc32(State, @Value, 1);
  end;

  // @nested $845770 AccumulateIntegrityBoolean
  procedure AccumulateIntegrityBoolean(Value: Boolean); // @addr 0x845770 @ida "void __usercall $name(bool Value@<al>, void *ParentFrame@<^0>);"
  begin
    State := UpdateCrc32(State, @Value, 1);
  end;

  // @nested $845794 AccumulateIntegrityWords
  procedure AccumulateIntegrityWords(Data: PWord; Count: Integer); // @addr 0x845794 @ida "void __usercall $name(unsigned __int16 *Data@<eax>, int Count@<edx>, void *ParentFrame@<^0>);"
  begin
    State := UpdateCrc32(State, Data, Count * 2);
  end;

  // @nested $8457C0 AccumulateIntegrityBytes
  procedure AccumulateIntegrityBytes(Data: PByte; Count: Integer); // @addr 0x8457C0 @ida "void __usercall $name(unsigned __int8 *Data@<eax>, int Count@<edx>, void *ParentFrame@<^0>);"
  begin
    State := UpdateCrc32(State, Data, Count);
  end;

  // @nested $8457E8 AccumulateIntegrityObject
  procedure AccumulateIntegrityObject(Instance: TObject); // @addr 0x8457E8 @ida "void __usercall $name(TObject *Instance@<eax>, void *ParentFrame@<^0>);" @note "Excludes the VMT pointer; nil contributes nothing."
  begin
    if Instance <> nil then
      AccumulateIntegrityBytes(PByte(PAnsiChar(Instance) + SizeOf(Pointer)), Instance.InstanceSize - SizeOf(Pointer));
  end;

  // @nested $845818 AccumulateIntegrityItem
  procedure AccumulateIntegrityItem(Item: TItem); // @addr 0x845818 @ida "void __usercall $name(TItem *Item@<eax>, void *ParentFrame@<^0>);" @note "Opaque interface avoids the aItem dependency cycle; the nested body uses TItem. Excludes the VMT and temporarily zeros Graphic plus cached action-code state; nil contributes nothing."
...
{ @routine $84680C TGalaxy_PrimeIntegrityChecksum }
...
{ @routine $846858 TGalaxy_PrimeIntegrityChecksum1 }
...
{ @routine $8468A8 TGalaxy_PrimeIntegrityChecksum2 }
...
{ @routine $8468F8 TGalaxy_CheckIntegrityChecksum }
...
{ @routine $84699C TGalaxy_CheckIntegrityChecksumAndSetStatus }
...
{ @routine $846A40 TGalaxy_CheckIntegrityChecksum1 }
...
{ @routine $846AE4 TGalaxy_CheckIntegrityChecksum2 }
...
{ @routine $846B88 TGalaxy_ClearIntegrityStatus }
...
{ @routine $846BB0 TGalaxy_AppendIntegritySnapshot }
...
begin
  ...
  Buffer.AddAnsiChar(AnsiChar(Ord(GetPlayer.OwnerId = Byte(oiPirate))));
...
{ @routine $8474BC TGalaxy_HasVisibleScoreModFlags }
...
{ @routine $84752C TGalaxy_GetCheatPoints }
...
{ @routine $847554 TGalaxy_SetCheatPoints }
...
{ @routine $847624 THole_InitializeGraphic }
...
{ @routine $847768 THole_SaveToBuffer }
...
{ @routine $84781C THole_LoadFromBuffer }
...
{ @routine $847980 THole_ResolveLoadedReferences }
...
{ @routine $8479D0 THole_SaveToBlock }
...
{ @routine $847DEC THole_LoadFromBlock }
...
{ @routine $848170 TCustomSystemInfo_Create }
...
{ @routine $8481B4 TCustomSystemInfo_Destroy }
...
{ @routine $8481E8 TCustomSystemInfo_LoadFromBuffer }
...
{ @routine $8482A0 TCustomSystemInfo_SaveToBuffer }
...
{ @routine $848600 TStar_GenerateSystemContents }
...
{ @routine $849198 TStar_SaveToBuffer }
...
var FileName: WideString;
  ...
  // @nested $8490FC CheckModuleCRC
  procedure CheckModuleCRC(ExpectedCRC: Cardinal); // @addr 0x8490FC @ida "void __usercall $name(unsigned int ExpectedCRC@<eax>, void *ParentFrame@<^0>);" @note "Caller-popped static link; filename at -4, star at -8. Updates global integrity status on mismatch."
...
{ @routine $849A68 TStar_LoadFromBuffer }
...
{ @routine $84A408 TStar_ResolveLoadedReferences }
...
{ @routine $84A5C4 TStar_SaveToBlock }
...
{ @routine $84AE18 TStar_LoadFromBlock }
...
{ @routine $84BC28 TStar_PruneWeaponTargetsAfterTurn }
...
{ @routine $84BE14 TStar_MarkConnectedCombatEvents }
...
{ @routine $84BEB8 TStar_DropMinerals }
...
{ @routine $84C310 TStar_ProcessPlayerAsteroidKill }
...
{ @routine $84C74C TStar_ClearTargetReferences }
...
{ @routine $84CA04 TStar_ClearShipReferences }
...
{ @routine $84CCE4 TStar_ClearItemReferences }
...
{ @routine $84CF68 TStar_ClearCombatEventWeaponReferences }
...
{ @routine $84CFC4 TStar_PrepareNextDay }
...
{ @routine $84D1CC TStar_HandleObjectLeavingStar }
...
{ @routine $84D744 TStar_AvoidShipPathCollisions }
...
{ @routine $84DBAC TStar_RebuildShipMovementPaths }
...
{ @routine $84DC08 TStar_OpenSpaceScene }
...
{ @routine $84E23C TStar_RefreshSpaceObjectPositions }
...
{ @routine $84E458 TStar_QueueSpaceImageLoads }
...
{ @routine $84E73C TStar_QueueHyperspaceShipImageLoads }
...
{ @routine $84E7A4 TStar_GetBackgroundImagePath }
...
{ @routine $84E884 TStar_ProcessItemScripts }
...
{ @routine $84EB6C GameTurnToDateTime }
...
{ @routine $84EB90 FormatGameTurnDate }
...
{ @routine $84ED48 ShouldContinuePlayerTravel }
...
{ @routine $84EEF0 EstimatePlayerTravelTurns }
...
{ @routine $84F0AC GetLocalObjectLink }
...
{ @routine $84F220 TGalaxy_FindConstellationIndexForStar }
...
{ @routine $84F28C TGalaxy_InitializeConstellationDistanceTiers }
...
{ @routine $84F418 TGalaxy_BuildConstellationOutlineJunctions }
...
{ @routine $84F800 TGalaxy_ShouldKeepConstellationOutlineVertex }
...
{ @routine $84F938 TGalaxy_SimplifyConstellationOutline }
...
{ @routine $84FB7C TGalaxy_BuildConstellationStarGraphs }
...
{ @routine $84FBD8 TGalaxy_BuildConstellationPolygonsAndAdjacency }
...
{ @routine $84FF74 TGalaxy_GenerateGalaxyLayout }
...
{ @routine $851710 TGalaxy_HideSpecialConstellation }
...
var I, J, K, SegmentIndex, EarIndex, LastIndex, Reserved20, MiddleIndex: Integer;
  ...
  // @nested $851614 PointInsideTriangle
  function PointInsideTriangle(PointX, PointY, AX, AY, BX, BY, CX, CY: Single): Boolean; // @addr 0x851614 @ida "bool __usercall $name@<al>(float PointX@<^28>, float PointY@<^24>, float AX@<^20>, float AY@<^16>, float BX@<^12>, float BY@<^8>, float CX@<^4>, float CY@<^0>, void *ParentFrame@<^32>);" @stackpop 0x20 @calls "0x85268B 0x852A25 0x852C45 0x852FE3 0x853053 0x853289" @note "Strict interior only; excludes edges and degenerate triangles. The caller-popped static link is unused."
  begin
    Result := False;
...
{ @routine $854A54 TConstellation_RestoreHiddenForm }
...
{ @routine $854ACC TGalaxy_CountVisibleConstellationsWithBoundaryPoints }
...
{ @routine $854B50 TConstellation_Create }
...
{ @routine $854C8C TConstellation_Destroy }
...
{ @routine $854D78 TConstellation_SaveToBuffer }
...
{ @routine $855224 TConstellation_LoadFromBuffer }
...
{ @routine $855744 TConstellation_ResolveLoadedReferences }
...
{ @routine $8557E8 TConstellation_ClearStarLinks }
...
{ @routine $85583C TConstellation_ClearBoundaryRaySamples }
...
{ @routine $855890 TConstellation_GenerateBoundaryRaySamples }
...
{ @routine $8559B4 TConstellation_SetOutlinePolygon }
...
{ @routine $8559F4 TConstellation_RebuildOutlineSegments }
...
{ @routine $855A30 TConstellation_ClearOutlineSegmentsAndBounds }
...
{ @routine $855AC0 TConstellation_AddStar }
...
{ @routine $855AEC TConstellation_AddAdjacentConstellation }
...
{ @routine $855B1C TConstellation_SharesOutlineSegment }
...
{ @routine $855B90 TConstellation_ResetGeneratedMapShape }
...
{ @routine $855BE8 TConstellation_ClearStars }
...
{ @routine $855C08 TConstellation_ClearAdjacentConstellations }
...
{ @routine $855C28 TConstellation_GetOutlineArea }
...
{ @routine $855C4C TConstellation_FindNextClosestStarPair }
...
{ @routine $855E70 TConstellation_HasStarGraphCycle }
...
{ @routine $85609C TConstellation_IsStarGraphConnected }
...
{ @routine $85623C TConstellation_BuildStarGraph }
...
{ @routine $8563E4 TConstellation_ExpandOutlineBounds }
...
{ @routine $856470 TConstellation_RefreshOutlineBounds }
...
{ @routine $856594 TConstellation_ContainsPoint }
...
{ @routine $8565E0 TConstellation_HasAdjacentConstellation }
...
{ @routine $856630 TConstellation_HasOutlineSegment }
...
{ @routine $8566AC TConstellation_AreBothPointsOnOutline }
...
{ @routine $856750 TConstellation_CalculateLabelPosition }
...
{ @routine $856918 TConstellation_HasOutlineVertex }
...
{ @routine $856990 TConstellation_IsPointNearOutline }
...
{ @routine $856A2C TConstellation_NormalizeOutlineSegmentOrder }
...
{ @routine $856B48 TConstellation_GetName }
...
{ @routine $856C04 TConstellation_HasDominatorPresence }
...
{ @routine $856C5C TConstellation_HasPirateClanPresence }
...
begin
  for I := 0 to Stars.Count - 1 do begin
    ...
    for J := 0 to Star.Ships.Count - 1 do begin
      Ship := TShip(Star.Ships[J]);
      if (Ship is TPirate) and (Ship.OwnerId = Byte(oiPirate)) and (TPirate(Ship).PirateType <> 0) then begin Result := True; Exit; end;
    end;
  end;
  Result := False;
end;
...
{ @routine $856D08 TConstellation_HasBertorOfSeries }
...
{ @routine $856DD4 TConstellation_CountShipsByTypeMask }
...
{ @routine $856E20 TGalaxy_CountEligibleRangers }
...
{ @routine $856E78 TGalaxy_RefreshRangerWealthStats }
    ...
    begin
      ...
      if CoalitionDefeatedTurn <> 0 then
        ...
        begin
          ...
          begin
            Ship := Star.Ships[J];
            if (Ship is TPirate) and ((Ship as TPirate).PirateType = 0) and (Ship.OwnerId = Byte(oiPirate)) then
            begin
...
{ @routine $85705C TGalaxy_RefreshRangerStrengthStats }
...
{ @routine $857238 TGalaxy_RefreshRangerRatingPlaces }
...
{ @routine $8573E0 TGalaxy_FindStrongestRanger }
...
{ @routine $85745C TGalaxy_FindWealthiestRanger }
...
{ @routine $8574DC TGalaxy_CountFactionStars }
...
{ @routine $857544 TGalaxy_GetFactionControlPercent }
...
{ @routine $857588 TGalaxy_GetDominatorSeriesControlShare }
...
{ @routine $857660 TGalaxy_CountStarsInBattle }
...
{ @routine $8576B4 TGalaxy_AssignTextQuestsToPlanets }
...
{ @routine $857B94 TGalaxy_HasPlayerQuestHistory }
...
{ @routine $857BFC TGalaxy_TurnToDateTime }
...
{ @routine $857C3C TGalaxy_FormatTurnDate }
...
{ @routine $857DF0 TGalaxy_AddPlanetNewsWithPlayerBubble }
...
{ @routine $857E64 TGalaxy_AddPlanetNews }
...
{ @routine $857FB4 TGalaxy_CountPlanetNewsByType }
...
{ @routine $858014 TGalaxy_PrunePlanetNews }
...
{ @routine $858088 TGalaxy_CreateDominatorSpawnProxy }
...
{ @routine $8580C8 TGalaxy_UpdateConstellationMilitaryStats }
...
{ @routine $8581D0 TGalaxy_RefreshTechLevel }
...
begin
  ...
  for I := 0 to Planets.Count - 1 do begin
    Planet := Planets[I];
    if Planet.IsCoalitionOwned or (Planet.OwnerId = Byte(oiPirate)) then begin
...
{ @routine $858338 TGalaxy_CancelEnemyJumpsToStar }
...
{ @routine $858484 TGalaxy_SelectStarForLiberationAttack }
...
{ @routine $85861C TGalaxy_ComputeGlobalGoodsPriceBands }
...
{ @routine $858844 TGalaxy_GetGoodsPricePercent }
...
{ @routine $8588B4 TGalaxy_ScaleGoodsPriceByGalaxyAge }
...
{ @routine $858920 TGalaxy_ScaleGoodsStockByGalaxyAge }
...
{ @routine $85898C TGalaxy_ScaleIntByTechLevel }
...
{ @routine $8589E8 TGalaxy_InterpolateSingleByTechLevel }
...
{ @routine $858AA4 TGalaxy_SelectWeaponInfo }
...
var
  ...
  // @nested $858A3C TechDistance
  function TechDistance(TechLevel: Integer): Integer; // @addr 0x858A3C @ida "int __usercall $name@<eax>(int TechLevel@<eax>, void *ParentFrame@<^0>);" @note "Nested helper with caller-popped static link. Minimum/maximum bytes are at ParentFrame+8/+12."
  begin
    Result := 0;
...
{ @routine $858C7C TGalaxy_SelectMicroModule }
...
{ @routine $859084 TGalaxy_SelectMicroModuleForEquipment }
...
{ @routine $859500 TGalaxy_SelectHullSeries }
...
{ @routine $859778 TGalaxy_IsDominatorSeriesUnresolved }
...
{ @routine $8597D0 TGalaxy_HasUnresolvedDominatorSeries }
...
{ @routine $859848 TGalaxy_ProcessPlayerSatelliteExploration }
...
{ @routine $859B0C TGalaxy_CountExistingSatellites }
...
{ @routine $859C98 TGalaxy_ComputeScaledMiniMoney }
...
{ @routine $859D18 TGalaxy_ComputeScaledSmallMoney }
...
{ @routine $859D98 TGalaxy_ComputeScaledAverageMoney }
...
{ @routine $859E18 TGalaxy_ComputeScaledBigMoney }
...
{ @routine $859E98 TGalaxy_ComputeScaledHugeMoney }
...
{ @routine $859F18 TGalaxy_ResolveMoneySizeTag }
function TGalaxy.ResolveMoneySizeTag(Tag: WideString; ScaleIndex: Byte): Integer;
begin
  if Tag = 'Zero' then Result := 0
  else if Tag = 'Mini' then Result := Galaxy.ComputeScaledMiniMoney(ScaleIndex)
  else if Tag = 'Small' then Result := Galaxy.ComputeScaledSmallMoney(ScaleIndex)
  else if Tag = 'Average' then Result := Galaxy.ComputeScaledAverageMoney(ScaleIndex)
  else if Tag = 'Big' then Result := Galaxy.ComputeScaledBigMoney(ScaleIndex)
  else if Tag = 'Huge' then Result := Galaxy.ComputeScaledHugeMoney(ScaleIndex)
  else begin RaiseWideMessage('Error! Указан неправильный формат размера у вещи ' + Tag); Result := -1; end;
end;
{ @end $859F18 }

{ @routine $85A11C TGalaxy_GetMiniGoodsQuantity }
...
{ @routine $85A160 TGalaxy_GetSmallGoodsQuantity }
...
{ @routine $85A19C TGalaxy_GetAverageGoodsQuantity }
...
{ @routine $85A1D4 TGalaxy_GetBigGoodsQuantity }
...
{ @routine $85A210 TGalaxy_GetHugeGoodsQuantity }
...
{ @routine $85A24C TGalaxy_GetGoodsQuantityBySize }
...
{ @routine $85A36C TGalaxy_ClassifyGoodsQuantity }
...
{ @routine $85A448 TGalaxy_GetMinimumGoodsPrice }
...
{ @routine $85A474 TGalaxy_GetLowGoodsPrice }
...
{ @routine $85A4BC TGalaxy_GetAverageGoodsPrice }
...
{ @routine $85A4E8 TGalaxy_GetHighGoodsPrice }
...
{ @routine $85A530 TGalaxy_GetMaximumGoodsPrice }
...
{ @routine $85A55C TGalaxy_GetGoodsPriceByLevel }
...
{ @routine $85A668 TGalaxy_ClassifyGoodsPrice }
...
{ @routine $85A744 TGalaxy_ProcessStationSpawning }
...
{ @routine $85A830 TGalaxy_ReplenishStationType }
...
{ @routine $85AC64 TGalaxy_ProcessDominatorResearchProgress }
...
{ @routine $85AF80 TGalaxy_IsDominatorResearchComplete }
...
{ @routine $85AFDC TGalaxy_GetDominatorResearchRate }
...
{ @routine $85B058 TGalaxy_GetDominatorResearchEfficiency }
...
{ @routine $85B0A8 TGalaxy_FindStationByTypeAndIndex }
...
{ @routine $85B168 TGalaxy_TryAwardDepositPrize }
...
{ @routine $85B4E8 TGalaxy_ProcessBankDebtAndDeposits }
...
{ @routine $85BCF4 TGalaxy_ProcessRangerCenterNewYearEvent }
...
{ @routine $85C2E8 TGalaxy_TryCreateLiberationGroup }
...
{ @routine $85C9A8 TGalaxy_TryDispatchMilitaryBaseToEnemyStar }
...
{ @routine $85D048 TGalaxy_FindMilitaryBaseInTransit }
...
{ @routine $85D120 TGalaxy_HasMilitaryBaseAssignedToStar }
...
{ @routine $85D1E0 TGalaxy_HasLiberationGroupTargetingStar }
...
{ @routine $85D248 TGalaxy_AssignSpecialStationService }
...
{ @routine $85D738 TGalaxy_ApplyWingmanLeadershipPenalty }
...
{ @routine $85D89C TGalaxy_ProcessCoalitionDefeat }
...
{ @routine $85DCAC TGalaxy_ComputeRangerSpawnQuotas }
...
begin
  ...
  Total := 0;
  for Race := 0 to 4 do Inc(Total, Counts[Race]);
  ...
  for Race := 0 to 4 do Sorted[Race] := Counts[Race];
  for I := 0 to 3 do
    for J := I + 1 to 4 do
...
{ @routine $85DFAC TGalaxy_PruneExpiredGalaxyEvents }
...
{ @routine $85E010 TGalaxy_GetCoalitionToPirateSystemRatio }
...
{ @routine $85E060 TGalaxy_GetEffectiveDifficultyLevel }
...
{ @routine $85E0B0 TGalaxy_GetDifficultyTierIndex }
...
{ @routine $85E114 TGalaxy_InterpolateDifficulty }
...
{ @routine $85E230 TGalaxy_ScaleDifficultyExponentially }
...
{ @routine $85E290 TGalaxy_GetDominatorBossHullScale }
...
{ @routine $85E2C4 TGalaxy_GetDominatorKillExperienceScale }
...
{ @routine $85E318 TGalaxy_GetTurnsBetweenLiberationGroups }
...
{ @routine $85E3AC TGalaxy_GetInitialDominatorControlPercent }
...
{ @routine $85E3E0 TGalaxy_GetDominatorAggressionLevel }
...
{ @routine $85E418 TGalaxy_GetDominatorSpawnLevel }
...
{ @routine $85E450 TGalaxy_GetPirateAggressionLevel }
...
{ @routine $85E48C TGalaxy_IsChaoticRandomEnabled }
...
{ @routine $85E4C0 TGalaxy_AreStationsNearStarsEnabled }
...
{ @routine $85E4F4 TGalaxy_IsFullStationTargetingEnabled }
...
{ @routine $85E528 TGalaxy_IsEquipmentKnowledgeUnrestricted }
...
{ @routine $85E55C TGalaxy_GetAsteroidModifier }
...
{ @routine $85E5A8 TGalaxy_GetStarDamageDifficultyScale }
...
{ @routine $85E5F8 TGalaxy_AreSpecialShipsEnabled }
...
{ @routine $85E62C TGalaxy_GetMicroModuleOfferRollThresholdPercent }
...
{ @routine $85E664 TGalaxy_GetNodeDropModifier }
...
{ @routine $85E6B0 TGalaxy_GetArcadeDropValueModifier }
...
{ @routine $85E6FC TGalaxy_GetDropValueModifier }
...
{ @routine $85E748 TGalaxy_GetAgriculturalPlanetWeight }
...
{ @routine $85E7A8 TGalaxy_GetMixedPlanetWeight }
...
{ @routine $85E808 TGalaxy_GetIndustrialPlanetWeight }
...
{ @routine $85E868 TGalaxy_IsZeroStartingExperienceEnabled }
...
{ @routine $85E89C TGalaxy_GetExtraRangerCount }
...
{ @routine $85E8CC TGalaxy_IsArcadeBattleRoyaleEnabled }
...
{ @routine $85E900 TGalaxy_GetArcadeHitpointsModifier }
...
{ @routine $85E94C TGalaxy_GetArcadeDamageModifier }
...
{ @routine $85E998 TGalaxy_AreDominatorRacialWeaponsEnabled }
...
{ @routine $85E9CC TGalaxy_GetAIJunkToleranceLevel }
...
{ @routine $85E9FC TGalaxy_AreMaxRangeMissilesEnabled }
...
{ @routine $85EA30 TGalaxy_IsOldHyperspaceEnabled }
...
{ @routine $85EA64 TGalaxy_ArePirateNodesEnabled }
...
{ @routine $85EA98 TGalaxy_IsAIShoppingEnabled }
...
{ @routine $85EACC TGalaxy_IsStationShopUpdateEnabled }
...
{ @routine $85EB00 TGalaxy_AreDuplicateArtefactsEnabled }
...
{ @routine $85EB34 TGalaxy_GetHullGrowthMod }
...
{ @routine $85EB64 TGalaxy_IsArcadeEquipmentChangeEnabled }
...
{ @routine $85EB98 TGalaxy_IsOldSpeedCalculationEnabled }
...
{ @routine $85EBCC TGalaxy_AreOldMissileBonusesEnabled }
...
{ @routine $85EC00 TStar_RefreshDominatorSeries }
...
{ @routine $85ED1C TStar_RefreshDerivedStats }
...
{ @routine $85EE90 TStar_GetControlPresence }
...
var
  ...
  // @nested $85EE04 AccumulateControlPresence
  procedure AccumulateControlPresence; // @addr 0x85EE04 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; ship -4, Coalition output -8, faction flags -9..-11, other outputs +8..+16."
  begin
...
begin
  ...
  begin
    ...
    if not Ship.InHyperspace and
      ((Ship.CurrentPlanet = nil) or (Ship.CurrentPlanet.OwnerId <> Byte(oiUninhabited))) and
  ...
  begin
    ...
    if CoalitionKills < PirateLeaningKills then CoalitionPresent := False
      ...
      if GetPlayer.OwnerId = Byte(oiPirate) then CoalitionPresent := False else PiratesPresent := False
    else PiratesPresent := False;
  end;
end;
{ @end $85EE90 }

{ @routine $85F4E4 TStar_UpdateControlFaction }
...
begin
  ...
  begin
    ...
    begin
      ...
      if Planet.OwnerId <> Byte(oiUninhabited) then
      begin
        if Planet.OwnerId = Byte(oiPirate) then Planet.Government := TPlanetGovernment(SeededRandomIntRange(0, 4, Planet.RandomState));
...
{ @routine $8606E8 TStar_ResetControlFaction }
...
var Ship: TShip;
  ...
  // @nested $860458 CountShipStanding
  procedure CountShipStanding; // @addr 0x860458 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; ship -4, standing counters -8..-28."
  begin
    case Ship.CurrentStanding of
  ...
  // @nested $8604C8 SetCoalition
  procedure SetCoalition; // @addr 0x8604C8 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; star -32. Also transfers planets and clears eligible NPC ranger prison terms."
  var J: Integer;
  begin
    ControlFaction := sfCoalition;
    ...
    for J := 0 to Planets.Count - 1 do begin
      ...
      if Planet.OwnerId <> Byte(oiUninhabited) then begin
        Planet.OwnerId := RaceToOwner(Planet.RaceId);
        Planet.UpdateOwnerFlags;
      end;
    end;
  ...
  // @nested $8605E4 SetPirates
  procedure SetPirates; // @addr 0x8605E4 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; star -32. Also transfers planets and clears eligible NPC pirate prison terms."
  var J: Integer;
  begin
    ControlFaction := sfPirates;
    ...
    for J := 0 to Planets.Count - 1 do begin
      Planet := TPlanet(Planets[J]);
      if Planet.OwnerId <> Byte(oiUninhabited) then begin Planet.OwnerId := Byte(oiPirate); Planet.UpdateOwnerFlags; end;
    end;
    for J := 0 to Ships.Count - 1 do begin
      Ship := TShip(Ships[J]);
...
begin
  ...
  for I := 0 to Ships.Count - 1 do begin
    Ship := TShip(Ships[I]);
    if not Ship.InHyperspace and ((Ship.CurrentPlanet = nil) or (Ship.CurrentPlanet.OwnerId <> Byte(oiUninhabited))) and
      ((Ship.DockedTo = nil) or (GetPlayer = Ship)) and
      ((GetPlayer <> Ship) or ((Ship.CurrentPlanet = nil) and (Ship.DockedTo = nil)) or
  ...
  if DominatorCount > CoalitionCount + PirateCount + CoalitionPassiveCount + PiratePassiveCount + NeutralCount then begin
    ...
    for I := 0 to Planets.Count - 1 do begin
      Planet := TPlanet(Planets[I]);
      if Planet.OwnerId <> Byte(oiUninhabited) then begin Planet.OwnerId := Byte(oiDominator); Planet.UpdateOwnerFlags; end;
    end;
...
{ @routine $860A54 TStar_RefreshMapDiameterAndStats }
...
{ @routine $860A74 TStar_RefreshMovementStepParameters }
...
{ @routine $860AF4 TStar_ComputeMapDiameter }
...
{ @routine $860B5C TStar_CountPlanetsByOwner }
...
{ @routine $860BB4 TStar_CountDistinctInhabitedPlanetOwners }
...
begin
  ...
  if CountPlanetsByOwner(Ord(oiPirate)) > 0 then Inc(Result);
end;
{ @end $860BB4 }

{ @routine $860BFC TStar_FindFirstInhabitedPlanet }
...
begin
  ...
  begin
    ...
    if (TObject(Result) as TPlanet).OwnerId <> Byte(oiUninhabited) then Break;
  end;
end;
{ @end $860BFC }

{ @routine $860C58 TStar_SelectRandomInhabitedPlanet }
...
begin
  ...
  Remaining := 0;
  for I := 0 to Planets.Count - 1 do
    if TPlanet(Planets[I]).OwnerId <> Byte(oiUninhabited) then Inc(Remaining);
...
{ @routine $860D14 TStar_FindFastestResearchPlanet }
...
{ @routine $860D9C TStar_RefreshShipTypeCounts }
...
{ @routine $860E3C TStar_CountEligibleRangersInSpace }
...
{ @routine $860EB0 TStar_CountShipsByTypeMask }
...
{ @routine $860F00 TStar_CountDominatorForces }
...
{ @routine $861054 TStar_CountStandardDominatorsOfLocalSeries }
...
{ @routine $86112C TStar_CountPirateForces }
...
{ @routine $86126C TStar_CountCustomFactionForces }
...
{ @routine $861360 TStar_CountOtherCustomFactionForces }
...
{ @routine $8614EC TStar_CountPirateShips }
...
begin
  ...
  for I := 0 to Ships.Count - 1 do begin
    ...
    if (not Ship.IsOutsideStarSpace or (IncludeOutsideStarSpace <> False)) and (Ship.CurrentStanding <> ssCustom) then begin
      if (Ship is TPirate) and (Ship.OwnerId = Byte(oiPirate)) then Inc(Count);
      if (Ship = GetPlayer) and (GetPlayer.OwnerId = Byte(oiPirate)) then Inc(Count);
    end;
  end;
  Result := Count;
end;
...
{ @routine $8616B8 TStar_CountForcesByOwnerGroups }
...
var
  ...
  // @nested $861590 AccumulateFactionForces
  procedure AccumulateFactionForces; cdecl; // @addr 0x861590 @ida "void __cdecl $name(void *ParentFrame);"
  begin
    if Ship.InHyperspace then Exit;
    if (Ship.CurrentPlanet <> nil) and (Ship.CurrentPlanet.OwnerId = Byte(oiUninhabited)) then Exit;
...
{ @routine $8617C4 TStar_CountRatedRangersByCareerMask }
...
{ @routine $861854 TStar_GetRangerNamesByCareerMask }
...
{ @routine $861934 TStar_IsConstellationVisible }
...
{ @routine $861954 TStar_GetCachedFactionStrength }
...
begin
  if Galaxy.CurrentTurn = FactionStrengthCacheTurn then Result := Status.CachedFactionStrength[FactionGroup]
    ...
    // Native $861CEC/$861CF5/$861CFE store these in TStar+$4C/$50/$54.
...
{ @routine $861D40 TStar_SumBestRangerRelativeStrength }
...
{ @routine $861DD4 TStar_RebuildStarDistances }
...
{ @routine $861F88 TStar_HasHostilePresenceForScriptBinding }
...
{ @routine $862000 TStar_FindNearestStarByFaction }
...
{ @routine $862088 TStar_GetBoundaryPointTowardStar }
...
{ @routine $862134 TStar_HasLiberationGroupOrder }
...
{ @routine $8621E8 TStar_TryGenerateSystemNews }
...
var
  ...
  Reserved357: Byte;
  Stage: Integer;
// @nested $8630B0 CompleteItemPickup
procedure CompleteItemPickup; // @addr 0x8630B0 @ida "void __cdecl $name(void *ParentFrame);" @note "Caller-popped static link; item -4, ship -8, star -12, film flag -13. Transfers, merges or consumes the completed pickup; may free the item."
...
{ @routine $86D4C0 TGalaxy_ShowLocalizedWarning }
...
{ @routine $86D528 TInterfaceStateOverride_Create }
...
{ @routine $86D56C TInterfaceStateOverride_Destroy }
...
{ @routine $86D608 TInterfaceStateOverride_Initialize }
...
{ @routine $86D85C TInterfaceStateOverride_SetState }
...
{ @routine $86DA38 TInterfaceStateOverride_GetState }
...
{ @routine $86DA54 TInterfaceStateOverride_SaveToBuffer }
...
{ @routine $86DA9C TInterfaceStateOverride_LoadFromBuffer }
...
{ @routine $86DB68 TInterfaceStateOverride_Reapply }
...
{ @routine $86DD30 TInterfaceTextOverride_Create }
...
{ @routine $86DD74 TInterfaceTextOverride_Destroy }
...
{ @routine $86DE98 TInterfaceTextOverride_Initialize }
...
{ @routine $86E158 TInterfaceTextOverride_SetText }
...
{ @routine $86E3A4 TInterfaceTextOverride_GetText }
...
{ @routine $86E3C4 TInterfaceTextOverride_SaveToBuffer }
...
{ @routine $86E40C TInterfaceTextOverride_LoadFromBuffer }
...
{ @routine $86E4C0 TInterfaceTextOverride_Reapply }
...
{ @routine $86E6EC TInterfaceImageOverride_Create }
...
{ @routine $86E730 TInterfaceImageOverride_Destroy }
...
begin
  ...
  if Form <> nil then begin
    ...
    if Control <> nil then begin
      ...
      end else if Control is TgaiGI then (Control as TgaiGI).SetImagePath(OriginalImagePath)
...
{ @routine $86E938 TInterfaceImageOverride_Initialize }
...
begin
  ...
  if Control = nil then
    ...
    if (CountDelimitedPartsW(Self.ImagePath, ':') > 1) and (ExtractDelimitedPartW(Self.ImagePath, 0, ':') = 'Style') then begin
      ...
      OriginalImagePath := (Control as TgaiGI).GetImagePath;
      (Control as TgaiGI).SetImagePath(Self.ImagePath);
...
{ @routine $86ED54 TInterfaceImageOverride_SetImagePath }
...
{ @routine $86F0A8 TInterfaceImageOverride_GetImagePath }
...
{ @routine $86F0C8 TInterfaceImageOverride_SaveToBuffer }
...
{ @routine $86F110 TInterfaceImageOverride_LoadFromBuffer }
...
{ @routine $86F1C4 TInterfaceImageOverride_Reapply }
...
{ @routine $86F4F8 TInterfacePosOverride_Create }
...
{ @routine $86F53C TInterfacePosOverride_Destroy }
...
{ @routine $86F5C8 TInterfacePosOverride_Initialize }
...
{ @routine $86F824 TInterfacePosOverride_SetPosition }
...
{ @routine $86FA18 TInterfacePosOverride_SaveToBuffer }
...
{ @routine $86FAA0 TInterfacePosOverride_LoadFromBuffer }
...
{ @routine $86FB78 TInterfacePosOverride_Reapply }
...
{ @routine $86FD34 TInterfaceSizeOverride_Create }
...
{ @routine $86FD78 TInterfaceSizeOverride_Destroy }
...
{ @routine $86FDF4 TInterfaceSizeOverride_Initialize }
...
{ @routine $870034 TInterfaceSizeOverride_SetSize }
...
{ @routine $870220 TInterfaceSizeOverride_SaveToBuffer }
...
{ @routine $870284 TInterfaceSizeOverride_LoadFromBuffer }
...
{ @routine $870340 TInterfaceSizeOverride_Reapply }
...
{ @routine $8704EC TStoredItem_CreateEmpty }
...
{ @routine $870540 TStoredItem_Create }
...
{ @routine $8705D0 TStoredItem_Destroy }
...
{ @routine $870620 TStoredItem_SaveToBuffer }
...
{ @routine $87065C TStoredItem_LoadFromBuffer }
...
{ @routine $8706E4 TGalaxy_StoreItem }
...
{ @routine $8709E8 TGalaxy_GetStoredItem }
function TGalaxy.GetStoredItem(Name: WideString; Remove: Boolean): TObject;
var
  Entry: TStoredItem;
  Index, LowIndex, HighIndex, Comparison: Integer;

  // @nested $87099C TakeEntry
  function TakeEntry: TObject; // @addr 0x87099C @ida "TObject *__cdecl $name(void *ParentFrame);" @note "Nested in GetStoredItem; caller-popped static link. Entry -4, Remove -5, Self -12, Index -16."
  begin
    Result := Entry.Item;
    if Remove then
    begin
...
{ @routine $870BC8 TGalaxy_GetOrCreateCustomWeaponInfo }
function TGalaxy.GetOrCreateCustomWeaponInfo(Name: WideString): PWeaponInfo;
var
  Info: PWeaponInfo;
  LowIndex, HighIndex, Middle, Comparison: Integer;
  // @nested $870B48 Allocate
  procedure Allocate; // @addr $870B48 @ida "void __cdecl $name(void *ParentFrame);"
  var State: Cardinal;
  begin
    New(Result);
...
{ @routine $870DB8 TGalaxy_RequireCustomWeaponInfo }
...
{ @routine $87108C TGalaxy_CanRecordAchievements }
