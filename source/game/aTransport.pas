unit aTransport;
// Unit bracket (inferred): .text 0x005BF71C..0x005C5E1E; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    function AdjustItemEvaluation(Item: TItem; PriceMode: Byte; Effectiveness: Single): Single; override; // @addr $5C477C @slot $50 @ida "float __userpurge $name@<st0>(TTransport *Self@<eax>, TItem *Item@<edx>, unsigned __int8 PriceMode@<cl>, float Effectiveness@<^0>);"
    function EvaluateStatBonus(BonusKind: TEquipmentBonusKind; Value: Integer): Single; override; // @addr $5C4EE8 @slot $54 @ida "float __usercall $name@<st0>(TTransport *Self@<eax>, TEquipmentBonusKind BonusKind@<dl>, int Value@<ecx>);"
    function EvaluateWeaponDamage(Weapon: TWeapon; IncludeAdditiveBonuses: Boolean; BaseDamage: Single): Single; override; // @addr $5C58B0 @slot $58 @ida "float __userpurge $name@<st0>(TTransport *Self@<eax>, TWeapon *Weapon@<edx>, bool IncludeAdditiveBonuses@<cl>, float BaseDamage@<^0>);"

    function RelationToRanger(Ranger: Pointer): Byte; override; // @addr $5C1B48 @slot $74
    procedure ChangeRelationToRanger(Ranger: Pointer; Amount: Integer); override; // @addr $5C1B90 @slot $78
    procedure ReactToAttack(Attacker: TShip); override; // @addr $5C1CEC @slot $7C
    function RelationToNonRanger(Ship: TShip): Byte; override; // @addr $5C19C4 @slot $80
    function AcceptsRansomDemandFrom(Ship: TShip): Boolean; override; // @addr $5C23C4 @slot $88
    function TrustsAttackRequester(Ship: TShip): Boolean; override; // @addr $5C24B0 @slot $8C
    function EvaluateAllyRelationAndStrength(Ship: TShip): Boolean; override; // @addr $5C24D4 @slot $90
    procedure ProcessCombatDialogue; override; // @addr $5C2F0C @slot $A0
    procedure ReactToExtortionDemand(Ranger: Pointer); override; // @addr $5C2FA0 @slot $A4
    function AcceptPartnershipOffer(OtherShip: TShip; var Response: WideString; PaymentAmount: Integer): Boolean; override; // @addr $5C46DC @slot $B8
    function BuildPartnershipOfferResponse(OtherShip: TShip; var Response: WideString; PaymentAmount: Integer): Boolean; override; // @addr $5C472C @slot $BC

    function BuildMoneyExtortionResponse(OtherShip: TShip; var Response: WideString; DemandedAmount: Integer): Boolean; override; // @addr $5C3298 @slot $A8
    function BuildCargoExtortionResponse(OtherShip: TShip; var Response: WideString): Boolean; override; // @addr $5C39E8 @slot $AC
    function BuildTrucePaymentResponse(OtherShip: TShip; var Response: WideString; OfferedAmount: Integer): Boolean; override; // @addr $5C3DDC @slot $B0
    function BuildAttackRequestResponse(Requester: TShip; var Response: WideString; Target: TShip): Boolean; override; // @addr $5C41C4 @slot $B4

    destructor Destroy; override; // @addr 0x5BF844 @ida "void __usercall $name(TTransport *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure InitGenerated(Planet: TPlanet; InitialMoney: Integer; SubType: TTransportType; RandomizeSubType: Boolean); // @addr 0x5BFC8C @note "Registers Self with the planet's star; requires a fresh instance."
    procedure SaveToBuffer(Buffer: TBufEC); override; // @addr 0x5C0260 @slot 0x00
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); override; // @addr 0x5C028C @slot 0x04
    procedure ResolveLoadedReferences(Galaxy: TGalaxy); override; // @addr 0x5C02C0 @slot 0x08

    function GetTypeNameKey: WideString; override; // @addr $5C165C @slot $2C @ida "void __usercall $name(TTransport *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetGreetingShipCategory: Byte; override; // @addr $5C1688 @slot $30
    function GetHomeStar: TStar; override; // @addr $5C1460 @slot $34
    function GetStrengthScaledPirateStatus: Byte; override; // @addr $5C16D0 @slot $3C
    function GetDominantCareer: TRangerCareer; override; // @addr 0x5C16BC @slot 0x38 @note "Always rcTrader."
    function GetName: WideString; override; // @addr 0x5C147C @slot 0x24 @ida "void __usercall $name(TTransport *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetFullName(const Separator: WideString): WideString; override; // @addr 0x5C149C @slot 0x28 @ida "void __usercall $name(TTransport *Self@<eax>, unsigned __int16 *Separator@<edx>, unsigned __int16 **Result@<ecx>);"

    procedure NextDay; override; // @addr 0x5C02DC @slot 0x18
    procedure NextDayLogic; override; // @addr 0x5C0550 @slot 0x1C @calls "0x5C03C4"
    function SelectRepairOrTradePlanet: TPlanet; // @addr 0x5C0B30 @note "May issue a movement order when repairs are needed; result is borrowed and may be nil."
    function SelectTradePlanet: TPlanet; // @addr 0x5C0B68 @note "Requires a non-nil PlanetQueue; an empty queue returns nil. Result is borrowed."
    procedure BuildReachablePlanetQueue; override; // @addr 0x5C0C14 @slot 0x64 @calls "0x5C072A"
    function CanQueueReachablePlanet(Planet: TPlanet): Boolean; override; // @addr 0x5C0E74 @slot 0x68 @note "AI ownership check only; does not test travel range."
    procedure ProcessTrading; // @addr 0x5C0E94 @note "Requires CurrentPlanet. Can sell below cost; purchases can exhaust money and cargo space."
    procedure RepairBrokenEquipmentAtLocation; override; // @addr 0x5C1384 @slot 0x60 @calls "0x5C05DB" @note "Restores equipment condition without charging Money."
    function GetDesiredCargoFreeSpace: Integer; override; // @addr 0x5C16E4 @slot 0x40
    procedure RefuelAtLocation; override; // @addr 0x5C1784 @slot 0x48 @calls "0x5C05F3" @note "Fills installed fuel tanks without charging Money."
    procedure ProcessUnseenProgression; // @addr 0x5C17B4
    function RecomputeFearState: Boolean; override; // @addr 0x5C1D9C @slot 0x84 @calls "0x5C0739" @note "Updates InFear and may replace EnemyShip."
    procedure TryOfferRansomToPursuer; // @addr 0x5C208C
    procedure AssignWeaponTargetsInStar; override; // @addr 0x5C2570 @slot 0x20 @calls "0x5C075B"
    procedure SelectEnemyShipInStar; override; // @addr 0x5C2D40 @slot 0x6C @calls "0x5C0776"
    procedure EngageEnemyShip; override; // @addr 0x5C2E4C @slot 0x70 @calls "0x5C077E"
    procedure RefreshCurrentStanding; override; // @addr 0x5C5DBC @slot 0xC4 @calls "0x5C0182"
  end;

var
  TransportSkillBonusWeights: array[22..27] of Integer = (80, 80, 100, 100, 60, 5); // @addr $87AB7C
  TransportSlotBonusWeights: array[13..20] of Integer = (200, 0, 150, 0, 150, 50, 0, 0); // @addr $87AB94 Native eight-entry slot table; artefact/afterburner weights at $87ABAC/$87ABB0 are zero.
...
{ @routine $5BF844 TTransport_Destroy }
...
{ @routine $5BFC8C TTransport_InitGenerated }
...
var I: Integer; Ranger: TRanger; Good: Byte;
  // @nested $5BF88C SelectUniqueName
  procedure SelectUniqueName(Config: TBlockParEC); // @addr $5BF88C @ida "void __usercall $name(TBlockParEC *Config@<eax>, void *ParentFrame@<^0>);" @note "Nested name-selection helper; caller-popped static link and parent Self at -4."
...
{ @routine $5C0260 TTransport_SaveToBuffer }
...
{ @routine $5C028C TTransport_LoadFromBuffer }
...
{ @routine $5C02C0 TTransport_ResolveLoadedReferences }
...
{ @routine $5C02DC TTransport_NextDay }
...
{ @routine $5C0550 TTransport_NextDayLogic }
...
{ @routine $5C0B30 TTransport_SelectRepairOrTradePlanet }
...
{ @routine $5C0B68 TTransport_SelectTradePlanet }
...
{ @routine $5C0C14 TTransport_BuildReachablePlanetQueue }
...
{ @routine $5C0E74 TTransport_CanQueueReachablePlanet }
...
{ @routine $5C0E94 TTransport_ProcessTrading }
...
{ @routine $5C1384 TTransport_RepairBrokenEquipmentAtLocation }
...
{ @routine $5C1460 TTransport_GetHomeStar }
...
{ @routine $5C147C TTransport_GetName }
...
{ @routine $5C149C TTransport_GetFullName }
...
{ @routine $5C165C TTransport_GetTypeNameKey }
...
{ @routine $5C1688 TTransport_GetGreetingShipCategory }
...
{ @routine $5C16BC TTransport_GetDominantCareer }
...
{ @routine $5C16D0 TTransport_GetStrengthScaledPirateStatus }
...
{ @routine $5C16E4 TTransport_GetDesiredCargoFreeSpace }
...
{ @routine $5C1784 TTransport_RefuelAtLocation }
...
{ @routine $5C17B4 TTransport_ProcessUnseenProgression }
...
{ @routine $5C19C4 TTransport_RelationToNonRanger }
function TTransport.RelationToNonRanger(Ship: TShip): Byte;
var Value: Integer;
begin
  if Ship.TypeId = stTransport then begin
    Value := Round(OwnerRelations[Integer(RaceToOwner(PilotRace)) and $7F, Integer(RaceToOwner(Ship.PilotRace)) and $7F] * (0.7 * PlanetRaceMarket[PilotRace].FriendlyRelationScale));
    if Value > 100 then Value := 100;
    Result := Value;
  end else if Ship.TypeId = stPirate then begin
    Value := Round(OwnerRelations[Integer(RaceToOwner(PilotRace)) and $7F, Integer(RaceToOwner(Ship.PilotRace)) and $7F] * (0.5 * PlanetRaceMarket[PilotRace].PirateRelationFactor));
...
{ @routine $5C1B48 TTransport_RelationToRanger }
...
{ @routine $5C1B90 TTransport_ChangeRelationToRanger }
...
{ @routine $5C1CEC TTransport_ReactToAttack }
...
{ @routine $5C1D9C TTransport_RecomputeFearState }
...
{ @routine $5C208C TTransport_TryOfferRansomToPursuer }
...
{ @routine $5C23C4 TTransport_AcceptsRansomDemandFrom }
...
{ @routine $5C24B0 TTransport_TrustsAttackRequester }
function TTransport.TrustsAttackRequester(Ship: TShip): Boolean;
begin Result := RelationToShip(Ship) >= 30; end;
{ @end $5C24B0 }

{ @routine $5C24D4 TTransport_EvaluateAllyRelationAndStrength }
...
{ @routine $5C2570 TTransport_AssignWeaponTargetsInStar }
...
{ @routine $5C2D40 TTransport_SelectEnemyShipInStar }
...
{ @routine $5C2E4C TTransport_EngageEnemyShip }
...
{ @routine $5C2F0C TTransport_ProcessCombatDialogue }
...
{ @routine $5C2FA0 TTransport_ReactToExtortionDemand }
...
{ @routine $5C3298 TTransport_BuildMoneyExtortionResponse }
...
var NextDemandTurn: Integer; LicenseFactor: Single;
  // @nested $5C301C PayDemand
  procedure PayDemand; // @addr $5C301C @ida "void __usercall $name(void *ParentFrame@<^0>);" @note "Nested response helper with caller-popped static link."
...
{ @routine $5C39E8 TTransport_BuildCargoExtortionResponse }
...
var Forced: Boolean; NextDemandTurn: Integer;
  // @nested $5C36F8 DropDemand
  procedure DropDemand; // @addr $5C36F8 @ida "void __usercall $name(void *ParentFrame@<^0>);" @note "Nested response helper with caller-popped static link."
...
{ @routine $5C3DDC TTransport_BuildTrucePaymentResponse }
...
var NextDemandTurn: Integer;
  // @nested $5C3D8C AcceptPayment
  procedure AcceptPayment; // @addr $5C3D8C @ida "void __usercall $name(void *ParentFrame@<^0>);" @note "Nested response helper with caller-popped static link."
  begin
...
begin
  ...
  if OtherShip.TruceShip = Self then Response := LookupVisibleTalkText('Talk.Truce.WeAlreadyHavePact', OtherShip)
    ...
    (OfferedAmount > RemapClamped(Integer(GetWinChancePercent(OtherShip)) and $7F, 0, 100, GetWealthScaledAmount(1), GetWealthScaledAmount(4))) then begin
...
{ @routine $5C41C4 TTransport_BuildAttackRequestResponse }
function TTransport.BuildAttackRequestResponse(Requester: TShip; var Response: WideString; Target: TShip): Boolean;
  // @nested $5C40EC AcceptRequest
  procedure AcceptRequest; // @addr $5C40EC @ida "void __usercall $name(void *ParentFrame@<^0>);" @note "Nested response helper with caller-popped static link."
  begin
...
{ @routine $5C46DC TTransport_AcceptPartnershipOffer }
...
{ @routine $5C4EE8 TTransport_EvaluateStatBonus }
...
begin
  ...
  case BonusKind of
    ...
    bonScan: Result := Value * 3 + Value * 20 * (Integer(CountWeaponsByDamageFlags(ScannerFlags)) and $7F);
    ...
    bonSlotRadar:
      ...
      else if (GetRadar <> nil) and (Value < 0) then Result := -TransportSlotBonusWeights[Ord(BonusKind)] - TransportSlotBonusWeights[18] * (Integer(CountMissileWeapons) and $7F)
    ...
    bonSlotWeapon:
      begin
        ...
        if (Integer(CountEquippedWeapons) and $7F) > Max(Value + GetSlotCount(sskWeapon), 1) then
          Result := Result - (TransportSlotBonusWeights[Ord(BonusKind)] * 0.6) * ((Integer(CountEquippedWeapons) and $7F) - Max(1, Value + GetSlotCount(sskWeapon)));
      end;
    bonSkill1..bonSkill6:
      begin
        if Value > 0 then
          Result := Min(6 - (Integer(GetEffectiveSkillLevel(TPilotSkill(EquipmentBonusSkills[Ord(BonusKind) - 22]))) and $7F), Value) * TransportSkillBonusWeights[Ord(BonusKind)];
        if (Value > 0) and (Value + (Integer(GetEffectiveSkillLevel(TPilotSkill(EquipmentBonusSkills[Ord(BonusKind) - 22]))) and $7F) > 6) then
          Result := Result + (TransportSkillBonusWeights[Ord(BonusKind)] * 0.05) * (Value + (Integer(GetEffectiveSkillLevel(TPilotSkill(EquipmentBonusSkills[Ord(BonusKind) - 22]))) and $7F) - 6);
        if Value < 0 then
          Result := Min(Integer(GetEffectiveSkillLevel(TPilotSkill(EquipmentBonusSkills[Ord(BonusKind) - 22]))) and $7F, -Value) * -TransportSkillBonusWeights[Ord(BonusKind)];
        if (Value < 0) and (Value + (Integer(GetEffectiveSkillLevel(TPilotSkill(EquipmentBonusSkills[Ord(BonusKind) - 22]))) and $7F) < 0) then
          Result := Result + (TransportSkillBonusWeights[Ord(BonusKind)] * 0.03) * (Value + (Integer(GetEffectiveSkillLevel(TPilotSkill(EquipmentBonusSkills[Ord(BonusKind) - 22]))) and $7F));
      end;
  else Result := 0;
  end;
...
{ @routine $5C58B0 TTransport_EvaluateWeaponDamage }
...
{ @routine $5C5DBC TTransport_RefreshCurrentStanding }
