unit aWarrior;
// Unit bracket (inferred): .text 0x005B5EA8..0x005BF719; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    procedure SaveToBuffer(Buffer: TBufEC); override; // @addr $005B6BE4 @slot $00
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); override; // @addr $005B6C10 @slot $04
    function AdjustItemEvaluation(Item: TItem; PriceMode: Byte; Effectiveness: Single): Single; override; // @addr $005BDE60 @slot $50 @ida "float __userpurge $name@<st0>(TWarrior *Self@<eax>, TItem *Item@<edx>, unsigned __int8 PriceMode@<cl>, float Effectiveness@<^0>);"
    function EvaluateStatBonus(BonusKind: TEquipmentBonusKind; Value: Integer): Single; override; // @addr $005BE3F4 @slot $54
    function EvaluateWeaponDamage(Weapon: TWeapon; IncludeAdditiveBonuses: Boolean; BaseDamage: Single): Single; override; // @addr $005BF068 @slot $58 @ida "float __userpurge $name@<st0>(TWarrior *Self@<eax>, TWeapon *Weapon@<edx>, bool IncludeAdditiveBonuses@<cl>, float BaseDamage@<^0>);"
    procedure RepairBrokenEquipmentAtLocation; override; // @addr $005B855C @slot $60
    procedure BuildReachablePlanetQueue; override; // @addr $005B8430 @slot $64
    procedure SelectEnemyShipInStar; override; // @addr $005BAAE8 @slot $6C
    procedure EngageEnemyShip; override; // @addr $005BB030 @slot $70
    function RelationToRanger(Ranger: Pointer): Byte; override; // @addr $005B8C1C @slot $74
    procedure ChangeRelationToRanger(Ranger: Pointer; Amount: Integer); override; // @addr $005B8C64 @slot $78
    procedure ReactToAttack(Attacker: TShip); override; // @addr $005B8DC8 @slot $7C
    function RelationToNonRanger(Ship: TShip): Byte; override; // @addr $005B8B0C @slot $80
    function RecomputeFearState: Boolean; override; // @addr $005B8EA0 @slot $84
    function AcceptsRansomDemandFrom(Ship: TShip): Boolean; override; // @addr $005B900C @slot $88
    function TrustsAttackRequester(Ship: TShip): Boolean; override; // @addr $005B90A8 @slot $8C
    function EvaluateAllyRelationAndStrength(Ship: TShip): Boolean; override; // @addr $005B90CC @slot $90
    function AcceptPickupItem(Item: TItem): Boolean; override; // @addr $005BF5C8 @slot $94
    function AcceptPickupDistance(Item: TItem; Distance: Double): Boolean; override; // @addr $005BF644 @slot $98 @ida "bool __userpurge $name@<al>(TWarrior *Self@<eax>, TItem *Item@<edx>, double Distance@<^0>);"
    procedure ProcessCombatDialogue; override; // @addr $005BCBD8 @slot $A0
    procedure ReactToExtortionDemand(Ranger: Pointer); override; // @addr $005BCBE4 @slot $A4
    function BuildMoneyExtortionResponse(OtherShip: TShip; var Response: WideString; DemandedAmount: Integer): Boolean; override; // @addr $005BCC50 @slot $A8
    function BuildCargoExtortionResponse(OtherShip: TShip; var Response: WideString): Boolean; override; // @addr $005BCD6C @slot $AC
    function BuildTrucePaymentResponse(OtherShip: TShip; var Response: WideString; OfferedAmount: Integer): Boolean; override; // @addr $005BD0AC @slot $B0
    function BuildAttackRequestResponse(Requester: TShip; var Response: WideString; Target: TShip): Boolean; override; // @addr $005BD53C @slot $B4
    function AcceptPartnershipOffer(OtherShip: TShip; var Response: WideString; PaymentAmount: Integer): Boolean; override; // @addr $005BDBE0 @slot $B8
    function BuildPartnershipOfferResponse(OtherShip: TShip; var Response: WideString; PaymentAmount: Integer): Boolean; override; // @addr $005BDC30 @slot $BC
    function UnknownVirtualC0(Argument: Pointer): Boolean; override; // @addr $005BDE04 @slot $C0
    procedure RefreshCurrentStanding; override; // @addr $005BF6E0 @slot $C4
    destructor Destroy; override; // @addr $5B5FD0 @ida "void __usercall $name(TWarrior *Self@<eax>, __int8 DestroyFlags@<dl>);"
    function FindNearestFriendlyFlagship: TShip; // @addr $5BAA04
    function NavigateToHomePlanet: Boolean; // @addr $5B832C
    procedure ProcessUnseenProgression; // @addr $5B8974
    procedure MoveToRandomPlanetOrbit; // @addr $5BDC80
    procedure ConsumeNodes(Amount: Integer); // @addr $5BDD30

    procedure InitGenerated(Planet: TPlanet; InitialMoney: Integer; Kind: Byte); // @addr 0x5B60E4 @note "Sets location, money and WarriorType; registers the ship with its star and home garrison."

    procedure NextDay; override; // @addr 0x5B6C5C @slot 0x18
    procedure NextDayLogic; override; // @addr 0x5B6ED0 @slot 0x1C @calls "0x5B6D4C"
    procedure ManeuverFlagship; // @addr $5BBFF4
    procedure NextDayFlagshipLogic; // @addr 0x5B7B30 @note "Flagship branch; its diagnostic retains TWarrior.NextDayLogic."
    procedure AssignWeaponTargetsInStar; override; // @addr 0x5B9168 @slot 0x20 @note "Native diagnostic name: TWarrior.ArmsToTarget."
    procedure AssignFlagshipWeaponTargets; // @addr 0x5BA058 @note "Flagship branch; shares the TWarrior.ArmsToTarget diagnostic."
    ...
    procedure ReassignFlagshipHomePlanet; // @addr $5BC66C Scores Coalition systems and same-race garrisons; moves the flagship between home rosters without changing its current position.
    procedure MoveToRandomPatrolPoint; // @addr 0x5B84FC
    function IsHomePatrolTurn: Boolean; // @addr 0x5BDCE8 @note "Requires the home system and (Id + CurrentTurn) mod 100 < 25."
    function GetDesiredCargoFreeSpace: Integer; override; // @addr 0x5B892C @slot 0x40
    procedure RefuelAtLocation; override; // @addr 0x5B8944 @slot 0x48 @note "Fills installed fuel tanks without charging Money."
    function CanQueueReachablePlanet(Planet: TPlanet): Boolean; override; // @addr 0x5B84CC @slot 0x68 @note "AI ownership check only; does not test travel range."
  end;

const
  WarriorSkillBonusWeights: array[22..27] of Integer = (100, 100, 80, 10, 10, 10); // @addr $87AB44
  // EvaluateStatBonus dispatches bonus kinds 13..20 to these slot cases.
  WarriorSlotBonusWeights: array[13..20] of Integer = (80, 80, 200, 0, 200, 150, 0, 10); // @addr $87AB5C @indexrefs "$5BE8EA,$5BE92A,$5BE96A,$5BE99B,$5BE9F7,$5BEA37,$5BEA68,$5BEA97,$5BEAD5,$5BEB06,$5BEB35,$5BEB73,$5BEBCF,$5BEC19,$5BECB0,$5BECE6,$5BED16"
...
{ @routine $5B5FD0 TWarrior_Destroy }
...
{ @routine $5B60E4 TWarrior_InitGenerated }
...
var FirstNameIndex, LastNameIndex: Integer; Ranger: TRanger; TechLevel: Byte; Config: TBlockParEC; Weapon: TWeapon;
  // @nested $5B6034 SelectTechLevel
  function SelectTechLevel(Minimum, Maximum: Integer): Integer; // @addr $5B6034 @ida "int __usercall $name@<eax>(int Minimum@<eax>, int Maximum@<edx>, void *ParentFrame@<^0>);" @note "Nested helper with caller-popped static link."
  begin
...
begin
  ...
  if WarriorType = wtFlagship then begin
    ...
    if GetSlotCount(sskWeapon) > WeaponCount then begin
      Weapon := CreateAndEquipWeapon(Ord(t_Weapon3), 2 * WeaponInfos[Ord(t_Weapon3)].AverageSize, 1, OwnerId);
    ...
    if GetSlotCount(sskWeapon) > WeaponCount then CreateAndEquipWeapon(Ord(t_Weapon3), WeaponInfos[Ord(t_Weapon3)].AverageSize, 1, OwnerId);
...
{ @routine $5B6BE4 TWarrior_SaveToBuffer }
...
{ @routine $5B6C10 TWarrior_LoadFromBuffer }
...
{ @routine $5B6C5C TWarrior_NextDay }
...
{ @routine $5B6ED0 TWarrior_NextDayLogic }
...
{ @routine $5B7B30 TWarrior_NextDayFlagshipLogic }
...
var Planet: TPlanet; Station, Ship: TShip; Stations: TList; I, Stage: Integer;
  // @nested $5B7560 RepairHullWithNodes
  procedure RepairHullWithNodes; // @addr $5B7560 @ida "void __usercall $name(void *ParentFrame@<^0>);"
  ...
  // @nested $5B784C RepairEquipmentWithNodes
  procedure RepairEquipmentWithNodes; // @addr $5B784C @ida "void __usercall $name(void *ParentFrame@<^0>);"
...
{ @routine $5B832C TWarrior_NavigateToHomePlanet }
...
{ @routine $5B8430 TWarrior_BuildReachablePlanetQueue }
...
{ @routine $5B84CC TWarrior_CanQueueReachablePlanet }
...
{ @routine $5B84FC TWarrior_MoveToRandomPatrolPoint }
...
{ @routine $5B855C TWarrior_RepairBrokenEquipmentAtLocation }
...
{ @routine $5B8638 TWarrior_GetHomeStar }
...
{ @routine $5B8654 TWarrior_GetName }
...
{ @routine $5B8674 TWarrior_GetFullName }
...
{ @routine $5B88BC TWarrior_GetTypeNameKey }
...
{ @routine $5B88F0 TWarrior_GetGreetingShipCategory }
...
{ @routine $5B8904 TWarrior_GetDominantCareer }
...
{ @routine $5B892C TWarrior_GetDesiredCargoFreeSpace }
...
{ @routine $5B8944 TWarrior_RefuelAtLocation }
...
{ @routine $5B8974 TWarrior_ProcessUnseenProgression }
...
{ @routine $5B8C1C TWarrior_RelationToRanger }
...
{ @routine $5B8C64 TWarrior_ChangeRelationToRanger }
...
{ @routine $5B8DC8 TWarrior_ReactToAttack }
...
{ @routine $5B8EA0 TWarrior_RecomputeFearState }
...
{ @routine $5B900C TWarrior_AcceptsRansomDemandFrom }
...
{ @routine $5B90A8 TWarrior_TrustsAttackRequester }
...
{ @routine $5B9168 TWarrior_AssignWeaponTargetsInStar }
...
{ @routine $5BA058 TWarrior_AssignFlagshipWeaponTargets }
...
var Scores: array[1..5] of Single; Assigned, I, J: Integer; Ship: TShip; Weapon: TWeapon; Distance: Single;
  ...
  // @nested $5B9EF8 ScoreTarget
  procedure ScoreTarget(Ship: TShip); // @addr $5B9EF8 @ida "void __usercall $name(TShip *Ship@<eax>, void *ParentFrame@<^0>);"
...
{ @routine $5BAA04 TWarrior_FindNearestFriendlyFlagship }
...
{ @routine $5BAAE8 TWarrior_SelectEnemyShipInStar }
...
{ @routine $5BB030 TWarrior_EngageEnemyShip }
...
{ @routine $5BBFF4 TWarrior_ManeuverFlagship }
...
var Enemies: TList; SupportWeight: Single; OwnMinRange, OwnMaxRange: Integer; OwnAttack: Single; Allies: TList;
  ...
  // @nested $5BB2B4 GetWeaponRangeBounds
  procedure GetWeaponRangeBounds(Ship: TShip; var Minimum, Maximum: Integer); // @addr $5BB2B4 @ida "void __usercall $name(TShip *Ship@<eax>, int *Minimum@<edx>, int *Maximum@<ecx>, void *ParentFrame@<^0>);"
  ...
  // @nested $5BB380 AreaWeaponScore
  function AreaWeaponScore(Ship: TShip): Integer; // @addr $5BB380 @ida "int __usercall $name@<eax>(TShip *Ship@<eax>, void *ParentFrame@<^0>);"
  ...
  // @nested $5BB458 ProjectOutsideStar
  function ProjectOutsideStar(Point: TPointF): TPointF; // @addr $5BB458 @ida "void __usercall $name(TPointF *Point@<eax>, TPointF *Result@<edx>, void *ParentFrame@<^0>);"
  ...
  // @nested $5BB5DC EvaluatePosition
  function EvaluatePosition(Point: TPointF): Single; // @addr $5BB5DC @ida "float __usercall $name@<st0>(TPointF *Point@<eax>, void *ParentFrame@<^0>);"
  ...
  // @nested $5BBE80 BoostWithNodes
  procedure BoostWithNodes; // @addr $5BBE80 @ida "void __usercall $name(void *ParentFrame@<^0>);"
  var Factor: Single;
  begin
...
{ @routine $5BC66C TWarrior_ReassignFlagshipHomePlanet }
...
{ @routine $5BCBD8 TWarrior_ProcessCombatDialogue }
procedure TWarrior.ProcessCombatDialogue;
begin end;
{ @end $5BCBD8 }

{ @routine $5BCBE4 TWarrior_ReactToExtortionDemand }
...
{ @routine $5BCC50 TWarrior_BuildMoneyExtortionResponse }
...
{ @routine $5BCD6C TWarrior_BuildCargoExtortionResponse }
...
{ @routine $5BD0AC TWarrior_BuildTrucePaymentResponse }
...
var NextDemandTurn: Integer;
  // @nested $5BCE84 AcceptPayment
  procedure AcceptPayment; // @addr $5BCE84 @ida "void __usercall $name(void *ParentFrame@<^0>);" @note "Nested helper with caller-popped static link."
...
{ @routine $5BD53C TWarrior_BuildAttackRequestResponse }
...
var I, HostileCount: Integer; Ship: TShip;
  // @nested $5BD464 AcceptRequest
  procedure AcceptRequest; // @addr $5BD464 @ida "void __usercall $name(void *ParentFrame@<^0>);" @note "Nested helper with caller-popped static link."
  begin
...
{ @routine $5BDBE0 TWarrior_AcceptPartnershipOffer }
...
{ @routine $5BDCE8 TWarrior_IsHomePatrolTurn }
...
{ @routine $5BDD30 TWarrior_ConsumeNodes }
...
{ @routine $5BDE04 TWarrior_UnknownVirtualC0 }
...
{ @routine $5BDE60 TWarrior_AdjustItemEvaluation }
...
{ @routine $5BE3F4 TWarrior_EvaluateStatBonus }
...
begin
  ...
  case BonusKind of
    ...
    bonSlotDroid:
      ...
      else if (GetRepairRobot <> nil) and (Value < 0) then Result := -WarriorSlotBonusWeights[Ord(BonusKind)]
      else if (GetSlotCount(sskRepairRobot) = 1) and (Value < 0) then Result := WarriorSlotBonusWeights[Ord(BonusKind)] * -0.3;
    bonSlotDef:
      if (GetSlotCount(sskDefGenerator) = 0) and (Value > 0) then Result := WarriorSlotBonusWeights[Ord(BonusKind)] * 0.3
      else if (GetDefGenerator <> nil) and (Value < 0) then Result := -WarriorSlotBonusWeights[Ord(BonusKind)]
      else if (GetSlotCount(sskDefGenerator) = 1) and (Value < 0) then Result := WarriorSlotBonusWeights[Ord(BonusKind)] * -0.3;
    bonSlotWeapon:
      begin
    ...
    bonSlotForsage:
      ...
      else if (GetSlotCount(sskAfterburner) = 1) and (Value < 0) then Result := -WarriorSlotBonusWeights[Ord(BonusKind)];
    bonSkill1..bonSkill6:
      begin
        if Value > 0 then
  ...
  if BonusKind in [bonSkill1..bonSkill6] then Result := Result * 0.01 * (100 + SeededRandomIntRange(-30, 30, Seed + 131 * Ord(BonusKind))) * RaceSkillEvaluationFactors[PilotRace, EquipmentBonusSkills[Ord(BonusKind) - 22]]
  else Result := Result * 0.01 * (100 + SeededRandomIntRange(-10, 10, Seed + 131 * Ord(BonusKind)));
end;
{ @end $5BE3F4 }

{ @routine $5BF068 TWarrior_EvaluateWeaponDamage }
...
begin
  ...
  case Byte(Weapon.GetWeaponInfo.ShotType) of
    ...
    Ord(wstMissile): Result := Result * (1.1 + Weapon.GetWeaponInfo.SecondaryDamageRadius * 1.0 * 0.01 + StatusFactor) * Weapon.GetShotCount;
    Ord(wstTorpedo): Result := Result * (1 + Weapon.GetWeaponInfo.SecondaryDamageRadius * 1.0 * 0.01 + StatusFactor);
    Ord(wstChain): Result := Result * (1.1 + (Weapon.GetShotCount - 1) * 0.2) * (1 + StatusFactor);
    Ord(wstSplash): Result := Result * (1 + Weapon.GetWeaponInfo.SecondaryDamageRadius * 1.5 * 0.01 * SpeedFactor + StatusFactor);
    Ord(wstExploder): Result := Result * (1 + Weapon.GetWeaponInfo.SecondaryDamageRadius * 0.8 * 0.01 * SpeedFactor + StatusFactor);
    Ord(wstAreaDamage): Result := Result * (1 + Weapon.Range * 2.2 * 0.01 * SpeedFactor + StatusFactor);
...
{ @routine $5BF5C8 TWarrior_AcceptPickupItem }
...
{ @routine $5BF644 TWarrior_AcceptPickupDistance }
...
{ @routine $5BF6E0 TWarrior_RefreshCurrentStanding }
