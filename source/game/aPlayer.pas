unit aPlayer;
// Unit bracket (inferred): .text 0x0078BD6C..0x0079A0DB; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create; // @addr $78BF5C @ida "TJournalRecord *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr $78BFB0 @ida "void __usercall $name(TJournalRecord *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure SaveToBuffer(Buffer: TBufEC); // @addr $78BFE4
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr $78C010
  end;
  ...
  public
    ...
    StationServiceLastUseTurns: array[0..11] of Integer; // @offset $5D0 Initialized to 150; last-use turns for investment/service cooldowns. Native reads $58686A/$57CD9D and write $589E9D.
    ...
    DeclinePlanetBattleOffers: Boolean; // @offset 0x6AC  Suppresses planetary-battle offers ($52ADF4); set by DeclineAllPlanetBattles ($52C341), editable key RejectPB.
    DiseaseContractionCount: Word; // @offset 0x6AE
    StimulantPurchaseCount: Word; // @offset 0x6B0
    PrisonStaysCompleted: Word; // @offset $6B2 PRISON: successful return from the prison quest; native increment $528BDE.
    SatelliteTilesExplored: Integer; // @offset $6B4 ARCHEOLOGY: accumulated water/land/hill exploration gains; native add $8599F8.
    NationalityChangeCount: Integer; // @offset $6B8 MANYFACES: accepted nationality changes ($577110). Serialized as a word.
    SideChangeCount: Integer; // @offset $6BC SIDECHANGER: switches between Coalition and pirate allegiance ($577E53). Serialized as a word.
    ...
    procedure InitializePlayerAtPlanet(Planet: TPlanet; InitialMoney, CharacterPreset: Integer); // @addr $78ED8C Inherited ranger registration followed by player career/skill defaults; CharacterPreset is unused here.
    procedure ApplyCharacterPreset(Planet: TPlanet; InitialMoney, CharacterPreset: Integer); // @addr $78EE6C Twenty-five race/preset loadouts, stored cargo and initial planet relations. Planet is unused.
    procedure BeginStorageTurn; // @addr $795E10 Native empty turn-start hook.
    procedure RechargeTransmitters; // @addr $793CAC Increments every carried transmitter in the global player's artefact list, including unequipped ones.
    procedure ApplyBioArtefactHealthEffects; // @addr $793D18 Each active Bio artefact may shorten a disease and extend a stimulant by one turn.
    function MayTakeSubCrack: Boolean; // @addr $793E88 Terron unresolved, late-game offer cadence and no program 4 already carried.
    function GetSubCrackCost: Integer; // @addr $793F0C Difficulty-scaled price.
    function GetPirateServiceDiscount: TPercent; // @addr $793F4C Rounded pirate career status / 1.3, plus one percentage point.
    function CountProgramRewardStocks: Integer; // @addr $793F8C
    function GetMaxPiratePartners: Integer; // @addr $79926C Pirate career thresholds, eminent title and active license.
    function GetMaxDominionShips: Integer; // @addr $7992C0 As pirate partners, with an additional threshold above career status 50.
    procedure AddJournalRecord(Text: WideString); // @addr $799324 Dates the new record with the current turn.
    procedure DeleteJournalRecord(Index: Integer); // @addr $7993A8 Native guard accepts Index=Count, leaving the list accessor to raise.
    procedure ClearJournal; // @addr $7993F8
    procedure SortNewsEntries; // @addr $799668 Ascending ID, using pairwise swaps.
    procedure MergeGalaxyNews; // @addr $7999DC Copies galaxy news whose IDs are absent locally.
    procedure RefreshNewsAtLocation; // @addr $799ADC Updates eligible docked players after turn 300 and retains the newest 100 entries.
    procedure CloseRuinsModeScreen; // @addr $799CD8 Returns to the saved location or star screen and requests screen closure.
    procedure RefreshCurrentStanding; override; // @addr $799E4C @slot $C4 Includes the player's current-system kill counts and main pirate planet exception.
    constructor Create; // @addr $78C0F0 @ida "TPlayer *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);" Native constructor initializes lists/defaults; does not register or generate the player loadout.
    destructor Destroy; override; // @addr $78C47C @ida "void __usercall $name(TPlayer *Self@<eax>, __int8 DestroyFlags@<dl>);" Requires the inherited ranger registration state for final cleanup.
    procedure SaveToBuffer(Buffer: TBufEC); override; // @addr $78C6D0 @slot $00
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); override; // @addr $78D044 @slot $04
    procedure ResolveLoadedReferences(Galaxy: TGalaxy); override; // @addr $78DD10 @slot $08
    procedure SaveToBlock(Block: TBlockParEC); override; // @addr $78E064 @slot $10
    procedure LoadFromBlock(Block: TBlockParEC); override; // @addr $78E76C @slot $14
    function ExportJournal: WideString; // @addr $799428 @ida "void __usercall $name(TPlayer *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function ExportNews: WideString; // @addr $7997B4 @ida "void __usercall $name(TPlayer *Self@<eax>, unsigned __int16 **Result@<edx>);"
    procedure TrimNewsEntries(KeepCount: Integer); // @addr $799750 Removes and finalizes the oldest Count-KeepCount entries; unchecked argument.
    function TryAwardDominatorPrograms(Victim: TShip): Boolean; // @addr $793FC0 Requires a TKling victim; records its hull capacity even when no reward is due.
    function GetSatelliteExplorationTurns(Satellite: TSatellite): Integer; // @addr $795960 @note "Remaining duration at the current planet using combined operational probe rates; capped at 999 per terrain."
    procedure ReportIdleSatellites(Star: TStar); // @addr $795C6C @note "Reports inactive deployed satellites in the entered star; records up to three target planets."
    procedure ExitRuinsMode; // @addr $799DB4 Restores the real docking target and rebuilds its temporary shop stock.
    procedure CreateRuinsProxy; // @addr $799B74 Creates a military-base proxy and removes it from the ordinary system ship list.
    procedure EnterRuinsMode(Mode: Integer); // @addr $799BE4 Nonpositive Mode uses the installed hull's CapitalShip kind.
    function CanSelectShipTarget(Ship: TShip): Boolean; // @addr $799F0C Honors scripted targeting restrictions, chameleon logic and friendly station standing masks.
    function CanScanShip(Ship: TShip): Boolean; // @addr $79A024 Checks station/Dominator restrictions and invokes the player's scan-permission item scripts.
    procedure SaveEquipmentConfiguration(Index: Integer); // @addr 0x798630 @note "Index is zero-based and unchecked; the selected preset is unchanged."
    procedure ApplyEquipmentConfiguration(Index: Integer); // @addr 0x798D90 @note "Can draw from storage at the current dock. Index is unchecked; the selected preset is unchanged."
    procedure NextDay; override; // @addr 0x791F68 @slot 0x18 @calls "0x7A9C35"
    function HasEquipmentConfiguration(Index: Integer): Boolean; // @addr 0x798DC8 @note "Does not validate Index."

    function CalculateSpeed: Integer; override; // @addr 0x799B58 @slot 0x4C
    function ComputeDepositAccruedValue: Integer; // @addr 0x793BB4 @note "Capped at 100000000; zero for nonpositive principal."
    function CanAccessStoredItem(Item: TItem): Boolean; // @addr $795E34 Native always-true permission hook used by storage lookup, counting and slot allocation.
    function CountStoredItemUnits(Location: TObject; ItemType: Byte): Integer; // @addr 0x795E4C @note "Location=nil includes all storage locations. Goods and item types 69/75 count by weight; other matching items count individually."
    function HasSatelliteOnPlanet(Planet: TPlanet): Boolean; // @addr 0x7949B0 @note "Requires Self=GetPlayer(): uses Self for the list count but fetches entries from the global player's deployed satellites."
    function CanAccessSurfaceLootItem(Item: TItem): Boolean; // @addr 0x795C54 @note "Native stub always returns true; Self and Item are unused. Called by treasure-map selection and planet loot reset."
    function FindNextStorageSlot(Location: TObject): Integer; // @addr 0x795FE0 @note "Returns a nonnegative slot local to Location."
    function FindStorageIndexByLocationAndSlot(Location: TObject; Slot: Integer): Integer; // @addr 0x7960F8 @note "Returns a zero-based StorageEntries index, or -1."
    function FindStorageGoodsByLocationAndType(Location: TObject; Good: Byte): Integer; // @addr 0x79617C @note "Returns a zero-based StorageEntries index, or -1."
    function FindMergeableStorageItemByLocation(Location: TObject; Item: TCountableItem): Integer; // @addr 0x7961F4 @note "Returns a zero-based StorageEntries index, or -1."
    procedure ShiftStorageSlotsAtOrAfter(Location: TObject; Slot: Integer); // @addr 0x796270
    function FindProfitableTradeRoute(Nearby: Boolean; Seed: Cardinal; var PurchasePlanet, SalePlanet: TPlanet; var Good: Byte; GoodsMask: TItemTypeMask): Boolean; // @addr $794274 @ida "bool __userpurge $name@<al>(TPlayer *Self@<eax>, bool Nearby@<dl>, unsigned int Seed@<ecx>, TPlanet **PurchasePlanet@<^12>, TPlanet **SalePlanet@<^8>, unsigned __int8 *Good@<^4>, TItemTypeMask *GoodsMask@<^0>);" @note "Returns the highest-scoring visible Coalition trade route; retains output arguments on failure and excludes their previous endpoints."
    function SelectPlanetBattleMap: Integer; // @addr $7980DC Returns a map ID or -1; updates shared map play-count scratch from the player's history.
    function CanAccessHoldGoods(Good: Byte): Boolean; // @addr $795E1C Native always-true permission hook; Good is passed in DL.
    procedure RepairDuplicateStorageSlots(Location: TObject); // @addr $795F04 Reassigns later accessible entries with duplicate slot indices.
    function GetStorageSlotExtent(Location: TObject): Integer; // @addr $796068 Maximum accessible slot index plus one, or zero.
    procedure CloseVacantStorageSlot(Location: TObject; Slot: Integer); // @addr $7962E8 Shifts later slots down only if Slot is unoccupied.
    function HasAccessibleStorageAt(Location: TObject): Boolean; // @addr $796374 With nil, returns whether the entire storage list is empty.
    function CountPartnersInNormalSpace: Byte; // @addr $796400 Counts ships following Self in the current system, wrapping at 256.
    function GetShipRatingComparison(Ship: TShip): Byte; // @addr $79646C Native compares ranger TotalExperience with the global player's PlaceInRating after refreshing rankings.
    function GetShipRankComparison(Ship: TShip): Byte; // @addr $7965C8 Returns 1..5 for relative Coalition rank, zero for non-normal ships.
    function GetShipPirateRankComparison(Ship: TShip): Byte; // @addr $7966E0 Returns 1..5 for relative pirate rank, zero for non-normal ships.
    function GetShipStrengthComparison(Ship: TShip): Byte; // @addr $7967F8 Returns 1..5 from strength relative to the global player.
    function GetAvailableNodeCount(Carrier: TShip): Integer; // @addr $798E34 Includes the carrier's hold and the player's current-location storage.
    procedure ConsumeAvailableNodes(Count: Integer; Carrier: TShip); // @addr $798F38 Uses the carrier's hold, then current-location storage; refreshes Self even when Carrier differs.
    procedure RefreshStorageBubbles; // @addr 0x7970E0
    function HasDeployedSatellites: Boolean; // @addr $794950 @note "Uses Self.Satellites.Count but reads the global player's list."
    function GetStorageColumnHeaderText: WideString; // @addr $794A14 @ida "void __usercall $name(TPlayer *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function GetStorageDividerText: WideString; // @addr $794C4C @ida "void __usercall $name(TPlayer *Self@<eax>, unsigned __int16 **Result@<edx>);"
    function BuildDeployedSatelliteSummary(var LineCount: Integer): WideString; // @addr $794CF4 @ida "void __usercall $name(TPlayer *Self@<eax>, int *LineCount@<edx>, unsigned __int16 **Result@<ecx>);"
    function BuildTranclucatorStorageSummary(var LineCount: Integer): WideString; // @addr $796914 @ida "void __usercall $name(TPlayer *Self@<eax>, int *LineCount@<edx>, unsigned __int16 **Result@<ecx>);"
    function CompareStorageEntries(Left, Right: PStorageEntry): Integer; // @addr $796C24 @note "Compares star/location IDs, type, module priority/index, weight and cost; ignores slot indices."
    procedure SortStorageEntries; // @addr $796FF8
    procedure BuildStorageBubbles; // @addr 0x7970F4 @note "Publishes paginated storage summaries, including deployed probes; uses the global player's bubble list."
  end;

procedure SetPlayer(Player: TPlayer; Galaxy: TGalaxy); // @addr 0x78C090 @note "Updates Galaxy.PlayerRangerIndex; a nil Galaxy leaves the current player unchanged."
function GetPlayer: TPlayer; // @addr 0x78C078

var
  ArcadeKellerDefeats: Integer; // @addr $889D08 Native campaign reward guard.
  ArcadeKellerReward: TObject; // @addr $889D0C Owned reward item pending transfer.
  // The native initial DWORD decodes to nil; zero would decode to a non-null pointer.
  EncodedPlayer: Cardinal = $B1CD15D3; // @addr $87C19C
  StorageHeaderColumns: TStorageHeaderColumnTable = ((Size: 300; Cost: 380), (Size: 400; Cost: 490)); // @addr $87C1A0
  StorageDividerLengths: TStorageDividerLengthTable = (76, 98); // @addr $87C1B0
  ProbeSummaryColumns: TProbeSummaryColumnTable = (
    (Heading: 190; Size: 160; Exploration: 220; Condition: 280; Status: 295),
    (Heading: 250; Size: 200; Exploration: 280; Condition: 370; Status: 390)); // @addr $87C1B8
  TranclucatorSummaryWidths: TStorageDividerLengthTable = (180, 240); // @addr $87C1E0
  StorageItemColumns: TStorageItemColumnTable = ((Heading: 190; Size: 300; Cost: 380), (Heading: 250; Size: 400; Cost: 490)); // @addr $87C1E8
...
{ @routine $78BF5C TJournalRecord_Create }
...
{ @routine $78BFB0 TJournalRecord_Destroy }
...
{ @routine $78BFE4 TJournalRecord_SaveToBuffer }
...
{ @routine $78C010 TJournalRecord_LoadFromBuffer }
...
{ @routine $78C47C TPlayer_Destroy }
...
{ @routine $78C6D0 TPlayer_SaveToBuffer }
...
{ @routine $78D044 TPlayer_LoadFromBuffer }
...
{ @routine $78DD10 TPlayer_ResolveLoadedReferences }
...
{ @routine $78E064 TPlayer_SaveToBlock }
...
{ @routine $78E76C TPlayer_LoadFromBlock }
...
{ @routine $78ED8C TPlayer_InitializePlayerAtPlanet }
...
{ @routine $78EE6C TPlayer_ApplyCharacterPreset }
...
begin
  ...
  case OwnerId * 5 + CharacterPreset of
    ...
    begin
      ...
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[1]), 3, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon2), Round(WeaponInfos[Ord(t_Weapon2)].AverageSize * EquipmentSizeFactors[2]), 2, OwnerId);
    end;
    2:
    begin
      ...
      CreateAndEquipWeapon(Ord(t_Weapon2), Round(WeaponInfos[Ord(t_Weapon2)].AverageSize * EquipmentSizeFactors[2]), 3, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon3), Round(WeaponInfos[Ord(t_Weapon3)].AverageSize * EquipmentSizeFactors[2]), 2, OwnerId);
    end;
    3:
    begin
      ...
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[2]), 1, OwnerId).ConditionPercent := NextRandomIntRange(20, 80, RandomState);
    end;
    4:
    begin
      ...
      CreateAndEquipWeapon(Ord(t_Weapon2), Round(WeaponInfos[Ord(t_Weapon2)].AverageSize * EquipmentSizeFactors[2]), 2, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon3), Round(WeaponInfos[Ord(t_Weapon3)].AverageSize * EquipmentSizeFactors[3]), 2, OwnerId);
    end;
    5:
    begin
      ...
      CreateAndEquipRadar(Round(RadarBaseSize * EquipmentSizeFactors[2]), 1, OwnerId);
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[2]), 1, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[3]), 1, OwnerId);
    end;
    6:
    begin
    ...
    begin
      ...
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[2]), 1, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[2]), 2, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[2]), 2, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon2), Round(WeaponInfos[Ord(t_Weapon2)].AverageSize * EquipmentSizeFactors[3]), 1, OwnerId);
    end;
    8:
    begin
      SetMoney(RoundAndTruncateToTens(SeededRandomFloatRange(Galaxy.GenerationSeed, 0.9, 1.1) * InitialMoney));
      ...
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[1]), 2, OwnerId);
      GetCargoHook.ConditionPercent := NextRandomIntRange(20, 80, RandomState);
      CreateAndEquipWeapon(Ord(t_Weapon2), Round(WeaponInfos[Ord(t_Weapon2)].AverageSize * EquipmentSizeFactors[2]), 3, OwnerId).ConditionPercent := NextRandomIntRange(20, 80, RandomState);
    end;
    9:
    begin
    ...
    begin
      ...
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[1]), 2, OwnerId);
      GetCargoHook.ConditionPercent := NextRandomIntRange(20, 80, RandomState);
      CreateAndEquipWeapon(Ord(t_Weapon4), Round(WeaponInfos[Ord(t_Weapon4)].AverageSize * EquipmentSizeFactors[3]), 1, OwnerId).ConditionPercent := NextRandomIntRange(60, 90, RandomState);
      New(Entry);
      GetPlayer.StorageEntries.Add(Entry);
      Entry.Item := TGoods.Create;
    ...
    begin
      ...
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[3]), 2, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon2), Round(WeaponInfos[Ord(t_Weapon2)].AverageSize * EquipmentSizeFactors[2]), 2, OwnerId);
    end;
    12:
    begin
      ...
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[2]), 1, OwnerId);
      GetCargoHook.ConditionPercent := NextRandomIntRange(20, 80, RandomState);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[3]), 1, OwnerId).ConditionPercent := NextRandomIntRange(20, 80, RandomState);
      New(Entry);
      GetPlayer.StorageEntries.Add(Entry);
      Entry.Item := TGoods.Create;
    ...
    begin
      ...
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[3]), 1, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[3]), 2, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon2), Round(WeaponInfos[Ord(t_Weapon2)].AverageSize * EquipmentSizeFactors[4]), 2, OwnerId);
    end;
    15:
    begin
      SetMoney(RoundAndTruncateToTens(SeededRandomFloatRange(Galaxy.GenerationSeed, 0.2, 0.3) * InitialMoney));
      ...
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[3]), 1, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[4]), 2, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[3]), 3, OwnerId);
    end;
    16:
    begin
      SetMoney(RoundAndTruncateToTens(SeededRandomFloatRange(Galaxy.GenerationSeed, 0.2, 0.3) * InitialMoney));
      ...
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[3]), 1, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[3]), 3, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon2), Round(WeaponInfos[Ord(t_Weapon2)].AverageSize * EquipmentSizeFactors[3]), 2, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon3), Round(WeaponInfos[Ord(t_Weapon3)].AverageSize * EquipmentSizeFactors[3]), 2, OwnerId);
    end;
    17:
    begin
      ...
      CreateAndEquipRadar(Round(RadarBaseSize * EquipmentSizeFactors[3]), 1, OwnerId);
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[3]), 1, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon4), Round(WeaponInfos[Ord(t_Weapon4)].AverageSize * EquipmentSizeFactors[3]), 2, OwnerId);
    end;
    18:
    begin
      ...
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[4]), 1, OwnerId).ConditionPercent := NextRandomIntRange(10, 50, RandomState);
    end;
    19:
    begin
      SetMoney(RoundAndTruncateToTens(SeededRandomFloatRange(Galaxy.GenerationSeed, 0.2, 0.3) * InitialMoney));
    ...
    begin
      ...
      CreateAndEquipRadar(Round(RadarBaseSize * EquipmentSizeFactors[2]), 1, OwnerId);
      CreateAndEquipCargoHook(Round(CargoHookBaseSize * EquipmentSizeFactors[2]), 1, OwnerId);
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[2]), 1, OwnerId);
      New(Entry);
      GetPlayer.StorageEntries.Add(Entry);
      Entry.Item := TGoods.Create;
    ...
    begin
      ...
      CreateAndEquipWeapon(Ord(t_Weapon1), Round(WeaponInfos[Ord(t_Weapon1)].AverageSize * EquipmentSizeFactors[2]), 1, OwnerId).ConditionPercent := NextRandomIntRange(10, 50, RandomState);
    end;
  end;
...
{ @routine $791F68 TPlayer_NextDay }
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Sqr(Max(0, StimulantExcess - CountActiveArtefacts(Ord(t_ArtBio)))) * 0.4 > NextRandomFloatRange(0.0, 1000.0, LocalSeed) then
...
{ @routine $793BB4 TPlayer_ComputeDepositAccruedValue }
...
{ @routine $793CAC TPlayer_RechargeTransmitters }
...
{ @routine $793D18 TPlayer_ApplyBioArtefactHealthEffects }
...
{ @routine $793E88 TPlayer_MayTakeSubCrack }
...
{ @routine $793F0C TPlayer_GetSubCrackCost }
...
{ @routine $793F4C TPlayer_GetPirateServiceDiscount }
...
{ @routine $793F8C TPlayer_CountProgramRewardStocks }
...
{ @routine $793FC0 TPlayer_TryAwardDominatorPrograms }
...
{ @routine $794274 TPlayer_FindProfitableTradeRoute }
...
{ @routine $794950 TPlayer_HasDeployedSatellites }
...
{ @routine $7949B0 TPlayer_HasSatelliteOnPlanet }
...
{ @routine $794A14 TPlayer_GetStorageColumnHeaderText }
...
{ @routine $794C4C TPlayer_GetStorageDividerText }
...
{ @routine $794CF4 TPlayer_BuildDeployedSatelliteSummary }
...
{ @routine $795960 TPlayer_GetSatelliteExplorationTurns }
...
{ @routine $795C54 TPlayer_CanAccessSurfaceLootItem }
...
{ @routine $795C6C TPlayer_ReportIdleSatellites }
...
{ @routine $795E10 TPlayer_BeginStorageTurn }
procedure TPlayer.BeginStorageTurn;
begin
end;
{ @end $795E10 }

{ @routine $795E1C TPlayer_CanAccessHoldGoods }
...
{ @routine $795E34 TPlayer_CanAccessStoredItem }
...
{ @routine $795E4C TPlayer_CountStoredItemUnits }
...
begin
  ...
  begin
    ...
    begin
      if (ItemType in [Ord(t_Food)..Ord(t_Narcotics)]) or
        (ItemType in [Ord(t_Protoplasm), Ord(t_UselessCountableItem)]) then
      begin
        if Byte(Entry.Item.ItemType) = ItemType then Inc(Result, Entry.Item.Weight);
      end
      else if Byte(Entry.Item.ItemType) = ItemType then Inc(Result);
    end;
  end;
end;
{ @end $795E4C }

{ @routine $795F04 TPlayer_RepairDuplicateStorageSlots }
...
{ @routine $795FE0 TPlayer_FindNextStorageSlot }
...
{ @routine $796068 TPlayer_GetStorageSlotExtent }
...
{ @routine $7960F8 TPlayer_FindStorageIndexByLocationAndSlot }
...
{ @routine $79617C TPlayer_FindStorageGoodsByLocationAndType }
...
{ @routine $7961F4 TPlayer_FindMergeableStorageItemByLocation }
...
{ @routine $796270 TPlayer_ShiftStorageSlotsAtOrAfter }
...
{ @routine $7962E8 TPlayer_CloseVacantStorageSlot }
...
{ @routine $796374 TPlayer_HasAccessibleStorageAt }
...
{ @routine $796400 TPlayer_CountPartnersInNormalSpace }
...
{ @routine $79646C TPlayer_GetShipRatingComparison }
...
{ @routine $7965C8 TPlayer_GetShipRankComparison }
...
{ @routine $7966E0 TPlayer_GetShipPirateRankComparison }
...
{ @routine $7967F8 TPlayer_GetShipStrengthComparison }
...
{ @routine $796914 TPlayer_BuildTranclucatorStorageSummary }
...
{ @routine $796C24 TPlayer_CompareStorageEntries }
...
{ @routine $796FF8 TPlayer_SortStorageEntries }
...
{ @routine $7970E0 TPlayer_RefreshStorageBubbles }
...
{ @routine $7970F4 TPlayer_BuildStorageBubbles }
...
{ @routine $7980DC TPlayer_SelectPlanetBattleMap }
...
{ @routine $798630 TPlayer_SaveEquipmentConfiguration }
...
begin
  ...
  begin
    ...
    begin
      Item := Inventory[I];
      if not (Item is TWeapon) and (Item.EquippedFlag <> 0) and (ItemTypeToSlotKind(Byte(Item.ItemType)) <> sskUnsupported) then
      begin
    ...
    begin
      ...
      if (Slot >= 0) and (DefaultHullSlotCounts[8] > Slot) then ArtefactIds[Slot] := Artefact.Id;
    end;
  end;
end;
{ @end $798630 }

{ @routine $798D90 TPlayer_ApplyEquipmentConfiguration }
procedure TPlayer.ApplyEquipmentConfiguration(Index: Integer);
  // @nested $7987CC IsEmpty
  function IsEmpty: Boolean; // @addr $7987CC @ida "bool __cdecl $name(void *ParentFrame);" @note "Nested in ApplyEquipmentConfiguration; caller-popped static link, player -4 and preset index -8."
  var
    I: Integer;
  begin
    Result := True;
  ...
  // @nested $798838 SupportsItem
  function SupportsItem(Item: TItem): Boolean; // @addr $798838 @ida "bool __usercall $name@<al>(TItem *Item@<eax>, void *ParentFrame@<^0>);" @note "Nested in ApplyEquipmentConfiguration; caller-popped static link, player -4 and preset index -8."
  begin
    Result := True;
  ...
  // @nested $7988FC FindSlot
  function FindSlot(Item: TItem): Integer; // @addr $7988FC @ida "int __usercall $name@<eax>(TItem *Item@<eax>, void *ParentFrame@<^0>);" @note "Nested in ApplyEquipmentConfiguration; caller-popped static link, player -4 and preset index -8."
  var
    I: Integer;
  begin
    Result := -1;
  ...
  // @nested $7989CC UnequipAll
  procedure UnequipAll; // @addr $7989CC @ida "void __cdecl $name(void *ParentFrame);" @note "Nested in ApplyEquipmentConfiguration; caller-popped static link, player -4 and preset index -8."
  var
    I: Integer;
    Item: TEquipment;
    Artefact: TArtefact;
  begin
    ...
    begin
      Item := Inventory[I];
      if Byte(Item.ItemType) in [Ord(t_Hull)..Ord(t_CustomWeapon)] then Item.Unequip;
    end;
    for I := 0 to Artefacts.Count - 1 do
    begin
      Artefact := Artefacts[I];
  ...
  // @nested $798AA0 ReleaseStorageEntry
  procedure ReleaseStorageEntry(Entry: PStorageEntry); // @addr $798AA0 @ida "void __usercall $name(PStorageEntry Entry@<eax>, void *ParentFrame@<^0>);" @note "Nested in ApplyEquipmentConfiguration; caller-popped static link, player -4 and preset index -8."
  begin
    Entry.Item := nil;
    GetPlayer.StorageEntries.Delete(GetPlayer.StorageEntries.IndexOf(Entry));
    Dispose(Entry);
  end;
  // @nested $798AE4 TakeStoredItem
  procedure TakeStoredItem(Entry: PStorageEntry; Slot: Integer); // @addr $798AE4 @ida "void __usercall $name(PStorageEntry Entry@<eax>, int Slot@<edx>, void *ParentFrame@<^0>);" @note "Nested in ApplyEquipmentConfiguration; caller-popped static link, player -4 and preset index -8."
  var
    Item: TItem;
    Artefact: TArtefact;
  ...
  // @nested $798BD4 EquipCarriedItems
  procedure EquipCarriedItems; // @addr $798BD4 @ida "void __cdecl $name(void *ParentFrame);" @note "Nested in ApplyEquipmentConfiguration; caller-popped static link, player -4 and preset index -8."
  var
    I, Slot: Integer;
    Equipment: TEquipment;
    Artefact: TArtefact;
  ...
  // @nested $798CC0 EquipStoredItems
  procedure EquipStoredItems; // @addr $798CC0 @ida "void __cdecl $name(void *ParentFrame);" @note "Nested in ApplyEquipmentConfiguration; caller-popped static link, player -4 and preset index -8."
  var
    I, Slot: Integer;
    Entry: PStorageEntry;
  begin
...
{ @routine $798DC8 TPlayer_HasEquipmentConfiguration }
...
{ @routine $798E34 TPlayer_GetAvailableNodeCount }
...
{ @routine $798F38 TPlayer_ConsumeAvailableNodes }
...
{ @routine $79926C TPlayer_GetMaxPiratePartners }
function TPlayer.GetMaxPiratePartners: Integer;
begin
  Result := 0;
  if CareerStatus[Ord(rcPirate)] > 60 then Inc(Result);
  if CareerStatus[Ord(rcPirate)] > 75 then Inc(Result);
  if Galaxy.EminentCareerShips[Ord(rcPirate)] = Self then Inc(Result);
  if PirateLicenseTicks > 0 then Inc(Result);
end;
{ @end $79926C }

{ @routine $7992C0 TPlayer_GetMaxDominionShips }
function TPlayer.GetMaxDominionShips: Integer;
begin
  Result := 0;
  if CareerStatus[Ord(rcPirate)] > 50 then Inc(Result);
  if CareerStatus[Ord(rcPirate)] > 60 then Inc(Result);
  if CareerStatus[Ord(rcPirate)] > 75 then Inc(Result);
  if Galaxy.EminentCareerShips[Ord(rcPirate)] = Self then Inc(Result);
  if PirateLicenseTicks > 0 then Inc(Result);
...
{ @routine $799324 TPlayer_AddJournalRecord }
...
{ @routine $7993A8 TPlayer_DeleteJournalRecord }
...
{ @routine $7993F8 TPlayer_ClearJournal }
...
{ @routine $799428 TPlayer_ExportJournal }
...
{ @routine $799668 TPlayer_SortNewsEntries }
...
{ @routine $799750 TPlayer_TrimNewsEntries }
...
{ @routine $7997B4 TPlayer_ExportNews }
...
{ @routine $7999DC TPlayer_MergeGalaxyNews }
...
{ @routine $799ADC TPlayer_RefreshNewsAtLocation }
...
{ @routine $799B58 TPlayer_CalculateSpeed }
...
{ @routine $799B74 TPlayer_CreateRuinsProxy }
...
{ @routine $799BE4 TPlayer_EnterRuinsMode }
...
{ @routine $799CD8 TPlayer_CloseRuinsModeScreen }
...
{ @routine $799DB4 TPlayer_ExitRuinsMode }
...
{ @routine $799E4C TPlayer_RefreshCurrentStanding }
...
{ @routine $799F0C TPlayer_CanSelectShipTarget }
...
{ @routine $79A024 TPlayer_CanScanShip }
