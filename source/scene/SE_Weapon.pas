unit SE_Weapon;
// Unit bracket (inferred): .text 0x007D4E24..0x007D95F9; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create(AEffectIndex: Integer; AOwner: TObjectGI); // @addr $7D8404 @ida "TWeaponEffect *__userpurge $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, int AEffectIndex@<ecx>, TObjectGI *AOwner@<^0>);"
    destructor Destroy; override; // @addr $7D85B8 @ida "void __usercall $name(TWeaponEffect *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Clear; // @addr $7D85E4
    function AddItem: PWeaponEffectItem; // @addr $7D8608
    procedure RemoveItem(Item: PWeaponEffectItem); // @addr $7D86AC
    procedure AddTargetEffect(Index: Integer); // @addr $7D8748
    procedure AddSourceEffect(Index: Integer); // @addr $7D8BE8
    procedure Start; // @addr $7D9088
    procedure Advance; // @addr $7D920C
    procedure AnimationComplete(Sender: TObjectGI); // @addr $7D93E0
    function IsFinished: Boolean; // @addr $7D9408
    procedure SetSourcePoint(Point: TPointF); // @addr $7D942C @ida "void __usercall $name(TWeaponEffect *Self@<eax>, TPointF *Point@<edx>);"
    procedure SetTargetPoint(Point: TPointF); // @addr $7D94E8 @ida "void __usercall $name(TWeaponEffect *Self@<eax>, TPointF *Point@<edx>);"
  end;
  ...
  public
    ...
    destructor Destroy; override; // @addr $7D50CC @ida "void __usercall $name(TWeaponSE *Self@<eax>, __int8 DestroyFlags@<dl>);"
    function GetTargetPoint: TPointF; // @addr $7D7640 @ida "void __usercall $name(TWeaponSE *Self@<eax>, TPointF *Result@<edx>);"
    function GetSourcePoint: TPointF; // @addr $7D76DC @ida "void __usercall $name(TWeaponSE *Self@<eax>, TPointF *Result@<edx>);"
    procedure LoadTemplate(Block: TBlockParEC); override; // @addr $7D821C
    procedure AttachToSpace(ASpace: TSpaceSE); override; // @addr $7D5128
    procedure DetachFromSpace; override; // @addr $7D7488
    procedure Advance; override; // @addr $7D779C
    procedure SetHit(Color, Damage: Integer; Destroyed, PlaySound: Boolean); // @addr $7D75D4
    procedure SetEndpoints(Source, Target: TObjectSE); // @addr $7D7610 Retains both scene references.
    constructor Create(const GraphKey: WideString; UnusedPosition: TPoint; ShotVisual, Variant: Integer); // @addr 0x7D4F5C @ida "TWeaponSE *__userpurge $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, unsigned __int16 *GraphKey@<ecx>, TPoint *UnusedPosition@<^8>, int ShotVisual@<^4>, int Variant@<^0>);" @note "Stores visual/variant and appends them to GraphKey. Position is passed through the base constructor."
  end;

// The owner-color helper's original unit ownership is unresolved.

procedure InitializeWeaponVisualResources; // @addr $7D95A4
...
{ @routine $7D4F5C TWeaponSE_Create }
...
{ @routine $7D50CC TWeaponSE_Destroy }
...
{ @routine $7D5128 TWeaponSE_AttachToSpace }
...
begin
  ...
  if (ShotVisual = 0) and (CountDelimitedPartsW(GraphKey, ',') > 1) then
  begin
    ShotVisual := StrToInt(ExtractDelimitedPartW(GraphKey, 1, ','));
    LoadTemplate(GameDataConfig.GetBlockByPath('SE.' + Key));
  end;
  UseRandomHit := True;
  ...
  else Projectile := TPSEyesGI.Create(Space.MapPanel);
...
{ @routine $7D7488 TWeaponSE_DetachFromSpace }
...
{ @routine $7D75D4 TWeaponSE_SetHit }
...
{ @routine $7D7610 TWeaponSE_SetEndpoints }
...
{ @routine $7D7640 TWeaponSE_GetTargetPoint }
...
{ @routine $7D76DC TWeaponSE_GetSourcePoint }
...
{ @routine $7D779C TWeaponSE_Advance }
...
{ @routine $7D821C TWeaponSE_LoadTemplate }
procedure TWeaponSE.LoadTemplate(Block: TBlockParEC);
var
  PaletteBlock, Palettes: TBlockParEC;

  // @nested $7D817C GetWeaponTemplateParam
  function GetWeaponTemplateParam(Name: WideString): WideString; // @addr $7D817C @ida "void __usercall $name(unsigned __int16 *Name@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x7D82CE, 0x7D82EB"
  begin
...
{ @routine $7D8404 TWeaponEffect_Create }
...
{ @routine $7D85B8 TWeaponEffect_Destroy }
...
{ @routine $7D85E4 TWeaponEffect_Clear }
...
{ @routine $7D8608 TWeaponEffect_AddItem }
...
{ @routine $7D86AC TWeaponEffect_RemoveItem }
...
{ @routine $7D8748 TWeaponEffect_AddTargetEffect }
...
{ @routine $7D8BE8 TWeaponEffect_AddSourceEffect }
...
{ @routine $7D9088 TWeaponEffect_Start }
...
{ @routine $7D920C TWeaponEffect_Advance }
...
{ @routine $7D93E0 TWeaponEffect_AnimationComplete }
...
{ @routine $7D9408 TWeaponEffect_IsFinished }
...
{ @routine $7D942C TWeaponEffect_SetSourcePoint }
...
{ @routine $7D94E8 TWeaponEffect_SetTargetPoint }
...
{ @routine $7D95A4 InitializeWeaponVisualResources }
