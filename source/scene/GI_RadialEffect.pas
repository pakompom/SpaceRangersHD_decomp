...
var
  AuraAnimationPaths: array of WideString; // @addr $889400
  RadiationRadii: array of Integer; // @addr $889404
  RadiationPalettes: array of TRadiationPalette; // @addr $889408
...
type
  ...
  public
    ...
    constructor Create(Owner: TObjectGI; APaletteIndex: Integer); // @addr $4EBD18 @ida "TPSRadEffectGI *__userpurge $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, TObjectGI *Owner@<ecx>, int APaletteIndex@<^0>);"
    destructor Destroy; override; // @addr $4EBDE8 @ida "void __usercall $name(TPSRadEffectGI *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure SetColor(Value: Word); // @addr $4EBE24
    procedure SetPosition(Position: TPoint); override; // @addr $4EBE44 @ida "void __usercall $name(TPSRadEffectGI *Self@<eax>, TPoint *Position@<edx>);"
    procedure SetTargetPoint(Point: TPoint); override; // @addr $4EC034 @ida "void __usercall $name(TPSRadEffectGI *Self@<eax>, TPoint *Point@<edx>);"
    procedure UpdateProjectionBounds; // @addr $4EC088
    procedure UpdateHitTestBounds; override; // @addr $4EC100
    function GetLocalBounds: TRect; override; // @addr $4EC160 @ida "void __usercall $name(TPSRadEffectGI *Self@<eax>, TRect *Result@<edx>);"
    function AddParticle: PRadiationParticle; // @addr $4EC1C4
    procedure ClearParticles; // @addr $4EC23C
    procedure InvalidateRect(Rect: TRect); override; // @addr $4EC290 @ida "void __usercall $name(TPSRadEffectGI *Self@<eax>, TRect *Rect@<edx>);"
    procedure Advance(Timer: PCallbackTimerGI; UserData: Integer); override; // @addr $4EC35C
    procedure Draw(ClipRect: TRect); override; // @addr $4ECDA4 @ida "void __usercall $name(TPSRadEffectGI *Self@<eax>, TRect *ClipRect@<edx>);"
  end;

procedure LoadRadiationPalettes; // @addr $4ED0B0

var
  RadiationEdgeStarts: array[0..23] of TPoint; // @addr $88940C
  RadiationEdgeEnds: array[0..23] of TPoint; // @addr $8894CC
  RadiationHexagon: array[0..5] of TPointF = (
    (X: -0.866025404; Y: 0.5), (X: 0; Y: 1),
    (X: 0.866025404; Y: 0.5), (X: 0.866025404; Y: -0.5),
    (X: 0; Y: -1), (X: -0.866025404; Y: -0.5)); // @addr $87A9C8
...
// @unit-initialization $8757F0
// @unit-finalization $4ED5F8

uses SysUtils, Math, EC_BlockPar, EC_Str, EC_Mem, GR_Main, GR_DX, aMyFunction, Globals;

{ @routine $4EBD18 TPSRadEffectGI_Create }
...
{ @routine $4EBDE8 TPSRadEffectGI_Destroy }
...
{ @routine $4EBE24 TPSRadEffectGI_SetColor }
...
{ @routine $4EBE44 TPSRadEffectGI_SetPosition }
...
{ @routine $4EC034 TPSRadEffectGI_SetTargetPoint }
...
{ @routine $4EC088 TPSRadEffectGI_UpdateProjectionBounds }
...
{ @routine $4EC100 TPSRadEffectGI_UpdateHitTestBounds }
...
{ @routine $4EC160 TPSRadEffectGI_GetLocalBounds }
...
{ @routine $4EC1C4 TPSRadEffectGI_AddParticle }
...
{ @routine $4EC23C TPSRadEffectGI_ClearParticles }
...
{ @routine $4EC290 TPSRadEffectGI_InvalidateRect }
...
{ @routine $4EC35C TPSRadEffectGI_Advance }
...
{ @routine $4ECDA4 TPSRadEffectGI_Draw }
procedure TPSRadEffectGI.Draw(ClipRect: TRect);
var
  DistanceSquared, FadeRadius, CenterX, CenterY: Integer;

  // @nested $4EC87C RadiationRingColor
  function RadiationRingColor(Value: Integer): Cardinal; // @addr $4EC87C @ida "unsigned int __usercall $name@<eax>(int Value@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x004ECC96, 0x004ECA8C"
  begin
    Result := SampleGradientColor(RadiationPalettes[PaletteIndex], Sqr(Value / 255.0));
  end;

  // @nested $4EC8D0 RadiationRingAlpha
  function RadiationRingAlpha(Distance: Single): Integer; // @addr $4EC8D0 @ida "int __userpurge $name@<eax>(float Distance@<^0>, void *ParentFrame@<^4>);" @stackpop 4 @calls "0x004ECC7C, 0x004ECA72"
  begin
  ...
  // @nested $4ECA3C QueueRadiationOctants
  procedure QueueRadiationOctants(X, Y: Integer); // @addr $4ECA3C @ida "void __usercall $name(int X@<eax>, int Y@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x004ECF53"
  var
    PixelColor: Cardinal;
    PixelAlpha: Integer;
    Distance: Single;
...
var
  ...
  // @nested $4ECC2C BlendRadiationOctants
  procedure BlendRadiationOctants(X, Y: Integer); // @addr $4ECC2C @ida "void __usercall $name(int X@<eax>, int Y@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x004ED09D"
  var
    PixelColor: Cardinal;
    PixelAlpha: Integer;

    // @nested $4ECBC4 BlendRadiationPoint
    procedure BlendRadiationPoint(X, Y: Integer); // @addr $4ECBC4 @ida "void __usercall $name(int X@<eax>, int Y@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x004ECCBA, 0x004ECCD7, 0x004ECCFE, 0x004ECD19, 0x004ECD3A, 0x004ECD57, 0x004ECD7E, 0x004ECD99"
    begin
...
{ @routine $4ED0B0 LoadRadiationPalettes }
