unit GI_Cursor;
// Unit bracket (inferred): .text 0x004B58A0..0x004B683E; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
// Native dynamic-array RTTI names GI_Cursor at $4B58A0, $4B58C4 and $4B58E8.
...
type
  ...
  public
    ...
    constructor Create(Owner: TObjectGI); // @addr $4B5A50 @ida "TCursorGI *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, TObjectGI *Owner@<ecx>);"
    destructor Destroy; override; // @addr $4B5AB8 @ida "void __usercall $name(TCursorGI *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Clear; override; // @addr $4B5BB8 @note "Clears cursor resources and the image child, retaining the child object."
    procedure SetImagePath(const Path: WideString); // @addr $4B5CA0
    procedure SetActive(Enabled: Boolean); override; // @addr $4B5D48
    procedure SetOrigin(Origin: TPoint); override; // @addr $4B5EB0 @ida "void __usercall $name(TCursorGI *Self@<eax>, TPoint *Origin@<edx>);"
    procedure Draw(ClipRect: TRect); override; // @addr $4B5F08 @ida "void __usercall $name(TCursorGI *Self@<eax>, TRect *ClipRect@<edx>);"
    procedure RebuildSystemCursor; // @addr $4B5F30 @note "Builds Windows cursor handles from GI/GAI resources and schedules animation when active."
    function CreateCursorBitmap(Buffer: TGraphBufGR): Cardinal; // @addr $4B65C8 @note "Copies a 32-bit image to a top-down Windows DIB; caller owns the bitmap."
    procedure AdvanceAnimation(Timer: PCallbackTimerGI; UserData: Integer); // @addr $4B6764 @note "Timer and UserData are unused; replaces AnimationTimer after advancing the sequence."
  end;
...
{ @routine $4B5A50 TCursorGI_Create }
...
{ @routine $4B5AB8 TCursorGI_Destroy }
...
{ @routine $4B5BB8 TCursorGI_Clear }
...
{ @routine $4B5CA0 TCursorGI_SetImagePath }
...
{ @routine $4B5D48 TCursorGI_SetActive }
...
{ @routine $4B5EB0 TCursorGI_SetOrigin }
...
{ @routine $4B5F30 TCursorGI_RebuildSystemCursor }
...
function CreateDIBSection(DC: HDC; const BitmapInfo: TBitmapInfo; Usage: Cardinal;
  ...
  external 'gdi32.dll' name 'CreateDIBSection'; // @addr $4B65C0

{ @routine $4B65C8 TCursorGI_CreateCursorBitmap }
...
{ @routine $4B6764 TCursorGI_AdvanceAnimation }
