unit GI_Edit;
// Unit bracket (inferred): .text 0x004A54F8..0x004A79E5; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create(Owner: TObjectGI); // @addr 0x4A5628 @ida "TEditGI *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, TObjectGI *Owner@<ecx>);"
    destructor Destroy; override; // @addr 0x4A5744 @ida "void __usercall $name(TEditGI *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Clear; override; // @addr 0x4A57A8
    procedure SetFontName(FontName: WideString); // @addr 0x4A585C
    procedure SetBorderEnabled(Value: Boolean); // @addr 0x4A58BC
    procedure SetText(Value: WideString); // @addr 0x4A58F4 @note "Resets CaretPosition on change; does not clamp to MaxLength or invoke ChangedCallback."
    function HasGlyph(Character: WideChar): Boolean; // @addr 0x4A5970
    procedure SetTextColor(Value: Cardinal); // @addr 0x4A59E0
    procedure SetBorderLightColor(Value: Cardinal); // @addr 0x4A5A18
    procedure SetBorderDarkColor(Value: Cardinal); // @addr 0x4A5A50
    procedure SetTextAlignX(Value: TTextAlignXGI); // @addr 0x4A5A88 @note "Only Left and Center are accepted; other values raise."
    procedure SetCaretPosition(Value: Integer); // @addr 0x4A5B10 @note "Clamps to 0..Length(Text)."
    procedure ProcessLeftButtonDown(KeyState: Cardinal; Point: TPoint); override; // @addr $4A5B78 @ida "void __usercall $name(TEditGI *Self@<eax>, unsigned int KeyState@<edx>, TPoint *Point@<ecx>);"
    procedure OnCaretBlink; override; // @addr $4A611C
    procedure OnFocusGained; override; // @addr 0x4A5BB8
    procedure OnFocusLost; override; // @addr 0x4A5BF8
    procedure ProcessKeyDown(Key: Integer); override; // @addr 0x4A5C40
    procedure ProcessCharacter(Character: WideChar); override; // @addr 0x4A5FF0 @note "Requires a font glyph, acceptance by the optional callback, and length below MaxLength."
    procedure LoadFromConfigPath(const Path: WideString); override; // @addr 0x4A6134
    procedure LoadFromBlock(Block: TBlockParEC); override; // @addr 0x4A689C
    procedure Draw(ClipRect: TRect); override; // @addr 0x4A7050 @ida "void __usercall $name(TEditGI *Self@<eax>, TRect *ClipRect@<edx>);"
    procedure QueueImageLoad(PendingLoads: TList); override; // @addr 0x4A79A8
  end;
...
{ @routine $4A5744 TEditGI_Destroy }
...
{ @routine $4A585C TEditGI_SetFontName }
...
{ @routine $4A58BC TEditGI_SetBorderEnabled }
...
{ @routine $4A58F4 TEditGI_SetText }
...
{ @routine $4A5970 TEditGI_HasGlyph }
...
{ @routine $4A59E0 TEditGI_SetTextColor }
...
{ @routine $4A5A18 TEditGI_SetBorderLightColor }
...
{ @routine $4A5A50 TEditGI_SetBorderDarkColor }
...
{ @routine $4A5A88 TEditGI_SetTextAlignX }
...
{ @routine $4A5B10 TEditGI_SetCaretPosition }
...
{ @routine $4A5B78 TEditGI_ProcessLeftButtonDown }
...
{ @routine $4A5BB8 TEditGI_OnFocusGained }
...
{ @routine $4A5BF8 TEditGI_OnFocusLost }
...
{ @routine $4A5C40 TEditGI_ProcessKeyDown }
...
{ @routine $4A5FF0 TEditGI_ProcessCharacter }
...
{ @routine $4A611C TEditGI_OnCaretBlink }
...
{ @routine $4A6134 TEditGI_LoadFromConfigPath }
...
{ @routine $4A689C TEditGI_LoadFromBlock }
...
{ @routine $4A79A8 TEditGI_QueueImageLoad }
