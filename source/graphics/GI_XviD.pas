unit GI_XviD;
// Unit bracket (inferred): .text 0x00472DCC..0x00473C00; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create(Owner: TObjectGI); // @addr $472F04 @ida "TxvidGI *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, TObjectGI *Owner@<ecx>);"
    destructor Destroy; override; // @addr $472F4C @ida "void __usercall $name(TxvidGI *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Clear; override; // @addr $472F88
    function ImageOpen(const FileName: WideString; FillViewport: Boolean): Boolean; // @addr $472FA4
    procedure XvidClose; // @addr $473720
    procedure ImageClose; // @addr $473838
    procedure LoadFromConfigPath(const Path: WideString); override; // @addr $4738A8
    procedure LoadFromBlock(Block: TBlockParEC); override; // @addr $4738DC
    procedure ReadVideoConfig(Block: TBlockParEC); // @addr $473904
    function DecodeNextFrame: Boolean; // @addr $473914
    function SetPlaybackTime(TimeMs: Double): Boolean; // @addr $473B24 @ida "bool __userpurge $name@<al>(TxvidGI *Self@<eax>, double TimeMs@<^0>);"
    procedure SetFramePosition(Frame: Integer); // @addr $473B8C
  end;

var
  XvidLibrary: Cardinal = 0; // @addr $87A6C8
  XvidGlobal: TXvidFunction = nil; // @addr $87A6CC
  XvidDecore: TXvidFunction = nil; // @addr $87A6D0

implementation

uses Windows, SysUtils, Direct3D9, GR_DX, GR_Main, EC_Str, EC_Struct;

{ @routine $472F04 TxvidGI_Create }
...
{ @routine $472F4C TxvidGI_Destroy }
...
{ @routine $472FA4 TxvidGI_ImageOpen }
...
{ @routine $473720 TxvidGI_XvidClose }
...
{ @routine $473838 TxvidGI_ImageClose }
...
{ @routine $4738A8 TxvidGI_LoadFromConfigPath }
...
{ @routine $4738DC TxvidGI_LoadFromBlock }
...
{ @routine $473904 TxvidGI_ReadVideoConfig }
procedure TxvidGI.ReadVideoConfig(Block: TBlockParEC);
begin
end;
{ @end $473904 }

{ @routine $473914 TxvidGI_DecodeNextFrame }
...
{ @routine $473B24 TxvidGI_SetPlaybackTime }
...
{ @routine $473B8C TxvidGI_SetFramePosition }
