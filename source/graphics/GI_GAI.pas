unit GI_GAI;
// Unit bracket (inferred): .text 0x004D036C..0x004D36F9; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    procedure SetHardwareMirrorHorizontal(Value: Boolean); // @addr $4D26C0

    constructor Create(Owner: TObjectGI); // @addr 0x4D049C @ida "TgaiGI *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, TObjectGI *Owner@<ecx>);"
    destructor Destroy; override; // @addr 0x4D05B8 @ida "void __usercall $name(TgaiGI *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Clear; override; // @addr 0x4D0680 @note "Preserves animation state."
    procedure SetImagePath(const ImagePath: WideString); // @addr 0x4D0694 @note "Resets sequence position even when the key is unchanged."
    function GetImagePath: WideString; // @addr 0x4D0708 @ida "void __usercall $name(TgaiGI *Self@<eax>, unsigned __int16 **Result@<edx>);"
    procedure SetFirstFrameImagePath(const ImagePath: WideString); // @addr 0x4D072C
    procedure SetSequenceFrame(FrameInSequence: Integer); // @addr 0x4D07D4 @note "Does not validate the index."
    procedure SetFramePosition(FrameInSequence: Integer; ForwardOnly: Boolean); // @addr 0x4D0820 @note "Accepted out-of-range positions become zero."
    function GetMainImageFrameCount: Integer; // @addr 0x4D0898 @note "Returns zero in FirstFrameOnly mode."
    procedure StopAutoPlayback; // @addr 0x4D0908
    procedure RestartPlayback; // @addr 0x4D0948 @note "Does not reset frame position; single-frame sequences remain timer-free."
    function GetContentSize: TPoint; // @addr 0x4D0A04 @ida "void __usercall $name(TgaiGI *Self@<eax>, TPoint *Result@<edx>);"
    function GetContentOrigin: TPoint; // @addr 0x4D0AFC @ida "void __usercall $name(TgaiGI *Self@<eax>, TPoint *Result@<edx>);"
    procedure SetImageKindX(Value: TImageKindXGI); // @addr 0x4D0C10
    procedure SetImageKindY(Value: TImageKindYGI); // @addr 0x4D0C48
    procedure SetAlpha(Value: Byte); // @addr 0x4D0C80
    procedure SetSize(Size: TPoint); override; // @addr 0x4D0CB8 @ida "void __usercall $name(TgaiGI *Self@<eax>, TPoint *Size@<edx>);"
    procedure ClearFrameSequence; // @addr 0x4D0CDC
    procedure LoadFrameSequenceFromText(const FrameSpec: WideString); // @addr 0x4D0D50 @note "Accepts ascending and descending ranges; changes the playback timer unless stopped."
    function GetSequenceCount: Integer; // @addr 0x4D0FE8
    function GetSequenceFrameSourceIndex(FrameInSequence: Integer): Integer; // @addr 0x4D1058 @note "Does not validate the index."
    procedure SetFrameDelay(FrameInSequence, DelayMs: Integer); // @addr 0x4D1090 @note "Does not validate the index."
    function GetFrameDelay(FrameInSequence: Integer): Integer; // @addr 0x4D10CC @note "Does not validate the index."
    function HitTestPixel(Point: TPoint): Boolean; // @addr 0x4D1104 @ida "bool __usercall $name@<al>(TgaiGI *Self@<eax>, TPoint *Point@<edx>);" @note "Black pixels do not count as hits; composed playback may require an existing composition buffer."
    procedure SetActive(Value: Boolean); override; // @addr 0x4D16D8
    procedure OnDeactivate; override; // @addr 0x4D178C
    procedure LoadFromConfigPath(const Path: WideString); override; // @addr 0x4D17F0
    procedure LoadFromBlock(Block: TBlockParEC); override; // @addr 0x4D1824
    procedure LoadAnimationProperties(Block: TBlockParEC); // @addr 0x4D184C
    procedure UpdateAutoGeometry; override; // @addr 0x4D1D20 @note "Also rebuilds frame tables when SequenceIndex is nonnegative."
    procedure SetOneCycleDuration(DurationMs: Integer); // @addr 0x4D1F48 @note "Enables StopAfterOneCycle; frame delays are rounded to milliseconds with a minimum of one."
    procedure AdvanceAutoFrame(Timer: PCallbackTimerGI; UserData: Integer); // @addr 0x4D1FD8
    procedure Invalidate; override; // @addr 0x4D2164
    procedure Draw(ClipRect: TRect); override; // @addr 0x4D26DC @ida "void __usercall $name(TgaiGI *Self@<eax>, TRect *ClipRect@<edx>);"
    procedure PrimeImageCaches; // @addr 0x4D35BC @note "Skips the main GAI in FirstFrameOnly mode."
    procedure QueueImageLoad(PendingLoads: TList); override; // @addr 0x4D3624
  end;

var
  GaiFrameHeap: Cardinal = 0; // @addr 0x87A804

procedure LoadGaiFrameToGraphBuf(const Path: WideString; GraphBuf: TGraphBufGR; Seed: Cardinal); // @addr $4D3648
...
{ @routine $4D0694 TgaiGI_SetImagePath }
...
{ @routine $4D0708 TgaiGI_GetImagePath }
...
{ @routine $4D072C TgaiGI_SetFirstFrameImagePath }
...
{ @routine $4D07D4 TgaiGI_SetSequenceFrame }
...
{ @routine $4D0820 TgaiGI_SetFramePosition }
...
{ @routine $4D0898 TgaiGI_GetMainImageFrameCount }
...
{ @routine $4D0908 TgaiGI_StopAutoPlayback }
...
{ @routine $4D0948 TgaiGI_RestartPlayback }
...
{ @routine $4D0A04 TgaiGI_GetContentSize }
...
{ @routine $4D0AFC TgaiGI_GetContentOrigin }
...
{ @routine $4D0C10 TgaiGI_SetImageKindX }
...
{ @routine $4D0C48 TgaiGI_SetImageKindY }
...
{ @routine $4D0C80 TgaiGI_SetAlpha }
...
{ @routine $4D0CDC TgaiGI_ClearFrameSequence }
...
{ @routine $4D0D50 TgaiGI_LoadFrameSequenceFromText }
...
{ @routine $4D0FE8 TgaiGI_GetSequenceCount }
...
{ @routine $4D1058 TgaiGI_GetSequenceFrameSourceIndex }
...
{ @routine $4D1090 TgaiGI_SetFrameDelay }
...
{ @routine $4D10CC TgaiGI_GetFrameDelay }
...
{ @routine $4D1104 TgaiGI_HitTestPixel }
...
{ @routine $4D16D8 TgaiGI_SetActive }
...
{ @routine $4D178C TgaiGI_OnDeactivate }
...
{ @routine $4D17F0 TgaiGI_LoadFromConfigPath }
...
{ @routine $4D1824 TgaiGI_LoadFromBlock }
...
{ @routine $4D184C TgaiGI_LoadAnimationProperties }
...
{ @routine $4D1D20 TgaiGI_UpdateAutoGeometry }
...
{ @routine $4D1F48 TgaiGI_SetOneCycleDuration }
...
{ @routine $4D1FD8 TgaiGI_AdvanceAutoFrame }
...
{ @routine $4D2164 TgaiGI_Invalidate }
...
{ @routine $4D26C0 TgaiGI_SetHardwareMirrorHorizontal }
...
{ @routine $4D35BC TgaiGI_PrimeImageCaches }
...
{ @routine $4D3624 TgaiGI_QueueImageLoad }
...
{ @routine $4D3648 LoadGaiFrameToGraphBuf }
