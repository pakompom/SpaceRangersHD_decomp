unit GR_GraphBuf;
// Unit bracket (inferred): .text 0x008304FC..0x00836A0B; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    function InterpolateRgb(First, Second: Cardinal; Amount: Single): Cardinal; // @addr $8308B8 @ida "unsigned int __userpurge $name@<eax>(TPixelFormatGR *Self@<eax>, unsigned int First@<edx>, unsigned int Second@<ecx>, float Amount@<^0>);"
    function UnpackRed(Color: Cardinal): Byte; // @addr $8309D0
    function UnpackGreen(Color: Cardinal): Byte; // @addr $830A00
    function UnpackBlue(Color: Cardinal): Byte; // @addr $830A30
    procedure RebuildChannelMetrics; // @addr 0x8305C4 @note "Ignores disjoint bits after each mask's first contiguous run; BytesPerPixel is unchanged."
    function PackRgbBytes(Red, Green, Blue: Byte): Cardinal; // @addr 0x83076C
    function PackRgb(Red, Green, Blue: Integer): Cardinal; // @addr 0x8307DC
    function PackNormalizedRgb(Red, Green, Blue: Double): Cardinal; // @addr 0x830834 @ida "unsigned int __userpurge $name@<eax>(TPixelFormatGR *Self@<eax>, double Red@<^16>, double Green@<^8>, double Blue@<^0>);" @note "Does not clamp inputs or include alpha."
  end;
  ...
  public
    function GetPixel16(X, Y: Integer): Cardinal; // @addr $83179C
    procedure SetPixel16(X, Y: Integer; Color: Cardinal); // @addr $8317E8
    function GetBrightness16(X, Y: Integer): Cardinal; // @addr $831834
    function GetPixel32(X, Y: Integer): Cardinal; // @addr $8319B8
    procedure DrawHorizontalLine16(X, Y, Count: Integer; Color: Cardinal); // @addr $831A04
    procedure DrawVerticalLine16(X, Y, Count: Integer; Color: Cardinal); // @addr $831A68
    procedure DrawHorizontalLine16Clipped(X, Y, Count: Integer; Color: Cardinal; Clip: TRect); // @addr $831ADC @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, int X@<edx>, int Y@<ecx>, int Count@<^8>, unsigned int Color@<^4>, TRect *Clip@<^0>);"
    procedure DrawVerticalLine16Clipped(X, Y, Count: Integer; Color: Cardinal; Clip: TRect); // @addr $831B80 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, int X@<edx>, int Y@<ecx>, int Count@<^8>, unsigned int Color@<^4>, TRect *Clip@<^0>);"
    procedure ClearPixels; // @addr $8327F0
    procedure FillPixels(Value: Byte); // @addr $832820
    procedure FillRect32(Rect: TRect; Color: Cardinal); // @addr $832898 @ida "void __usercall $name(TGraphBufGR *Self@<eax>, TRect *Rect@<edx>, unsigned int Color@<ecx>);"
    procedure BlendPixel16(X, Y: Integer; Color: Cardinal; Alpha: Byte); // @addr $831904
    procedure DrawAlphaLine16(X1, Y1, X2, Y2: Integer; Color: Word; Alpha: Byte; Clip: TRect); // @addr $83195C @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, int X1@<edx>, int Y1@<ecx>, int X2@<^16>, int Y2@<^12>, unsigned __int16 Color@<^8>, unsigned __int8 Alpha@<^4>, TRect *Clip@<^0>);"
    procedure DrawLine16(First, Last: TPoint; Color: Cardinal); // @addr $831C24 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *First@<edx>, TPoint *Last@<ecx>, unsigned int Color@<^0>);"
    procedure DrawAnimatedLine16(First, Last: TPoint; Color: Cardinal; Phase: Integer; Clip: TRect); // @addr $831CBC @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *First@<edx>, TPoint *Last@<ecx>, unsigned int Color@<^8>, int Phase@<^4>, TRect *Clip@<^0>);"
    procedure DrawShadowLine16(First, Last: TPoint; Color: Cardinal; Phase: Integer; Clip: TRect; ShadowPixels: Pointer; ShadowPitch: Integer); // @addr $831D20 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *First@<edx>, TPoint *Last@<ecx>, unsigned int Color@<^16>, int Phase@<^12>, TRect *Clip@<^8>, void *ShadowPixels@<^4>, int ShadowPitch@<^0>);"
    procedure DrawAlphaTrapezium16(X1, Y1, X2, Y2, X3, X4: Integer; Color: Word; Alpha: Byte; Clip: TRect); // @addr $831D8C @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, int X1@<edx>, int Y1@<ecx>, int X2@<^24>, int Y2@<^20>, int X3@<^16>, int X4@<^12>, unsigned __int16 Color@<^8>, unsigned __int8 Alpha@<^4>, TRect *Clip@<^0>);"
    procedure DrawLine16Clipped(First, Last: TPoint; Color: Cardinal; Clip: TRect); // @addr $831E2C @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *First@<edx>, TPoint *Last@<ecx>, unsigned int Color@<^4>, TRect *Clip@<^0>);"
    procedure FillPixels16(Color: Word); // @addr $832858
    procedure ScaleAlpha(Rect: TRect; Alpha: Byte); // @addr $832928 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TRect *Rect@<edx>, unsigned __int8 Alpha@<cl>);"
    procedure FlipHorizontal16; // @addr $8329D4
    procedure RotateLeft16; // @addr $832A8C
    procedure Stretch16(Width, Height: Cardinal); // @addr $832BFC
    procedure ConvertRgbTo565; // @addr $832CAC
    procedure Convert565ToRgb; // @addr $832DEC
    procedure ShiftLight16(Shift: Integer; Rect: TRect); // @addr $832EC8 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, int Shift@<edx>, TRect *Rect@<ecx>);"
    procedure DrawCircle16(Center: TPoint; Radius: Integer; OutlineColor, FillColor: Cardinal; Clip: TRect); // @addr $832F38 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *Center@<edx>, int Radius@<ecx>, unsigned int OutlineColor@<^8>, unsigned int FillColor@<^4>, TRect *Clip@<^0>);"
    procedure DrawCircle8(Center: TPoint; Radius: Integer; OutlineColor, FillColor: Cardinal; Clip: TRect); // @addr $832FD8 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *Center@<edx>, int Radius@<ecx>, unsigned int OutlineColor@<^8>, unsigned int FillColor@<^4>, TRect *Clip@<^0>);"
    procedure ApplyOperations(const Operations: WideString); // @addr $833074
    procedure RescaleRgb(Width, Height: Integer); // @addr $833234
    procedure RescaleRgba(Width, Height, Filter: Integer); // @addr $833318
    procedure RescaleBilinearRgba(Width, Height: Integer); // @addr $833400
    procedure FillPolygon32(Points: array of TPoint; Color: Cardinal); // @addr $833704
    function GetPixelCentroid: TPoint; // @addr $833A2C @ida "void __usercall $name(TGraphBufGR *Self@<eax>, TPoint *Result@<edx>);"
    procedure CopyRect32(Dest: TPoint; Source: TGraphBufGR; Rect: TRect); // @addr $833AF4 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *Dest@<edx>, TGraphBufGR *Source@<ecx>, TRect *Rect@<^0>);"
    procedure BlendRect32(Dest: TPoint; Source: TGraphBufGR; Rect: TRect); // @addr $833BCC @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *Dest@<edx>, TGraphBufGR *Source@<ecx>, TRect *Rect@<^0>);"
    procedure MakeShadow; // @addr $833D54
    procedure AdjustBrightness(Percent: Integer); // @addr $83511C
    procedure ConvertToGrayscale; // @addr $83534C
    function GetTexture: IDirect3DTexture9; // @addr $835640 @ida "void __usercall $name(TGraphBufGR *Self@<eax>, IDirect3DTexture9 **Result@<edx>);"
    procedure Crop(Rect: TRect); // @addr $834ED4 @ida "void __usercall $name(TGraphBufGR *Self@<eax>, TRect *Rect@<edx>);"
    procedure DrawNinePatch(X, Y, Width, Height: Integer; Source: TGraphBufGR; SourceRect, Borders: TRect); // @addr $8342E4 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, int X@<edx>, int Y@<ecx>, int Width@<^16>, int Height@<^12>, TGraphBufGR *Source@<^8>, TRect *SourceRect@<^4>, TRect *Borders@<^0>);"
    procedure RescaleWithAspect(Width, Height: Cardinal; CropToAspect: Boolean; HorizontalAlign, VerticalAlign, Filter: Integer); // @addr $8345B8
    procedure RescaleRGBA_HW(Width, Height: Cardinal; CropToAspect: Boolean; HorizontalAlign, VerticalAlign: Integer); // @addr 0x83491C @note "Alignment values: 0=start, 1=center, 2=end."
    procedure LoadFromScreen(UnusedOption: Byte); // @addr $83587C @note "The byte-sized option is ignored in this build."
    procedure ConvertBgraToRgb24; // @addr $835F04
    procedure DrawAntialiasedCircle16(Center: TPoint; Radius: Integer; Color: Cardinal; Clip: TRect); // @addr $836110 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *Center@<edx>, int Radius@<ecx>, unsigned int Color@<^4>, TRect *Clip@<^0>);"
    procedure DrawAntialiasedLine16(X1, Y1, X2, Y2: Integer; Color: Cardinal; Alpha: Integer; Clip: TRect); // @addr $836410 @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, int X1@<edx>, int Y1@<ecx>, int X2@<^16>, int Y2@<^12>, unsigned int Color@<^8>, int Alpha@<^4>, TRect *Clip@<^0>);"
    ...
    constructor Create(AUseTexture: Boolean); // @addr 0x830A4C @ida "TGraphBufGR *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>, bool AUseTexture@<cl>);"
    destructor Destroy; override; // @addr 0x830AF0 @ida "void __usercall $name(TGraphBufGR *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Clear; // @addr 0x830B2C
    function GetPixels: Pointer; // @addr 0x830BB8 @note "Locks texture storage for writing if necessary."
    procedure AllocateNativePitch(Width, Height, PitchBytes: Integer); // @addr $830D50
    procedure AttachPixels(Width, Height, PitchBytes: Integer; Data: Pointer); // @addr $830E54
    procedure AllocateNative(Width, Height: Integer); // @addr 0x830BDC @note "Records 16-bit pixels; software pitch uses CurrentPixelFormat.BytesPerPixel and four-byte alignment."
    procedure AllocateRgbaTight(Width, Height: Integer); // @addr 0x830EC4 @note "Software storage uses Width*4 pitch; texture storage uses the returned surface pitch."
    procedure AllocateRgba(Width, Height, PitchBytes: Integer); // @addr 0x830FC8
    procedure AllocateRgbTight(Width, Height: Integer); // @addr 0x8310CC
    procedure AllocateRgb(Width, Height, PitchBytes: Integer); // @addr 0x831130 @note "Always allocates software storage, even when UseTexture is enabled."
    procedure AllocateGrayscale(Width, Height: Integer); // @addr 0x831194 @note "Eight-bit software pixels with four-byte-aligned pitch."
    // Decode the entire file payload, ignoring Buffer.Position. Failures raise.
    procedure LoadImage(Buffer: TBufEC); // @addr 0x831260 @note "Uses CurrentPixelFormat masks and byte width."
    procedure LoadImageRgba(Buffer: TBufEC); // @addr 0x8314D8 @note "Produces BGRA byte order, with alpha in the high byte."
    procedure LoadImageRgb(Buffer: TBufEC); // @addr 0x8315C8 @note "Produces RGB byte order."
    procedure LoadImageGrayscale(Buffer: TBufEC); // @addr 0x8316B0
    procedure SaveToBuffer(Buffer: TBufEC); // @addr 0x833DC4 @note "Replaces Buffer with width, height, pitch and raw pixels. Leaves Position at 12, before the pixel data."
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr 0x833E50 @note "Accepts zlib-packed or raw buffer data. Allocates software pixels and leaves Position immediately after the 12-byte image header."
    procedure SavePng(FileName: WideString); // @addr 0x833F38 @note "Assumes four-byte BGRA pixels. Converts FileName to ANSI and ignores the writer's status."
    procedure SaveBmp(FileName: WideString); // @addr 0x833FCC @note "Assumes 32-bit BGRA pixels; alpha is excluded. Converts FileName to ANSI and ignores the writer's status."
    procedure SaveJpeg(FileName: WideString; Quality: Integer); // @addr 0x834070 @note "Writes an intermediate BMP to FileName, then replaces it with JPEG. Quality is truncated to one byte; exceptions after the BMP write are swallowed."
    procedure DrawAntialiasedLine(FirstPoint, SecondPoint: TPoint; Color: Cardinal); // @addr 0x8321DC @ida "void __userpurge $name(TGraphBufGR *Self@<eax>, TPoint *FirstPoint@<edx>, TPoint *SecondPoint@<ecx>, unsigned int Color@<^0>);" @note "RGBA pixels; coverage replaces the color alpha. Blending preserves an existing pixel's alpha unless coverage is fully opaque."
    procedure LockTexture(ReadOnly: Boolean); // @addr 0x835E14
    procedure UnlockTexture; // @addr 0x835EB8
  end;
...
{ @routine $8305C4 TPixelFormatGR_RebuildChannelMetrics }
...
{ @routine $83076C TPixelFormatGR_PackRgbBytes }
...
{ @routine $8307DC TPixelFormatGR_PackRgb }
...
{ @routine $830834 TPixelFormatGR_PackNormalizedRgb }
...
{ @routine $8308B8 TPixelFormatGR_InterpolateRgb }
...
{ @routine $8309D0 TPixelFormatGR_UnpackRed }
...
{ @routine $830A00 TPixelFormatGR_UnpackGreen }
...
{ @routine $830A30 TPixelFormatGR_UnpackBlue }
...
{ @routine $830A4C TGraphBufGR_Create }
...
{ @routine $830AF0 TGraphBufGR_Destroy }
...
{ @routine $830B2C TGraphBufGR_Clear }
...
{ @routine $830BB8 TGraphBufGR_GetPixels }
...
{ @routine $830BDC TGraphBufGR_AllocateNative }
...
{ @routine $830D50 TGraphBufGR_AllocateNativePitch }
...
{ @routine $830E54 TGraphBufGR_AttachPixels }
...
{ @routine $830EC4 TGraphBufGR_AllocateRgbaTight }
...
{ @routine $830FC8 TGraphBufGR_AllocateRgba }
...
{ @routine $8310CC TGraphBufGR_AllocateRgbTight }
...
{ @routine $831130 TGraphBufGR_AllocateRgb }
...
{ @routine $831194 TGraphBufGR_AllocateGrayscale }
...
{ @routine $831260 TGraphBufGR_LoadImage }
...
{ @routine $8314D8 TGraphBufGR_LoadImageRgba }
...
{ @routine $8315C8 TGraphBufGR_LoadImageRgb }
...
{ @routine $8316B0 TGraphBufGR_LoadImageGrayscale }
...
{ @routine $83179C TGraphBufGR_GetPixel16 }
...
{ @routine $8317E8 TGraphBufGR_SetPixel16 }
...
{ @routine $831834 TGraphBufGR_GetBrightness16 }
...
{ @routine $831904 TGraphBufGR_BlendPixel16 }
...
{ @routine $83195C TGraphBufGR_DrawAlphaLine16 }
...
{ @routine $8319B8 TGraphBufGR_GetPixel32 }
...
{ @routine $831A04 TGraphBufGR_DrawHorizontalLine16 }
...
{ @routine $831A68 TGraphBufGR_DrawVerticalLine16 }
...
{ @routine $831ADC TGraphBufGR_DrawHorizontalLine16Clipped }
...
{ @routine $831B80 TGraphBufGR_DrawVerticalLine16Clipped }
...
{ @routine $831C24 TGraphBufGR_DrawLine16 }
...
{ @routine $831CBC TGraphBufGR_DrawAnimatedLine16 }
...
{ @routine $831D20 TGraphBufGR_DrawShadowLine16 }
...
{ @routine $831D8C TGraphBufGR_DrawAlphaTrapezium16 }
...
{ @routine $831E2C TGraphBufGR_DrawLine16Clipped }
...
{ @routine $8321DC TGraphBufGR_DrawAntialiasedLine }
...
var
  ...
  // @nested $831F4C LineFraction
  function LineFraction(Value: Double): Double; // @addr $831F4C @ida "double __userpurge $name@<st0>(double Value@<^0>, void *ParentFrame@<^8>);" @stackpop 8 @calls "0x0083243E 0x00832482 0x0083249C 0x008325AE 0x008325F7 0x0083260E 0x0083270F 0x00832729"
  begin
    Result := Value - Floor(Value);
  end;

  // @nested $831FB8 PlotLinePixel
  procedure PlotLinePixel(X, Y: Integer; Color: Cardinal; Alpha: Integer); // @addr $831FB8 @ida "void __userpurge $name(int X@<eax>, int Y@<edx>, unsigned int Color@<ecx>, int Alpha@<^0>, void *ParentFrame@<^4>);" @stackpop 4 @calls "0x008324CF 0x008324F6 0x0083251E 0x00832545 0x0083264A 0x0083267D 0x008326B3 0x008326E8 0x0083275C 0x00832783 0x008327AB 0x008327D2"
  var Source, Dest: PColorRGBA; Denominator: Cardinal;

    // @nested $831F78 BlendLineChannel
    function BlendLineChannel(DestColor, DestAlpha, SourceColor, SourceAlpha, Denominator: Cardinal): Cardinal; // @addr $831F78 @ida "unsigned int __userpurge $name@<eax>(unsigned int DestColor@<eax>, unsigned int DestAlpha@<edx>, unsigned int SourceColor@<ecx>, unsigned int SourceAlpha@<^4>, unsigned int Denominator@<^0>, void *ParentFrame@<^8>);" @stackpop 8 @calls "0x0083216B 0x0083219A 0x008321CA"
    begin
...
{ @routine $8327F0 TGraphBufGR_ClearPixels }
...
{ @routine $832820 TGraphBufGR_FillPixels }
...
{ @routine $832858 TGraphBufGR_FillPixels16 }
...
{ @routine $832898 TGraphBufGR_FillRect32 }
...
{ @routine $832928 TGraphBufGR_ScaleAlpha }
...
{ @routine $8329D4 TGraphBufGR_FlipHorizontal16 }
...
{ @routine $832A8C TGraphBufGR_RotateLeft16 }
...
{ @routine $832BFC TGraphBufGR_Stretch16 }
...
{ @routine $832CAC TGraphBufGR_ConvertRgbTo565 }
...
{ @routine $832DEC TGraphBufGR_Convert565ToRgb }
...
{ @routine $832EC8 TGraphBufGR_ShiftLight16 }
...
{ @routine $832F38 TGraphBufGR_DrawCircle16 }
...
{ @routine $832FD8 TGraphBufGR_DrawCircle8 }
...
{ @routine $833074 TGraphBufGR_ApplyOperations }
...
{ @routine $833234 TGraphBufGR_RescaleRgb }
...
{ @routine $833318 TGraphBufGR_RescaleRgba }
...
{ @routine $833400 TGraphBufGR_RescaleBilinearRgba }
...
{ @routine $833704 TGraphBufGR_FillPolygon32 }
...
{ @routine $833A2C TGraphBufGR_GetPixelCentroid }
...
{ @routine $833AF4 TGraphBufGR_CopyRect32 }
...
{ @routine $833BCC TGraphBufGR_BlendRect32 }
...
{ @routine $833D54 TGraphBufGR_MakeShadow }
...
{ @routine $833DC4 TGraphBufGR_SaveToBuffer }
...
{ @routine $833E50 TGraphBufGR_LoadFromBuffer }
...
{ @routine $833F38 TGraphBufGR_SavePng }
...
{ @routine $833FCC TGraphBufGR_SaveBmp }
...
{ @routine $834070 TGraphBufGR_SaveJpeg }
...
{ @routine $8342E4 TGraphBufGR_DrawNinePatch }
procedure TGraphBufGR.DrawNinePatch(X, Y, Width, Height: Integer; Source: TGraphBufGR; SourceRect, Borders: TRect);
var TileRect: TRect;

  // @nested $8341BC TilePatch
  procedure TilePatch(X, Y, Width, Height: Integer; Rect: TRect); // @addr $8341BC @ida "void __userpurge $name(int X@<eax>, int Y@<edx>, int Width@<ecx>, int Height@<^4>, TRect *Rect@<^0>, void *ParentFrame@<^8>);" @stackpop 8 @calls "0x834491 0x8344D7 0x834517 0x83455D 0x8345A9"
...
{ @routine $8345B8 TGraphBufGR_RescaleWithAspect }
...
{ @routine $83491C TGraphBufGR_RescaleRGBA_HW }
...
{ @routine $834ED4 TGraphBufGR_Crop }
...
{ @routine $83511C TGraphBufGR_AdjustBrightness }
...
{ @routine $83534C TGraphBufGR_ConvertToGrayscale }
...
{ @routine $835640 TGraphBufGR_GetTexture }
function TGraphBufGR.GetTexture: IDirect3DTexture9;
var Locked: TD3DLockedRect; Y: Cardinal; RowBytes: Integer;

  // @nested $835580 CopyRgbToOpaqueRgba
  procedure CopyRgbToOpaqueRgba(Dest: Pointer; DestPitch: Integer; Source: Pointer; SourcePitch, Width, Height: Integer); // @addr $835580 @ida "void __userpurge $name(void *Dest@<eax>, int DestPitch@<edx>, void *Source@<ecx>, int SourcePitch@<^8>, int Width@<^4>, int Height@<^0>, void *ParentFrame@<^12>);" @stackpop 12 @calls "0x8356EA"
  var X, Y: Integer;
  begin
    Y := 0;
    while Y < Height do
...
{ @routine $83587C TGraphBufGR_LoadFromScreen }
...
var
  ...
  // @nested $83582C SetRowAlpha
  procedure SetRowAlpha(Pixels: Pointer; Count, Alpha: Integer); // @addr $83582C @ida "void __usercall $name(void *Pixels@<eax>, int Count@<edx>, int Alpha@<ecx>, void *ParentFrame@<^0>);"
  var X: Integer;
  begin
    Alpha := Alpha and $FF;
...
{ @routine $835E14 TGraphBufGR_LockTexture }
...
{ @routine $835EB8 TGraphBufGR_UnlockTexture }
...
{ @routine $835F04 TGraphBufGR_ConvertBgraToRgb24 }
...
{ @routine $836110 TGraphBufGR_DrawAntialiasedCircle16 }
...
var X, Y, SignX, SignY, Quadrant: Integer; PreviousCoverage, Coverage: Single; PreviousX: Integer;
  ...
  // @nested $8360B4 PlotCirclePixel16
  procedure PlotCirclePixel16(X, Y, Alpha: Integer); // @addr $8360B4 @ida "void __usercall $name(int X@<eax>, int Y@<edx>, int Alpha@<ecx>, void *ParentFrame@<^0>);" @calls "0x83619b 0x8361bb 0x8362b3 0x8362e4 0x836319 0x836345"
  begin
...
{ @routine $836410 TGraphBufGR_DrawAntialiasedLine16 }
...
var
  ...
  // @nested $836380 LineFraction16
  function LineFraction16(Value: Double): Double; // @addr $836380 @ida "double __userpurge $name@<st0>(double Value@<^0>, void *ParentFrame@<^8>);" @stackpop 8 @calls "0x8366a6 0x8366ea 0x836704 0x836802 0x83684b 0x836862 0x83694f 0x836969"
  begin
    Result := Value - Floor(Value);
  end;

  // @nested $8363AC PlotLinePixel16
  procedure PlotLinePixel16(X, Y, Alpha: Integer); // @addr $8363AC @ida "void __usercall $name(int X@<eax>, int Y@<edx>, int Alpha@<ecx>, void *ParentFrame@<^0>);" @calls "0x836732 0x836754 0x836777 0x836799 0x836899 0x8368c7 0x8368f8 0x836928 0x836997 0x8369b9 0x8369dc 0x8369fe"
  begin
