unit GR_gi;
// Unit bracket (inferred): .text 0x004761FC..0x00478090; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create; // @addr 0x476250 @ida "TgiGR *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x476294 @ida "void __usercall $name(TgiGR *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure ClearData; // @addr 0x4762D0 @note "Borrowed data is detached without freeing it."
    function IsEmpty: Boolean; // @addr 0x476318
    procedure LoadRawGiBytes(BufferPtr: Pointer; ByteCount: Integer); // @addr 0x476334 @note "BufferPtr is borrowed; its header and length are not validated."
    procedure LoadRawGiFromBuffer(SourceBuffer: TBufEC); // @addr 0x476374 @note "Owns its copy; ignores SourceBuffer.Position."
    procedure LoadCompressedGiBytes(BufferPtr: Pointer; ByteCount: Integer); // @addr 0x4763D4 @note "Owns decompressed storage; empty on decompression failure."
    function GetBoundsRect: TRect; // @addr 0x47647C @ida "void __usercall $name(TgiGR *Self@<eax>, TRect *Result@<edx>);"
    function GetContentSize: TPoint; // @addr 0x4764A4 @ida "void __usercall $name(TgiGR *Self@<eax>, TPoint *Result@<edx>);"
    function GetTopLeft: TPoint; // @addr 0x4764E4 @ida "void __usercall $name(TgiGR *Self@<eax>, TPoint *Result@<edx>);"
    function GetFormat: Integer; // @addr 0x476514
    function GetPlane(PlaneIndex: Integer): PgiPlaneGR; // @addr 0x476530 @note "Does not validate PlaneIndex."
    function GetClipRectCount: Integer; // @addr 0x476558
    function GetClipRect(RectIndex: Integer): TRect; // @addr 0x476574 @ida "void __usercall $name(TgiGR *Self@<eax>, int RectIndex@<edx>, TRect *Result@<ecx>);" @note "Does not validate RectIndex."
    procedure BuildPalettedFormat4ColorCache; // @addr 0x4765D4 @note "Modifies Data even when borrowed."
    procedure DrawToGraphBuf(GraphBuf: TGraphBufGR; X, Y: Integer; DrawRect: TRect; BlendMode, Alpha: Byte); // @addr 0x4766D0 @ida "void __userpurge $name(TgiGR *Self@<eax>, TGraphBufGR *GraphBuf@<edx>, int X@<ecx>, int Y@<^12>, TRect *DrawRect@<^8>, unsigned __int8 BlendMode@<^4>, unsigned __int8 Alpha@<^0>);"
    procedure DecodeToGraphBuf(GraphBuf: TGraphBufGR; Keep16BitPixels: Boolean); // @addr 0x476D20 @note "Formats 0..4 create a 32-bit destination; formats 5/6 decode into an existing sufficiently large buffer. Keep16BitPixels affects format 0 without an alpha mask."
    procedure DecodeRawRegion(Destination: Pointer; PitchBytes, SourceX, SourceY, Width, Height: Integer; Keep16BitPixels: Boolean); // @addr 0x477480 @note "Only format 0 is supported; other formats report an error."
    procedure DecodeToPixels(Destination: Pointer; PitchBytes, Width, Height: Integer; Keep16BitPixels: Boolean); // @addr 0x4776E0
    procedure CreateFromGraphBuf(GraphBuf: TGraphBufGR; StorageMode: Integer); // @addr 0x477BF8 @note "Mode 1 uses RGB565; mode 0 uses ARGB masks. Native allocation reserves two bytes per pixel except for mode 2, whose payload is left uninitialized."
    procedure CreateFormat2FromGraphBuf(GraphBuf: TGraphBufGR; TopLeft: TPoint); // @addr 0x477DEC @ida "void __usercall $name(TgiGR *Self@<eax>, TGraphBufGR *GraphBuf@<edx>, TPoint *TopLeft@<ecx>);" @note "Uses RGB565 masks."
  end;

procedure PrepareRawGiColorCache(Data: Pointer); // @addr $478094

implementation

uses EC_Mem, EC_OKGF, Windows, GR_Main, GlobalsV, EC_Str;

{ @routine $476250 TgiGR_Create }
...
{ @routine $4762D0 TgiGR_ClearData }
...
{ @routine $476334 TgiGR_LoadRawGiBytes }
...
{ @routine $476374 TgiGR_LoadRawGiFromBuffer }
...
{ @routine $4763D4 TgiGR_LoadCompressedGiBytes }
...
{ @routine $47647C TgiGR_GetBoundsRect }
...
{ @routine $4764A4 TgiGR_GetContentSize }
...
{ @routine $4764E4 TgiGR_GetTopLeft }
...
{ @routine $476514 TgiGR_GetFormat }
...
{ @routine $476558 TgiGR_GetClipRectCount }
...
{ @routine $476574 TgiGR_GetClipRect }
...
{ @routine $4765D4 TgiGR_BuildPalettedFormat4ColorCache }
...
{ @routine $4766D0 TgiGR_DrawToGraphBuf }
...
{ @routine $476D20 TgiGR_DecodeToGraphBuf }
...
{ @routine $477480 TgiGR_DecodeRawRegion }
...
{ @routine $4776E0 TgiGR_DecodeToPixels }
...
{ @routine $477BF8 TgiGR_CreateFromGraphBuf }
procedure TgiGR.CreateFromGraphBuf(GraphBuf: TGraphBufGR; StorageMode: Integer);
var Plane: PgiPlaneGR; ByteCount: Cardinal;

  // @nested $477BB0 SwapGiSourceRedBlue
  procedure SwapGiSourceRedBlue; // @addr $477BB0 @ida "void __usercall $name(void *ParentFrame@<^0>);" @stackpop 0 @calls "0x00477D6A 0x00477DE0"
...
{ @routine $477DEC TgiGR_CreateFormat2FromGraphBuf }
...
{ @routine $478094 PrepareRawGiColorCache }
