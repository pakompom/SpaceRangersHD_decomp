unit fEquipmentShop;
// Unit bracket (inferred): .text 0x006AA500..0x006B17CE; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    procedure SaveToBuffer(Buffer: TBufEC); // @addr $6AADB0
    procedure LoadFromBuffer(Buffer: TBufEC; Galaxy: TGalaxy); // @addr $6AAE44
    // Owns Item while the location's shop list is detached, and frees remaining controls.
    constructor Create; // @addr 0x6AAC74 @ida "TShopSlot *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x6AACB8 @ida "void __usercall $name(TShopSlot *Self@<eax>, __int8 DestroyFlags@<dl>);"
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x6AAEC0 @ida "TfEquipmentShop *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x6AAF40 @ida "void __usercall $name(TfEquipmentShop *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure OnOpen; override; // @addr 0x6AB5C4
    procedure OnClose; override; // @addr 0x6ABD6C
    procedure SelectMusic; override; // @addr 0x6B15C4
    procedure ProcessMouseWheel(KeyState: Cardinal; Point: TPoint; Delta: Integer); override; // @addr 0x6B1310 @ida "void __userpurge $name(TfEquipmentShop *Self@<eax>, unsigned int KeyState@<edx>, TPoint *Point@<ecx>, int Delta@<^0>);"
    procedure InitializeLayout; override; // @addr 0x6AAFE0
    procedure ExecuteUiCode(Block: TBlockParEC; Key: Cardinal); override; // @addr 0x6B1550
    procedure BuildGoodsControls; // @addr 0x6ABF68
    procedure ClearGoodsControls; // @addr 0x6ACEBC
    procedure UpdateScrollButtons; // @addr 0x6AD434
    procedure ScrollLeft(Sender: TObjectGI); // @addr 0x6AD8C4
    procedure ScrollRight(Sender: TObjectGI); // @addr 0x6AD9B0
    procedure ItemMouseEnter(Sender: TObjectGI); // @addr 0x6AD5FC
    procedure ItemMouseLeave(Sender: TObjectGI); // @addr 0x6AD7A8
    procedure RefreshItemInfo(Item: TItem); // @addr 0x6AEE28

    procedure EndTurnClicked(Sender: TObjectGI); // @addr $6AD02C
    procedure ShipClicked(Sender: TObjectGI); // @addr $6AD304
    procedure ScrollTick(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6ADAD0
    procedure ItemMouseUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6ADC60 @ida "void __userpurge $name(TfEquipmentShop *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);"
    procedure ChooseAnimatedPreview(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6AEBD0
    procedure PreviewCycleComplete(Sender: TObjectGI); // @addr $6AECD8
    procedure HideItemInfo(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6AECFC
    procedure BuildHullSlotOverlays(Parent: TObjectGI; Hull: THull; OffsetX, OffsetY: Integer); // @addr $6AFCA8
    procedure RefreshHullInfo(Target: TMessageLoopGI; Hull: THull; Text: WideString; SuppressImage: Boolean); // @addr $6B00CC
    procedure MainPanelKeyDown(Sender: TObjectGI; Key: Cardinal); // @addr $6B13C8

    procedure StartSlotAnimatedPreview(Slot: TShopSlot); // @addr 0x6AD544
    procedure ScheduleSlotPreviewStop(Slot: TShopSlot); // @addr 0x6AD5C8 @note "Playback continues until the current animation cycle ends."
    procedure PanelScrollChanged(Sender: TObjectGI); // @addr 0x6ADC0C
  end;

procedure TemporaryShopStockHook(Argument: Pointer); // @addr $6AA5F4 Native no-op. Sole caller passes nil; original parameter meaning is unresolved.
procedure BuildTemporaryShopSlotGrid; // @addr 0x6AA600 @note "Transfers ownership of market inventory into the temporary slots."
procedure RestoreTemporaryShopStock; // @addr 0x6AA9BC @note "Returns ownership of remaining items to the original market."
procedure ClearTemporaryShopSlotGrid; // @addr 0x6AAAAC @note "Frees remaining items without returning them to the market."
function FindShopSlotByGridPoint(Point: TPoint): TShopSlot; // @addr 0x6AAB28 @ida "TShopSlot *__usercall $name@<eax>(TPoint *Point@<eax>);"
function FindShopSlotByItem(Item: TItem): TShopSlot; // @addr 0x6AAB9C
function GetShopItemIconName(Item: TItem): WideString; // @addr 0x6AABF8 @ida "void __usercall $name(TItem *Item@<eax>, unsigned __int16 **Result@<edx>);"

var
  ShopGridRowCount: Integer = 4; // @addr 0x87B878
  ShopVisibleColumnCount: Integer = 6; // @addr 0x87B87C
  TemporaryShopSlots: TList = nil; // @addr 0x87B880
  TemporaryShopPlanet: TPlanet = nil; // @addr 0x87B884
  TemporaryShopStation: TRuins = nil; // @addr 0x87B888
...
{ @routine $6AA5F4 TemporaryShopStockHook }
procedure TemporaryShopStockHook(Argument: Pointer);
begin
end;
{ @end $6AA5F4 }

{ @routine $6AA600 BuildTemporaryShopSlotGrid }
...
{ @routine $6AA9BC RestoreTemporaryShopStock }
...
{ @routine $6AAAAC ClearTemporaryShopSlotGrid }
...
{ @routine $6AAB28 FindShopSlotByGridPoint }
...
{ @routine $6AAB9C FindShopSlotByItem }
...
{ @routine $6AABF8 GetShopItemIconName }
...
{ @routine $6AAC74 TShopSlot_Create }
...
{ @routine $6AACB8 TShopSlot_Destroy }
...
{ @routine $6AADB0 TShopSlot_SaveToBuffer }
...
{ @routine $6AAE44 TShopSlot_LoadFromBuffer }
...
{ @routine $6AAEC0 TfEquipmentShop_Create }
...
{ @routine $6AAF40 TfEquipmentShop_Destroy }
...
{ @routine $6AAFE0 TfEquipmentShop_InitializeLayout }
...
{ @routine $6AB5C4 TfEquipmentShop_OnOpen }
...
{ @routine $6ABD6C TfEquipmentShop_OnClose }
...
{ @routine $6ABF68 TfEquipmentShop_BuildGoodsControls }
...
{ @routine $6ACEBC TfEquipmentShop_ClearGoodsControls }
...
{ @routine $6AD02C TfEquipmentShop_EndTurnClicked }
...
{ @routine $6AD304 TfEquipmentShop_ShipClicked }
...
{ @routine $6AD434 TfEquipmentShop_UpdateScrollButtons }
...
{ @routine $6AD544 TfEquipmentShop_StartSlotAnimatedPreview }
...
{ @routine $6AD5C8 TfEquipmentShop_ScheduleSlotPreviewStop }
...
{ @routine $6AD5FC TfEquipmentShop_ItemMouseEnter }
...
{ @routine $6AD7A8 TfEquipmentShop_ItemMouseLeave }
...
{ @routine $6AD8C4 TfEquipmentShop_ScrollLeft }
...
{ @routine $6AD9B0 TfEquipmentShop_ScrollRight }
...
{ @routine $6ADAD0 TfEquipmentShop_ScrollTick }
...
{ @routine $6ADC0C TfEquipmentShop_PanelScrollChanged }
...
{ @routine $6ADC60 TfEquipmentShop_ItemMouseUp }
...
{ @routine $6AEBD0 TfEquipmentShop_ChooseAnimatedPreview }
...
{ @routine $6AECD8 TfEquipmentShop_PreviewCycleComplete }
...
{ @routine $6AECFC TfEquipmentShop_HideItemInfo }
...
{ @routine $6AEE28 TfEquipmentShop_RefreshItemInfo }
...
{ @routine $6AFCA8 TfEquipmentShop_BuildHullSlotOverlays }
procedure TfEquipmentShop.BuildHullSlotOverlays(Parent: TObjectGI; Hull: THull; OffsetX, OffsetY: Integer);
var
  Root: TObjectGI;
  I: Integer;

  // @nested $6AFB6C AddOverlay
  procedure AddOverlay(Name: WideString); // @addr $6AFB6C @ida "void __usercall $name(unsigned __int16 *Name@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6afcf1,0x6afd0a,0x6afd25,0x6afd40,0x6afd5b,0x6afd76,0x6afdc0,0x6afde1,0x6afdfa,0x6afe13,0x6afe2c,0x6afe45,0x6afe5e,0x6afe77" @note "Nested in BuildHullSlotOverlays; captures root, parent and offsets."
  var
    Control: TObjectGI;
    Path: WideString;
    Graph: TGraphBufGI;
...
var
  // Native managed-string initialization pairs at $6B1888, $6B1880 and $6B1878.
  ShopDominatorImagePrefixes: array[0..2] of WideString = ('B', 'K', 'T'); // @addr $87B88C

{ @routine $6B00CC TfEquipmentShop_RefreshHullInfo }
...
{ @routine $6B1310 TfEquipmentShop_ProcessMouseWheel }
...
{ @routine $6B13C8 TfEquipmentShop_MainPanelKeyDown }
...
{ @routine $6B1550 TfEquipmentShop_ExecuteUiCode }
...
{ @routine $6B15C4 TfEquipmentShop_SelectMusic }
