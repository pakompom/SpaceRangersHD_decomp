unit fShip2;
// Unit bracket (inferred): .text 0x006D34E0..0x006F720D; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    function CanUseLocalStorage: Boolean; // @addr $6D8F0C Native Self/result stack ordering establishes this as a method.
    function GetLocalStorageOwner: TObject; // @addr $6D8F6C Borrows the current planet or docked ship.
    function SlotToTip(SlotName: WideString): Byte; // @addr 0x6DB4B8 @note "Matches the second underscore-delimited component against eight equipment slot names; raises on no match."
    ...
    constructor Create; // @addr 0x6D4780 @ida "TfShip2 *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x6D4828 @ida "void __usercall $name(TfShip2 *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure ProcessWindowMessage(Message, WParam: Cardinal; LParam: Integer); override; // @addr 0x6F4C98
    procedure OnOpen; override; // @addr 0x6D6574
    procedure OnClose; override; // @addr 0x6D8A28
    procedure ProcessCallbackTimers; override; // @addr 0x6F4C58
    procedure SelectMusic; override; // @addr 0x6F4CF0
    procedure AdvanceScriptVideo(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6EA98C
    function StopScriptVideo: Boolean; // @addr $6EAA44 Returns whether a video timer was active.
    procedure ProcessMouseWheel(KeyState: Cardinal; Point: TPoint; Delta: Integer); override; // @addr 0x6E5988 @ida "void __userpurge $name(TfShip2 *Self@<eax>, unsigned int KeyState@<edx>, TPoint *Point@<ecx>, int Delta@<^0>);"
    procedure InitializeLayout; override; // @addr 0x6D4868
    procedure UpdateActionCursor(CanTake: Boolean); override; // @addr 0x6DFFB8
    function GetActionParentLoop: TMessageLoopGI; override; // @addr 0x6F716C
    procedure ExecuteUiCode(Block: TBlockParEC; Key: Cardinal); override; // @addr 0x6F7188
    procedure LayoutItemInfo(Window: TWindowGI; Title, Text: TLabelGI; KeepMinimumHeight, WordWrap: Boolean; MinimumWidth: Integer); // @addr $6EDEC0
    procedure LayoutObjectInfo(Window: TWindowGI; Title: TLabelGI; Left1, Right1, Left2, Right2, Left3, Right3, Left4, Right4, Left5, Right5, Left6, Right6, Left7, Right7, Left8, Right8: TLabelGI; Emblem: TObjectGI; KeepMinimumHeight: Boolean; MinimumWidth: Integer); // @addr $6EE380 Two-column planet/ship statistics, shared with live and recorded star-map panels.
    procedure CloseClicked(Sender: TObjectGI); // @addr $6D9CEC Checks integrity outside inspection mode, restores the return screen, closes and raises BreakUiMessage.
    procedure EnterBridgeClicked(Sender: TObjectGI); // @addr $6F7118 Verified callback assignment in InitializeLayout.
    procedure HoldLeftReleased(Sender: TObjectGI); // @addr $6E47E8 Verified callback assignment in InitializeLayout.
    procedure HoldLeftPressed(Sender: TObjectGI); // @addr $6E4778 Verified callback assignment in InitializeLayout.
    procedure HoldRightReleased(Sender: TObjectGI); // @addr $6E4890 Verified callback assignment in InitializeLayout.
    procedure HoldRightPressed(Sender: TObjectGI); // @addr $6E4820 Verified callback assignment in InitializeLayout.
    function ConfigureChameleon: Boolean; // @addr $6E452C
    procedure MainKeyDown(Sender: TObjectGI; Key: Cardinal); // @addr $6E5254
    procedure MainKeyUp(Sender: TObjectGI; Key: Cardinal); // @addr $6E5900 Verified callback assignment in InitializeLayout.
    procedure MainLeftButtonUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6E5C80 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);" Verified callback assignment in InitializeLayout.
    procedure MainRightButtonDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6E65C8 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);" Verified callback assignment in InitializeLayout.
    procedure GateMouseEnter(Sender: TObjectGI); // @addr $6F0E40 Verified callback assignment in InitializeLayout.
    procedure GateMouseLeave(Sender: TObjectGI); // @addr $6F0E9C Verified callback assignment in InitializeLayout.
    procedure GateMouseUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6F1108 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);" Verified callback assignment in InitializeLayout.
    procedure UseMouseEnter(Sender: TObjectGI); // @addr $6F113C Verified callback assignment in InitializeLayout.
    procedure UseMouseLeave(Sender: TObjectGI); // @addr $6F116C Verified callback assignment in InitializeLayout.
    procedure DropSelectedInArcade; // @addr $6E898C Transfers the selected cargo to arcade space, or destroys it in arcade view mode.
    procedure UseMouseUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6EAB14 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);" Verified callback assignment in InitializeLayout.
    procedure StorageUpClicked(Sender: TObjectGI); // @addr $6F1EA0 Verified callback assignment in InitializeLayout.
    procedure StorageDownClicked(Sender: TObjectGI); // @addr $6F1DFC Verified callback assignment in InitializeLayout.
    procedure ToggleRemoteHoldClicked(Sender: TObjectGI); // @addr $6F50E8 Verified callback assignment in InitializeLayout.
    procedure RemoteHoldItemMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6E1834 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);" Verified callback assignment in InitializeLayout.
    procedure SortRemoteHoldClicked(Sender: TObjectGI); // @addr $6F55C4 Verified callback assignment in InitializeLayout.
    procedure RemoteHoldUpPressed(Sender: TObjectGI); // @addr $6F52A8 Verified callback assignment in InitializeLayout.
    procedure RemoteHoldUpReleased(Sender: TObjectGI); // @addr $6F5340 Verified callback assignment in InitializeLayout.
    procedure RemoteHoldDownPressed(Sender: TObjectGI); // @addr $6F5378 Verified callback assignment in InitializeLayout.
    procedure RemoteHoldDownReleased(Sender: TObjectGI); // @addr $6F5420 Verified callback assignment in InitializeLayout.
    procedure StorageToShipClicked(Sender: TObjectGI); // @addr $6F56CC Verified callback assignment in InitializeLayout.
    procedure SellStorageClicked(Sender: TObjectGI); // @addr $6F5790
    procedure ShipToStorageClicked(Sender: TObjectGI); // @addr $6F59F4 Verified callback assignment in InitializeLayout.
    procedure SortStorageByTypeClicked(Sender: TObjectGI); // @addr $6F5AB4 Verified callback assignment in InitializeLayout.
    procedure SortStorageBySizeClicked(Sender: TObjectGI); // @addr $6F5AD8 Verified callback assignment in InitializeLayout.
    procedure SortStorageByPriceClicked(Sender: TObjectGI); // @addr $6F5B00 Verified callback assignment in InitializeLayout.
    procedure LoadHoldRocketsClicked(Sender: TObjectGI); // @addr $6F5B28 Verified callback assignment in InitializeLayout.
    procedure SellHoldClicked(Sender: TObjectGI); // @addr $6F5B50 Verified callback assignment in InitializeLayout.
    procedure LoadEquippedRocketsClicked(Sender: TObjectGI); // @addr $6F5E88 Verified callback assignment in InitializeLayout.
    procedure EquipmentConfigurationClicked(Sender: TObjectGI); // @addr $6E522C Verified callback assignment in InitializeLayout.
    procedure RefreshShipView; // @addr $6DBBAC
    procedure ReturnSelectedHoldEntry; // @addr $6E49B4
    procedure ScrollHoldTimer(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6E48C8
    procedure ScrollRemoteHoldTimer(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F5458
    function GetRemoteHoldScrollLimit: Integer; // @addr $6F551C
    procedure SlideStorageTimer(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F1F44
    procedure OpenGate; // @addr $6F0EB4
    procedure CloseGate; // @addr $6F0F44
    procedure OpenUsePanel; // @addr $6F1184
    procedure CloseUsePanel; // @addr $6F1210
    procedure SlideGateTimer(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F0F9C
    procedure SlideUseTimer(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F1268
    procedure SortStorage(Sort: TPlayerHoldSort); // @addr $6F61A8
    procedure RefreshStorageView; // @addr $6F2094
    procedure LoadRockets(Equipped: Boolean); // @addr $6EC84C
    procedure SelectEquipmentConfiguration(Index: Integer); // @addr $6E5074
    procedure ScrollStorageUp(Sender: TObjectGI); // @addr $6F2368
    procedure ScrollStorageDown(Sender: TObjectGI); // @addr $6F23B0
    function IsHoldNormalShip: Boolean; // @addr $6F5048
    function CanUsePlayerExperience: Boolean; // @addr $6F506C Ruins with modernization sponsorship or a player-owned tranclucator.
    procedure StorageItemMouseUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6F2440 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);"
    procedure RefreshRewards(Ship: TNormalShip); // @addr $6D8FB8
    procedure RewardsMouseMove(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6D9550 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);"
    procedure RewardsMouseLeave(Sender: TObjectGI); // @addr $6D96F4
    procedure ShowRewardTooltip(Ship: TNormalShip; Award: Integer); // @addr $6D970C
    procedure HideRewardTooltip; // @addr $6D9B70
    procedure SlideRightPanelTimer(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6D9B98
    procedure RewardsMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6D9D50 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);"
    procedure UpdateInfoHint(First, Second: Integer); // @addr $6DB4A4 Native empty three-register method; argument purposes unresolved.
    RightPanelSlideStep: Integer; // @offset $370
    UsePanelSlideOffset: Integer; // @offset $3F8
    SelectedReward: Integer; // @offset $3C0
    procedure ShipNameMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6D9E70 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);"
    procedure RefreshEquipmentConfigurationButtons; // @addr $6E4F58
    procedure SlideUsePanelTimer(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F148C
    procedure SpecialSlot1Clicked(Sender: TObjectGI); // @addr $6E900C
    procedure OpenSpecialSlot1; // @addr $6F15A4
    procedure CloseSpecialSlot1; // @addr $6F167C
    procedure AnimateSpecialSlot1(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F171C
    procedure SpecialSlot2Clicked(Sender: TObjectGI); // @addr $6E9AFC
    procedure OpenSpecialSlot2; // @addr $6F186C
    procedure CloseSpecialSlot2; // @addr $6F1944
    procedure AnimateSpecialSlot2(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F19E4
    procedure SpecialSlot3Clicked(Sender: TObjectGI); // @addr $6EB0C0
    procedure OpenSpecialSlot3; // @addr $6F1B34
    procedure CloseSpecialSlot3; // @addr $6F1C0C
    procedure AnimateSpecialSlot3(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F1CAC
    ...
    function IsCompatibleSlot(ItemType, SlotType: TItemType): Boolean; // @addr $6DB588
    procedure RefreshActionPanels(Kind: TPlayerHoldKind; Good: Byte; Quantity, Cost: Integer; Item: TItem; Origin: Integer); // @addr $6DF02C @ida "void __userpurge $name(TfShip2 *Self@<eax>, TPlayerHoldKind Kind@<edx>, unsigned __int8 Good@<cl>, int Quantity@<^12>, int Cost@<^8>, TItem *Item@<^4>, int Origin@<^0>);"
    ...
    procedure RefreshEquipmentSlotControls; // @addr $6DB5C4
    procedure ShowShipPropertyInfo(Sender: TObjectGI); // @addr $6D9FC4
    procedure TrainSkillClicked(Sender: TObjectGI); // @addr $6E01A0
    procedure RepairAllClicked(Sender: TObjectGI); // @addr $6EBD50
    HighlightRepairableEquipment: Boolean; // @offset $490
    procedure RepairAllMouseEnter(Sender: TObjectGI); // @addr $6EC7B8
    procedure RepairAllMouseLeave(Sender: TObjectGI); // @addr $6EC828
    DisplayedItemKey: Integer; // @offset $3B8 Zero for no item, Good+1 for hold goods, or an item pointer for equipment/storage.
    procedure HideItemInfo(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6EDDEC
    MoneyWarningVisible: Boolean; // @offset $3C4
    MoneyWarningTicks: Integer; // @offset $3C8
    procedure HideShipPropertyInfo(Sender: TObjectGI); // @addr $6DB46C
    procedure ToggleAfterburner(Sender: TObjectGI); // @addr $6ED224
    procedure ShowItemInfo; // @addr $6ED3D4
    procedure ShowItemInfoTimer(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6EDDC4
    procedure RefreshMoneyWarning; // @addr $6F0AF4
    procedure StartMoneyWarning; // @addr $6F0C44
    procedure AdvanceMoneyWarning(Timer: PCallbackTimerGI; UserData: Integer); // @addr $6F0CBC
    procedure ShowNoDropMessage(Code: Integer); // @addr $6F0D20
    procedure DropSelectedOutside(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6E77E8 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);"
    procedure OpenUseSidePanel; // @addr $6F13D0
    procedure CloseUseSidePanel; // @addr $6F1434
    procedure HideSender(Sender: TObjectGI); // @addr $6F4C3C
    procedure RefreshLoadEquippedRocketsButton; // @addr $6F6378
    function TryDeployTranclucator(Ship: TShip): Boolean; // @addr $6E76B4
    function CreateShipInfoImage(Sender: TLabelGI; Item: PFontObjectEC): TObjectGI; // @addr $6F30C4
    procedure SellAllItems(Origin: Integer); // @addr $6F6818 Zero sells hold contents; one sells local storage.
    procedure TakeHoldEntry(Entry: TPlayerHoldUnit; Slot: Integer; UsesDisplayOrder: Boolean); // @addr $6E3A44
    procedure HullMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6E3C4C @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0.4>);"
    procedure EquipmentSlotMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6E0308 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0.4>);"
    procedure ArtefactSlotMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6E114C @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);"
    procedure UseOnArtefactSlot(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint); // @addr $6E1464 @ida "void __userpurge $name(TfShip2 *Self@<eax>, TObjectGI *Sender@<edx>, unsigned int KeyState@<ecx>, TPoint *Point@<^0>);"
    ...
    procedure ShowEquipmentInfo(Item: TItem; FromStorage: Boolean); // @addr $6EE69C
    procedure ShowHoldGoodsInfo(Good: Byte); // @addr $6EFB7C
    procedure ShowStoredGoodsInfo(Goods: TGoods); // @addr $6F044C
    procedure BuildAdditionalInfoPanel; // @addr 0x6F35F0
  end;

function RunShipEquipment(ParentLoop: TMessageLoopGI): Boolean; // @addr $6F55EC

function CompareStoredItems(Left, Right: PStorageEntry; Sort: TPlayerHoldSort): Integer; // @addr $6F5F0C

function RankToImage(Rank: Byte): WideString; // @addr $6D362C @ida "void __usercall $name(unsigned __int8 Rank@<al>, unsigned __int16 **Result@<edx>);"

function RankToImageSmall(Rank: Byte): WideString; // @addr $6D3754 @ida "void __usercall $name(unsigned __int8 Rank@<al>, unsigned __int16 **Result@<edx>);"

function PirateRankToImage(Rank: Byte): WideString; // @addr $6D3888 @ida "void __usercall $name(unsigned __int8 Rank@<al>, unsigned __int16 **Result@<edx>);"

function PirateRankToImageSmall(Rank: Byte): WideString; // @addr $6D39CC @ida "void __usercall $name(unsigned __int8 Rank@<al>, unsigned __int16 **Result@<edx>);"

procedure InitializePlayerHoldView; // @addr 0x6D3B34
procedure FinalizePlayerHoldView; // @addr 0x6D3B4C
procedure RefreshPlayerHoldView(IncludeFilteredItems: Boolean); // @addr 0x6D3B6C @note "Uses PlayerHoldShip, or the player when nil; preserves display order."
procedure RemoveEmptyPlayerHoldSlot(Index: Integer); // @addr 0x6D4054
procedure ClearPlayerHoldEntries; // @addr 0x6D40A4
procedure RemoveEmptyPlayerHoldSlots; // @addr 0x6D40F8
function IsPlayerHoldSlotEmpty(Index: Integer): Boolean; // @addr 0x6D4154 @note "Returns true for out-of-range indices as well as empty slots."
function ComparePlayerHoldEntries(Left, Right: TPlayerHoldUnit; Sort: TPlayerHoldSort): Integer; // @addr 0x6D4198 @note "Ascending order; goods prices are total purchase costs, not unit prices."
procedure SortPlayerHoldEntries(Sort: TPlayerHoldSort); // @addr 0x6D44AC
procedure RestorePlayerHoldDisplayOrder; // @addr 0x6D45DC
function FindPlayerHoldIndexByOrder(DisplayOrder: Integer): Integer; // @addr 0x6D46B4 @note "Returns -1 when absent."
function FindFreePlayerHoldOrder: Integer; // @addr 0x6D4718

var
  StorageImageCount: Integer = 21; // @addr $87BBD0
  PlayerHoldEntries: TList = nil; // @addr 0x87BBD4
  PlayerHoldShip: TShip = nil; // @addr 0x87BBD8

implementation

// @unit-initialization $876A08
// @unit-finalization $6F7210
...
var
  SelfSkillPointColor: Cardinal; // @addr $889CF0
  OtherSkillPointColor: Cardinal; // @addr $889CF4

{ @routine $6D362C RankToImage }
...
{ @routine $6D3754 RankToImageSmall }
...
{ @routine $6D3888 PirateRankToImage }
...
{ @routine $6D39CC PirateRankToImageSmall }
...
{ @routine $6D3B34 InitializePlayerHoldView }
...
{ @routine $6D3B4C FinalizePlayerHoldView }
...
{ @routine $6D3B6C RefreshPlayerHoldView }
...
{ @routine $6D4054 RemoveEmptyPlayerHoldSlot }
...
{ @routine $6D40A4 ClearPlayerHoldEntries }
...
{ @routine $6D40F8 RemoveEmptyPlayerHoldSlots }
...
{ @routine $6D4154 IsPlayerHoldSlotEmpty }
...
{ @routine $6D4198 ComparePlayerHoldEntries }
...
{ @routine $6D44AC SortPlayerHoldEntries }
...
{ @routine $6D45DC RestorePlayerHoldDisplayOrder }
...
{ @routine $6D46B4 FindPlayerHoldIndexByOrder }
...
{ @routine $6D4718 FindFreePlayerHoldOrder }
...
{ @routine $6D4828 TfShip2_Destroy }
...
{ @routine $6D4868 TfShip2_InitializeLayout }
...
{ @routine $6D8A28 TfShip2_OnClose }
...
{ @routine $6D8F0C TfShip2_CanUseLocalStorage }
...
{ @routine $6D8F6C TfShip2_GetLocalStorageOwner }
...
{ @routine $6D8FB8 TfShip2_RefreshRewards }
...
{ @routine $6D9550 TfShip2_RewardsMouseMove }
...
{ @routine $6D96F4 TfShip2_RewardsMouseLeave }
...
{ @routine $6D970C TfShip2_ShowRewardTooltip }
...
{ @routine $6D9B70 TfShip2_HideRewardTooltip }
...
{ @routine $6D9B98 TfShip2_SlideRightPanelTimer }
...
{ @routine $6D9CEC TfShip2_CloseClicked }
...
{ @routine $6D9D50 TfShip2_RewardsMouseDown }
...
{ @routine $6D9E70 TfShip2_ShipNameMouseDown }
...
{ @routine $6D9FC4 TfShip2_ShowShipPropertyInfo }
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ReplaceTextToken(CustomDescription,'<Data1>',IntToStr(Info.Data[1]),'<color=255,240,100>');
        ReplaceTextToken(CustomDescription,'<Data2>',IntToStr(Info.Data[2]),'<color=255,240,100>');
        ReplaceTextToken(CustomDescription,'<Data3>',IntToStr(Info.Data[3]),'<color=255,240,100>');
      ...
      begin
        ...
        begin
          if Ship.GetRankPointsToNextRank > 0 then
            Description := Description + ' ' + FormatText2(LocalizedText('Rank.NextRankText'),'<color=255,240,100>','<NextRank>',Ship.GetNextRankName,'<WarPoints>',IntToStr(Ship.GetRankPointsToNextRank))
          else
            Description := Description + ' ' + FormatText1(LocalizedText('Rank.NextRankGetText'),'<color=255,240,100>','<NextRank>',Ship.GetNextRankName);
        end;
      end
      ...
      begin
        ...
        begin
          if Ship.GetPirateRankPointsToNextRank > 0 then
            Description := Description + ' ' + FormatText2(LocalizedText('RankPirate.NextRankText'),'<color=255,240,100>','<NextRank>',Ship.GetNextPirateRankName,'<WarPoints>',IntToStr(Ship.GetPirateRankPointsToNextRank))
          else
            Description := Description + ' ' + FormatText1(LocalizedText('RankPirate.NextRankGetText'),'<color=255,240,100>','<NextRank>',Ship.GetNextPirateRankName);
        end;
      end;
    end
...
{ @routine $6DB46C TfShip2_HideShipPropertyInfo }
...
{ @routine $6DB4A4 TfShip2_UpdateInfoHint }
procedure TfShip2.UpdateInfoHint(First, Second: Integer);
begin
end;
{ @end $6DB4A4 }

{ @routine $6DB4B8 TfShip2_SlotToTip }
...
{ @routine $6DB588 TfShip2_IsCompatibleSlot }
...
{ @routine $6DB5C4 TfShip2_RefreshEquipmentSlotControls }
...
{ @routine $6DBBAC TfShip2_RefreshShipView }
...
begin
  ...
  if PlayerHoldShip.CalculateSpeed <= 0 then Text := '<color=255,0,0>' else Text := '';
  (GetByName('ISpeed') as TLabelGI).SetText(WrapTextInColor(IntToStr(PlayerHoldShip.CalculateSpeed),Text));
  if PlayerHoldShip.GetCargoFreeSpace < 0 then Text := '<color=255,0,0>' else Text := '';
  ...
  with GetByName('IDestrEnergy') as TLabelGI do SetText(FormatText2(LocalizedText('FormShip.DestrEnergy'),'<color=0,71,234>','<Value1>',IntToStr(PlayerHoldShip.GetHull.Energy),'<Value2>',IntToStr(PlayerHoldShip.GetHull.EnergyMax)));
  ...
  with GetByName('IDestrCount') as TLabelGI do SetText(FormatText2(LocalizedText('FormShip.DestrCount'),'<color=0,71,234>','<Count>',IntToStr(PlayerHoldShip.CountActiveInterceptorTargets),'<Cost>',IntToStr(PlayerHoldShip.GetInterceptorEnergyCost)));
  with GetByName('IDestrEnergyOut') as TLabelGI do SetText('-' + FormatText1(LocalizedText('FormShip.DestrPerDay'),'<color=0,71,234>','<Value>',IntToStr(PlayerHoldShip.CountActiveInterceptorTargets * 3)));
  with GetByName('IDestrEnergyIn') as TLabelGI do SetText('+' + FormatText1(LocalizedText('FormShip.DestrPerDay'),'<color=0,71,234>','<Value>',IntToStr(PlayerHoldShip.GetHullEnergyRegeneration)));
  RefreshLoadEquippedRocketsButton;
...
{ @routine $6DF02C TfShip2_RefreshActionPanels }
...
var I, TotalRepair: Integer; Equipment: TEquipment; OrdinaryShip: Boolean;
  ...
  // @nested $6DEBEC Skill
  procedure Skill(Index, BaseLevel, EffectiveLevel: Integer; CanTrain: Boolean); // @addr $6DEBEC @ida "void __userpurge $name(int Index@<eax>, int BaseLevel@<edx>, int EffectiveLevel@<ecx>, bool CanTrain@<^0>, void *ParentFrame@<^4>);" @stackpop 4 @calls "0x6DF126,0x6DF162,0x6DF19E,0x6DF1DD,0x6DF21C,0x6DF25B" Nested in RefreshActionPanels; captures Self.
...
{ @routine $6DFFB8 TfShip2_UpdateActionCursor }
...
{ @routine $6E01A0 TfShip2_TrainSkillClicked }
...
{ @routine $6E0308 TfShip2_EquipmentSlotMouseDown }
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        SoundManager.PlaySound('Sound.NoMoney');
        ShowMessageBoxGI(Self,FormatText1(LocalizedColorText('FormShip.RuinMoveItemInvalid'),'<color=255,240,100>','<Item>',RemoveTextTagsW(Item.GetDisplayName)),mbgCancel or mbgWarning);
        Exit;
      end;
      if (PlayerHoldShip is TTranclucator) and ((Item is TEngine) or (Item is TFuelTanks)) then
      begin
        SoundManager.PlaySound('Sound.NoMoney');
        ShowMessageBoxGI(Self,FormatText1(LocalizedColorText('FormShip.TrancMoveItemInvalid'),'<color=255,240,100>','<Item>',RemoveTextTagsW(Item.GetDisplayName)),mbgCancel or mbgWarning);
        Exit;
      end;
  ...
  begin
    ...
    begin
      Item := PlayerHoldShip.FindEquippedItemInSlot(ItemType,Slot);
      Text := FormatText2(LocalizedText('MicroModuls.AddToItem'),'<color=255,240,100>','<ModuleName>',(SelectedHoldItem as TMicroModule).GetPlainName,'<ItemName>',RemoveTextTagsW(Item.GetDisplayName));
...
{ @routine $6E114C TfShip2_ArtefactSlotMouseDown }
...
{ @routine $6E1464 TfShip2_UseOnArtefactSlot }
...
{ @routine $6E1834 TfShip2_RemoteHoldItemMouseDown }
...
var
  ...
  ActionResult: Integer;
  // @nested $6E1798 RefreshAfterTransfer
  procedure RefreshAfterTransfer; // @addr $6E1798 @ida "void __usercall $name(void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6E19CE,0x6E1FF6,0x6E272D,0x6E333E,0x6E3787,0x6E3790" Captures Self and NeedsRefresh.
  begin
    Galaxy.PrimeIntegrityChecksum1(445);
...
begin
    ...
    begin
      ...
      begin
        ...
        if ShowCountDialog(Self,'GI,' + GetItemTypeBitmapPath(TItemType(SelectedGoodsIndex)),
          FormatText1(LocalizedText('FormShip.FromStorageItem'),'<color=0,50,200>','<Name>',LowerCaseWideString(GetStackableItemTypeName(TItemType(SelectedGoodsIndex)))),
      ...
      begin
        if Count > 1 then
          if ShowCountDialog(Self,'GI,' + GetShopItemIconName(SelectedHoldItem) + 's',
            FormatText1(LocalizedText('FormShip.FromStorageItem'),'<color=0,50,200>','<Name>',LowerCaseWideString(GetStackableItemName(SelectedHoldItem))),
  ...
  begin
    ...
    begin
      ...
      begin
        Text := FormatText2(LocalizedText('MicroModuls.AddToItem'),'<color=255,240,100>','<ModuleName>',(SelectedHoldItem as TMicroModule).GetPlainName,'<ItemName>',RemoveTextTagsW(Entry.Item.GetDisplayName));
        if ShowMessageBoxGI(Self,Text,mbgOK or mbgCancel or mbgQuestion) = mbgResultOK then
        begin
...
{ @routine $6E3A44 TfShip2_TakeHoldEntry }
...
{ @routine $6E3C4C TfShip2_HullMouseDown }
...
begin
  ...
  begin
    Text := FormatText2(LocalizedText('MicroModuls.AddToItem'),'<color=255,240,100>','<ModuleName>',(SelectedHoldItem as TMicroModule).GetPlainName,'<ItemName>',RemoveTextTagsW(PlayerHoldShip.GetHull.GetDisplayName));
    if ShowMessageBoxGI(Self,Text,mbgOK or mbgCancel or mbgQuestion) = mbgResultOK then
    begin
...
{ @routine $6E452C TfShip2_ConfigureChameleon }
...
var
  ...
  // @nested $6E44F8 ChameleonDialogChoiceToSeries
  function ChameleonDialogChoiceToSeries(Choice: Integer): TDominatorSeries; // @addr $6E44F8 @ida "unsigned __int8 __usercall $name@<al>(int Choice@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6E46A0" Nested helper; does not access its parent frame.
  begin
    Result := dsBlazer;
    case Choice of
      2: Result := dsBlazer;
...
{ @routine $6E4778 TfShip2_HoldLeftPressed }
...
{ @routine $6E47E8 TfShip2_HoldLeftReleased }
...
{ @routine $6E4820 TfShip2_HoldRightPressed }
...
{ @routine $6E4890 TfShip2_HoldRightReleased }
...
{ @routine $6E48C8 TfShip2_ScrollHoldTimer }
...
{ @routine $6E49B4 TfShip2_ReturnSelectedHoldEntry }
...
{ @routine $6E4F58 TfShip2_RefreshEquipmentConfigurationButtons }
...
{ @routine $6E5074 TfShip2_SelectEquipmentConfiguration }
...
{ @routine $6E522C TfShip2_EquipmentConfigurationClicked }
...
{ @routine $6E5254 TfShip2_MainKeyDown }
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessageBoxGI(Self,FormatText1(LocalizedText('FormShip.SetHotEqu'),'<color=255,240,100>','<Key>','"' + Chr(Key) + '"'),mbgOK or mbgUnused04);
    end
...
{ @routine $6E5900 TfShip2_MainKeyUp }
...
{ @routine $6E5988 TfShip2_ProcessMouseWheel }
...
{ @routine $6E5C80 TfShip2_MainLeftButtonUp }
...
var
  // Native initialized-WideString descriptor at $6F7258; compiler-generated
  // finalizer at $6F7210 clears these 13 entries (unit counter $889CEC).
...
{ @routine $6E65C8 TfShip2_MainRightButtonDown }
...
var
  ...
  // @nested $6E62A0 EquipSelected
  procedure EquipSelected; // @addr $6E62A0 @ida "void __usercall $name(void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6E6D94,0x6E6E9D,0x6E6FAB,0x6E735C,0x6E7430" Nested helper captures selected item types, KeyState, Zone and Self.
  var
    Slot, Index: Integer;
    Equipment: TEquipment;
  begin
...
{ @routine $6E76B4 TfShip2_TryDeployTranclucator }
...
{ @routine $6E77E8 TfShip2_DropSelectedOutside }
...
begin
  ...
  begin
    ...
    if not RemoteHoldVisible then
      if ShowCountDialog(Self,'GI,' + GetShopItemIconName(SelectedHoldItem) + 's',
        FormatText1(LocalizedText('FormShip.ThrowItem'),'<color=0,50,200>','<Name>',LowerCaseWideString(GetStackableItemName(SelectedHoldItem))),
  ...
  begin
    ...
    begin
      ...
      begin
        ShowMessageBoxGI(Self,FormatText1(LocalizedColorText('FormShip.RuinMoveItemInvalid'),'<color=255,240,100>','<Item>',RemoveTextTagsW(SelectedHoldItem.GetDisplayName)),mbgCancel or mbgWarning);
        Exit;
      end;
    end
    ...
    begin
      ...
      begin
        ShowMessageBoxGI(Self,FormatText1(LocalizedColorText('FormShip.TrancMoveItemInvalid'),'<color=255,240,100>','<Item>',RemoveTextTagsW(SelectedHoldItem.GetDisplayName)),mbgCancel or mbgWarning);
    ...
    begin
      ...
      begin
        if PlayerHoldShip is TRuins then
          Text := FormatText1(LocalizedColorText('FormShip.RuinMoveItemInvalid'),'<color=255,240,100>','<Item>',RemoveTextTagsW(SelectedHoldItem.GetDisplayName))
  ...
  begin
    ...
    begin
      if not RemoteHoldVisible then
        if ShowCountDialog(Self,'GI,' + GetItemTypeBitmapPath(TItemType(SelectedGoodsIndex)),
          FormatText1(LocalizedText('FormShip.ThrowItem'),'<color=0,50,200>','<Name>',LowerCaseWideString(GetStackableItemTypeName(TItemType(SelectedGoodsIndex)))),
...
{ @routine $6E898C TfShip2_DropSelectedInArcade }
...
{ @routine $6E900C TfShip2_SpecialSlot1Clicked }
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          SoundManager.PlaySound('Sound.NoMoney');
          ShowMessageBoxGI(Self,FormatText1(LocalizedText('FormShip.RepairMsgNeedNode'),'<color=255,240,100>','<NeedNode>',IntToStr(Nodes - GetPlayer.GetAvailableNodeCount(PlayerHoldShip))),mbgCancel);
          Exit;
        end;
        if ShowMessageBoxGI(Self,FormatText1(LocalizedText('FormShip.RepairMsgEnoughNode'),'<color=255,240,100>','<NeedNode>',IntToStr(Nodes)),mbgOK or mbgCancel) <> mbgResultOK then Exit;
...
{ @routine $6E9AFC TfShip2_SpecialSlot2Clicked }
...
begin
  ...
  begin
    ...
    if (SelectedHoldItem is TArtefactTranclucator) and
      (ShowMessageBoxGI(Self,FormatText1(LocalizedColorText('FormShip.SellItemQuestion'),'<color=255,240,100>','<Item>',SelectedHoldItem.GetDisplayName),mbgOK or mbgCancel or mbgQuestion) = 2) then Exit;
    ...
    begin
      ...
      if (ShowCountDialog(Self,'GI,' + GetShopItemIconName(SelectedHoldItem) + 's',
        FormatText1(LocalizedText('FormShip.SellItem'),'<color=0,50,200>','<Name>',LowerCaseWideString(GetStackableItemName(SelectedHoldItem))),
...
{ @routine $6EA98C TfShip2_AdvanceScriptVideo }
...
{ @routine $6EAA44 TfShip2_StopScriptVideo }
...
{ @routine $6EAB14 TfShip2_UseMouseUp }
...
{ @routine $6EB0C0 TfShip2_SpecialSlot3Clicked }
...
begin
  ...
  begin
    ...
    if IsVirtualKeyDown(VK_CONTROL) then
    begin
      Text := LocalizedText('FormShip.FuelAct') + #13#10 + '<color=0,50,200>' + LocalizedText('Items.Cistern.Name2') + '</color>';
  ...
  begin
    ...
    if IsVirtualKeyDown(VK_CONTROL) then
    begin
      Text := LocalizedText('FormShip.FuelAct') + #13#10 + '<color=0,50,200>' + FuelTanks.GetDisplayName + '</color>';
  ...
  begin
    ...
    begin
      ...
      if IsVirtualKeyDown(VK_CONTROL) then
      begin
        Text := LocalizedText('FormShip.MissileAct') + #13#10 + '<color=0,50,200>' + Weapon.GetDisplayName + '</color>';
...
{ @routine $6EBD50 TfShip2_RepairAllClicked }
...
begin
  ...
  begin
    ...
    if GetPlayer.GetAvailableNodeCount(PlayerHoldShip) < Nodes then
    begin
      ShowMessageBoxGI(Self,FormatText1(LocalizedText('FormShip.RepairMsgAllNeedNode'),'<color=255,240,100>','<NeedNode>',IntToStr(Nodes)),mbgCancel);
      Nodes := 0;
    end
    else if ShowMessageBoxGI(Self,FormatText1(LocalizedText('FormShip.RepairMsgEnoughNode'),'<color=255,240,100>','<NeedNode>',IntToStr(Nodes)),mbgOK or mbgCancel) = mbgResultOK then
      GetPlayer.ConsumeAvailableNodes(Nodes,PlayerHoldShip)
    else Nodes := 0;
  end;
...
{ @routine $6EC7B8 TfShip2_RepairAllMouseEnter }
...
{ @routine $6EC828 TfShip2_RepairAllMouseLeave }
...
{ @routine $6EC84C TfShip2_LoadRockets }
...
begin
  ...
  if GetPlayer.Money < Cost then
  begin
    ShowMessageBoxGI(Self,FormatText1(LocalizedColorText('FormShip.ReloadAll.' + Suffix + '.NoMoney'),'<color=255,240,100>','<Money>',IntToStr(Cost)),mbgCancel or mbgWarning);
    Exit;
  end;
  if ShowMessageBoxGI(Self,FormatText1(LocalizedColorText('FormShip.ReloadAll.' + Suffix + '.Confirm'),'<color=255,240,100>','<Money>',IntToStr(Cost)),mbgOK or mbgCancel or mbgQuestion) = 2 then Exit;
  if GetPlayer.Money < Cost then
  begin
    SoundManager.PlaySound('Sound.NoMoney');
    StartMoneyWarning;
...
{ @routine $6ED224 TfShip2_ToggleAfterburner }
...
{ @routine $6ED3D4 TfShip2_ShowItemInfo }
...
{ @routine $6EDDC4 TfShip2_ShowItemInfoTimer }
...
{ @routine $6EDDEC TfShip2_HideItemInfo }
...
{ @routine $6EDEC0 TfShip2_LayoutItemInfo }
...
{ @routine $6EE380 TfShip2_LayoutObjectInfo }
...
var
  ...
  // @nested $6EE14C MeasurePair
  procedure MeasurePair(Left, Right: TLabelGI); // @addr $6EE14C @ida "void __usercall $name(TLabelGI *Left@<eax>, TLabelGI *Right@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6ee3e3,0x6ee3f0,0x6ee3fd,0x6ee40a,0x6ee417,0x6ee424,0x6ee431,0x6ee43e" @note "Nested in LayoutObjectInfo; captures column widths, total height and row count."
  begin
  ...
  // @nested $6EE22C PlacePair
  procedure PlacePair(Left, Right: TLabelGI); // @addr $6EE22C @ida "void __usercall $name(TLabelGI *Left@<eax>, TLabelGI *Right@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6ee5a5,0x6ee5b2,0x6ee5bf,0x6ee5cc,0x6ee5d9,0x6ee5e6,0x6ee5f3,0x6ee600" @note "Nested in LayoutObjectInfo; captures window, border dimensions and accumulated row position."
  begin
    if (Left <> nil) and (Right <> nil) and Left.Active and Right.Active then
    begin
...
{ @routine $6EE69C TfShip2_ShowEquipmentInfo }
...
begin
  ...
  begin
    ...
    begin
      EquipmentShopScreen.RefreshHullInfo(Self,Item as THull,Equipment.GetInfoText('<color=255,240,100>',PlayerHoldShip),True);
...
{ @routine $6EFB7C TfShip2_ShowHoldGoodsInfo }
...
begin
  ...
  begin
    ...
    Text := Text + #13#10 + FormatText1(LocalizedText('FormShip.CostGoods'),'<color=255,240,100>','<OldCost>',IntToStr(Round(PlayerHoldShip.GetAverageCargoCost(Good))));
...
{ @routine $6F044C TfShip2_ShowStoredGoodsInfo }
...
begin
  ...
  begin
    ...
    Text := Text + #13#10 + FormatText1(LocalizedText('FormShip.CostGoods'),'<color=255,240,100>','<OldCost>',IntToStr(Round(AverageCost)));
...
{ @routine $6F0AF4 TfShip2_RefreshMoneyWarning }
...
{ @routine $6F0C44 TfShip2_StartMoneyWarning }
...
{ @routine $6F0CBC TfShip2_AdvanceMoneyWarning }
...
{ @routine $6F0D20 TfShip2_ShowNoDropMessage }
...
{ @routine $6F0E40 TfShip2_GateMouseEnter }
...
{ @routine $6F0E9C TfShip2_GateMouseLeave }
...
{ @routine $6F0EB4 TfShip2_OpenGate }
...
{ @routine $6F0F44 TfShip2_CloseGate }
...
{ @routine $6F0F9C TfShip2_SlideGateTimer }
...
{ @routine $6F1108 TfShip2_GateMouseUp }
...
{ @routine $6F113C TfShip2_UseMouseEnter }
...
{ @routine $6F116C TfShip2_UseMouseLeave }
...
{ @routine $6F1184 TfShip2_OpenUsePanel }
...
{ @routine $6F1210 TfShip2_CloseUsePanel }
...
{ @routine $6F1268 TfShip2_SlideUseTimer }
...
{ @routine $6F13D0 TfShip2_OpenUseSidePanel }
...
{ @routine $6F1434 TfShip2_CloseUseSidePanel }
...
{ @routine $6F148C TfShip2_SlideUsePanelTimer }
...
{ @routine $6F15A4 TfShip2_OpenSpecialSlot1 }
...
{ @routine $6F167C TfShip2_CloseSpecialSlot1 }
...
{ @routine $6F171C TfShip2_AnimateSpecialSlot1 }
...
{ @routine $6F186C TfShip2_OpenSpecialSlot2 }
...
{ @routine $6F1944 TfShip2_CloseSpecialSlot2 }
...
{ @routine $6F19E4 TfShip2_AnimateSpecialSlot2 }
...
{ @routine $6F1B34 TfShip2_OpenSpecialSlot3 }
...
{ @routine $6F1C0C TfShip2_CloseSpecialSlot3 }
...
{ @routine $6F1CAC TfShip2_AnimateSpecialSlot3 }
...
{ @routine $6F1DFC TfShip2_StorageDownClicked }
...
{ @routine $6F1EA0 TfShip2_StorageUpClicked }
...
{ @routine $6F1F44 TfShip2_SlideStorageTimer }
...
{ @routine $6F2094 TfShip2_RefreshStorageView }
...
{ @routine $6F2368 TfShip2_ScrollStorageUp }
...
{ @routine $6F23B0 TfShip2_ScrollStorageDown }
...
{ @routine $6F2440 TfShip2_StorageItemMouseUp }
...
begin
  ...
  begin
    ...
    if not RemoteHoldVisible and (SelectedHoldOrigin = 0) and (Count > 1) then
      if ShowCountDialog(Self,'GI,' + GetShopItemIconName(SelectedHoldItem) + 's',
        FormatText1(LocalizedText('FormShip.StorageItem'),'<color=0,50,200>','<Name>',LowerCaseWideString(GetStackableItemName(SelectedHoldItem))),
  ...
  begin
    ...
    if not RemoteHoldVisible and (SelectedHoldOrigin = 0) and (Count > 1) then
      if ShowCountDialog(Self,'GI,' + GetItemTypeBitmapPath(TItemType(SelectedGoodsIndex)),
        FormatText1(LocalizedText('FormShip.StorageItem'),'<color=0,50,200>','<Name>',LowerCaseWideString(GetStackableItemTypeName(TItemType(SelectedGoodsIndex)))),
...
{ @routine $6F30C4 TfShip2_CreateShipInfoImage }
...
{ @routine $6F35F0 TfShip2_BuildAdditionalInfoPanel }
...
var
  ...
  // @nested $6F31D0 AddLine
  procedure AddLine(Icon: Integer; Text, Hint: WideString; Data: Integer); // @addr $6F31D0 @ida "void __userpurge $name(int Icon@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 *Hint@<ecx>, int Data@<^0>, void *ParentFrame@<^4>);" @stackpop 4 @calls "0x6F37AD,0x6F3906,0x6F39EA,0x6F3ABE,0x6F3C1E,0x6F3CB8,0x6F3FB9,0x6F4047,0x6F4381" Nested helper captures the panel, accumulated height and Self.
  var
  ...
  // @nested $6F352C GetShipInfoColor
  function GetShipInfoColor(ColorIndex: Byte): WideString; // @addr $6F352C @ida "void __usercall $name(unsigned __int8 ColorIndex@<al>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6F3F36" Nested in BuildAdditionalInfoPanel; does not access its parent frame.
  begin
    Result := '';
    case ColorIndex of
  ...
  for I := 1 to 24 do
    ...
    begin
      ...
      begin
        ...
        if Kind = 1 then
          Name := Name + #13#10 + FormatText1(LocalizedText('Illness.Illness.EndDate'),'<color=255,240,100>','<Date>',Galaxy.FormatTurnDate(Turn))
        else
          Name := Name + #13#10 + FormatText1(LocalizedText('Illness.Stimulant.EndDate'),'<color=255,240,100>','<Date>',Galaxy.FormatTurnDate(Turn));
      end;
  ...
  begin
    if GetPlayer.MedicalPolicyTicks > 0 then
      AddLine(0,LocalizedText('ShipInfo.AddInfo.MedPolicy.Name'),
        ...
        FormatText1(LocalizedText('ShipInfo.AddInfo.MedPolicy.Text'),'<color=255,240,100>','<Date>',Galaxy.FormatTurnDate(Galaxy.CurrentTurn + GetPlayer.MedicalPolicyTicks)),0);
    if GetPlayer.DebtAmount > 0 then
    begin
      Name := LocalizedText('ShipInfo.AddInfo.DebtInfo.Name');
      AddLine(0,Name,Name + '~' +
        FormatText2(LocalizedText('ShipInfo.AddInfo.DebtInfo.Text'),'<color=255,240,100>','<Money>',IntToStr(GetPlayer.DebtAmount),'<Date>',Galaxy.FormatTurnDate(GetPlayer.DebtDueTurn)),0);
    end;
    ...
    begin
      ...
      ReplaceTextToken(Text,'<Date>',Galaxy.FormatTurnDate(GetPlayer.DepositStartTurn),'<color=255,240,100>');
      ReplaceTextToken(Text,'<Money>',IntToStr(GetPlayer.DepositAmount),'<color=255,240,100>');
      ReplaceTextToken(Text,'<Percent>',FloatToStrF(GetPlayer.DepositInterestRate,ffFixed,1,1),'<color=255,240,100>');
      ReplaceTextToken(Text,'<Sum>',IntToStr(GetPlayer.ComputeDepositAccruedValue),'<color=255,240,100>');
    ...
    begin
      ...
      ReplaceTextToken(Text,'<Date>',Galaxy.FormatTurnDate(Galaxy.CurrentTurn + GetPlayer.PirateLicenseTicks),'<color=255,240,100>');
      AddLine(0,Name,Name + '~' + Text,0);
    end;
    ...
    begin
      ...
      begin
        ...
        Text := FormatText3(LocalizedText('ShipInfo.AddInfo.Chameleon.Text'),'<color=255,240,100>','<Series>',SeriesName,'<Type>',TypeName,'<Count>',CountText);
      end;
      ...
      begin
        ...
        for ColorIndex := 0 to 2 do
          ...
          begin
            ...
            CountText := FormatText1(LocalizedText('ShipInfo.AddInfo.Chameleon.Count'),'<color=255,240,100>','<Count>',IntToStr(GetPlayer.ChameleonCharges[ColorIndex]));
...
{ @routine $6F4C3C TfShip2_HideSender }
...
{ @routine $6F4C58 TfShip2_ProcessCallbackTimers }
...
{ @routine $6F4C98 TfShip2_ProcessWindowMessage }
...
{ @routine $6F4CF0 TfShip2_SelectMusic }
...
{ @routine $6F5048 TfShip2_IsHoldNormalShip }
...
{ @routine $6F506C TfShip2_CanUsePlayerExperience }
...
{ @routine $6F50E8 TfShip2_ToggleRemoteHoldClicked }
...
{ @routine $6F52A8 TfShip2_RemoteHoldUpPressed }
...
{ @routine $6F5340 TfShip2_RemoteHoldUpReleased }
...
{ @routine $6F5378 TfShip2_RemoteHoldDownPressed }
...
{ @routine $6F5420 TfShip2_RemoteHoldDownReleased }
...
{ @routine $6F5458 TfShip2_ScrollRemoteHoldTimer }
...
{ @routine $6F551C TfShip2_GetRemoteHoldScrollLimit }
...
{ @routine $6F55C4 TfShip2_SortRemoteHoldClicked }
...
{ @routine $6F55EC RunShipEquipment }
...
{ @routine $6F56CC TfShip2_StorageToShipClicked }
...
{ @routine $6F5790 TfShip2_SellStorageClicked }
...
{ @routine $6F59F4 TfShip2_ShipToStorageClicked }
...
{ @routine $6F5AB4 TfShip2_SortStorageByTypeClicked }
...
{ @routine $6F5AD8 TfShip2_SortStorageBySizeClicked }
...
{ @routine $6F5B00 TfShip2_SortStorageByPriceClicked }
...
{ @routine $6F5B28 TfShip2_LoadHoldRocketsClicked }
...
{ @routine $6F5B50 TfShip2_SellHoldClicked }
...
{ @routine $6F5E88 TfShip2_LoadEquippedRocketsClicked }
...
{ @routine $6F5F0C CompareStoredItems }
...
var LeftValue, RightValue: Integer; LeftType, RightType: Byte; LeftClass, RightClass: Integer;
  ...
  // @nested $6F5EB0 Classify
  function Classify(ItemType: Byte): Integer; // @addr $6F5EB0 @ida "int __usercall $name@<eax>(unsigned __int8 ItemType@<al>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6F5F7D,0x6F5F8A" Nested helper; does not read the parent frame.
  begin
    Result := 0;
...
{ @routine $6F61A8 TfShip2_SortStorage }
...
{ @routine $6F6378 TfShip2_RefreshLoadEquippedRocketsButton }
...
{ @routine $6F6818 TfShip2_SellAllItems }
...
var
  ...
  // @nested $6F647C SellItem
  function SellItem(Item: TItem; IgnoreRequiredEquipment: Boolean): Boolean; // @addr $6F647C @ida "bool __usercall $name@<al>(TItem *Item@<eax>, bool IgnoreRequiredEquipment@<dl>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x6F6A3C,0x6F6B5D,0x6F6EE7" Nested helper captures sale/change flags.
  var
    ...
    begin
      ...
      begin
        if Item is TArtefactTranclucator then
          if ShowMessageBoxGI(Self,FormatText1(LocalizedColorText('FormShip.SellItemQuestion'),'<color=255,240,100>','<Item>',Item.GetDisplayName),mbgOK or mbgCancel or mbgQuestion) = 2 then
          begin
            Dec(I);
            Continue;
          end;
...
{ @routine $6F7118 TfShip2_EnterBridgeClicked }
...
{ @routine $6F716C TfShip2_GetActionParentLoop }
...
{ @routine $6F7188 TfShip2_ExecuteUiCode }
