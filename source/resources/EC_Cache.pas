unit EC_Cache;
// Unit bracket (inferred): .text 0x007F3258..0x007F492A; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create; // @addr 0x7F33AC @ida "TCacheControlEC *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x7F33F0 @ida "void __usercall $name(TCacheControlEC *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Reset; virtual; // @addr 0x7F3428 @slot 0x00 @note "Drops all retains and the data binding."
    procedure SetCacheKey(const NewKey: WideString); virtual; // @addr 0x7F3484 @slot 0x04 @calls "0x487B6E 0x482E32" @note "Drops existing retains and the data binding; may apply configured key substitutions."
    function HasEmptyCacheKey: Boolean; // @addr 0x7F3554
    procedure QueueLoadIfMissing(PendingLoads: TList); virtual; // @addr 0x7F357C @slot 0x08 @calls "0x485ED6 0x48913A 0x4933EE 0x4D363E"
    function CreateData: TCacheDataEC; virtual; // @addr 0x7F358C @slot 0x0C @note "Base implementation returns nil."
    function AcquireData: TCacheDataEC; virtual; // @addr 0x7F3B48 @slot 0x10 @note "Base implementation returns nil."
    procedure Release; virtual; // @addr 0x7F3B60 @slot 0x14 @calls "0x487BAC 0x482EC3" @note "Saturates at zero; the data remains bound."
    ...
    function AcquireDataFromConfig(CacheDataClass: TCacheDataClass): TCacheDataEC; // @addr 0x7F35A4
    function AcquireDataFromDirectKey(CacheDataClass: TCacheDataClass): TCacheDataEC; // @addr 0x7F38C8
    procedure EvictData(CacheDataClass: TCacheDataClass); // @addr 0x7F3B84 @note "Only checks this control's RetainCount; frees the shared entry and detaches all its controls."
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x7F3C18 @ida "TCacheDataEC *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x7F3C5C @ida "void __usercall $name(TCacheDataEC *Self@<eax>, __int8 DestroyFlags@<dl>);" @note "Detaches all controls without freeing them."
    procedure AppendControl(Control: TCacheControlEC); // @addr 0x7F3CA8 @note "Caller must set Control.BoundData."
    procedure UnlinkControl(Control: TCacheControlEC); // @addr 0x7F3CFC @note "Clears BoundData but preserves RetainCount."
    // Base load hooks are empty in the native implementation.
    procedure LoadFromConfigBuffer(SourceBuffer: TBufEC; const LoadOption: WideString); virtual; // @addr 0x7F3D84 @slot 0x00
    procedure LoadFromKey(const Key: WideString); virtual; // @addr 0x7F3D98 @slot 0x04
  end;
  ...
  public
    ...
    constructor Create; // @addr 0x7F3DA8 @ida "TCacheEC *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr 0x7F3DFC @ida "void __usercall $name(TCacheEC *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure Clear; // @addr 0x7F3E40 @note "Invalidates all entries, including retained ones."
    procedure SetDataRoot(Root: TDataEC); // @addr 0x7F3E6C @note "Root is borrowed; invalidates existing cached entries."
    procedure ResetControl(Control: TCacheControlEC); // @addr 0x7F3E90
    // List and lookup helpers below do not acquire CacheLock.
    procedure AddDataToLruHead(Data: TCacheDataEC); // @addr 0x7F3EA8
    procedure RemoveAndFreeData(Data: TCacheDataEC); // @addr 0x7F3EFC @note "Does not adjust ResidentBytes. Accepts nil."
    function FindDataByKeyAndClass(const Key: WideString; CacheDataClass: TCacheDataClass): TCacheDataEC; // @addr 0x7F3F78 @note "Case-sensitive key and exact class match; returns nil when absent."
    procedure TouchData(Data: TCacheDataEC); // @addr 0x7F3FD8
    function OpenDataBuffer(const Path: WideString): TBufEC; // @addr 0x7F405C @note "Caller owns the returned buffer."
    procedure TrimToBudget(BudgetBytes: Integer); // @addr 0x7F4098 @note "Retained entries can prevent reaching the budget."
    procedure QueueNamedLoadIfMissing(PendingLoads: TList; const CacheKind, Key: WideString); // @addr 0x7F415C @note "PendingLoads owns added controls; duplicate pending entries are possible."
  end;

procedure EvictMainMenuShipCachesWhenAddressSpaceHigh; // @addr 0x7F4468 @note "The threshold is 0x30000000 bytes of used virtual address space."
procedure EvictRuinsAndGovernmentCaches; // @addr $7F4738
procedure EvictStarAndBackgroundCaches; // @addr $7F47B8
procedure EvictBlockChildrenFromCache(BlockPath: WideString; CacheDataClass: TCacheDataClass); // @addr 0x7F4804 @note "Requires an exact cache-data class match."
...
{ @routine $7F33AC TCacheControlEC_Create }
...
{ @routine $7F33F0 TCacheControlEC_Destroy }
...
{ @routine $7F3428 TCacheControlEC_Reset }
...
{ @routine $7F3484 TCacheControlEC_SetCacheKey }
...
{ @routine $7F3554 TCacheControlEC_HasEmptyCacheKey }
...
{ @routine $7F357C TCacheControlEC_QueueLoadIfMissing }
procedure TCacheControlEC.QueueLoadIfMissing(PendingLoads: TList);
begin
end;
{ @end $7F357C }

{ @routine $7F358C TCacheControlEC_CreateData }
...
{ @routine $7F35A4 TCacheControlEC_AcquireDataFromConfig }
...
{ @routine $7F38C8 TCacheControlEC_AcquireDataFromDirectKey }
...
{ @routine $7F3B48 TCacheControlEC_AcquireData }
...
{ @routine $7F3B60 TCacheControlEC_Release }
...
{ @routine $7F3B84 TCacheControlEC_EvictData }
...
{ @routine $7F3C18 TCacheDataEC_Create }
...
{ @routine $7F3C5C TCacheDataEC_Destroy }
...
{ @routine $7F3CA8 TCacheDataEC_AppendControl }
...
{ @routine $7F3CFC TCacheDataEC_UnlinkControl }
...
{ @routine $7F3DFC TCacheEC_Destroy }
...
{ @routine $7F3E6C TCacheEC_SetDataRoot }
...
{ @routine $7F3E90 TCacheEC_ResetControl }
...
{ @routine $7F3EA8 TCacheEC_AddDataToLruHead }
...
{ @routine $7F3EFC TCacheEC_RemoveAndFreeData }
...
{ @routine $7F3F78 TCacheEC_FindDataByKeyAndClass }
...
{ @routine $7F3FD8 TCacheEC_TouchData }
...
{ @routine $7F405C TCacheEC_OpenDataBuffer }
...
{ @routine $7F4098 TCacheEC_TrimToBudget }
...
{ @routine $7F415C TCacheEC_QueueNamedLoadIfMissing }
...
{ @routine $7F4468 EvictMainMenuShipCachesWhenAddressSpaceHigh }
...
{ @routine $7F4738 EvictRuinsAndGovernmentCaches }
...
{ @routine $7F47B8 EvictStarAndBackgroundCaches }
...
{ @routine $7F4804 EvictBlockChildrenFromCache }
