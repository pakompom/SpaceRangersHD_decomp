unit Achievements;
// Unit bracket (inferred): .text 0x007FD94C..0x007FF59E; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    AsteroidsDestroyed: Integer; // @offset $04 ASTEROID: player asteroid kills in TStar.ProcessPlayerAsteroidKill ($84C310).
    EnemiesDestroyedByStarHeat: Integer; // @offset $08 FRY counter.
    SystemsDefended: Integer; // @offset $0C DEFENDER: qualifying Coalition or pirate defenses; native increments $85F978/$85FA03.
    ...
    SuccessfulDominatorHacks: Integer; // @offset $18 HACKER: accepted programs in TfTalk.RunDominatorProgram; native increment $616CEA.
    PrisonersBailedOut: Integer; // @offset $1C PRISONBAIL: ships released by TfGov.PayPrisonBail; native increment $52D92D.
    ...
    constructor Create; // @addr $7FDC40 @ida "TAchievementStats *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    destructor Destroy; override; // @addr $7FDCD8 @ida "void __usercall $name(TAchievementStats *Self@<eax>, __int8 DestroyFlags@<dl>);"
    procedure LoadFromBuffer(Buffer: TBufEC); // @addr $7FDD0C Includes the pre-version-99 counter layout.
    procedure SaveToBuffer(Buffer: TBufEC); // @addr $7FDE80
    ...
    procedure CheckAllSkillsAchievement; // @addr $7FE4B4 SKILL: all six player base skills are at least level six.
    procedure CheckMoneyAchievement; // @addr $7FE24C MONEY: at least 10000000 credits.
    procedure CheckMasterAchievement; // @addr $7FE294 MASTER: at least six player wingmen.
    procedure CheckAllDiseasesAchievement; // @addr $7FE930 ILL: the player has experienced every one of the twelve diseases.
    procedure CheckNoShotsArcadeVictoryAchievement; // @addr $7FE20C @note "Caller establishes a victory without firing; native HOLEPEACE unlock."
    procedure CheckScratchDamageAchievement(HitsReceived: Integer); // @addr $7FE9F4 SCRATCHDAMAGE: twenty one-point hits on the same target.
    procedure CheckStarFuelAchievement; // @addr $7FEA48 SUNFUEL threshold: 40 units in one fuel tank.
    procedure CheckSpeedAchievement; // @addr $7FE51C Native SPEED threshold: calculated speed 2300.
    procedure CheckBestEquipmentAchievement; // @addr $7FE790 Native BEST check: all direct slots and five weapons have nonstandard stats.
    procedure CheckBaronAchievement; // @addr $7FE610 Requires player pirate rank 7.
    procedure CheckAllAwardsAchievement; // @addr $7FDF74 Native award-presence buffer is left uninitialized by reversed FillChar arguments.
    procedure CheckHaterAchievement; // @addr 0x7FE6A8 @note "HATER requires at least one inhabited planet in Coalition-controlled systems and hostile relations with every such planet."
    procedure CheckNoQuestVictoryAchievement; // @addr 0x7FE0F8
    procedure CheckChampionVictoryAchievement(Score: Integer); // @addr 0x7FE174 @note "CHAMPION requires at least 50000 points."
    procedure CheckNoLoadVictoryAchievement; // @addr 0x7FE1C4
    procedure CheckLongGameVictoryAchievement(Score, FinishedTurn: Integer); // @addr 0x7FE328 @note "DOLGOZHID requires at least 20000 points and turn 36800."
    procedure CheckPacifistVictoryAchievement; // @addr 0x7FE378
    procedure CheckFirstPlaceRatingAchievement; // @addr 0x7FE458 @note "Requires PlaceInRating=1 and CurrentTurn>=300."
    procedure CheckNodesAchievement; // @addr $7FE2E0
    procedure CheckCommanderAchievement; // @addr $7FE658
    procedure CheckAllDrugsAchievement; // @addr $7FE98C
    procedure CheckMapBuilderAchievement; // @addr $7FEA8C
    procedure CheckInvestorAchievement(Amount: Integer); // @addr $7FEB74
    procedure CheckScienceAchievement; // @addr $7FE5CC
    procedure CheckFastVictoryAchievement; // @addr 0x7FE564 @note "SPRINTER requires fewer than seven elapsed years after turn 300."
    procedure CheckAllPirateSystemsAchievement; // @addr $7FE3CC PIRATESYSTEMS: every registered star is pirate-controlled.
    procedure CheckBomberAchievement(KillsThisTurn: Integer); // @addr 0x7FDF2C @note "BOMBER requires at least five kills and a current player/galaxy."
    procedure CheckTranclucatorFleetAchievement; // @addr 0x7FEBC4 @note "TRANCLUCATORS requires ten live, normal-space Tranclucators owned by the player in the current star."
  end;

function GetCurrentAchievementProgress(Key: WideString; StoredValue: Integer): Integer; // @addr $7FD9AC

function GetAchievementBackend: Byte; // @addr $7FEE74 1=Steam, 2=Steam without achievement support, 3=local.
function GetAvailableAchievementCount: Integer; // @addr $7FEEA8 Capped at 82.
function CreateAchievementData: PAchievementData; // @addr $7FF4E4 Allocates three 255-character caller-owned string buffers.
procedure FreeAchievementData(Data: PAchievementData); // @addr $7FF564 Nil-safe; releases all three string cells and the record.
function GetAchievementData(Key: WideString): PAchievementData; // @addr $7FF3EC Caller owns the result; nil for an unknown key.

function TryUnlockAchievement(Key: WideString): Boolean; // @addr 0x7FEF08 @note "Checks availability, unlock budget and per-save duplicates; records successful unlocks in the current player's AwardedAchievementKeys."

function TrySetAchievementProgress(Key: WideString; Value: Integer): Boolean; // @addr $7FF210 @note "Raises progress to the supplied value, capped at the target; never reduces existing progress."

function TryAddAchievementProgress(Key: WideString; Amount: Integer): Boolean; // @addr 0x7FF05C @note "Caps the increment at the configured achievement target; checks platform availability and per-save completion."


var
  AchievementDefinitions: TBlockParEC = nil; // @addr $881EF8

procedure InitializeAchievementDefinitions; // @addr $7FECAC

implementation

// @unit-initialization $876A7C
// @unit-finalization $7FF5A0
...
{ @routine $7FD9AC GetCurrentAchievementProgress }
...
{ @routine $7FDC40 TAchievementStats_Create }
...
{ @routine $7FDCD8 TAchievementStats_Destroy }
...
{ @routine $7FDD0C TAchievementStats_LoadFromBuffer }
...
{ @routine $7FDE80 TAchievementStats_SaveToBuffer }
...
{ @routine $7FDF2C TAchievementStats_CheckBomberAchievement }
...
{ @routine $7FDF74 TAchievementStats_CheckAllAwardsAchievement }
...
{ @routine $7FE0F8 TAchievementStats_CheckNoQuestVictoryAchievement }
...
{ @routine $7FE174 TAchievementStats_CheckChampionVictoryAchievement }
...
{ @routine $7FE1C4 TAchievementStats_CheckNoLoadVictoryAchievement }
...
{ @routine $7FE20C TAchievementStats_CheckNoShotsArcadeVictoryAchievement }
...
{ @routine $7FE24C TAchievementStats_CheckMoneyAchievement }
...
{ @routine $7FE294 TAchievementStats_CheckMasterAchievement }
...
{ @routine $7FE2E0 TAchievementStats_CheckNodesAchievement }
...
{ @routine $7FE328 TAchievementStats_CheckLongGameVictoryAchievement }
...
{ @routine $7FE378 TAchievementStats_CheckPacifistVictoryAchievement }
...
{ @routine $7FE3CC TAchievementStats_CheckAllPirateSystemsAchievement }
...
{ @routine $7FE458 TAchievementStats_CheckFirstPlaceRatingAchievement }
...
{ @routine $7FE4B4 TAchievementStats_CheckAllSkillsAchievement }
...
{ @routine $7FE51C TAchievementStats_CheckSpeedAchievement }
...
{ @routine $7FE564 TAchievementStats_CheckFastVictoryAchievement }
...
{ @routine $7FE5CC TAchievementStats_CheckScienceAchievement }
...
{ @routine $7FE610 TAchievementStats_CheckBaronAchievement }
...
{ @routine $7FE658 TAchievementStats_CheckCommanderAchievement }
...
{ @routine $7FE6A8 TAchievementStats_CheckHaterAchievement }
...
{ @routine $7FE790 TAchievementStats_CheckBestEquipmentAchievement }
...
{ @routine $7FE930 TAchievementStats_CheckAllDiseasesAchievement }
...
{ @routine $7FE98C TAchievementStats_CheckAllDrugsAchievement }
...
{ @routine $7FE9F4 TAchievementStats_CheckScratchDamageAchievement }
...
{ @routine $7FEA48 TAchievementStats_CheckStarFuelAchievement }
...
{ @routine $7FEA8C TAchievementStats_CheckMapBuilderAchievement }
...
{ @routine $7FEB74 TAchievementStats_CheckInvestorAchievement }
...
{ @routine $7FEBC4 TAchievementStats_CheckTranclucatorFleetAchievement }
...
{ @routine $7FECAC InitializeAchievementDefinitions }
...
{ @routine $7FEE74 GetAchievementBackend }
...
{ @routine $7FEEA8 GetAvailableAchievementCount }
...
{ @routine $7FEF08 TryUnlockAchievement }
...
{ @routine $7FF05C TryAddAchievementProgress }
...
{ @routine $7FF210 TrySetAchievementProgress }
...
{ @routine $7FF3EC GetAchievementData }
...
{ @routine $7FF4E4 CreateAchievementData }
...
{ @routine $7FF564 FreeAchievementData }
