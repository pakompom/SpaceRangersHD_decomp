unit CalcParseClass;
// Unit bracket (inferred): .text 0x004DBED8..0x004E0DCC; inclusive evidence, not full bounds. See docs/declarations.md#unit-coverage-and-address-brackets.
...
type
  ...
  public
    ...
    constructor Create; // @addr 0x4E0864 @ida "TCalcParse *__usercall $name@<eax>(void *SelfOrClass@<eax>, unsigned __int8 Allocate@<dl>);"
    procedure Reset; // @addr 0x4E08A4

    // External spellings -> internal tokens: pct %, div f, mod g, in #,
    // to $, or |, and &, <> e, >= c, <= b, .. h, and decimal dot -> comma.
    function NormalizeTokens(var Text: WideString): WideString; // @addr 0x4DD3C4 @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 **Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Text is read-only despite var. Uses ANSI lowercase and boundary-free substitutions; always wraps the result in parentheses."
    function FormatTokens(var Text: WideString): WideString; // @addr 0x4DD998 @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 **Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Text is read-only despite var. Removes at most one enclosing parenthesis pair; leaves outer whitespace."
    // Lower ranks bind tighter; -1 means not an operator.
    // 1: ^ / f g; 2: * %; 3: -; 4: +; 5: $; 6: #;
    // 7: < > = b c e; 8: &; 9: |.
    function GetOperatorRank(Token: WideChar): Integer; // @addr 0x4DDDAC
    function CollapseOperatorRun(const Text: WideString): WideString; // @addr 0x4DDF3C @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Requires a nonempty operator run. Minus parity controls the sign; ties choose the leftmost weakest operator."
    function NormalizeParameterReference(Text: WideString): WideString; // @addr 0x4DF33C @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Uses only the first three digits; zero/missing digits produce [err]. Does not check parameter-list bounds."
    function FindTopLevelOperator(const Text: WideString; TextLength: Integer): Integer; // @addr 0x4DFE70 @note "One-based; zero when absent. Rightmost ties give left associativity. Delimiter balance is unchecked."
    function HasBalancedParenthesesInSlice(const Text: WideString; FirstIndex, LastIndex: Integer): Boolean; // @addr 0x4E090C @note "One-based inclusive bounds, unchecked. Empty slices pass; square brackets are ignored."
    function HasBalancedParentheses(const Text: WideString): Boolean; // @addr 0x4E0B1C @note "Empty text passes."

    procedure Prepare(Text: WideString; DefaultParameterIndex: Integer); // @addr 0x4E0610 @note "Resets state; stores Expression even on error. Empty input becomes (), not the default parameter."
    function NormalizeFragments(const Text: WideString): WideString; // @addr 0x4DE0C8 @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Square brackets do not nest. An unmatched opening bracket silently discards the remaining suffix."
    function NormalizeScalarFragment(Text: WideString): WideString; // @addr 0x4DE298 @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Silently discards unsupported characters, including decimal dots; call NormalizeTokens first."
    function NormalizeBracketFragment(Text: WideString): WideString; // @addr 0x4DF2A0 @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Any lowercase p selects parameter parsing, even outside the [pN] form."
    function NormalizeRangeLiteral(Text: WideString): WideString; // @addr 0x4DF4AC @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Requires internal h notation, not '..'. Empty or rejected input yields [err]; existing errors remain set."
    function InsertImplicitMultiplication(Text: WideString): WideString; // @addr 0x4DF7D4 @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 **Result@<ecx>);"
    function ClampNumericLiterals(Text: WideString): WideString; // @addr 0x4E0B54 @ida "void __usercall $name(TCalcParse *Self@<eax>, unsigned __int16 *Text@<edx>, unsigned __int16 **Result@<ecx>);" @note "Nonzero limits: 0.0001..999999999. The lower clamp emits a dot-decimal literal that evaluation rejects. Drops trailing numbers; conversion errors set flags and leave the caller's result storage unchanged."

    // Parameters: borrowed, non-nil TList of TParameter; [pN] is one-based.
    function SubstituteParameters(Parameters: TList): WideString; // @addr 0x4E0984 @ida "void __usercall $name(TCalcParse *Self@<eax>, TList *Parameters@<edx>, unsigned __int16 **Result@<ecx>);" @note "Unmatched references remain unchanged; negative values are parenthesized."
    function EvaluateExpression(Text: WideString): TCPVariant; // @addr 0x4DFF3C @note "Caller owns the result. Evaluates right before left without short-circuiting. EvaluationError blocks evaluation; HasError alone does not. Native recursive intermediates leak."
    procedure Evaluate(Parameters: TList); // @addr 0x4E04F4 @note "Existing HasError preserves ResultValue; flags are not reset. Native scratch and returned variants leak."
    ...
    procedure ApplyPower(var Left, Right, OutValue: TCPVariant); // @addr 0x4DBF4C @note "Negative bases stay negative even for even exponents; integer results round."
    procedure ApplyAdd(var Left, Right, OutValue: TCPVariant); // @addr 0x4DC16C
    procedure ApplySubtract(var Left, Right, OutValue: TCPVariant); // @addr 0x4DC2D8
    procedure ApplyMultiply(var Left, Right, OutValue: TCPVariant); // @addr 0x4DC444
    procedure ApplyPercentChange(var Left, Right, OutValue: TCPVariant); // @addr 0x4DC5B0 @note "Returns float Left * (1 + Right * 0.01)."
    procedure ApplyDivide(var Left, Right, OutValue: TCPVariant); // @addr 0x4DC61C @note "Exact integer quotients stay integer. Zero divisor: +/-2000000000 for integers; float operands incorrectly leave integer zero."
    procedure ApplyIntDivide(var Left, Right, OutValue: TCPVariant); // @addr 0x4DC7E0 @note "Truncates toward zero; zero-divisor behavior matches ApplyDivide."
    procedure ApplyModulo(var Left, Right, OutValue: TCPVariant); // @addr 0x4DC934 @note "Float operands produce a float remainder after truncation. Zero-divisor behavior matches ApplyDivide."
    procedure ApplyRange(var Left, Right, OutValue: TCPVariant); // @addr 0x4DCB64 @note "Uses operand extrema, rounds floats and swaps reversed bounds. Range operands must be nonempty."
    procedure ApplyMembership(var Left, Right, OutValue: TCPVariant); // @addr 0x4DCC78 @note "Two ranges sample Left once; scalar/range membership rounds the scalar."
    // Comparisons return integer 0 or 1 and sample each range operand once.
    procedure ApplyLessThan(var Left, Right, OutValue: TCPVariant); // @addr 0x4DCDFC
    procedure ApplyGreaterThan(var Left, Right, OutValue: TCPVariant); // @addr 0x4DCE60
    procedure ApplyLessOrEqual(var Left, Right, OutValue: TCPVariant); // @addr 0x4DCEC4
    procedure ApplyGreaterOrEqual(var Left, Right, OutValue: TCPVariant); // @addr 0x4DCF28
    procedure ApplyEqual(var Left, Right, OutValue: TCPVariant); // @addr 0x4DCF8C
    procedure ApplyNotEqual(var Left, Right, OutValue: TCPVariant); // @addr 0x4DCFF0
    procedure ApplyAnd(var Left, Right, OutValue: TCPVariant); // @addr 0x4DD054 @note "Range operands concatenate rather than intersect; duplicates remain."
    procedure ApplyOr(var Left, Right, OutValue: TCPVariant); // @addr 0x4DD210 @note "Range operands concatenate; duplicates remain."
  end;
...
{ @routine $4DBF4C TCalcParse_ApplyPower }
...
{ @routine $4DC16C TCalcParse_ApplyAdd }
...
{ @routine $4DC2D8 TCalcParse_ApplySubtract }
...
{ @routine $4DC444 TCalcParse_ApplyMultiply }
...
{ @routine $4DC5B0 TCalcParse_ApplyPercentChange }
...
{ @routine $4DC61C TCalcParse_ApplyDivide }
...
{ @routine $4DC7E0 TCalcParse_ApplyIntDivide }
...
{ @routine $4DC934 TCalcParse_ApplyModulo }
...
{ @routine $4DCB64 TCalcParse_ApplyRange }
...
{ @routine $4DCC78 TCalcParse_ApplyMembership }
...
{ @routine $4DCDFC TCalcParse_ApplyLessThan }
...
{ @routine $4DCE60 TCalcParse_ApplyGreaterThan }
...
{ @routine $4DCEC4 TCalcParse_ApplyLessOrEqual }
...
{ @routine $4DCF28 TCalcParse_ApplyGreaterOrEqual }
...
{ @routine $4DCF8C TCalcParse_ApplyEqual }
...
{ @routine $4DCFF0 TCalcParse_ApplyNotEqual }
...
{ @routine $4DD054 TCalcParse_ApplyAnd }
...
{ @routine $4DD210 TCalcParse_ApplyOr }
...
{ @routine $4DD3C4 TCalcParse_NormalizeTokens }
...
{ @routine $4DD998 TCalcParse_FormatTokens }
...
{ @routine $4DDDAC TCalcParse_GetOperatorRank }
...
{ @routine $4DDF3C TCalcParse_CollapseOperatorRun }
...
{ @routine $4DE0C8 TCalcParse_NormalizeFragments }
...
{ @routine $4DE298 TCalcParse_NormalizeScalarFragment }
...
{ @routine $4DF2A0 TCalcParse_NormalizeBracketFragment }
...
{ @routine $4DF33C TCalcParse_NormalizeParameterReference }
...
{ @routine $4DF4AC TCalcParse_NormalizeRangeLiteral }
...
{ @routine $4DF7D4 TCalcParse_InsertImplicitMultiplication }
...
{ @routine $4DFE70 TCalcParse_FindTopLevelOperator }
...
{ @routine $4DFF3C TCalcParse_EvaluateExpression }
...
{ @routine $4E04F4 TCalcParse_Evaluate }
...
{ @routine $4E0610 TCalcParse_Prepare }
...
{ @routine $4E0864 TCalcParse_Create }
...
{ @routine $4E08A4 TCalcParse_Reset }
...
{ @routine $4E090C TCalcParse_HasBalancedParenthesesInSlice }
...
{ @routine $4E0984 TCalcParse_SubstituteParameters }
...
{ @routine $4E0B1C TCalcParse_HasBalancedParentheses }
...
{ @routine $4E0B54 TCalcParse_ClampNumericLiterals }
