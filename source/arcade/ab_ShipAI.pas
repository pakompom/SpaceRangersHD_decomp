...
procedure SelectArcadeRewardWeapon(const Ship: TabShipAI; const Tech: Integer;
  var Info: PWeaponInfo); inline;
var
  MaximumTech, MinimumTech: Integer;
  Seed: Cardinal;
begin
  Seed := Ship.RandomRange(1, 100000);
  if Tech + 1 < 8 then MaximumTech := Tech + 1 else MaximumTech := 8;
  if Tech - 1 < 1 then MinimumTech := 1 else MinimumTech := Tech - 1;
  Info := Galaxy.SelectWeaponInfo(Seed, [0], RewardTechArgument(MaximumTech), RewardTechArgument(MinimumTech));
end;
...
type
  TUnusedCompilerLocal = record end;
var
